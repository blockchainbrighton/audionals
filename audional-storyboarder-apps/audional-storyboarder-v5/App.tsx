...
import { generateAudionalBooklet, generateBookletIllustration } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handleGenerate = async (targetName: string, vibe: string, category: 'creative' | 'industry') => {
    setAppState(AppState.GENERATING);
    try {
      const data = await generateAudionalBooklet(targetName, vibe, category);
      setBookletData(data);
  ...
  const handleGenerateImages = async () => {
    ...
    // Clone the data so we can update it incrementally
    const newBookletData = { ...bookletData, pages: [...bookletData.pages] };

    // We generate sequentially to be polite to the API rate limit, 
    // though parallel is possible if quota allows.
    for (let i = 0; i < newBookletData.pages.length; i++) {
      const page = newBookletData.pages[i];
      if (!page.imageUrl) {
        const imageUrl = await generateBookletIllustration(page.visualDescription, bookletData.tone);
        if (imageUrl) {
          newBookletData.pages[i] = { ...page, imageUrl };
          // Update state after each image so user sees progress
          setBookletData({ ...newBookletData });
        }
      }
    }
