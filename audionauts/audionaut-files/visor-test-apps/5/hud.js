/**
 * B.A.M. HUD Programmer v2.1
 * Manages a complex, multi-layered HUD with programmable scenes
 * and dynamic playlist generation from an asset library.
 */
...
class HUDManager {
    ...
    init() {
        this.bindEvents();
        this.renderAll();
        console.log('B.A.M. HUD v2.1 Initialized.');
    }
    ...
    handleControlClick(e) {
        ...
        const actions = {
            ...
            'load-demo': () => this.loadDemoPlaylist(), // The original simple demo loader
            ...
            // --- New actions for playlist generation ---
    ...
    generatePlaylistFromLibrary(filters = {}) {
        ...
        if (filters.collection) {
            filteredAssets = assetLibrary.filter(a => a.collection === filters.collection);
        }
        if (filters.rarity) {
            filteredAssets = assetLibrary.filter(a => a.rarity === filters.rarity);
        }
        ...
        filteredAssets.forEach((asset, index) => {
            ...
            if (asset.type === 'text') {
                layerContent = `[Text from ${asset.id}]\n${asset.name}`;
            }

            const newScene = {
                id: `gen_${asset.id}_${index}`,
                name: sceneName,
                duration: 6000,
                layers: [
                    {
                        ...
                        style: {
                            ...
                            opacity: 1, fontSize: 4, color: '#00ffd0', textAlign: 'center',
                    ...
                    {
                        id: `gen_info_layer_${asset.id}`, name: 'Info Text', type: 'text',
                        content: `COLLECTION: ${asset.collection}\nRARITY: ${asset.rarity}`,
                        style: {
                            x: 5, y: 88, width: 90, height: 10,
                            opacity: 0.8, fontSize: 3.5, color: '#fca311', textAlign: 'left',
        ...
        this.activeEditingLayerId = null;
        this.renderAll();
        alert(`Generated a new playlist with ${newPlaylist.length} scenes!`);
    }
    ...
    loadDemoPlaylist() {
        // This is the original, simple demo loader from before the asset library was added.
    ...
    renderHUD(scene) {
        ...
        scene.layers.forEach(layer => {
            ...
            const s = layer.style, f = s.filter;
