...
import { isFlashOn } from './flashUtils.js';
...
export function loop(ctx, cvs, geom, CFG, chars, helmet, noise, nCtx, animationState, lastHeartbeatTimeRef, lastTimeRef, pxOffsetRef, charShiftRef) {
  return function _loop(t) {
    ...
    if (CFG.sineWave) {
      ...
      // ---------- HUD TEXT: FLASH AT BEAT ----------
      const e = (t - lastTimeRef.value) / 1e3; lastTimeRef.value = t;
      pxOffsetRef.value += CFG.speed * e;
      if (pxOffsetRef.value >= CFG.step) {
        charShiftRef.value = (charShiftRef.value + ((pxOffsetRef.value / CFG.step) | 0)) % chars.length;
        pxOffsetRef.value %= CFG.step;
      }
      ...
      // 🔥 BPM-SYNC FLASH CONTROL ————
      if (CFG.flashing) {
        // Use window.fxAudioContext for real playback sync
        let playbackTime = 0;
        if (window.fxAudioContext && window.fxPlaybackState?.isPlaying) {
          playbackTime = window.fxAudioContext.currentTime - (window.fxPlaybackState.playbackStartTime || 0);
        } else {
          playbackTime = t / 1000;
        }
        if (!isFlashOn(playbackTime, CFG, window.fxInitialBPM || 104.15)) {
          ctx.globalAlpha = 0;
        }
      }

      // (Pulse effect applies if not flashing off)
      if (CFG.pulse && ctx.globalAlpha !== 0)
        ctx.globalAlpha *= .6 + .4 * Math.sin(t / 183.36);
