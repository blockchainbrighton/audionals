 ...
 *****************************************************************/
import { ogOneShotSampleUrls } from './samples.js';
...
function buildInlineSampleSelect(onChange) {
  ...
  ogOneShotSampleUrls.forEach(({ value, text }) => sel.append(new Option(text, value)));
  sel.onchange = () => {
...
document.getElementById('addChannelBtn').onclick = () =>
  addChannel(`Ch ${State.channels.length + 1}`, ogOneShotSampleUrls[0].value);
