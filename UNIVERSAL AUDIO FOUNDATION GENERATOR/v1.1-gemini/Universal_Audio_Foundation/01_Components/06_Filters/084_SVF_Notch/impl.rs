...
impl AudioNode for SVF_NotchNode {
...
mod tests {
    ...
    fn test_instantiation_and_metadata() {
        let node = SVF_NotchNode::new();
        let meta = node.metadata();
        assert_eq!(meta.name, "SVF_Notch");
        assert_eq!(meta.category, "06_Filters");
    }

    #[test]
    fn test_determinism() {
        // Rule: Same input + Same State = Same Output
        let mut node = SVF_NotchNode::new();
    ...
    fn test_reset_behavior() {
        ...
        let mut clean_node = SVF_NotchNode::new();
