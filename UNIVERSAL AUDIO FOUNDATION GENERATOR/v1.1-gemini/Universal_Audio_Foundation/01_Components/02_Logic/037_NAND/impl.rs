...
mod tests {
    ...
    fn test_instantiation_and_metadata() {
        let node = NANDNode::new();
        let meta = node.metadata();
        assert_eq!(meta.name, "NAND");
        assert_eq!(meta.category, "02_Logic");
    }

    #[test]
    fn test_determinism() {
        // Rule: Same input + Same State = Same Output
        let mut node = NANDNode::new();
    ...
    fn test_reset_behavior() {
        ...
        let mut clean_node = NANDNode::new();
