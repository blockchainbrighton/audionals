...
impl AudioNode for MS_EncodeNode {
...
mod tests {
    ...
    fn test_instantiation_and_metadata() {
        let node = MS_EncodeNode::new();
        let meta = node.metadata();
        assert_eq!(meta.name, "MS_Encode");
        assert_eq!(meta.category, "09_Effects");
    }

    #[test]
    fn test_determinism() {
        // Rule: Same input + Same State = Same Output
        let mut node = MS_EncodeNode::new();
    ...
    fn test_reset_behavior() {
        ...
        let mut clean_node = MS_EncodeNode::new();
