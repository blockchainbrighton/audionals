use crate::traits::{AudioNode, AudioNodeMetadata, InputDescriptor, OutputDescriptor};

pub struct SelectNode;

impl SelectNode {
    pub fn new() -> Self {
        Self {}
    }
}

impl AudioNode for SelectNode {
    fn metadata(&self) -> AudioNodeMetadata {
        AudioNodeMetadata {
            id: 38,
            name: "Select",
            category: "02_Logic",
            inputs: &[
                InputDescriptor { name: "Control", min: 0.0, max: 1.0, default: 0.0 },
                InputDescriptor { name: "Input A", min: -1.0, max: 1.0, default: 0.0 },
                InputDescriptor { name: "Input B", min: -1.0, max: 1.0, default: 0.0 },
            ],
            outputs: &[
                OutputDescriptor { name: "Output" },
            ],
        }
    }

    fn process(&mut self, inputs: &[f64], outputs: &mut [f64], _sample_rate: f64) {
        if inputs.len() < 3 { return; }
        let ctrl = inputs[0];
        let a = inputs[1];
        let b = inputs[2];
        
        // standard threshold
        outputs[0] = if ctrl > 0.5 { a } else { b };
    }

    fn reset(&mut self) {}
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_math_accuracy() {
        let mut node = SelectNode::new();
        let mut out = [0.0];
        // Ctrl high -> A
        node.process(&[1.0, 0.8, 0.2], &mut out, 44100.0);
        assert_eq!(out[0], 0.8);
        // Ctrl low -> B
        node.process(&[0.0, 0.8, 0.2], &mut out, 44100.0);
        assert_eq!(out[0], 0.2);
    }
}