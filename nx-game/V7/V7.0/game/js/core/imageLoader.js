...
// Local Asset Cache (Game Sprites)
...
export const imageLoader = {
    ...
    init: function() {
        // Preload core assets
        this.loadLocal('neon_shade_npc', 'artwork/neon_shade_npc.svg');
        this.loadLocal('hack_icon', 'artwork/hack_icon.svg'); // Future use
    },

    loadLocal: function(key, path) {
        const img = new Image();
        img.src = path;
        img.onload = () => {
            localCache.set(key, img);
            console.log(`[ImageLoader] Loaded asset: ${key}`);
        };
        img.onerror = () => {
            console.warn(`[ImageLoader] Failed to load local asset: ${path}`);
        };
    },
