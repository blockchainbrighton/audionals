...
import { mapManager, TILE_PROPERTIES } from '../managers/mapManager.js'; // Import TILE_PROPERTIES
...
import { floatingTextManager } from '../managers/floatingTextManager.js'; // Import Floating Text Manager
import { soundManager } from '../managers/soundManager.js'; // Import Sound Manager
import { minimap } from '../ui/minimap.js'; // Import Minimap
import { particleManager } from '../managers/particleManager.js'; // Import Particle Manager
import { stashManager } from '../managers/stashManager.js'; // Import Stash Manager
import { imageLoader } from './imageLoader.js';
import { SlotsGame } from '../casino/SlotsGame.js';
import { RouletteGame } from '../casino/RouletteGame.js';
import { HighLowGame } from '../casino/HighLowGame.js';
import { discoveryManager } from '../managers/discoveryManager.js';
import { InputManager } from './InputManager.js'; // Import InputManager
import { ProgressionManager } from '../managers/progressionManager.js';
import { barManager } from '../managers/barManager.js';
import { DialogueManager } from '../managers/dialogueManager.js';
import { HackingGame } from '../minigames/HackingGame.js';
...
export const game = {
    ...
    utils: utils,
    events: new EventBus(), 
    input: InputManager, // Expose InputManager
    canvas: null,
    ctx: null,
    gameState: 'PLAYING',
    gameTime: 0,
    ...
    // keysPressed: {}, // Deprecated: Use game.input
    ...
    casino: { // Aggregated Casino Manager
        Slots: SlotsGame,
    ...
    dialogueManager: new DialogueManager(null), // Placeholder until init
    hackingGame: new HackingGame(null), // Placeholder

    init: function(canvas, ctx) {
        this.canvas = canvas;
        this.ctx = ctx;
        this.imageLoader.init(); // Init Image Loader (Local Assets)
        this.input.init(this); // Init Input
        this.setupInput(); // Setup Action Handlers
        ...
        this.hackingGame = new HackingGame(this); // Init Hacking Game

        this.entities = []; // Initialize entities list
        
        // Input State
        this.mouse = { x: 0, y: 0, screenX: 0, screenY: 0 }; // Kept for compat, synced by InputManager
        ...
        this.casino.init(this); // Init Casino Games
        ...
        this.hud.update();
        this.utils.addMessage("Digital Metaverse Online. Welcome, User Agent.");
        this.discovery.startIntro();

        this.utils.queueTutorial("Initiate Movement Matrix: WASD or Arrow Keys.", () => true);
        this.utils.queueTutorial("Access Inventory: Press 'I' at any time to Equip, Drop, or Sell items.", (g) => g.player.inventory.items.length > 0 || g.player.money > 10);
    ...
    setupInput: function() {
        this.events.on('ACTION_TRIGGERED', (action) => {
            // Hacking Game Input Intercept
            if (this.hackingGame && this.hackingGame.active) {
                if (action === 'ATTACK' || action === 'INTERACT') { // Space or E
                    this.hackingGame.attemptHit();
                }
            ...
            if (action === 'CLOSE_UI') {
                ...
                if (this.gameState === 'NFT_VIEW') this.closeNftPopup();
                else if (shopInterfaceEl && shopInterfaceEl.style.display === 'block') this.closeShop();
                else if (inventoryDisplayEl && inventoryDisplayEl.style.display === 'block') this.toggleInventory();
                else if (questLogEl && questLogEl.style.display === 'block') this.toggleQuestLog();
                else if (this.gameState === 'PLAYING') { 
                    this.utils.addMessage("Pause sequence (ESC)... Resuming operations."); 
                }
                return;
            }

            if (this.gameState === 'GAME_OVER' && action === 'RESTART') {
                this.restartGame();
                return;
            }
            ...
            if (action === 'ABILITY_1') this.player.useAbility('1');
            if (action === 'ABILITY_2') this.player.useAbility('2');
            if (action === 'ABILITY_3') this.player.useAbility('3');
            
            if (action === 'TOGGLE_INVENTORY') this.toggleInventory();
            if (action === 'TOGGLE_QUESTS') this.toggleQuestLog();

            if (this.gameState === 'PLAYING' || this.gameState === 'INVENTORY_OPEN' || this.gameState === 'QUESTLOG_OPEN') {
                if (action === 'INTERACT') this.interact();
                if (action === 'RELOAD') this.player.reload();
    ...
    update: function(deltaTime) {
        ...
        if (this.paused) {
            if (this.hackingGame && this.hackingGame.active) {
                this.hackingGame.update(deltaTime);
                // Also update HUD/Input so we don't freeze inputs completely?
                // Actually hackingGame handles its own state, but we need to ensure input makes it there.
            }
            return; // Stop update loop if paused
        }

        if (this.progression && this.progression.update) this.progression.update(deltaTime);
    ...
    render: function() {
        ...
        if (this.progression) this.progression.renderObjectiveMarkers();
        ...
        if (this.hackingGame && this.hackingGame.active) {
            this.hackingGame.render(this.ctx);
        }
    ...
    gameOver: function() {
        ...
        this.utils.addMessage("Critical System Failure. User Agent signature lost.");
    },
    ...
    interact: function(isAuto = false) {
        ...
        // Auto interaction now checks immediate neighbors to detect collisions with locked doors
        const dirs = [[0,0],[0,-1],[0,1],[-1,0],[1,0]]; 
        if (!isAuto) dirs.push([-1,-1],[-1,1],[1,-1],[1,1]);
                ...
                if (tileData) {
                    ...
                    if((tileProps && tileProps.interactive) || tileData.isDoor){
                        ...
                        if (this.utils.distance(this.player.x + this.player.width/2, this.player.y + this.player.height/2, tileWorldX, tileWorldY) <= this.config.TILE_SIZE * 1.2) {
                            ...
                            // Check for locks before allowing interaction
                            if (this.mapManager.checkLock(tileData)) {
                                this.interactionCooldown = Date.now() + 1000;
                                return;
                            }

                            // 1. Door Transition
                            if (tileData.isDoor) {
                                // For auto-interact (walking), only trigger if we are ON the tile (0,0)
                                // This prevents ping-ponging from adjacent tiles
                                if (isAuto && (dx !== 0 || dy !== 0)) continue;
                            ...
                            if(tileData.interactionType && tileData.interactionTargetId){
                                ...
                                if (tileData.interactionType === 'xlounge_stash') {
                                    ...
                                    this.enterLocation('CASINO', tileData.interactionTargetId, 'SLOTS'); return; // Default / Legacy
                                } else if (tileData.interactionType === 'casino_slots') {
                                    this.enterLocation('CASINO', tileData.interactionTargetId, 'SLOTS'); return;
                                } else if (tileData.interactionType === 'casino_roulette') {
                                    this.enterLocation('CASINO', tileData.interactionTargetId, 'ROULETTE'); return;
                                } else if (tileData.interactionType === 'casino_highlow') {
                                    this.enterLocation('CASINO', tileData.interactionTargetId, 'HIGHLOW'); return;
                                } else if (tileData.interactionType === 'neon_shade') {
                                    this.dialogueManager.startDialogue('neon_shade_intro'); return;
                                } else if (tileData.interactionType === 'hack_terminal') {
                                    this.hackingGame.start(1, (success) => {
                                        if (success) {
                                            this.utils.addMessage("[ ROOT ACCESS GRANTED ] - 500 Credits Transferred.");
                                            this.player.earnMoney(500);
                                        } else {
                                            this.utils.addMessage("[ ACCESS DENIED ] - Security Alert Triggered.");
                                        }
                                    });
                                    return;
                                }
    ...
    enterLocation: function(type, targetId, subType = null) {
        this.gameState = 'LOCATION_TRANSITION';
        this.soundManager.playTheme(type); // Play Location Theme
        this.events.emit('LOCATION_ENTERED', { type, targetId, subType }); // Emit event for progression
        ...
        switch(type) {
            case 'SHOP': 
                ...
                break;
            case 'STASH': 
                titleText = "ARCHIVE STORAGE"; 
                descText = "Encryption Handshake Required..."; 
                color = "#F0F";
                break;
            case 'QUEST': 
                titleText = "COMPILER"; 
                ...
                break;
            case 'BAR': 
                titleText = "STREAM BUFFER"; 
                descText = "Restoring Integrity... or reducing it."; 
                color = "#F80";
                break;
            case 'ARMOURY': 
        ...
        setTimeout(() => {
            ...
            switch(type) {
                ...
                case 'QUEST':
                    ...
                    break;
                case 'BAR':
                    this.barManager.renderUI(contentEl, actionsEl);
                ...
                case 'ARMOURY':
                    ...
                    break;
                case 'CASINO':
                    this.renderCasinoUI(contentEl, actionsEl, subType);
    ...
    renderCasinoUI: function(container, actions, subType) {
        console.log(`[Casino] Rendering Casino UI: ${subType}`);
        if (subType === 'ROULETTE') {
            this.casino.Roulette.renderUI(container, actions);
        } else if (subType === 'HIGHLOW') {
            this.casino.HighLow.renderUI(container, actions);
        } else {
            // Default to Slots
            this.casino.Slots.renderUI(container, actions);
        }
    },
