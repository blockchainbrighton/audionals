...
export function attack() {
    ...
    // Play Sound
    this.game.events.emit('WEAPON_FIRED', this.equippedWeapon);

    // this.game.utils.addMessage(`${this.equippedWeapon.name} deployed!`); // Too spammy?
    let attacked = false;

    // Apply Damage Multiplier
    let damageMult = this.damageMultiplier || 1.0;
    
    // Apply Gang Damage Mod
    if (this.gang && this.gang.DamageMod) {
        damageMult *= this.gang.DamageMod;
    }
    ...
    if (this.equippedWeapon.type === Weapons.weaponTypes.MELEE || this.equippedWeapon.type === Weapons.weaponTypes.UNARMED) {
        ...
        for (const enemy of sortedEnemies) {
            ...
            if (distToEnemy < this.equippedWeapon.effectiveRange) {
                ...
                if (angleDiff < accuracy) { 
                    ...
                    // --- ARC RIFLE LOGIC (Chain Lightning) ---
                    if (this.equippedWeapon.id === 'arc_rifle' && wasAlive) {
                        const chainDist = 150;
                        const chainCount = 2;
                        let chained = 0;
                        const potentialTargets = this.game.enemyManager.list.filter(e => e !== enemy && e.currentHp > 0);
                        
                        for (const secondary of potentialTargets) {
                            if (chained >= chainCount) break;
                            if (this.game.utils.distance(enemy.x, enemy.y, secondary.x, secondary.y) < chainDist) {
                                secondary.takeDamage(Math.floor(finalDamage * 0.6), 'energy');
                                if (this.game.particleManager) {
                                     this.game.particleManager.createEffect(secondary.x + secondary.width/2, secondary.y + secondary.height/2, 'spark', 5);
                                }
                                chained++;
                            }
                        }
                    ...
                    // --- GLITCH GRENADE LOGIC (AOE Stun) ---
                    if (this.equippedWeapon.id === 'glitch_grenade') {
                        const blastRadius = 100;
                        const potentialTargets = this.game.enemyManager.list.filter(e => e !== enemy && e.currentHp > 0);
                        for (const secondary of potentialTargets) {
                            if (this.game.utils.distance(enemy.x, enemy.y, secondary.x, secondary.y) < blastRadius) {
                                secondary.takeDamage(Math.floor(finalDamage * 0.5), 'glitch');
                                // Apply Stun/Slow (Simulated by damage for now, or add Status System to enemies later)
                            }
                        }
                        if (this.game.particleManager) {
                             this.game.particleManager.createEffect(enemy.x, enemy.y, 'explosion', 15);
                        }
                    }
        ...
        this.game.events.emit('WEAPON_UPDATED', this.equippedWeapon);
    }
...
export function attemptActiveReload() {
    ...
    if (progress >= 0.5 && progress <= 0.7) {
        ...
        this.completeReload();
        this.game.events.emit('RELOAD_PERFECT'); 
    } else {
...
export function takeDamage(amount, damageType = 'kinetic') {
    ...
    this.game.events.emit('PLAYER_HIT');

    this.game.utils.addMessage(`User Agent integrity compromised by ${finalDamage}! Integrity: ${this.hp}/${this.maxHp}`);
    if (this.hp <= 0) { 
        this.hp = 0; 
        this.game.gameOver(); 
    }
