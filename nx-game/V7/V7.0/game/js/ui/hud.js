...
// Helper for mapping POI types to display names
    poiNames: {
        'xlounge_stash': 'Localhost Cache',
        'exchange_node': 'Data Exchange',
        'bar': 'Stream Buffer',
        'armoury': 'Bit Depot',
        'casino': 'RNG Node',
        'xemist_contact': 'Compiler',
        'default': 'Node'
    },
    ...
    renderOverlay: function(ctx) {
        ...
        // Get revealed POI types from progression
        const revealedPois = new Set();
        if (this.game.progression) {
            this.game.progression.campaign.forEach(mission => {
                const isCompleted = this.game.progression.completedMissions.has(mission.id);
                const isActive = this.game.progression.activeMissionId === mission.id;
                if ((isCompleted || isActive) && mission.suggestedPoi) {
                    revealedPois.add(mission.suggestedPoi);
                }
            });
        }
        ...
        this.game.minimap.poiCache.forEach(poi => {
            ...
            // Show if: Admin Mode OR (revealed by mission AND discovered by exploration)
            // Wait, user said 'arrow should appear in the HUD' when suggested.
            // So mission suggestion is enough to show the arrow.
            
            let shouldShow = this.showAllPois;
            if (!shouldShow) {
                // If it's the current mission's target OR a past mission's target
                if (revealedPois.has(poi.type)) shouldShow = true;
                
                // Also check discovery manager if it exists
                if (!shouldShow && this.game.discovery && this.game.discovery.shouldShowPoi(poi.type)) shouldShow = true;
            }
