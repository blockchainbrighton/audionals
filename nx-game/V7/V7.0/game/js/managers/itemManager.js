...
import { itemDefinitions } from '../data/items.js'; // Import centralized items
        ...
        if (this.game.collectionData && this.game.collectionData.length > 0) {
                    ...
                    this.game.collectionData.forEach(row => {
                        ...
                        this.itemDefinitions[itemId] = {
                            id: itemId,
                            name: row.name || `Betav3rse #${row.id}`,
                            description: row.effect || "A rare substance from the Betav3rse collection.",
    ...
    getTaxonomyMatches: function(description, itemId) {
        ...
        applyTaxonomyEffect: function(player, taxonomyEntries, originalRow) {
            if (!taxonomyEntries || taxonomyEntries.length === 0) {
                ...
                player.heal(15);
                this.game.utils.addMessage("System stability restored (+15 Integrity).");
                return;
            }
                    ...
                    case 'TANK_MODE':
                        ...
                        break;
                                     ...
                                     case 'LUCKY_DIP':
                                         ...
                                         if (Math.random() > 0.5) {
                                             player.money += 100;
                                             this.game.utils.addMessage("Windfall detected! +100 Cycles.");
                                         } else {
                                             player.money = Math.max(0, player.money - 50);
                                             this.game.utils.addMessage("Wallet integrity compromised! -50 Cycles.");
                                         }
                                         break;
                                     case 'PYRO':
                                     ...
                                     case 'MIDAS_TOUCH':
                                         player.money += 200;
                                         this.game.utils.addMessage("Matter transmutation complete. +200 Cycles.");
                                         break;
                                     case 'PSYCHIC_WAVE':
                                         // Reveal map?
                                     ...
                                     case 'TRIVIA_GOD':
                                         ...
                                         this.game.utils.addMessage("Knowledge database expanded. +50 Cycles");
                                         break;
                                     case 'SUMMONER':
