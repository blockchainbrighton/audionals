export const zoneManager = {
    ...
    init: function(gameInstance) {
        ...
        this.zones=[]; 
        // Default static zones if any (moving towards dynamic)
        this.addZone({id:'zone_epsilon',name:"Corrupted Zone Epsilon",x:12*this.game.config.TILE_SIZE,y:12*this.game.config.TILE_SIZE,width:6*this.game.config.TILE_SIZE,height:6*this.game.config.TILE_SIZE,isControlledByPlayer:false,controlTimer:0,incomeTimer:0,enemiesInZone:0,initialEnemiesToClear:2, incomeValue: 75});
        this.addZone({id:'exchange_fringe',name:"Exchange Node Fringe",x:5*this.game.config.TILE_SIZE,y:5*this.game.config.TILE_SIZE,width:12*this.game.config.TILE_SIZE,height:12*this.game.config.TILE_SIZE,isControlledByPlayer:false,controlTimer:0,incomeTimer:0,enemiesInZone:0,initialEnemiesToClear:3, incomeValue: 120});
    },

    addZone: function(zoneData) {
        if (!this.zones.find(z => z.id === zoneData.id)) {
            const z = {
                ...zoneData,
                controlTimer: 0,
                incomeTimer: 0,
        ...
        this.zones.forEach(z=>{
            ...
            if(z.enemiesInZone===0 && !z.isControlledByPlayer){
                ...
                if(z.controlTimer>=this.controlDurationThreshold){
                    ...
                    z.incomeTimer=0;
                    this.game.utils.addMessage(`Zone "${z.name}" Secured. Yielding passive compounds (Cycles).`);
                }
