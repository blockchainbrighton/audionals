...
export const TILE_PROPERTIES = {
    ...
    // NPCs
    [TILE_TYPES.NEON_SHADE]: { color: '#0FF', collision: true, interactive: true, type: 'neon_shade' },
    
    // Dev Tools
    [TILE_TYPES.HACK_TERMINAL]: { color: '#0F0', collision: true, interactive: true, type: 'hack_terminal' },

    // Undercity Tiles
    [TILE_TYPES.SLUDGE]: { color: '#242', collision: false, speedModifier: 0.5, damage: 2 }, // Damages player
    [TILE_TYPES.RUST_WALL]: { color: '#432', collision: true, speedModifier: 0 },
    [TILE_TYPES.UNDERCITY_ENTRANCE]: { color: '#333', collision: false, speedModifier: 1, interactive: true, type: 'undercity_entrance' }
};
...
export const mapManager = {
    ...
    generateMap: function() {
        ...
        poiLocations.push({x: safehouseX, y: safehouseY, id: 'localhost', type: 'xlounge_stash'});
        ...
        // 5. Place Police Barricades around Localhost (Mission 2 constraint)
        const radius = 3; 
        ...
        // 5. Place Undercity Entrance (Hidden/Far away)
        const undercityLoc = findSpot(80);
        this.placeBuilding(overworld, undercityLoc.x, undercityLoc.y, 3, 3, TILE_TYPES.UNDERCITY_ENTRANCE, 'undercity', TILE_TYPES.RUST_WALL, 'undercity_entrance', 5, 5, 'zone_undercity');
        this.generateUndercity('undercity', 40, 40);

        // 6. Scatter Hostile Spawns & Coolant
        for (let r = 1; r < H-1; r++) {
            for (let c = 1; c < W-1; c++) {
    ...
    generateInterior: function(id, w, h, floorType, interactId, interactType, exitX, exitY) {
        ...
        if (interactType === 'casino') {
            ...
            // Place Neon Shade (The Broker) - Near the back/VIP area
            const brokerX = Math.floor(w - 2);
            const brokerY = 2; 
            map[brokerY][brokerX] = { type: TILE_TYPES.NEON_SHADE, interactionTargetId: 'neon_shade', interactionType: 'neon_shade' };
            ...
            // DEV: Place Hacking Terminal in Safehouse
            if (interactType === 'xlounge_stash') {
                map[cy+2][cx-2] = { type: TILE_TYPES.HACK_TERMINAL, interactionType: 'hack_terminal' };
            }
        }
    ...
    generateUndercity: function(id, w, h) {
        const map = [];
        // Fill with Rust Walls
        for (let r = 0; r < h; r++) {
            map[r] = [];
            for (let c = 0; c < w; c++) {
                map[r][c] = { type: TILE_TYPES.RUST_WALL };
            }
        }

        // Drunken Walk to carve paths
        let miners = [{x: 5, y: 5}]; // Start point
        let ticks = 0;
        const maxTicks = w * h * 0.8; // Carve 80%?

        while (miners.length > 0 && ticks < maxTicks) {
            ticks++;
            const minerIndex = Math.floor(Math.random() * miners.length);
            const miner = miners[minerIndex];
            
            // Carve
            if (map[miner.y][miner.x].type !== TILE_TYPES.SLUDGE) {
                // Chance to be plain or sludge
                map[miner.y][miner.x].type = Math.random() < 0.2 ? TILE_TYPES.SLUDGE : TILE_TYPES.LOW_REZ_PLAIN;
            }
            ...
            if (nx > 1 && nx < w - 2 && ny > 1 && ny < h - 2) {
                ...
                miners.splice(minerIndex, 1); // Hit wall, die
            }
        }

        // Ensure Entrance
        map[5][5] = { type: TILE_TYPES.LOW_REZ_PLAIN, isDoor: true, targetMap: 'overworld', targetX: 20, targetY: 20 }; // Placeholder exit
        // We need to link this back to the specific entrance loc, but for now generic is fine

        // Place Boss Room (Far Corner)
        const bx = w - 5;
        const by = h - 5;
        for(let r=by-3; r<=by+3; r++){
            for(let c=bx-3; c<=bx+3; c++){
                if(r>0 && r<h && c>0 && c<w) map[r][c] = { type: TILE_TYPES.LOW_REZ_PLAIN };
            }
        }
        
        // Place Boss (We don't have the entity yet, so just a marker?)
        // Use HOSTILE_SPAWN_VECTOR for now
        map[by][bx] = { type: TILE_TYPES.HOSTILE_SPAWN_VECTOR }; 
        ...
        this.game.zoneManager.addZone({
            id: 'zone_undercity',
            name: "Sector Zero",
            x: 0, y: 0, width: 0, height: 0, // Abstract zone
            isControlledByPlayer: false,
            enemiesInZone: 0,
            initialEnemiesToClear: 20,
            incomeValue: 500
        });
    },
    ...
    render: function() {
        ...
        for (let r = sR; r < eR; r++) {
            for (let c = sC; c < eC; c++) {
                ...
                } else if (tile.type === TILE_TYPES.NEON_SHADE) {
                    // Render NPC SVG
                    const img = this.game.imageLoader.getImage('neon_shade_npc');
                    if (img) {
                        this.game.ctx.drawImage(img, c * tileSize, r * tileSize, tileSize, tileSize);
                    } else {
                        // Fallback
                        this.game.ctx.fillStyle = tileDef.color;
                        this.game.ctx.fillRect(c * tileSize, r * tileSize, tileSize, tileSize);
                    }
                    continue; // Skip standard fillRect
                } else {
                    this.game.ctx.fillStyle = tileDef.color;
                }
                ...
                if (tile.interactionType) {
                    ...
                    switch(tile.interactionType) {
                        ...
                        case 'xlounge_stash': label = "CACHE"; break;
    ...
    checkLock: function(tileData) {
        ...
        if (this.currentMapId === 'safehouse_interior' && tileData.targetMap === 'overworld') {
            if (!this.game.progression.checkFlag('zone_street')) {
                this.game.utils.addMessage("[ SECURE LOCKDOWN ] - Access your CACHE to authorize exit.", '#F0F');
                if (this.game.soundManager) this.game.soundManager.playError();
                if (this.game.floatingTextManager) {
