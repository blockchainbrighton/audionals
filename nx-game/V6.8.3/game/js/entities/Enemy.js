...
export class Enemy extends Entity {
    ...
    die() {
        ...
        if (lootDrop) {
            ...
            if (lootDrop.item) {
                // Drop item using itemManager (which will create a WorldItem eventually)
                // For now, assume itemManager still handles spawning map items or we convert here.
                // Since we are transitioning, let's call itemManager.dropItem which pushes to game.entities if we update it.
                // Or directly call dropItem logic.
                // existing logic: this.game.itemManager.onMapItems.push({...})
                
                // We should use the itemManager's dropItem to ensure consistency
                this.game.itemManager.dropItem(lootDrop.item, this.x, this.y);
