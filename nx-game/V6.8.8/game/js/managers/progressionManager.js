...
export class ProgressionManager {
    ...
    setupEventListeners() {
        ...
        this.game.events.on('WEAPON_UPDATED', (weapon) => this.checkMissionProgress('EQUIP', weapon));
    }
    ...
    update(dt) {
         ...
         if (mission.type === 'GOTO') {
             ...
             } else if (mission.targetZone === 'zone_undercity' || mission.targetZone === 'undercity_entrance') {
                 // Check if player is on the Undercity Entrance Tile OR inside the Undercity Map
                 if (this.game.mapManager.currentMapId === 'undercity') {
                      this.completeMission(this.activeMissionId);
                 }
                 // If mission is to find the entrance (Signal Trace)
                 if (mission.id === 'mission_08_signal_trace') {
                     // Check proximity to entrance
                     const entrance = this.game.mapManager.poiLocations.find(p => p.type === 'undercity_entrance');
                     if (entrance) {
                         const dist = this.game.utils.distance(this.game.player.x, this.game.player.y, entrance.x * this.game.config.TILE_SIZE, entrance.y * this.game.config.TILE_SIZE);
                         if (dist < 100) this.completeMission(this.activeMissionId);
                     }
                 }
             }
         } else if (mission.type === 'ACCUMULATE') {
             if (this.game.player.money >= mission.amount) {
                 this.completeMission(this.activeMissionId);
             }
         }
    }
    ...
    checkMissionProgress(eventType, data) {
        ...
        if (mission.type === eventType) {
            ...
            else if (eventType === 'EQUIP') {
                if (data && data.id === mission.targetId) {
                    this.completeMission(this.activeMissionId);
                }
            }
        }
    ...
    startMission(missionId) {
        ...
        this.game.events.emit('MISSION_STARTED', mission);
    ...
    renderObjectiveMarkers() {
        ...
        if (mission.type === 'INTERACT') {
            // Check various interaction types
            targetLoc = this.game.questManager.findLocation(mission.targetId, 'xlounge_stash') || 
                        this.game.questManager.findLocation(mission.targetId, 'casino_slots') ||
                        this.game.questManager.findLocation(mission.targetId, 'casino_roulette') ||
                        this.game.questManager.findLocation(mission.targetId, 'casino_highlow') ||
                        this.game.questManager.findLocation(mission.targetId, 'casino') ||
                        this.game.questManager.findLocation(mission.targetId, 'neon_shade') ||
                        this.game.questManager.findLocation(mission.targetId, 'hack_terminal') ||
                        this.game.questManager.findLocation(mission.targetId, 'undercity_entrance');
            char = "!";
            color = "#F0F";
        } else if (mission.type === 'KILL') {
            const target = this.game.enemyManager.list.find(e => e.typeId === mission.targetType || mission.targetType === 'any');
            if (target) targetLoc = { x: target.x + target.width/2, y: target.y + target.height/2 };
            char = "TARGET";
            color = "#F00";
        } else if (mission.type === 'BUY') {
            targetLoc = this.game.questManager.findLocation('central_exchange', 'exchange_node');
            char = "SHOP";
            color = "#0FF";
        } else if (mission.type === 'GOTO') {
            const zone = this.game.zoneManager.getZoneById(mission.targetZone);
            if (zone) targetLoc = { x: zone.x + zone.width/2, y: zone.y + zone.height/2 };
            char = "GO";
            color = "#FF0";
        }

        if (targetLoc) {
            this.game.questManager.drawMarker(targetLoc.x, targetLoc.y, char, color);
        }
    }
}