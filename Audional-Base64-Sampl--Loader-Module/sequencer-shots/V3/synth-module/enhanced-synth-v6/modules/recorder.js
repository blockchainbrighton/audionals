...
export const Recorder = {
    ...
    initAudio() {
        ...
        import('./envelope.js').then(({ AudioSafety, EnvelopeManager }) => {
            ...
            a.reverb = new Tone.Reverb({ decay:2, wet:.3 });
            a.delay  = new Tone.FeedbackDelay({ delayTime:.25, feedback:.3, wet:.2 });
            a.filter = new Tone.Filter(5000, "lowpass");
            
            // Connect to audio safety chain
            const safetyInput = AudioSafety.getInputNode();
            a.filter.connect(safetyInput);
            a.reverb.connect(safetyInput);
            a.delay.connect(safetyInput);
            ...
            console.log('[Recorder] Audio initialized with safety features');
        }).catch(err => {
            console.error('[Recorder] Failed to initialize audio safety:', err);
    ...
    playNote(note){
        ...
        import('./envelope.js').then(({ AudioSafety }) => {
            if (!AudioSafety.canPlayNote()) {
                console.warn(`[Recorder] Cannot play note ${note}: voice limit reached`);
                return;
            }
            ...
            console.error('[Recorder] Audio safety not available, using fallback:', err);
            // Fallback behavior
            synthApp.activeNotes.add(note); 
            Keyboard.updateKeyVisual(note, 1);
            if(synthApp.isArmed && !synthApp.isRec) this.startRec();
    ...
    releaseNote(note){
        ...
        import('./envelope.js').then(({ AudioSafety }) => {
            ...
            console.error('[Recorder] Audio safety not available for note release:', err);
        });
        ...
        try {
            ...
            console.warn(`[Recorder] Error releasing note ${note}:`, err);
        }
    },
    startRec(){
