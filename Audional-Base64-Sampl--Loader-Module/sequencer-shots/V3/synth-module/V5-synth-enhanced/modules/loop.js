...
export const LoopManager = {
    ...
    startLoop(sequence = null) {
        ...
        // Connect crossfade gain if available
        if (this.crossfadeGain && window.synthApp?.synth) {
            try {
                window.synthApp.synth.disconnect();
                window.synthApp.synth.connect(this.crossfadeGain);
                this.crossfadeGain.connect(window.synthApp.filter);
            } catch (err) {
                console.warn('[LoopManager] Could not connect crossfade gain:', err);
            }
        }
    ...
    completeLoopStop() {
        ...
        if (this.crossfadeGain && window.synthApp?.synth) {
            try {
                ...
                window.synthApp.synth.disconnect();
                window.synthApp.synth.connect(window.synthApp.filter);
            } catch (err) {
                console.warn('[LoopManager] Error restoring audio routing:', err);
            }
        }
