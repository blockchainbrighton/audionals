...
export const LoopManager = {
    ...
    init() {
        ...
        if (window.synthApp)
            window.synthApp.loop = { enabled: false, start: 0, end: 0, quantized: false, grid: 0.25, tempo: 120, crossfade: true };
    ...
    _setLoopBounds(start, end, silent) {
        ...
        if (!silent) console.log(`[LoopManager] Loop bounds set: ${this.loopStart}s - ${this.loopEnd}s`);
    ...
    prepareLoopedSequence(seq = null) {
        ...
        const filtered = s.filter(n => n.start < this.loopEnd && n.start + n.dur > this.loopStart);
        let adjusted = filtered.map(n => {
