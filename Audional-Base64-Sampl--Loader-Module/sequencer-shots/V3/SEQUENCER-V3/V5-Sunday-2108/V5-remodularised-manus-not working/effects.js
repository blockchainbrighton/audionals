// effects.js - Enhanced audio effects system with multiple effects and LFOs
...
const EnhancedEffects = {
    ...
    getEffectChainOrder() {
        return [
            'inputGain',
            'distortion', 'compressor', // Chain 1
            'chorus', 'phaser', 'tremolo', 'vibrato', // Chain 2
            'bitCrusher', // Chain 3
            'mixer',
            'filter', 'delay', 'reverb',
            'outputGain'
    ...
    modulateParameter(effectName, paramName, lfoRate, depth, min, max) {
        const effect = this.effects[effectName];
        if (!effect || !effect[paramName]) {
            console.warn(`[EnhancedEffects] Cannot modulate ${paramName} on ${effectName}`);
            return;
        }
        ...
        lfo.connect(effect[paramName]);
        lfo.start();
        
        console.log(`[EnhancedEffects] Started modulation on ${effectName}.${paramName}`);
        return lfo;
    },

    // Real-time parameter automation
    automateParameter(effectName, paramName, values, duration) {
        const effect = this.effects[effectName];
        if (!effect || !effect[paramName]) {
            console.warn(`[EnhancedEffects] Cannot automate ${paramName} on ${effectName}`);
            return;
        }
        ...
        values.forEach((value, index) => {
            const time = Tone.now() + (index * stepDuration);
        ...
        console.log(`[EnhancedEffects] Started automation on ${effectName}.${paramName}`);
    },

    // Effect bypass utilities
    bypassAll() {
        Object.keys(this.enabled).forEach(effectName => {
            this.toggleEffect(effectName, false);
        });
        console.log('[EnhancedEffects] Bypassed all effects');
    },
    ...
    enableAll() {
        ...
        console.log('[EnhancedEffects] Enabled all effects');
    },

    // Performance monitoring
    getPerformanceStats() {
        const activeEffects = Object.entries(this.enabled)
            .filter(([name, enabled]) => enabled)
            .map(([name]) => name);
        
        return {
            activeEffects: activeEffects.length,
            totalEffects: Object.keys(this.enabled).length,
            activeEffectNames: activeEffects,
            estimatedCpuUsage: activeEffects.length * 2 // Rough estimate
        };
    },

    dispose() {
        Object.values(this.effects).forEach(e => e?.dispose?.());
        console.log('[EnhancedEffects] Disposed all effects');
    }
};
