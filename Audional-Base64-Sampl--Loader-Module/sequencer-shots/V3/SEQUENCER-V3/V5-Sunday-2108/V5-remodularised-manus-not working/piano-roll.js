// piano-roll.js - Piano roll editor interface for note editing and visualization
// Dependencies: Tone.js (global), LoopManager (global)
...
const PianoRoll = {
    ...
    draw() {
        ...
        const quantGrid = window.LoopManager?.quantizeEnabled ? window.LoopManager.quantizeGrid : null;
        ...
        const onMove = (e) => {
            ...
            const currentQuantGrid = window.LoopManager?.quantizeEnabled ? window.LoopManager.quantizeGrid : null;
        ...
        if (window.synthApp.isPlaying) {
            this._drawPlayhead(window.synthApp.currentTime || 0, cellW, cellH);
    ...
    scrollToNote(noteIndex) {
        const seq = window.synthApp.seq || [];
        if (noteIndex < 0 || noteIndex >= seq.length) return;
        
        const note = seq[noteIndex];
        const midi = Tone.Frequency(note.note).toMidi();
        const cellH = this.CELL_H * this.zoomY;
        const targetY = (this.MIDI_HIGH - midi) * cellH;
        
        this.scrollContainer.scrollTo({
            top: targetY - this.scrollContainer.clientHeight / 2,
            behavior: 'smooth'
        });
    },
    ...
    scrollToTime(time) {
        const cellW = 40 * this.zoomX;
        const targetX = time * cellW;
        
        this.scrollContainer.scrollTo({
            left: targetX - this.scrollContainer.clientWidth / 2,
            behavior: 'smooth'
        });
    },

    selectNote(noteIndex) {
        window.synthApp.selNote = noteIndex;
        this.draw();
        this.scrollToNote(noteIndex);
    },

    deleteSelectedNote() {
        if (typeof window.synthApp.selNote === "number" && window.synthApp.selNote >= 0) {
            window.synthApp.seq.splice(window.synthApp.selNote, 1);
            window.synthApp.selNote = null;
            this.draw();
        }
    ...
    duplicateSelectedNote() {
        if (typeof window.synthApp.selNote === "number" && window.synthApp.selNote >= 0) {
            const note = window.synthApp.seq[window.synthApp.selNote];
            const duplicate = { ...note, start: note.start + note.dur };
            window.synthApp.seq.push(duplicate);
            window.synthApp.selNote = window.synthApp.seq.length - 1;
            this.draw();
        }
    },

    quantizeSelectedNote() {
        if (typeof window.synthApp.selNote === "number" && window.synthApp.selNote >= 0 && window.LoopManager?.quantizeEnabled) {
            const note = window.synthApp.seq[window.synthApp.selNote];
            const grid = window.LoopManager.quantizeGrid;
