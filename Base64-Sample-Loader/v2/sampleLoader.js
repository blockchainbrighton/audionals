/*****************************************************************
 * Standalone Sample Loader Module
 *****************************************************************/

// ---------- Sample List ----------
// Export the list of sample URLs
export const ogSampleUrls = [
    { value: 'https://ordinals.com/content/e7d44ef3098d0889856978c4d2e81ccf2358f7f8b66feecc71e03036c59ad48i0#', text: 'OB1 #1 - 808 Kick' },
  ...
  // ---------- Internal State ----------
  // Store loaded AudioBuffers
  const loadedSamples = new Map();
  let audioContext = null;
  
  // ---------- Utility Functions ----------
  // Helper to trim whitespace and handle potential hash fragments
  function normalizeUrl(url) {
    return url.trim().split('#')[0]; // Remove potential fragment
  }
  
  // ---------- Core Loading Logic ----------
  // Adapted from the original loader.loadSample function
  async function loadSample(src) {
    ...
    if (loadedSamples.has(url)) {
      return loadedSamples.get(url);
    }
    ...
    try {
      ...
      if (contentType.startsWith('audio/')) {
         // Direct audio file (e.g., audio/wav, audio/mpeg)
        arrayBuffer = await response.arrayBuffer();
      } else if (contentType.includes('application/json')) {
         // JSON wrapper (like Tone.js format)
        const jsonData = await response.json();
        if (jsonData.audioData) {
          // Assume audioData is base64 encoded
          const byteString = atob(jsonData.audioData.split(',')[1]);
          const mimeString = jsonData.audioData.split(',')[0].split(':')[1].split(';')[0];
          ...
          arrayBuffer = ab;
           ...
           // Assume audioData is an ArrayBuffer or similar binary data
          arrayBuffer = jsonData.audioData;
        }
      } else {
         // Fallback: Try to parse as HTML and look for <audio src>
        const htmlText = await response.text();
        const audioMatch = htmlText.match(/<audio[^>]+src=["']([^"']+)["']/i) ||
                           htmlText.match(/src=["'](data:audio\/[^"']+)["']/i);
        if (audioMatch) {
          const audioSrc = audioMatch[1].startsWith('data:') ? audioMatch[1] : new URL(audioMatch[1], url).href;
          // Recursively call loadSample with the found audio source
          return await loadSample(audioSrc);
        } else {
          throw new Error(`Unsupported content type or format for ${url}`);
        }
      }
  
      // 4. Decode the audio data into an AudioBuffer
      const audioBuffer = await audioContext.decodeAudioData(arrayBuffer);
      loadedSamples.set(url, audioBuffer); // Cache the loaded buffer
      return audioBuffer;
  
    } catch (error) {
      console.error(`Error loading sample from ${url}:`, error);
      throw error; // Re-throw to be caught by caller (e.g., loadAllSamples)
    }
  }
  
  // ---------- Public API ----------
  /**
   * Initializes the AudioContext required for decoding audio.
  ...
  /**
   * Checks if the AudioContext has been initialized.
   */
  ...
  /**
   * Loads a single sample by its URL.
   * @param {string} url The URL of the sample to load.
   * @returns {Promise<AudioBuffer>} A promise resolving to the loaded AudioBuffer.
   */
  export async function loadSingleSample(url) {
  ...
  /**
   * Loads all samples defined in ogSampleUrls.
   * @param {Function} [onProgress] Optional callback (index, total, url, error) to report progress.
   * @returns {Promise<void>} A promise that resolves when all loading attempts are complete.
   */
  export async function loadAllSamples(onProgress) {
    if (!isAudioContextInitialized()) {
      throw new Error("AudioContext not initialized. Call initAudioContext first.");
    }
  
    const total = ogSampleUrls.length;
    let loadedCount = 0;
    let errorCount = 0;
  
    console.log("Starting to load all samples...");
    const loadPromises = ogSampleUrls.map(async ({ value: url }, index) => {
      const normalizedUrl = normalizeUrl(url);
      try {
        await loadSample(normalizedUrl);
        loadedCount++;
        if (onProgress) onProgress(index, total, normalizedUrl, null);
        // console.log(`Loaded (${loadedCount}/${total}): ${normalizedUrl}`);
      } catch (error) {
        errorCount++;
        if (onProgress) onProgress(index, total, normalizedUrl, error);
        console.error(`Failed to load (${errorCount}/${total}): ${normalizedUrl}`, error);
      }
    });
  
    await Promise.allSettled(loadPromises); // Wait for all promises, even if some reject
    console.log(`Finished loading samples. Success: ${loadedCount}, Failed: ${errorCount}`);
  }
  
  /**
   * Checks if all samples are successfully loaded and decodable.
   * @returns {boolean} True if all listed samples are loaded, false otherwise.
   */
  export function checkAllSamplesLoaded() {
    return ogSampleUrls.every(({ value: url }) => loadedSamples.has(normalizeUrl(url)));
  }
  
  /**
   * Retrieves a loaded AudioBuffer by its index in the ogSampleUrls array.
   * @param {number} index The index of the sample in ogSampleUrls.
   * @returns {AudioBuffer | null} The AudioBuffer if loaded, null otherwise.
   */
  export function getSampleByIndex(index) {
    if (index >= 0 && index < ogSampleUrls.length) {
      const url = normalizeUrl(ogSampleUrls[index].value);
      return loadedSamples.get(url) || null;
    }
    return null;
  }
  
  /**
   * Retrieves a loaded AudioBuffer by its URL.
   * @param {string} url The URL of the sample.
   * @returns {AudioBuffer | null} The AudioBuffer if loaded, null otherwise.
   */
  export function getSampleByUrl(url) {
    const normalizedUrl = normalizeUrl(url);
    return loadedSamples.get(normalizedUrl) || null;
  }
  
  /**
   * Gets the map of all loaded samples (URL -> AudioBuffer).
   * @returns {Map<string, AudioBuffer>} The map of loaded samples.
   */
  export function getLoadedSamplesMap() {
      return new Map(loadedSamples); // Return a copy to prevent direct modification
  }
  
  /**
   * Gets the list of sample metadata (URLs and names).
   * @returns {Array<{value: string, text: string}>} The list of sample URLs and names.
   */
  export function getSampleList() {
      return [...ogSampleUrls]; // Return a copy of the list
  }
