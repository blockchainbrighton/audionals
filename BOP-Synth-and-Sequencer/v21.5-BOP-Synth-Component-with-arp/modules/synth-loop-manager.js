...
export class LoopManager {
    ...
    setState(state = {}) {
        if ('enabled' in state) this.setLoopEnabled(!!state.enabled);
        if ('start' in state || 'end' in state) {
            const start = typeof state.start === 'number' ? state.start : this.loopStart;
            const end = typeof state.end === 'number' ? state.end : this.loopEnd;
            this.setLoopBounds(start, end);
        }
        if ('quantize' in state || 'grid' in state) {
            const enabled = 'quantize' in state ? !!state.quantize : this.quantizeEnabled;
            const gridValue = state.grid !== undefined ? state.grid : this.quantizeGridValue;
            this.setQuantization(enabled, gridValue);
        }
        if ('swing' in state) this.setSwing(state.swing);
    }
