...
function scheduleStep(time, stepIndex) {
    ...
    if (projectState.playMode === 'all' && projectState.sequences.length > 1) {
        const sequenceStepCount = getSequenceStepCount(seqData);
        const isLastStep = (stepIndex + 1) >= sequenceStepCount;
        if (isLastStep) {
...
function advanceToNextSequence() {
    ...
    const expectedSteps = getSequenceStepCount(nextSequence);
    if (toneSequence && toneSequence.events?.length !== expectedSteps) {
        createToneSequence();
    }
}
