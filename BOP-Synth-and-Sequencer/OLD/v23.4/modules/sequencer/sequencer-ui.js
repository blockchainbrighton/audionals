...
const advancedControlsState = {
    expanded: false,
    heightRaf: null
};

function applyAdvancedControlsState() {
    const { toggleControlsBtn, advancedControls } = elements;
    if (!toggleControlsBtn || !advancedControls) return;

    const expanded = advancedControlsState.expanded;
    toggleControlsBtn.setAttribute('aria-expanded', expanded ? 'true' : 'false');
    toggleControlsBtn.textContent = expanded ? '▲ Show Fewer Controls' : '▼ Show More Controls';

    advancedControls.setAttribute('aria-hidden', expanded ? 'false' : 'true');
    advancedControls.style.overflow = expanded ? 'visible' : 'hidden';
    if (expanded) {
...
function refreshAdvancedControlsHeight() {
    if (!advancedControlsState.expanded) return;
    const adv = elements.advancedControls;
    if (!adv) return;

    if (advancedControlsState.heightRaf !== null) {
        cancelAnimationFrame(advancedControlsState.heightRaf);
    }
    advancedControlsState.heightRaf = requestAnimationFrame(() => {
        advancedControlsState.heightRaf = null;
...
export function bindEventListeners() {
    ...
    if (elements.toggleControlsBtn && elements.advancedControls) {
        advancedControlsState.expanded = elements.toggleControlsBtn.getAttribute('aria-expanded') === 'true';
        applyAdvancedControlsState();
        elements.toggleControlsBtn.onclick = () => {
            advancedControlsState.expanded = !advancedControlsState.expanded;
            applyAdvancedControlsState();
            refreshAdvancedControlsHeight();
        };
    }
