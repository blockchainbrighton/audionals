...
export class ArpeggiatorEngine {
    ...
    startPlayback(syncDetail) {
        ...
        this.sequence.forEach(evt => {
            ...
            const scaledDur = Math.max(evt.dur * timeScale, 0.001);
            const offset = hasHost ? Math.max(0, baseDelta + scaledStart) : scaledStart;
            ...
            const id = transport.schedule((t) => {
                ...
                this.synthEngine.triggerAttackRelease(evt.note, scaledDur, t, evt.vel);
        ...
        if (hasHost) {
            const stopOffset = Math.max(0, baseDelta + sequenceDuration + 0.1);
            const stopId = transport.scheduleOnce(() => this.stopPlayback(), `+${stopOffset}`);
            this.scheduledEventIds.push(stopId);
        } else {
            const dur = sequenceDuration + 0.1;
            this.scheduledEventIds.push(
                transport.scheduleOnce(() => this.stopPlayback(), `+${dur}`)
            );
            transport.start();
