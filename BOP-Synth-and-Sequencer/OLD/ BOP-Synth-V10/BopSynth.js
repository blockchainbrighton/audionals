...
export class BopSynth {
    ...
    init() {
        try {
            ...
            this.transport = new Transport(
                this.uiElements.transport || '#transport',
                this.eventBus
            );
    ...
    wireUpEvents() {
        ...
        this.eventBus.addEventListener('save-project', (e) => {
            this.saveLoad.saveProject();
        });
        
        this.eventBus.addEventListener('load-project', (e) => {
            const { data } = e.detail;
            this.saveLoad.loadProject(data);
        });
        
        // Piano roll events
        this.eventBus.addEventListener('note-selected', (e) => {
            const { note } = e.detail;
            this.state.selNote = note;
        });
        
        this.eventBus.addEventListener('note-edited', (e) => {
            const { noteId, changes } = e.detail;
            this.recorder.editNote(noteId, changes);
        });
