...
export function createDefaultInsertSettings() {
    return {
        eq: { ...DEFAULT_INSERT_SETTINGS.eq },
        compressor: { ...DEFAULT_INSERT_SETTINGS.compressor },
        gate: { ...DEFAULT_INSERT_SETTINGS.gate }
    };
}
...
export function ensureChannelInsertSettings(channel) {
    ...
    ['eq', 'compressor', 'gate'].forEach(key => {
        const defaults = DEFAULT_INSERT_SETTINGS[key];
...
export function createNewChannel(type = 'sampler') {
    const channel = {
        ...
        solo: false,
        volume: 1,
        insertSettings: createDefaultInsertSettings()
    };
    if (type === 'sampler') {
    ...
    const clonedChannels = templateSequence.channels.map(sourceChannel => {
        ...
        const baseChannel = {
            ...
            volume: (typeof sourceChannel.volume === 'number' && !Number.isNaN(sourceChannel.volume))
                ...
                : 1,
            insertSettings: sourceChannel.insertSettings
                ? {
                    eq: { ...DEFAULT_INSERT_SETTINGS.eq, ...(sourceChannel.insertSettings.eq || {}) },
                    compressor: { ...DEFAULT_INSERT_SETTINGS.compressor, ...(sourceChannel.insertSettings.compressor || {}) },
                    gate: { ...DEFAULT_INSERT_SETTINGS.gate, ...(sourceChannel.insertSettings.gate || {}) }
                }
                : createDefaultInsertSettings()
        };
