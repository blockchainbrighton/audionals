...
function renderChannelInsertControls(channelData, chIndex) {
    ...
    wrapper.className = 'channel-inserts';
    const hasActivePlugin = INSERT_PLUGIN_CONFIG.some(plugin => channelData.insertSettings[plugin.id]?.enabled);
    wrapper.open = hasActivePlugin;

    const summary = document.createElement('summary');
    summary.textContent = 'Insert Plugins';
    ...
    INSERT_PLUGIN_CONFIG.forEach(plugin => {
        ...
        panel.className = 'channel-plugin-panel';
        panel.hidden = !pluginState.enabled;
        ...
        nameButton.addEventListener('click', () => {
            ...
            nameButton.classList.toggle('active', !panel.hidden);
        });

        header.append(toggleLabel, nameButton, status);
        pluginRow.append(header, panel);
        wrapper.appendChild(pluginRow);
    });
