...
export class EnhancedControls {
    ...
    setupEventListeners() {
        ...
        this.eventBus.addEventListener('preset-loaded', e => {
            const { preset, meta } = e.detail;
            this.loadPreset(preset, meta);
        });
    }
    ...
    updateControlValue(parameter, value) {
        const elements = this.panel.querySelectorAll(`[data-path="${parameter}"]`);
        ...
        elements.forEach((element) => {
            ...
            if (valueDisplay) {
                const formatter = getFormatter(element.id);
                valueDisplay.textContent = formatter(Number(element.value));
            }
        });
