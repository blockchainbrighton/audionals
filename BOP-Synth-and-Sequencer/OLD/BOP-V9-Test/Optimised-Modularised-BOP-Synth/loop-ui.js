...
/**
 * @file loop-ui.js
 * @description UI controls for the LoopManager.
 */

// Correctly imports the LoopManager singleton
...
export const LoopUI = {
    ...
    // --- FIX 1: Add the missing functions for PianoRoll to call ---
    isQuantizeEnabled() {
        // Provide a public method to get the state of the UI element
        return this.elements.quantizeEnabled?.checked ?? false;
    },

    getQuantizeGrid() {
        const gridKey = this.elements.quantizeGrid?.value;
        const gridMap = {
        ...
        const beatDuration = 60 / (window.Tone?.Transport.bpm.value || 120);
        return gridMap[gridKey] * beatDuration; // Return grid value in seconds
    },
    ...
    createUI() {
        ...
        // Your existing innerHTML code for the UI goes here...
        container.innerHTML = `
        ...
        bindEvents() {
            ...
            // The calls to LoopManager will now work because it's a singleton object
            ...
            this._on(els.quantizeEnabled, 'change', e => {
                const gridValue = this.getQuantizeGrid();
                LoopManager.setQuantization(e.target.checked, gridValue);
