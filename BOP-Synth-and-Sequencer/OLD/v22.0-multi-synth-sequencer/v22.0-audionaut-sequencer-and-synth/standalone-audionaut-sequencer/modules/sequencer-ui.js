...
import { listAvailableSynths } from './synth-registry.js';


let resizeRafId = null;

let synthOptions = [];
let synthOptionsPromise = null;
let synthOptionsError = null;

function ensureSynthOptionsLoaded() {
    if (!synthOptionsPromise) {
        synthOptionsPromise = listAvailableSynths()
            .then(options => {
                synthOptions = options;
                return options;
            })
            .catch(err => {
                synthOptionsError = err;
                console.error('[UI] Failed to load synth manifest:', err);
                return [];
            });
    }
    return synthOptionsPromise;
}

function getSynthNameById(synthId) {
    if (!synthId) return 'Empty Instrument';
    const entry = synthOptions.find(item => item.id === synthId);
    return entry ? entry.name : synthId;
}
...
function renderInstrumentControls(infoContainer, channelData, chIndex) {
    ...
    if (synthOptions.length) {
        ...
        select.value = channelData.synthId || select.options[0]?.value || '';
    } else if (synthOptionsError) {
        const opt = document.createElement('option');
        opt.value = '';
        opt.textContent = 'Synth manifest failed';
        select.appendChild(opt);
        select.disabled = true;
    } else {
        const opt = document.createElement('option');
        opt.value = '';
        opt.textContent = 'Loading synths...';
        select.appendChild(opt);
        select.disabled = true;
        ensureSynthOptionsLoaded().then(() => render());
    }
    ...
    select.onchange = () => {
        ...
        if (value) {
            ...
            sequence.channels[chIndex].synthId = value;
        }
    };

    instrumentControls.appendChild(select);

    const loadBtn = document.createElement('button');
    loadBtn.textContent = channelData.instrumentId ? 'Replace' : 'Load';
    loadBtn.disabled = !synthOptions.length;
    loadBtn.onclick = async () => {
        const synthId = select.value || channelData.synthId;
        if (!synthId) {
            alert('Choose a synth before loading.');
            return;
        }
        await createInstrumentForChannel(projectState.currentSequenceIndex, chIndex, synthId);
        render();
    };
...
export function render() {
    ...
    if (!currentSeq) return;
        ...
        if (channelData.type === 'sampler') {
            ...
            label.textContent = getSynthNameById(channelData.synthId) || 'Empty Instrument';
