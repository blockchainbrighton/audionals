...
import { projectState, runtimeState, initializeProject, syncNextInstrumentIdAfterLoad } from './sequencer-state.js';
                ...
                return {
                    channels: seq.channels.map((chan, chanIndex) => {
                        ...
                        if (chan.type === 'sampler') {
                            ...
                            newChan.synthId = chan.synthId || null;
                            console.log(`[SAVE]  -> Instrument channel ${chanIndex} synthId: ${newChan.synthId}`);
                            const instrument = chan.instrumentId ? runtimeState.instrumentRack[chan.instrumentId] : null;
                            if (instrument) {
                                try {
                                    newChan.patch = instrument.capturePatch();
                                ...
                                console.warn(`[SAVE]  -> Instrument ${chan.instrumentId} not active, saving stored patch.`);
                                newChan.patch = chan.patch || null;
                            }
                        }
                        return newChan;
        ...
        projectState.sequences.forEach((seq, seqIndex) => {
            ...
            seq.channels.forEach((chan, chanIndex) => {
                if (chan.type === 'instrument') {
                    ...
                    try {
                        loadPromises.push(createInstrumentForChannel(seqIndex, chanIndex, chan.synthId, chan.patch));
                    } catch (error) {
