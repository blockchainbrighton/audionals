...
const expandedStepChannels = new Set();

const getExpandedStepsKey = (sequenceIndex, channelIndex) => `${sequenceIndex}:${channelIndex}`;
...
function renderStepGrid(channelEl, channelData, chIndex) {
    ...
    const sequenceIndex = projectState.currentSequenceIndex ?? 0;
    const expandedKey = getExpandedStepsKey(sequenceIndex, chIndex);
    const isExpanded = expandedStepChannels.has(expandedKey);
    if (isExpanded) {
        stepsContainer.classList.add('steps--expanded');
    ...
    stepsContainer.addEventListener('dblclick', event => {
        event.preventDefault();
        event.stopPropagation();
        if (expandedStepChannels.has(expandedKey)) {
            expandedStepChannels.delete(expandedKey);
        } else {
            expandedStepChannels.add(expandedKey);
        }
        render();
    });

    channelEl.classList.toggle('channel--steps-expanded', isExpanded);
