...
export class WaveformEditorModal {
    ...
    getAudioCurrentTime() {
        ...
        if (!Tone) return null;
        try {
            const contextCandidate = typeof Tone.getContext === 'function' ? Tone.getContext() : Tone.context;
            const raw =
                contextCandidate?.rawContext ||
                contextCandidate?.context ||
                contextCandidate?._context ||
                contextCandidate?._nativeAudioContext ||
                contextCandidate?.audioContext ||
            ...
            console.warn('[WaveformModal] Unable to resolve audio context time:', err);
        }
        return null;
    }
