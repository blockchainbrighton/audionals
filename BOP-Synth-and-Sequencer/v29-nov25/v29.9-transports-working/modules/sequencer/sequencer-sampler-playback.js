...
export function playSamplerChannel(time, channelData, samplerOverride = null, allowOverlapOverride = null) {
    ...
    const samplePlaybackRate = typeof source?.playbackRate === 'number' && !Number.isNaN(source.playbackRate)
        ? source.playbackRate
        : 1;
    const sampleFadeIn = typeof source?.fadeIn === 'number' ? Math.max(0, source.fadeIn) : 0.005;
    const sampleFadeOut = typeof source?.fadeOut === 'number' ? Math.max(0, source.fadeOut) : 0.05;
    ...
    const releaseEstimate = selectionDuration + sampleFadeOut + 0.05;
    ...
    const availableTail = Math.max(0, bufferDuration - offsetSeconds - selectionDuration);
    const tailAllowance = Math.min(sampleFadeOut, availableTail);
    const playbackDuration = selectionDuration + Math.max(0, tailAllowance);

    voiceEntry.ampEnv.triggerAttackRelease(selectionDuration, time);
    voiceEntry.player.start(time, offsetSeconds, playbackDuration);
    voiceEntry.isPlaying = true;
    voiceEntry.busyUntil = time + releaseEstimate;
    ...
    if (typeof window !== 'undefined' && window?.dispatchEvent && audioStartTime !== null) {
        const actualDuration = Math.max(0.01, playbackDuration / Math.max(0.0001, samplePlaybackRate));
        try {
