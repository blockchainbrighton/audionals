...
export class EnhancedRecorder {
    ...
    startPlayback(syncDetail) {
        ...
        playableEvents.forEach(item => {
            ...
            const offset = hasHost
                ? Math.max(0, baseDelta + scaledStart)
                : Math.max(0, loopActive ? (effectiveStart - loopStart) : effectiveStart);
        ...
        const transportStart = loopActive
            ? loopStart
            : (hasHost ? (hostTransportTime ?? transport.seconds) : transport.seconds);
