...
function configureToneTransportTiming() {
    const Tone = runtimeState.Tone;
    if (!Tone?.Transport) return;
    try {
        Tone.Transport.lookAhead = 0;
        if (typeof Tone.Transport.updateInterval === 'number') {
            Tone.Transport.updateInterval = Math.max(0.01, Math.min(0.05, TRANSPORT_START_DELAY / 2));
        }
    } catch (err) {
        console.warn('[AUDIO] Unable to configure Tone.Transport timing:', err);
    }
}
...
export function stopPlayback() {
    ...
    runtimeState.lastStepIndex = -1;
    runtimeState.currentPlaybackSequenceIndex = projectState.currentSequenceIndex;
    runtimeState.currentStepScheduledTime = null;
    runtimeState.stepDurationSeconds = null;
