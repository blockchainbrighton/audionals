...
function handleNoteOn(instrumentId, note, velocity) {
    ...
    if (session.mode === 'live' && projectState.isPlaying) {
        ...
        const stepsPerLoop = Math.max(Array.isArray(session.channel?.steps) ? session.channel.steps.length : TOTAL_STEPS, TOTAL_STEPS);
        ...
        const baseSteps = Math.max(0, currentCycle * TOTAL_STEPS + currentStep);
        ...
        const audibleStepTime = Number.isFinite(scheduledTime) ? scheduledTime + latency : null;
        let stepProgress = Number.isFinite(audibleStepTime) ? now - audibleStepTime : 0;
        ...
        const absoluteTime = baseTime + Math.max(0, stepProgress);
    ...
    try {
        console.groupCollapsed('[RecordDiag] Note captured');
        console.log({
            instrumentId,
            note,
            ...
            relStart,
            now,
            transportSeconds: runtimeState.Tone?.Transport?.seconds ?? null,
            timingDetail
        });
        console.groupEnd();
    } catch {}
