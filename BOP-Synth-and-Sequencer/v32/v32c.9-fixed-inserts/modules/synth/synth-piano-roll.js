/**
 * @file PianoRoll.js
 * @description Piano roll UI component shared by sequencer instruments.
 */
...
export class PianoRoll {
    ...
    quantizeSnapshots = new Map();
    _quantizeGlobalMode = false;
    _quantizeRebroadcasting = false;
    ...
    constructor(containerElement, eventBus, state) {
        ...
        this._boundGlobalStep = this.#handleTransportStep.bind(this);
        this._boundTransportStop = this.#handleTransportStop.bind(this);
        ...
        this.drawModeButton = null;
        this.isDragging = false;
        this._keyListenersAttached = false;
        this._shortcutListenerAttached = false;
        ...
        this._pendingDrawWhileHidden = false;
        this._pendingPlayheadOnResume = false;
    ...
    #createUI() {
        ...
        this.quantizeToggle.addEventListener('change', () => {
            this.#setQuantizeEnabled(this.quantizeToggle.checked, { preserveToggle: true });
        });
        ...
        this.quantizeSelect.addEventListener('change', () => {
            ...
            if (Number.isFinite(value)) {
                ...
                if (this.quantizeEnabled && this.quantizeSnapshots.size) {
                    mutated = this.#applyQuantizeSnapshots({ gridSeconds: this.#gridSeconds(value) });
                }
                this.draw();
            }
            if (mutated) {
        ...
        this.#ensureGlobalEventHooks();
        this.#updateVelocityControls();
        this.#syncQuantizeControls();
        this.#updateCursorVisual();
    }

    #ensureGlobalEventHooks() {
        if (!this._viewportActive) return;
        if (!this._keyListenersAttached && document?.addEventListener) {
            document.addEventListener('keydown', this._boundKeyDown);
            document.addEventListener('keyup', this._boundKeyUp);
            this._keyListenersAttached = true;
        }
        if (this._keyListener && !this._shortcutListenerAttached && document?.addEventListener) {
            document.addEventListener('keydown', this._keyListener);
            this._shortcutListenerAttached = true;
        }
        if (!this._resizeListenerAttached && typeof window?.addEventListener === 'function') {
            window.addEventListener('resize', this._boundResize);
            this._resizeListenerAttached = true;
        }
        if (!this._transportListenersAttached && typeof window?.addEventListener === 'function') {
            window.addEventListener('step', this._boundGlobalStep);
            window.addEventListener('transport-stop', this._boundTransportStop);
            this._transportListenersAttached = true;
        }
        this.#attachGlobalDragHandlers();
    }

    #releaseGlobalEventHooks() {
        if (this._keyListenersAttached && document?.removeEventListener) {
            document.removeEventListener('keydown', this._boundKeyDown);
            document.removeEventListener('keyup', this._boundKeyUp);
            this._keyListenersAttached = false;
        }
        if (this._shortcutListenerAttached && document?.removeEventListener) {
            document.removeEventListener('keydown', this._keyListener);
            this._shortcutListenerAttached = false;
        }
        if (this._resizeListenerAttached && typeof window?.removeEventListener === 'function') {
            window.removeEventListener('resize', this._boundResize);
            this._resizeListenerAttached = false;
        }
        if (this._transportListenersAttached && typeof window?.removeEventListener === 'function') {
            window.removeEventListener('step', this._boundGlobalStep);
            window.removeEventListener('transport-stop', this._boundTransportStop);
    ...
    #createToolControls() {
        ...
        const drawBtn = this.#btn('Draw', () => this.#toggleDrawMode());
        drawBtn.title = 'Toggle draw mode to add new notes';
        drawBtn.setAttribute('aria-pressed', 'false');
        drawBtn.classList.add('piano-roll-tool-button');
        this.drawModeButton = drawBtn;
    ...
    #toggleDrawMode(forceState = null) {
        const nextState = typeof forceState === 'boolean' ? forceState : !this.isDrawMode;
        this.isDrawMode = nextState;
        if (this.drawModeButton) {
            this.drawModeButton.setAttribute('aria-pressed', String(nextState));
            this.drawModeButton.classList.toggle('is-active', nextState);
        }
        if (this.scrollContainer) {
            this.scrollContainer.classList.toggle('draw-mode', nextState);
            this.scrollContainer.style.cursor = nextState ? 'crosshair' : '';
        }
        if (nextState) {
    ...
    #quantizeSelection() {
        ...
        if (!Number.isFinite(gridSeconds) || gridSeconds <= 0) return;
        const targetIds = [];
        selected.forEach(note => {
            const id = this.#ensureNoteId(note);
            if (!id) return;
            if (!this.quantizeSnapshots.has(id)) {
                this.quantizeSnapshots.set(id, {
                    start: Number.isFinite(note.start) ? note.start : 0,
                    dur: Number.isFinite(note.dur) ? note.dur : gridSeconds
                });
            }
            targetIds.push(id);
        });
        if (!targetIds.length) return;
        const mutated = this.#applyQuantizeSnapshots({ targetIds, gridSeconds });
        this.#setQuantizeEnabled(true, { applyToAll: false });
        if (!mutated) return;
    ...
    #applyQuantizeSnapshots({ targetIds = null, gridSeconds = null } = {}) {
        if (!this.quantizeSnapshots.size) return false;
        this.#pruneQuantizeSnapshots();
        const seq = this.state.seq || [];
        const targetSet = targetIds ? new Set(targetIds) : null;
        const grid = Number.isFinite(gridSeconds) && gridSeconds > 0
            ? gridSeconds
            : this.#gridSeconds();
        ...
        seq.forEach(note => {
            const id = this.#ensureNoteId(note);
            if (!this.quantizeSnapshots.has(id)) return;
            if (targetSet && !targetSet.has(id)) return;
            const snapshot = this.quantizeSnapshots.get(id);
            const snappedStart = this.#quantizeValue(snapshot?.start ?? note.start ?? 0, grid);
            const snappedDur = Math.max(grid, this.#quantizeValue(snapshot?.dur ?? note.dur ?? grid, grid));
            if (note.start !== snappedStart || note.dur !== snappedDur) {
    ...
    #restoreQuantizeSnapshots() {
        ...
        seq.forEach(note => {
            ...
            const snapshot = this.quantizeSnapshots.get(id);
            if (!snapshot) return;
            if (note.start !== snapshot.start || note.dur !== snapshot.dur) {
                note.start = snapshot.start;
                note.dur = snapshot.dur;
                mutated = true;
            }
            this.quantizeSnapshots.delete(id);
        });
        if (mutated) {
            this.#sortSequenceByStart();
        }
    ...
    #snapshotAllNotes() {
        ...
        seq.forEach(note => {
            const id = this.#ensureNoteId(note);
            if (!id || this.quantizeSnapshots.has(id)) return;
            const start = Number.isFinite(note.start) ? note.start : 0;
            const dur = Number.isFinite(note.dur) ? note.dur : this.#defaultStepSeconds();
            this.quantizeSnapshots.set(id, { start, dur });
            added = true;
        });
        return added;
    }

    #pruneQuantizeSnapshots() {
        if (!this.quantizeSnapshots.size) return;
        const seq = this.state.seq || [];
        if (!seq.length) {
            this.quantizeSnapshots.clear();
            return;
        }
        const validIds = new Set(seq.map(note => this.#ensureNoteId(note)));
        Array.from(this.quantizeSnapshots.keys()).forEach(id => {
            if (!validIds.has(id)) this.quantizeSnapshots.delete(id);
        });
    }

    #quantizeValue(value, gridSeconds) {
        if (!Number.isFinite(value)) return 0;
        if (!Number.isFinite(gridSeconds) || gridSeconds <= 0) return value;
        const ticks = Math.round(value / gridSeconds);
        return Number((ticks * gridSeconds).toFixed(6));
    }

    #setQuantizeEnabled(next, { emitEvent = true, preserveToggle = false, applyToAll = true } = {}) {
        const normalized = !!next;
        const previous = this.quantizeEnabled;
        if (!preserveToggle && this.quantizeToggle) {
            this.quantizeToggle.checked = normalized;
        }
        this.quantizeEnabled = normalized;
        let mutated = false;
        if (normalized && !previous) {
            this._quantizeGlobalMode = !!applyToAll;
            if (applyToAll) {
                this.#snapshotAllNotes();
                mutated = this.#applyQuantizeSnapshots({ gridSeconds: this.#gridSeconds() }) || mutated;
            }
        } else if (!normalized && previous) {
            this._quantizeGlobalMode = false;
            if (this.quantizeSnapshots.size) {
                mutated = this.#restoreQuantizeSnapshots() || mutated;
            }
        }
        if (emitEvent && previous !== normalized) {
        ...
        if (previous !== normalized || mutated) {
            this.draw();
        }
        return mutated;
    }
    ...
    #deleteSelection() {
        ...
        for (let i = seq.length - 1; i >= 0; i--) {
            const noteId = this.#ensureNoteId(seq[i]);
            if (this.selectedNotes.has(noteId)) {
                this.quantizeSnapshots.delete(noteId);
                seq.splice(i, 1);
            }
        }
        this.selectedNotes.clear();
    ...
    #handleDrawPointer(event) {
        ...
        const seq = Array.isArray(this.state?.seq) ? this.state.seq : (this.state.seq = []);
        const rect = this.scrollContainer.getBoundingClientRect();
        const scrollLeft = this.scrollContainer.scrollLeft;
        const scrollTop = this.scrollContainer.scrollTop;
        const relX = event.clientX - rect.left + scrollLeft;
        const relY = event.clientY - rect.top + scrollTop;
        const cellW = Math.max(1e-6, this.cellWidth || (40 * this.zoomX));
        const cellH = Math.max(1e-6, this.CELL_H * this.zoomY);
        const totalRows = (this.MIDI_HIGH - this.MIDI_LOW) + 1;
        const clampedY = Math.max(0, Math.min(relY, totalRows * cellH - 1));
        const rowIndex = Math.floor(clampedY / cellH);
        const midi = Math.max(this.MIDI_LOW, Math.min(this.MIDI_HIGH, this.MIDI_HIGH - rowIndex));
        const noteName = Tone.Frequency(midi, 'midi').toNote();
        const baseGrid = this.quantizeEnabled ? this.#gridSeconds() : this.#defaultStepSeconds();
        const safeGrid = Math.max(1e-3, baseGrid || this.#defaultStepSeconds());
        const startSecondsRaw = Math.max(0, relX / cellW);
        const startSeconds = this.quantizeEnabled
            ? Math.round(startSecondsRaw / safeGrid) * safeGrid
            : startSecondsRaw;
        const velocityValue = parseFloat(this.velocitySlider?.value ?? '1');
        const vel = Number.isFinite(velocityValue) ? Math.max(0, Math.min(1, velocityValue)) : 1;
        const noteEntry = {
            note: noteName,
            start: startSeconds,
            dur: safeGrid,
        ...
        noteEntry.id = this.#generateNoteId(noteEntry);
        seq.push(noteEntry);
        this.#sortSequenceByStart();
        this.selectedNotes = new Set([noteEntry.id]);
        this.#onSelectionChanged();
        this.eventBus.dispatchEvent(new CustomEvent('sequence-changed'));
        this.draw();
    }
    ...
    setViewportActive(active) {
        const next = !!active;
        if (this._viewportActive === next) return;
        this._viewportActive = next;
        if (next) {
            this.#ensureGlobalEventHooks();
            const shouldRedraw = this._pendingDrawWhileHidden;
            this._pendingDrawWhileHidden = false;
            if (shouldRedraw) {
                this.draw();
            }
            const shouldResume = this._pendingPlayheadOnResume || !!this.state?.isPlaying;
            this._pendingPlayheadOnResume = false;
            if (shouldResume) {
        ...
        this._pendingPlayheadOnResume = this.state?.isPlaying || !!this.playheadRaf;
        this.stopPlayheadAnimation({ preserveResumeFlag: true });
        this.#releaseGlobalEventHooks();
    }
    ...
    #setupEventListeners() {
        ...
        this.eventBus.addEventListener('sequence-changed', () => {
            ...
            this.#pruneQuantizeSnapshots();
            if (this.quantizeEnabled && this._quantizeGlobalMode && !this._quantizeRebroadcasting) {
                const addedSnapshots = this.#snapshotAllNotes();
                if (addedSnapshots) {
                    const mutatedQuant = this.#applyQuantizeSnapshots({ gridSeconds: this.#gridSeconds() });
                    if (mutatedQuant) {
                        this._quantizeRebroadcasting = true;
                        try {
                            this.eventBus.dispatchEvent(new CustomEvent('sequence-changed'));
                        } finally {
                            this._quantizeRebroadcasting = false;
                        }
                    }
                }
            }
        ...
        this.eventBus.addEventListener('loop-state-update', e => {
            ...
            let mutated = false;
            if (typeof detail.quantizeEnabled === 'boolean') {
                this.#setQuantizeEnabled(detail.quantizeEnabled, { emitEvent: false, applyToAll: true });
            }
            if (typeof detail.quantizeGridValue === 'number') {
                this.quantizeGrid = detail.quantizeGridValue;
                if (this.quantizeEnabled && this.quantizeSnapshots.size) {
                    mutated = this.#applyQuantizeSnapshots({ gridSeconds: this.#gridSeconds() }) || mutated;
                }
            }
        ...
        this.eventBus.addEventListener('quantization-grid-changed', e => {
            if (typeof e.detail?.gridValue === 'number') {
                ...
                if (this.quantizeEnabled && this.quantizeSnapshots.size) {
                    mutated = this.#applyQuantizeSnapshots({ gridSeconds: this.#gridSeconds() });
                }
                this.#syncQuantizeControls();
                if (mutated) {
                    this.eventBus.dispatchEvent(new CustomEvent('sequence-changed'));
    ...
    draw() {
        ...
        if (!this._viewportActive) {
            this._pendingDrawWhileHidden = true;
            return;
        }
        this._pendingDrawWhileHidden = false;
    ...
    startPlayheadAnimation() {
        if (!this._viewportActive) {
            this._pendingPlayheadOnResume = true;
            return;
        }
    ...
    stopPlayheadAnimation({ preserveResumeFlag = false } = {}) {
        if (this.playheadRaf) {
    ...
    updatePlayhead() {
        if (!this.innerContent || !this.scrollContainer || !this._viewportActive) return;
        ...
        this.#renderPlayheadAt(displayTime);
    }

    #renderPlayheadAt(displayTime) {
        if (!this.scrollContainer || !this.innerContent) return;
        const pixelScale = this.cellWidth || (40 * this.zoomX);
        ...
        ph.style.left = `${Math.max(0, displayTime) * pixelScale + 48}px`;
        ph.style.height = `${this.innerContent.scrollHeight}px`;
    }

    #handleTransportStep(event) {
        if (!this._viewportActive) return;
        const detail = event?.detail || {};
        const stepIndex = Number(detail.stepIndex);
        if (!Number.isFinite(stepIndex)) return;
        const normalized = ((stepIndex % this.DEFAULT_SEQUENCE_STEPS) + this.DEFAULT_SEQUENCE_STEPS) % this.DEFAULT_SEQUENCE_STEPS;
        const stepSeconds = this.#defaultStepSeconds();
        const displayTime = normalized * stepSeconds;
        if (this.transportInfo) {
            this.transportInfo.textContent = `Transport: ${this.#formatTransport(displayTime)}`;
        }
        this.#renderPlayheadAt(displayTime);
    }
    ...
    destroy() {
        ...
        this.stopPlayheadAnimation();
        if (this._keyListener && document?.removeEventListener) {
            document.removeEventListener('keydown', this._keyListener);
        }
