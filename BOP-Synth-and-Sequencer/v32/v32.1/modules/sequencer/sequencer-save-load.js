...
import { projectState, runtimeState, initializeProject, syncNextInstrumentIdAfterLoad, ensureChannelMixDefaults, ensureSamplerChannelDefaults, ensureChannelInsertSettings, createDefaultInsertSettings, pruneInactiveSampleCaches } from './sequencer-state.js';
        ...
        projectState.sequences.forEach(seq => {
            ...
            seq.channels.forEach(chan => {
                ...
                if (chan.type === 'instrument') {
                    if (chan.instrumentClip && typeof chan.instrumentClip === 'object') {
                        chan.instrumentClip = JSON.parse(JSON.stringify(chan.instrumentClip));
                    } else {
                        chan.instrumentClip = null;
                    }
                }
            });
        });
