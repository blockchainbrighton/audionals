...
function cloneClipPayload(clip) {
    if (!clip) return null;
    return {
        meta: { ...(clip.meta || {}) },
        events: Array.isArray(clip.events) ? clip.events.map(evt => ({ ...evt })) : []
    };
}

function normalizeClipPayload(detail) {
    if (!detail) return null;
    const raw = detail.clip || detail;
    if (!raw) return null;
    const normalized = {
        meta: { ...(raw.meta || {}) },
        events: Array.isArray(raw.events) ? raw.events.map(evt => {
            const vel = typeof evt.vel === 'number'
                ? evt.vel
                : (typeof evt.velocity === 'number' ? evt.velocity : 0.85);
            const start = typeof evt.start === 'number' ? Math.max(0, evt.start) : 0;
            const dur = typeof evt.dur === 'number' ? Math.max(0.001, evt.dur) : 0.001;
            return {
                id: evt.id || null,
                note: evt.note,
                start,
    ...
    if (typeof normalized.meta.recordBpm !== 'number') {
        normalized.meta.recordBpm = projectState.bpm || 120;
    }
    return normalized;
}

function storeInstrumentClip(channel, instrumentId, clipDetail) {
    const clip = cloneClipPayload(clipDetail);
    if (channel) {
        channel.instrumentClip = clip ? cloneClipPayload(clip) : null;
    }
    if (!instrumentId) return;
    if (clip) {
        runtimeState.instrumentClips.set(instrumentId, cloneClipPayload(clip));
    } else {
        runtimeState.instrumentClips.delete(instrumentId);
    }
}
...
export function registerInstrumentLiveSession({ instrumentId, logic, channel }) {
    ...
    const clipHandler = event => {
        if (!logic?.isSequencerEmbed) return;
        const clip = normalizeClipPayload(event?.detail);
        storeInstrumentClip(channel, instrumentId, clip);
        ensureInstrumentStep(channel);
        notifySubscribers();
    };
    ...
    if (logic?.isSequencerEmbed) {
        eventBus.addEventListener('sequencer-clip-updated', clipHandler);
        if (typeof logic.publishSequencerClip === 'function') {
            logic.publishSequencerClip('registration');
        }
    }
    instrumentBindings.set(instrumentId, { eventBus, recordingHandler, sequenceHandler, clipHandler });
    if (logic?.isSequencerEmbed && channel.instrumentClip) {
        storeInstrumentClip(channel, instrumentId, channel.instrumentClip);
    }
...
export function unregisterInstrumentLiveSession(instrumentId) {
    ...
    if (binding) {
        const { eventBus, recordingHandler, sequenceHandler, clipHandler } = binding;
        ...
        if (clipHandler) {
            try { eventBus.removeEventListener('sequencer-clip-updated', clipHandler); } catch {}
        }
