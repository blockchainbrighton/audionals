...
function getDisplaySequence(record) {
    if (!record) return [];
    const channelClip = record.currentChannel?.instrumentClip;
    if (channelClip && Array.isArray(channelClip.events) && channelClip.events.length) {
        return channelClip.events;
    }
    const seq = record.instrument?.logic?.state?.seq;
...
function updateNoteSummary(record) {
    ...
    const seq = getDisplaySequence(record);
    if (!seq.length) {
