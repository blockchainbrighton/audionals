...
export class SequencerSchedulerHost {
    ...
    triggerInstrument(channel, trigger, scheduledTime, stepIndex, cycle, globalStep = null, options = {}) {
        ...
        const isEmbed = instrument.isSequencerEmbed || logic?.isSequencerEmbed;
        if (!recorder && !isEmbed) return;
        ...
        let playbackPromise;
        if (isEmbed) {
            playbackPromise = this.playEmbeddedInstrument(channel, instrument, scheduledTime);
        } else {
            const transportTime = this.computeTransportTimeForGlobalStep(
                Number.isFinite(globalStep)
                    ? globalStep
                    : (cycle * this.latestStepCount) + stepIndex
            );
            playbackPromise = instrument.playInternalSequence({
                transportTime: typeof transportTime === 'number' ? transportTime : undefined,
                audioTime: scheduledTime,
                stepIndex,
                cycle,
                forceRestart: force || !!playbackState
            });
        }
    ...
    playEmbeddedInstrument(channel, instrument, scheduledTime) {
        if (!channel || !instrument) return null;
        const clip = channel.instrumentClip || runtimeState.instrumentClips?.get?.(channel.instrumentId);
        if (!clip || !Array.isArray(clip.events) || !clip.events.length) return null;
        const synthEngine = instrument.logic?.modules?.synthEngine;
        if (!synthEngine?.triggerAttackRelease) return null;
        try {
            synthEngine.releaseAll?.();
        } catch (err) {
            console.warn('[SchedulerHost] Failed to reset synth before scheduling clip:', err);
        }
        const currentBpm = projectState.bpm || 120;
        const sourceBpm = clip.meta?.recordBpm || currentBpm;
        const timeScale = sourceBpm / currentBpm;
        const baseTime = Number.isFinite(scheduledTime)
            ? scheduledTime
            : (synthEngine.getImmediateTime?.() ?? runtimeState.Tone?.now?.() ?? 0);
        clip.events.forEach(evt => {
            if (!evt || typeof evt.note === 'undefined') return;
            const scaledStart = Math.max(0, (evt.start ?? 0) * timeScale);
            const scaledDur = Math.max((evt.dur ?? 0) * timeScale, 0.001);
            const velocity = typeof evt.vel === 'number'
                ? evt.vel
                : (typeof evt.velocity === 'number' ? evt.velocity : 0.85);
            const scheduledNoteTime = baseTime + scaledStart;
            try {
                synthEngine.triggerAttackRelease(evt.note, scaledDur, scheduledNoteTime, velocity);
            } catch (err) {
                console.warn('[SchedulerHost] Failed to trigger embedded synth note:', err);
            }
        });
    ...
    async primeInstrumentStep(stepIndex, cycle, scheduledTime, sequenceOverride = null, globalStep = 0) {
        ...
        channels.forEach((channel, index) => {
            ...
            const result = this.triggerInstrument(channel, trigger, scheduledTime, stepIndex, cycle, globalStep, { force: true });
            if (result && typeof result.then === 'function') {
                readinessPromises.push(result.catch(err => {
                    console.warn('[SchedulerHost] Instrument prime failed:', err);
                }));
            }
        });
        if (readinessPromises.length) {
            ...
            const timeoutPromise = new Promise(resolve => {
                timeoutId = setTimeout(() => resolve('timeout'), INSTRUMENT_PLAYBACK_READY_TIMEOUT_MS);
            });
            await Promise.race([
                Promise.allSettled(readinessPromises),
                timeoutPromise
            ]);
            if (timeoutId) {
