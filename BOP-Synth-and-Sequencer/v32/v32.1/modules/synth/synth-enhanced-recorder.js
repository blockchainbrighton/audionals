...
export class EnhancedRecorder {
    constructor(state, synthEngine, eventBus, options = {}) {
        this.state = state;
        this.synthEngine = synthEngine;
        this.eventBus = eventBus;
        this.embedMode = !!options.embedMode;
    ...
    abortCurrentPlayback({
        ...
        const transport = this.embedMode ? null : this.synthEngine?.Tone?.Transport;
    ...
    startRecording(firstNote = null, velocity = 0.8) {
        ...
        this.emitClipUpdate('start-recording');
        if (firstNote) {
    ...
    startPlayback(syncDetail) {
        ...
        if (this.embedMode) {
            ...
            this.startedOwnTransport = false;
            let hostTransportTime = 0;
            if (typeof syncDetail === 'number') hostTransportTime = syncDetail;
    ...
    reduceDensity(maxEvents = 8) {
        ...
        this.emitClipUpdate('reduce-density');
        return true;
    }
    ...
    getSequenceSnapshot() {
        const snapshot = this.getSequence({ clone: true });
        if (!snapshot) return null;
        if (typeof snapshot.meta.recordBpm !== 'number') {
            snapshot.meta.recordBpm = this.recordBpm || this.getTransportBpm();
        }
        return snapshot;
    }

    emitClipUpdate(reason = 'manual') {
        if (!this.embedMode) return;
        const clip = this.getSequenceSnapshot();
        this.eventBus.dispatchEvent(new CustomEvent('sequencer-clip-updated', {
            detail: {
                clip,
                reason,
                mode: 'sequencer-embed'
    ...
    setSequence(sequencePayload, meta = null) {
        ...
        if (Array.isArray(events)) {
            ...
            this.updateState();
            this.emitClipUpdate('set-sequence');
        } else {
