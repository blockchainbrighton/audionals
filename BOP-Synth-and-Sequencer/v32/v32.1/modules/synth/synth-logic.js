...
export class BopSynthLogic {
    constructor(Tone, options = {}) {
        ...
        this.mode = options.hostMode || options.mode || 'standalone';
        this.isSequencerEmbed = this.mode === 'sequencer' || !!options.sequencerEmbed;
    ...
    init() {
        ...
        this.modules.recorder = new EnhancedRecorder(
            this.state,
            this.modules.synthEngine,
            this.eventBus,
            { embedMode: this.isSequencerEmbed }
        );
        ...
        this.wireUpEvents();
        if (this.isSequencerEmbed) {
            this.setupSequencerEmbedBridge();
        }
    ...
    setupSequencerEmbedBridge() {
        const emitClip = (reason = 'sequence-changed') => {
            const recorder = this.modules?.recorder;
            if (recorder?.emitClipUpdate) {
                recorder.emitClipUpdate(reason);
            }
        };
        this.publishSequencerClip = reason => emitClip(reason || 'manual');
        emitClip('init');
        this.eventBus.addEventListener('sequence-changed', () => emitClip('sequence-changed'));
    }
