...
- Each entry in `runtimeState.instrumentRack` stores `{ logic, getPatch }`; channel clips live on the sequencer via `ensureInstrumentClip`, and the scheduler schedules those events directly while patches only cover engine/UI state (`modules/sequencer/sequencer-state.js`, `modules/sequencer/sequencer-scheduler-host.js:360`).
- Channels now expose a record toggle in the instrument controls that activates `SimpleSynthLogic` note capture into `channel.instrumentClip`, using `modules/sequencer/instrument-live-controller.js` helpers to manage clip writes and playback resync.
- Third-party logic controllers must ship an `eventBus` that reacts to keyboard/MIDI + parameter events (`'keyboard-note-on'`, `'keyboard-note-off'`, `'midi-note-on'`, `'note-preview'`, `'parameter-change'`, etc.) as demonstrated in `SimpleSynthLogic.wireUpEvents()` (`modules/simple-synth/simple-synth-logic.js`) so the piano roll can preview through the synth.
- UI embedding is handled by `<simple-synth-ui>` which calls `logic.connectUI()` and cleans up with `disconnectUI()`; mirror that pattern or register your own custom element before attaching it inside the modal (`modules/simple-synth/simple-synth-component.js:40`, `modules/sequencer/sequencer-instrument.js:145`).
