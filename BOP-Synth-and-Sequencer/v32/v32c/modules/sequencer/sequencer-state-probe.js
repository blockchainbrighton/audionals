// Sequencer state probe utilities
...
export function installStateProbeButton() {
    ...
    probeBtn.onclick = () => {
        ...
        if (!inst || typeof inst.getPatch !== 'function') {
            alert('State Probe: Instrument found, but it has no getPatch() method.');
            console.error(`[STATE PROBE] Instrument ${chan.instrumentId} is invalid or lacks getPatch().`);
            return;
        }

        // Fetch and pretty-print the full patch/state
        const state = inst.getPatch(); // This will also trigger the log inside getPatch()
