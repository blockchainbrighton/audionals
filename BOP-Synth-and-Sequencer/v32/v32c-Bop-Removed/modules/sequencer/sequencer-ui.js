...
import { setChannelInsertEnabled, setChannelInsertParameter, updateInsertSuspensionForSequence } from './plugins/channel-insert-manager.js';
...
import { activateInstrumentChannel, deactivateInstrumentChannel, getInstrumentLiveState, isInstrumentChannelActive, onInstrumentLiveStateChange, triggerInstrumentRecord } from './instrument-live-controller.js';
import { getInstrumentDefinition, listInstrumentOptions, instrumentSupportsRecording } from './instrument-registry.js';
...
let unsubscribeInstrumentLive = null;
...
export function initSequencerUI({ document: doc, window: win } = {}) {
    ...
    playbackChangeHandlerInstalled = false;
     if (!unsubscribeInstrumentLive) {
         unsubscribeInstrumentLive = onInstrumentLiveStateChange(scheduleRenderFromLiveState);
     }
    return elements;
}
...
function renderInstrumentControls(targetContainer, channelData, chIndex) {
    ...
    if (instrumentSupportsRecording(instrument?.type)) {
        const liveState = getInstrumentLiveState(instrumentId);
        const isRecording = !!liveState?.isRecording;
        const isArmed = !!liveState?.isArmed;
        const recordBtn = document.createElement('button');
        recordBtn.type = 'button';
        recordBtn.className = 'instrument-button instrument-button--record';
        recordBtn.textContent = isRecording ? 'Recording' : (isArmed ? 'Armed' : 'Record');
        recordBtn.title = 'Arm or start recording directly into this piano roll.';
        recordBtn.setAttribute('aria-pressed', String(isRecording || isArmed));
        recordBtn.classList.toggle('is-active', isRecording || isArmed);
        recordBtn.classList.toggle('is-recording', isRecording);
        recordBtn.onclick = () => {
            if (!isInstrumentChannelActive(channelData)) {
                activateInstrumentChannel(sequenceIndex, chIndex);
            }
            triggerInstrumentRecord(instrumentId);
        };
