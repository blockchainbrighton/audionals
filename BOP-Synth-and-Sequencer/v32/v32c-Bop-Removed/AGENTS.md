...
- Third-party logic controllers must ship an `eventBus` that reacts to the transport/note contract (`'transport-play'`, `'transport-stop'`, `'keyboard-note-on'`, `'sequence-changed'`, etc.) as demonstrated in `SimpleSynthLogic.wireUpEvents()` (`modules/simple-synth/simple-synth-logic.js`) so the piano roll and scheduler stay in sync.
