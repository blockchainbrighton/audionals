...
export class LoopManager {
    constructor(state, eventBus) {
        ...
        // Tempo
        this.originalTempo = state?.seqMeta?.recordBpm || 120;
        this.targetTempo = this.originalTempo;
        ...
        this.init();
        this.state.loopSettings = this.getLoopStatus();
    }
    ...
    ensureOriginalSeq() {
        ...
        const needsRefresh = () => {
            if (!this.originalSeq || this.originalSeq.length !== this.state.seq.length) return true;
            for (let i = 0; i < this.state.seq.length; i++) {
                const current = this.state.seq[i];
                const cached = this.originalSeq[i];
                if (!cached || current.id !== cached.id) return true;
                if (
                    current.note !== cached.note ||
                    current.start !== cached.start ||
                    current.dur !== cached.dur ||
                    current.vel !== cached.vel
                ) {
                    return true;
                }
            }
        ...
        if (typeof this.state?.seqMeta?.recordBpm === 'number') {
            this.currentTempo = this.state.seqMeta.recordBpm;
        }
    }
    ...
    dispatchLoopState() {
        const status = this.getLoopStatus();
        this.state.loopSettings = status;
        this.syncTransportLoop(status);
    ...
    quantizeSequenceArray(seq, grid, swing) {
        ...
        const gridSeconds = this.getGridSeconds(grid);
        return seq.map(note => {
            let snapped = Math.round((note.start || 0) / gridSeconds) * gridSeconds;
            let dur = Math.max(gridSeconds, Math.round(((note.dur || gridSeconds)) / gridSeconds) * gridSeconds);
            if (swing > 0) {
                const beatPos = (snapped / gridSeconds) % 2;
                if (Math.abs(beatPos - 1) < 1e-6) snapped += gridSeconds * swing * 0.1;
            }
            // Always return a new note object (deep copy)
            return { ...note, start: snapped, dur };
        });
    }
    ...
    getGridSeconds(gridValue = this.quantizeGridValue) {
        const fraction = Number.isFinite(gridValue) ? gridValue : 0.125;
        return fraction * this.secondsPerBeat();
    }

    secondsPerBeat() {
        const tempo = Number.isFinite(this.currentTempo)
            ? this.currentTempo
            : Number.isFinite(this.state?.seqMeta?.recordBpm)
                ? this.state.seqMeta.recordBpm
                : Number.isFinite(this.targetTempo)
                    ? this.targetTempo
                    : Number.isFinite(this.originalTempo)
                        ? this.originalTempo
                        : 120;
    ...
    quantizeTime(time) {
        ...
        const gridSeconds = this.getGridSeconds(this.quantizeGridValue);
        let quantized = Math.round(time / gridSeconds) * gridSeconds;
        if (this.swingAmount > 0) {
            const beatPosition = (quantized / gridSeconds) % 2;
            if (Math.abs(beatPosition - 1) < 1e-6) quantized += gridSeconds * this.swingAmount * 0.1;
        }
        return quantized;
    }
    ...
    syncTransportLoop(status = this.getLoopStatus()) {
        const transport = this.getTransport();
        if (!transport) return;

        const start = Number.isFinite(status.start) ? Math.max(0, status.start) : 0;
        const end = Number.isFinite(status.end) ? Math.max(start + 0.001, status.end) : start;

        if (!status.enabled || end <= start) {
            transport.loop = false;
            return;
        }

        if (typeof transport.setLoopPoints === 'function') {
    ...
    getTransport() {
        const tone = this.state.synth?.Tone || this.state.recorder?.synthEngine?.Tone;
