...
import { forceChannelSilence } from './sequencer-channel-mixer.js';
...
export class WaveformEditorModal {
    constructor({ container, audioBuffer, channelData, sampleName, onStateChange, onRequestClose }) {
        ...
        this.handleSamplerPlayback = this.handleSamplerPlayback.bind(this);
        this.playheadFrame = 0;
        this.playheadTimeout = null;
    }
    ...
    open() {
        ...
        if (typeof window !== 'undefined' && window?.addEventListener) {
            this.addListener(window, 'sampler-playback', this.handleSamplerPlayback);
        }
    ...
    destroy() {
        ...
        this.cancelAutoFit();
        this.cancelPlayheadAnimation();
        if (this.drawThrottleTimeout) {
            clearTimeout(this.drawThrottleTimeout);
            this.drawThrottleTimeout = null;
        }
        if (this.pendingDraw) {
    ...
    buildDom() {
        ...
        playhead.className = 'waveform-modal-playhead';
        ...
        const createNudgeButtons = (sliderEl, labelText, stepOverride = null) => {
            const group = document.createElement('div');
            group.className = 'waveform-nudge-group';

            const resolvedStep = (() => {
                if (stepOverride !== null) return stepOverride;
                const parsed = Number(sliderEl.step);
                return Number.isFinite(parsed) && parsed > 0 ? parsed : 0.1;
            })();

            const stepDigits = (() => {
                const stepSource = sliderEl.step ?? resolvedStep.toString();
                const dots = String(stepSource).split('.');
                return dots[1] ? dots[1].length : 0;
            })();

            const min = Number(sliderEl.min ?? sliderEl.getAttribute('min') ?? 0);
            const max = Number(sliderEl.max ?? sliderEl.getAttribute('max') ?? 1);

            const clampValue = value => Math.min(max, Math.max(min, value));
            const formatValue = value => (stepDigits > 0 ? value.toFixed(stepDigits) : String(Math.round(value)));
            const dispatchValue = value => {
                const next = clampValue(value);
                sliderEl.value = formatValue(next);
                sliderEl.dispatchEvent(new Event('input', { bubbles: false }));
                sliderEl.dispatchEvent(new Event('change', { bubbles: false }));
            };

            const adjustBy = delta => {
                const current = toNumber(sliderEl.value);
                dispatchValue(current + delta);
            };

            const minus = document.createElement('button');
            minus.type = 'button';
            minus.className = 'waveform-nudge-button';
            minus.textContent = '−';
            minus.setAttribute('aria-label', `Decrease ${labelText}`);
            minus.onclick = () => adjustBy(-resolvedStep);

            const plus = document.createElement('button');
            plus.type = 'button';
            plus.className = 'waveform-nudge-button';
            plus.textContent = '+';
            plus.setAttribute('aria-label', `Increase ${labelText}`);
            plus.onclick = () => adjustBy(resolvedStep);

            group.append(minus, plus);
            return group;
        };

        const createControlRow = (labelText, sliderEl, valueEl, options = {}) => {
            const { sliderExtras = [], rowExtras = [] } = options;
            ...
            sliderWrap.className = 'waveform-control-sliderwrap';
            sliderWrap.appendChild(sliderEl);

            const inlineExtras = Array.isArray(sliderExtras) ? sliderExtras : [sliderExtras];
            inlineExtras.filter(Boolean).forEach(extra => sliderWrap.appendChild(extra));

            const appendedExtras = Array.isArray(rowExtras) ? rowExtras : [rowExtras];

            row.append(labelEl, sliderWrap);
            appendedExtras.filter(Boolean).forEach(extra => row.append(extra));
            row.append(valueEl);
            return row;
        };
        ...
        const startNudge = createNudgeButtons(startSlider, 'Start', 0.1);
        const endNudge = createNudgeButtons(endSlider, 'End', 0.1);
        const fadeInNudge = createNudgeButtons(fadeInSlider, 'Fade In', Number(fadeInSlider.step) || 5);
        const fadeOutNudge = createNudgeButtons(fadeOutSlider, 'Fade Out', Number(fadeOutSlider.step) || 5);

        controls.append(
            createControlRow('Start', startSlider, startValue, { sliderExtras: [startNudge] }),
            createControlRow('End', endSlider, endValue, { sliderExtras: [endNudge] }),
            createControlRow('Playback Rate', playbackRateSlider, playbackRateValue),
            createControlRow('Fade In', fadeInSlider, fadeInValue, { sliderExtras: [fadeInNudge], rowExtras: [fadeInShapes] }),
            createControlRow('Fade Out', fadeOutSlider, fadeOutValue, { sliderExtras: [fadeOutNudge], rowExtras: [fadeOutShapes] })
        );
        ...
        stopBtn.type = 'button';
        stopBtn.className = 'waveform-modal-button';
        ...
        actions.append(previewBtn, stopBtn, resetBtn);
    ...
    attachUiEventHandlers() {
        ...
        this.addListener(this.stopBtn, 'click', () => {
            this.cancelPlayheadAnimation();
            forceChannelSilence(this.channelData);
        });
    ...
    scheduleDraw(immediate = false) {
        ...
        if (!immediate && this.lastDrawAt && (now - this.lastDrawAt) < this.drawThrottleMs) {
            if (!this.drawThrottleTimeout) {
                const delay = Math.max(4, this.drawThrottleMs - (now - this.lastDrawAt));
    ...
    performDraw() {
        if (!this.canvas || !this.audioBuffer) return;
        this.lastDrawAt = (typeof performance !== 'undefined' && typeof performance.now === 'function')
            ? performance.now()
            : Date.now();
    ...
    cancelPlayheadAnimation() {
        if (this.playheadTimeout) {
            clearTimeout(this.playheadTimeout);
            this.playheadTimeout = null;
        }
        ...
        if (this.playheadEl) {
            this.playheadEl.classList.remove('is-visible');
            this.playheadEl.hidden = true;
        }
    }

    startPlayheadAnimation(detail) {
        if (!this.playheadEl || this.isDestroyed) return;
        this.cancelPlayheadAnimation();

        const startTime = Number.isFinite(detail?.startTime) ? detail.startTime : null;
        const duration = Math.max(0.01, Number(detail?.duration) || 0.01);
        const rangeStart = clamp01(detail?.range?.start ?? 0);
        const rangeEnd = clamp01(detail?.range?.end ?? rangeStart);
        const span = rangeEnd - rangeStart;
        const rangeSpan = Math.abs(span) <= 1e-6 ? 0 : span;

        let effectiveStart = startTime;
        const run = () => {
            if (this.isDestroyed || !this.playheadEl || !this.canvasWrap?.isConnected) {
                this.cancelPlayheadAnimation();
                return;
            }
            const now = getAudioContextCurrentTime(runtimeState.Tone);
            if (typeof now !== 'number' || typeof effectiveStart !== 'number') {
                this.cancelPlayheadAnimation();
                return;
            }
            ...
            const normalized = rangeStart + clamped * rangeSpan;
            const viewRatio = this.viewSpan > 0 ? (normalized - this.viewportStart) / this.viewSpan : normalized;
            if (viewRatio >= 0 && viewRatio <= 1) {
                this.playheadEl.hidden = false;
                this.playheadEl.classList.add('is-visible');
                this.playheadEl.style.left = `${Math.min(100, Math.max(0, viewRatio * 100))}%`;
            } else {
                this.playheadEl.classList.remove('is-visible');
                this.playheadEl.hidden = true;
            }
            ...
            this.playheadFrame = requestAnimationFrame(run);
        };

        const begin = () => {
            const now = getAudioContextCurrentTime(runtimeState.Tone);
            if (typeof effectiveStart !== 'number') {
                effectiveStart = typeof now === 'number' ? now : 0;
            }
            this.playheadFrame = requestAnimationFrame(run);
        };
        ...
        if (typeof now === 'number' && typeof startTime === 'number') {
            ...
            if (delayMs > 1) {
                this.playheadTimeout = setTimeout(() => {
                    this.playheadTimeout = null;
                    begin();
                }, delayMs);
                return;
    ...
    handleSamplerPlayback(event) {
        const detail = event?.detail;
        if (!detail || detail.channel !== this.channelData) return;
        this.startPlayheadAnimation(detail);
    }
