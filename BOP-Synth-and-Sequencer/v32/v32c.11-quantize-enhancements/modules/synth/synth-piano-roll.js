...
export class PianoRoll {
    ...
    #quantizeSelection() {
        ...
        selected.forEach(note => {
            ...
            const dur = Number.isFinite(note.dur) ? note.dur : gridSeconds;
    ...
    #applyQuantizeSnapshots({ targetIds = null, gridSeconds = null } = {}) {
        ...
        seq.forEach(note => {
            ...
            const preservedDur = Number.isFinite(snapshot?.dur)
                ? snapshot.dur
                : (Number.isFinite(note.dur) ? note.dur : grid);
            const nextDur = Math.max(0.001, preservedDur);
            if (note.start !== snappedStart || note.dur !== nextDur) {
