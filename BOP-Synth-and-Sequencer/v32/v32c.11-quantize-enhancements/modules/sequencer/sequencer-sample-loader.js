...
const AudioContextCtor = globalScope?.AudioContext || globalScope?.webkitAudioContext || null;
const OfflineAudioContextCtor = globalScope?.OfflineAudioContext || globalScope?.webkitOfflineAudioContext || null;
let sharedRealtimeDecodingContext = null;
let sharedOfflineDecodingContext = null;

async function getDecodingAudioContext() {
  if (OfflineAudioContextCtor) {
    if (!sharedOfflineDecodingContext) {
      try {
        sharedOfflineDecodingContext = new OfflineAudioContextCtor(2, 44100, 44100);
      } catch (err) {
        console.warn('[SampleLoader] Failed to create OfflineAudioContext for decoding. Falling back to real-time AudioContext.', err);
        sharedOfflineDecodingContext = null;
      }
    }
    if (sharedOfflineDecodingContext) {
      return sharedOfflineDecodingContext;
    }
  }

  if (!AudioContextCtor) {
    throw new Error('Web Audio API is unavailable; cannot decode Ordinals samples.');
  }

  if (!sharedRealtimeDecodingContext) {
    sharedRealtimeDecodingContext = new AudioContextCtor();
  }
  if (sharedRealtimeDecodingContext.state === 'suspended' && typeof sharedRealtimeDecodingContext.resume === 'function') {
    try {
      await sharedRealtimeDecodingContext.resume();
    } catch (err) {
      console.warn('[SampleLoader] Unable to resume shared AudioContext for decoding.', err);
    }
  }
  return sharedRealtimeDecodingContext;
}
...
async function loadSample(src) {
  ...
  if (type.includes('application/json')) {
    ...
    const audioCtx = await getDecodingAudioContext();
    return await audioCtx.decodeAudioData(bin);
  }
