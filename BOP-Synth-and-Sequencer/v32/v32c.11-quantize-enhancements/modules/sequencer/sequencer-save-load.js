// Sequencer save/load helpers

import { projectState, runtimeState, initializeProject, syncNextInstrumentIdAfterLoad, ensureChannelMixDefaults, ensureSamplerChannelDefaults, ensureChannelInsertSettings, createDefaultInsertSettings, pruneInactiveSampleCaches, ensureInstrumentClip } from './sequencer-state.js';
...
function safeClone(value) {
    ...
    try {
        ...
        console.warn('[SAVE-LOAD] Failed to clone value, returning null.', err);
        return null;
    }
}
            ...
            sequences: projectState.sequences.map((seq, seqIndex) => {
                ...
                return {
                    channels: seq.channels.map((chan, chanIndex) => {
                        ...
                        newChan.insertSettings = (chan.type === 'sampler')
                            ? JSON.parse(JSON.stringify(chan.insertSettings || createDefaultInsertSettings()))
                            : null;
                        ...
                        } else if (chan.type === 'instrument') {
                            console.log(`[SAVE]  -> Found instrument channel ${chanIndex} with ID: ${chan.instrumentId || 'unloaded'}.`);
                            const instrument = chan.instrumentId
                                ? runtimeState.instrumentRack[chan.instrumentId]
                                : null;
                            const fallbackPatch = safeClone(chan.patch);
                            let capturedPatch = fallbackPatch;
                            if (instrument?.logic?.getFullState) {
                                try {
                                    const snapshot = instrument.logic.getFullState();
                                    capturedPatch = safeClone(snapshot);
                                    console.log(`%c[SAVE]  -> Captured patch for ${chan.instrumentId}:`, 'color: blue;', capturedPatch);
                                } catch (error) {
                                    console.error(`[SAVE]  -> FAILED to capture patch for ${chan.instrumentId}`, error);
                                    capturedPatch = fallbackPatch;
                                }
                            } else if (instrument) {
                                console.warn(`[SAVE]  -> Instrument ${chan.instrumentId} missing getFullState(); falling back to stored patch.`);
                            } else if (!fallbackPatch) {
                                console.warn(`[SAVE]  -> Instrument ${chan.instrumentId} not found and no stored patch. Using null.`);
                            }
                            if (capturedPatch) {
                                chan.patch = capturedPatch;
                                chan.patchInstrumentType = chan.instrumentType || chan.patchInstrumentType || instrument?.type || null;
                            }
                            newChan.patch = safeClone(capturedPatch);
                            newChan.patchInstrumentType = chan.patchInstrumentType || chan.instrumentType || instrument?.type || null;
                            newChan.instrumentClip = chan.instrumentClip
                                ? safeClone(chan.instrumentClip)
                                : null;
                            newChan.instrumentId = chan.instrumentId || null;
                            newChan.instrumentType = chan.instrumentType || instrument?.type || null;
                            newChan.pendingInstrumentType = chan.pendingInstrumentType || chan.instrumentType || instrument?.type || null;
                        }
                        return newChan;
                    })
                };
...
export async function loadProject(jsonString) {
    ...
    try {
        ...
        for (const id in runtimeState.instrumentRack) {
            ...
            if (!inst?.logic) continue;
            const snapshot = inst.logic.getFullState?.()
                || inst.logic.modules?.synthEngine?.getState?.()
                || null;
            if (snapshot) {
                console.info(`[LOAD] Old Synth State (${id}):`, snapshot);
            }
        }
        console.groupEnd();
        ...
        runtimeState.sampleCacheMeta.clear();
        ...
        projectState.sequences.forEach(seq => {
            ...
            seq.channels.forEach(chan => {
                ...
                if (chan.type === 'sampler') {
                    ...
                    normalizeInstrumentPatch(chan);
                    ensureInstrumentClip(chan);
                    if (!chan.instrumentId) {
                        chan.instrumentId = `inst-${projectState.nextInstrumentId++}`;
                    }
                    if (!chan.patchInstrumentType) {
                        chan.patchInstrumentType = chan.instrumentType || null;
                    }
                    if (!chan.pendingInstrumentType) {
                        chan.pendingInstrumentType = chan.instrumentType || null;
                    }
                }
        ...
        for (const id in runtimeState.instrumentRack) {
            const inst = runtimeState.instrumentRack[id];
            if (!inst?.logic) continue;
            const patch = inst.logic.getFullState?.()
                || inst.logic.modules?.synthEngine?.getState?.()
                || null;
            if (!patch) continue;
...
function normalizeInstrumentPatch(chan) {
    if (!chan || chan.type !== 'instrument' || !chan.patch || typeof chan.patch !== 'object') return;
    const hasFullState = typeof chan.patch.version === 'string'
        && chan.patch.engine
        && typeof chan.patch.engine === 'object';
    if (hasFullState) {
        chan.patch = safeClone(chan.patch) || chan.patch;
        return;
    }
    const engineSource = chan.patch.engine && typeof chan.patch.engine === 'object'
        ? chan.patch.engine
        : { params: { ...(chan.patch.params || chan.patch) } };
    const uiState = chan.patch.uiState && typeof chan.patch.uiState === 'object'
        ? chan.patch.uiState
        : null;
    chan.patch = {
        version: chan.patch.version || 'simple-synth-legacy',
        engine: safeClone(engineSource) || { params: {} },
        uiState: safeClone(uiState)
    };
}
