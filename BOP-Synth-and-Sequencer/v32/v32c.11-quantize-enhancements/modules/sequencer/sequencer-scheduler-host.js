...
const playbackDiagnostics = {
    ...
    loggedSampler: false,
    loggedInstrument: false
};

function preparePlaybackDiagnostics(sequence) {
    const channels = Array.isArray(sequence?.channels) ? sequence.channels : [];
    const samplerIndex = channels.findIndex(channel => channel?.type === 'sampler');
    const instrumentIndex = channels.findIndex(channel => channel?.type === 'instrument');
    playbackDiagnostics.samplerIndex = samplerIndex >= 0 ? samplerIndex : null;
    playbackDiagnostics.instrumentIndex = instrumentIndex >= 0 ? instrumentIndex : null;
    playbackDiagnostics.loggedSampler = false;
    playbackDiagnostics.loggedInstrument = false;
    playbackDiagnostics.armed = samplerIndex >= 0 || instrumentIndex >= 0;
    if (playbackDiagnostics.armed) {
        console.groupCollapsed('[PlaybackDiag] Playback started – diagnostics armed');
        console.log('Sampler channel index:', playbackDiagnostics.samplerIndex);
        console.log('Instrument channel index:', playbackDiagnostics.instrumentIndex);
        console.log('BPM:', projectState.bpm, 'Sequence:', sequence ? projectState.currentSequenceIndex : 'n/a');
        console.groupEnd();
    }
}

function resetPlaybackDiagnostics() {
    playbackDiagnostics.armed = false;
    playbackDiagnostics.samplerIndex = null;
    playbackDiagnostics.instrumentIndex = null;
    playbackDiagnostics.loggedSampler = false;
    playbackDiagnostics.loggedInstrument = false;
}
...
function scheduleInstrumentClipPlayback(channel, instrument, scheduledTime, context = {}) {
    const clip = ensureInstrumentClip(channel);
    const seq = Array.isArray(clip?.seq) ? clip.seq : [];
    if (!seq.length) return false;
    const logic = instrument?.logic;
    const engine = logic?.modules?.synthEngine;
    ...
    if (typeof logic?.stopClipPlayback === 'function') {
        try { logic.stopClipPlayback(); } catch (err) { console.warn('[SCHED] Failed to stop previous clip playback', err); }
    }
    if (typeof logic?.scheduleClipPlayback === 'function') {
        try {
            const scheduled = !!logic.scheduleClipPlayback(seq, { startTime: scheduledTime, timeScale });
            logInstrumentClipSchedule(channel, instrument, scheduledTime, seq, timeScale, context);
            return scheduled;
        } catch (err) {
            console.warn('[SCHED] Logic-level clip scheduling failed, falling back to engine trigger.', err);
        }
    }
    seq.forEach(note => {
        if (!note) return;
    ...
    logInstrumentClipSchedule(channel, instrument, scheduledTime, seq, timeScale, context, { fallback: true });
    return true;
}
...
function logSamplerDiagnostic(channel, trigger, scheduledTime, stepIndex, cycle) {
    const transportSeconds = runtimeState.Tone?.Transport?.seconds ?? null;
    console.groupCollapsed('[PlaybackDiag] Kick Sample Trigger');
    console.log('Step', stepIndex, 'Cycle', cycle, 'ScheduledTime', scheduledTime, 'TransportSeconds', transportSeconds);
    console.log('Channel Index', trigger.channelIndex, 'Sample Index', channel?.selectedSampleIndex, 'AllowOverlap', trigger.allowOverlap);
    console.log('Sample Descriptor', channel?.sampleDescriptor || null);
    console.groupEnd();
}

function logInstrumentDiagnostic(channel, scheduledTime, stepIndex, cycle) {
    const transportSeconds = runtimeState.Tone?.Transport?.seconds ?? null;
    const clip = ensureInstrumentClip(channel);
    const seq = Array.isArray(clip?.seq) ? clip.seq.slice(0, 8) : [];
    console.groupCollapsed('[PlaybackDiag] Synth Clip Trigger');
    console.log('Step', stepIndex, 'Cycle', cycle, 'ScheduledTime', scheduledTime, 'TransportSeconds', transportSeconds);
    console.log('Channel Index', channel ? channel.instrumentId : null, 'Instrument Type', channel?.instrumentType);
    console.log('Clip Notes (first 8)', seq);
    console.groupEnd();
}

function logInstrumentClipSchedule(channel, instrument, scheduledTime, seq, timeScale, context = {}, options = {}) {
    const bpm = projectState.bpm || 120;
    const secondsPerBeat = 60 / bpm;
    const secondsPerStep = secondsPerBeat / STEP_SUBDIVISION;
    const stepsPerLoop = Array.isArray(channel?.steps) && channel.steps.length
        ? channel.steps.length
        : TOTAL_STEPS;
    const loopDurationSeconds = stepsPerLoop * secondsPerStep;
    const diagNotes = seq.slice(0, 8).map(note => {
        const clipStart = Number(note?.start ?? 0);
        const clipDur = Number(note?.dur ?? 0);
        const scheduledOffset = Math.max(0, clipStart) * timeScale;
        const absPlayback = Number.isFinite(scheduledTime) ? scheduledTime + scheduledOffset : null;
        const loopPercent = loopDurationSeconds > 0
            ? ((clipStart % loopDurationSeconds) / loopDurationSeconds) * 100
            : 0;
        return {
            note: note?.note ?? '',
            clipStart: clipStart.toFixed(6),
            clipDur: clipDur.toFixed(6),
            loopPercent: Number(loopPercent.toFixed(2)),
            scheduledOffset: scheduledOffset.toFixed(6),
            absPlayback: absPlayback !== null ? absPlayback.toFixed(6) : 'n/a',
            velocity: Number(note?.vel ?? 0).toFixed(3)
        };
    });
    const label = options.fallback
        ? '[PlaybackDiag] Instrument Schedule (fallback)'
        : '[PlaybackDiag] Instrument Schedule';
    console.groupCollapsed(label);
    console.log({
        instrumentId: instrument?.id ?? null,
        channelId: channel?.instrumentId ?? null,
        stepIndex: context.stepIndex ?? null,
        cycle: context.cycle ?? null,
        scheduledTime,
        timeScale,
        loopDurationSeconds: Number(loopDurationSeconds.toFixed(6))
    });
    console.table(diagNotes);
    console.groupEnd();
}
...
export class SequencerSchedulerHost {
    ...
    async start(mode) {
        ...
        preparePlaybackDiagnostics(sequence);
    ...
    handleScheduleStep({ scheduledTime, stepIndex, cycle, globalStep, triggers }) {
        ...
        runtimeState.currentStepScheduledTime = typeof scheduledTime === 'number' ? scheduledTime : null;
        runtimeState.stepDurationSeconds = computeStepDurationSeconds();
        ...
        triggers.forEach(trigger => {
            ...
            if (!channel) return;
            if (playbackDiagnostics.armed && cycle === 0) {
                if (!playbackDiagnostics.loggedSampler
                    && playbackDiagnostics.samplerIndex === trigger.channelIndex
                    && trigger.type === 'sampler') {
                    logSamplerDiagnostic(channel, trigger, scheduledTime, stepIndex, cycle);
                    playbackDiagnostics.loggedSampler = true;
                }
                if (!playbackDiagnostics.loggedInstrument
                    && playbackDiagnostics.instrumentIndex === trigger.channelIndex
                    && trigger.type === 'instrument') {
                    logInstrumentDiagnostic(channel, scheduledTime, stepIndex, cycle);
                    playbackDiagnostics.loggedInstrument = true;
                }
                if (playbackDiagnostics.loggedSampler && playbackDiagnostics.loggedInstrument) {
                    playbackDiagnostics.armed = false;
                }
            }
