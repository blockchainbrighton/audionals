...
import { runtimeState, ensureInstrumentClip } from './sequencer-state.js';
...
function setRecordViewportState(record, active) {
    if (!record?.pianoRoll?.setViewportActive) return;
    try {
        record.pianoRoll.setViewportActive(!!active);
    } catch (err) {
        console.warn('[InstrumentPianoRoll] Failed to toggle piano roll viewport state', err);
    }
}
...
function destroyEditor(record) {
    ...
    record.cleanup = [];
    if (record.pianoRoll) {
        setRecordViewportState(record, false);
        try {
...
function hydrateEditor(channelData, instrument, hostElement) {
    ...
    const clipState = ensureInstrumentClip(channelData);
    const pianoRoll = new PianoRoll(rollHost, instrument.logic.eventBus, clipState);
    ...
    const sequenceChangedHandler = () => {
        ...
        channel.instrumentClip = record.pianoRoll?.state || ensureInstrumentClip(channel);
...
function setCollapse(record, collapsed, { centerOnNotes: shouldCenter = false, skipCenter = false } = {}) {
    ...
    if (collapsed) {
        ...
        setRecordViewportState(record, true);
        record.pianoRoll?.draw();
...
function updateNoteSummary(record) {
    ...
    const seq = Array.isArray(record.currentChannel?.instrumentClip?.seq)
        ? record.currentChannel.instrumentClip.seq
        : [];
    if (!seq.length) {
