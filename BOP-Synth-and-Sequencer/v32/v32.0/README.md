...
- `synth-layout.js` - Shared HTML template & UI element helpers for standalone/component builds
...
- `synth-ui-components.js` - Custom element that reuses the shared synth layout inside other hosts

### Shared Layout Utilities
- `synth-layout.js` exposes a single HTML template used by both the standalone host and any embedded components
- `renderSynthLayout(target)` mounts the layout into a host container and returns cached DOM references
- `getUIElements(scope)` normalises element lookups for `BopSynthUI`, keeping selectors consistent across environments
...
- Standalone synth and reusable component now consume the same `synth-layout.js` markup helpers, removing duplicated templates & CSS
