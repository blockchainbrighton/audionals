...
export const runtimeState = {
    ...
    currentStepScheduledTime: null,
    stepDurationSeconds: null,
    instrumentRack: {},
    instrumentLiveChannels: new Map(),
...
function disposeInstrumentRack() {
    if (runtimeState.instrumentRack && typeof runtimeState.instrumentRack === 'object') {
        Object.entries(runtimeState.instrumentRack).forEach(([instrumentId, instrument]) => {
            ...
            try {
                ...
                if (engine) {
                    ...
                    if (typeof engine.disconnect === 'function') {
                        engine.disconnect();
                    }
                }
            } catch (err) {
                console.warn(`[STATE] Failed to dispose instrument ${instrumentId}:`, err);
            }
        });
    }
    runtimeState.instrumentRack = {};
    if (runtimeState.instrumentLiveChannels?.clear) {
        runtimeState.instrumentLiveChannels.clear();
    } else {
    ...
    if (typeof runtimeState.__resetInstrumentLiveState === 'function') {
        try {
            runtimeState.__resetInstrumentLiveState();
        } catch (err) {
            console.warn('[STATE] Failed to reset instrument live controller during project init:', err);
        }
    }
}
...
export function createEmptyInstrumentClip(bpm = projectState.bpm || 120) {
    return {
        seq: [],
        seqMeta: { recordBpm: typeof bpm === 'number' && !Number.isNaN(bpm) ? bpm : 120 },
        loopSettings: null
    };
}

export function ensureInstrumentClip(channel) {
    if (!channel || channel.type !== 'instrument') return null;
    if (!channel.instrumentClip || typeof channel.instrumentClip !== 'object') {
        channel.instrumentClip = createEmptyInstrumentClip();
    } else {
        if (!Array.isArray(channel.instrumentClip.seq)) {
            channel.instrumentClip.seq = [];
        }
        const meta = channel.instrumentClip.seqMeta;
        if (!meta || typeof meta !== 'object') {
            channel.instrumentClip.seqMeta = { recordBpm: projectState.bpm || 120 };
        } else if (typeof meta.recordBpm !== 'number' || Number.isNaN(meta.recordBpm)) {
            channel.instrumentClip.seqMeta.recordBpm = projectState.bpm || 120;
        }
        if (channel.instrumentClip.loopSettings && typeof channel.instrumentClip.loopSettings !== 'object') {
            channel.instrumentClip.loopSettings = null;
        }
    }
    return channel.instrumentClip;
}
