...
export class SimpleSynthEngine {
    ...
    scheduleClip(notes, startTime, timeScale = 1) {
        ...
        notes.forEach(note => {
            ...
            scheduleEvent(noteStart, () => {
                this.poly.triggerAttack(noteName, noteStart, velocity);
                console.log('[PlaybackDiag] Tone Attack', {
                    note: noteName,
                    transportTime: noteStart,
                    scheduledFrom: startTime,
                    velocity,
                    duration: duration
                });
            });
            scheduleEvent(noteEnd, () => {
                this.poly.triggerRelease(noteName, noteEnd);
                console.log('[PlaybackDiag] Tone Release', {
                    note: noteName,
                    transportTime: noteEnd
                });
            });
        });
        return true;
