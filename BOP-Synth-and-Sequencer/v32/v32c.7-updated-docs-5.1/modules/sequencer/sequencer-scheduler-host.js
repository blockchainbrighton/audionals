...
function scheduleInstrumentClipPlayback(channel, instrument, scheduledTime) {
    ...
    if (typeof logic?.scheduleClipPlayback === 'function') {
        try {
            ...
            console.groupCollapsed('[PlaybackDiag] Instrument Schedule');
            console.log('Instrument:', instrument.id, 'Scheduled Time:', scheduledTime, 'Time Scale:', timeScale);
            console.table(
                seq.slice(0, 8).map(note => ({
                    start: Number(note?.start ?? 0).toFixed(6),
                    dur: Number(note?.dur ?? 0).toFixed(6),
                    vel: Number(note?.vel ?? 0).toFixed(3),
                    note: note?.note
                }))
            );
            console.groupEnd();
    ...
    console.groupCollapsed('[PlaybackDiag] Instrument Schedule (fallback)');
    console.log('Instrument:', instrument.id, 'Scheduled Time:', scheduledTime, 'Time Scale:', timeScale);
    console.table(
        seq.slice(0, 8).map(note => ({
            start: Number(note?.start ?? 0).toFixed(6),
