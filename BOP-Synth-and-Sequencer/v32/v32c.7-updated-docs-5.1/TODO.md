...
8. **Presets** Enable shareable preset bundles (`.audpr`) that serialize insert chains and synth patches, surfaced via a preset browser UI.
...
19. **Extensibility** Expose a scripting sandbox tied to `SimpleSynthLogic.eventBus`, letting advanced users author scripts that respond to transport events.
