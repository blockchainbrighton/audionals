...
function computeLoopTiming(channel) {
    ...
    const stepCount = Array.isArray(channel?.steps) && channel.steps.length
        ...
        : TOTAL_STEPS;
    const loopDuration = Math.max(secondsPerStep, stepCount * secondsPerStep);
...
function startRecordingSession(instrumentId, binding) {
    ...
    const currentStep = Number.isFinite(runtimeState.currentStepIndex)
        ? runtimeState.currentStepIndex
        : 0;
    const initialLoopOffset = playbackActive ? currentStep * secondsPerStep : 0;
...
function handleNoteOn(instrumentId, note, velocity) {
    ...
    if (session.mode === 'live') {
        ...
        const rawPosition = session.initialLoopOffset + Math.max(0, elapsed);
        relStart = normalizeLoopPosition(rawPosition, session.loopDuration);
    } else {
