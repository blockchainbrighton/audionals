...
export class PianoRoll {
    ...
    draw() {
        ...
        const timeMax = Math.max(stepSeconds * this.DEFAULT_SEQUENCE_STEPS, actualSequenceLength);
        ...
        this.#autoFitZoomX(gridTimeCount);
    ...
    #autoFitZoomX(totalDurationSeconds) {
        ...
        if (!container) return;
        const trackWidth = Math.max(120, (container.clientWidth || 0) - 48);
        if (!Number.isFinite(trackWidth) || trackWidth <= 0) return;
        const duration = Math.max(this.#defaultStepSeconds(), totalDurationSeconds || 0.0001);
