...
function isChannelWaveformVisible(sequenceIndex, channelIndex) {
    const key = getChannelLayoutKey(sequenceIndex, channelIndex);
    const state = channelLayoutState.get(key);
    if (!state || typeof state.waveformVisible === 'undefined') return true;
    return state.waveformVisible !== false;
}

function setChannelWaveformVisible(sequenceIndex, channelIndex, visible) {
    const key = getChannelLayoutKey(sequenceIndex, channelIndex);
    const state = channelLayoutState.get(key) || {};
    state.waveformVisible = !!visible;
    channelLayoutState.set(key, state);
}
...
export function render() {
    ...
    currentSeq.channels.forEach((channelData, chIndex) => {
        ...
        const detailsVisible = channelData.type === 'sampler'
            ...
            : true;
        const waveformVisible = channelData.type === 'sampler'
            ? isChannelWaveformVisible(sequenceIndex, chIndex)
            : true;
        ...
        if (channelData.type === 'sampler') {
            ...
            waveformToggle.type = 'button';
            waveformToggle.className = 'channel-layout-toggle';
            waveformToggle.textContent = waveformVisible ? 'Hide waveform' : 'Show waveform';
            waveformToggle.disabled = !detailsVisible;
            waveformToggle.onclick = () => {
                setChannelWaveformVisible(sequenceIndex, chIndex, !waveformVisible);
                render();
            };
            headerActions.appendChild(waveformToggle);

            if (waveformVisible) {
                waveformEl = createChannelWaveform(channelData);
            } else {
        ...
        if (channelData.type === 'sampler') {
            if (detailsVisible) {
                ...
                if (waveformEl) controlsRow.appendChild(waveformEl);
