...
export class EnhancedRecorder {
    ...
    startPlayback(syncDetail) {
        ...
        const loopEnd = loopActive ? loopStatus.end : Infinity;
        ...
        playableEvents.forEach(item => {
            ...
            if (hostTransportBase !== null) {
                scheduleTarget = Math.max(0, hostTransportBase + loopRelativeStart);
            } else if (hasHost) {
                scheduleTarget = `+${Math.max(0, baseDelta + loopRelativeStart)}`;
            } else {
                scheduleTarget = `+${Math.max(0, loopRelativeStart)}`;
            }
        ...
        const transportStart = loopActive
            ? loopStart
            : (hostTransportBase ?? (hasHost ? (transport.seconds + baseDelta) : transport.seconds));
        this.playbackStartTransport = transportStart || 0;
