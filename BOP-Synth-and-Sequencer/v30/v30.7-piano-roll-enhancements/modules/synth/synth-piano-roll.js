...
export class PianoRoll {
    ...
    #pasteSelection({ referenceOffset = null } = {}) {
        ...
        const seq = this.state.seq || [];
        const selected = this.#getSelectedNotes();
        const grid = this.quantizeGrid || 0.25;
        const reference = Number.isFinite(referenceOffset)
            ? referenceOffset
            : (selected.length
                ? Math.max(...selected.map(note => (note.start || 0) + (note.dur || 0))) + grid
                : (this.sequenceLength || 0) + grid);
        const newIds = [];
    ...
    #duplicateSelection() {
        ...
        this.#copySelection();
        const grid = this.quantizeGrid || 0.25;
        const offset = Math.max(...selected.map(note => (note.start || 0) + (note.dur || 0))) + grid;
    ...
    #quantizeSelection() {
        ...
        selected.forEach(note => {
            note.start = Math.max(0, Math.round((note.start || 0) / grid) * grid);
            note.dur = Math.max(grid, Math.round((note.dur || grid) / grid) * grid);
    ...
    #deactivateCloneDrag(event) {
        ...
        this.dragData.notes.forEach(entry => {
            ...
            entry.origDur = entry.note.dur || (this.quantizeGrid || 0.25);
    ...
    #handleNoteMouseDown(evt, noteObj, noteId, midi) {
        ...
        const originalNotes = selectionIds.map(id => {
            ...
            return {
                ...
                origDur: targetNote.dur || (this.quantizeGrid || 0.25),
                origMidi: noteMidi
            };
        });
    ...
    #onGlobalDragMove(e) {
        ...
        if (this.dragData.mode === 'resize-duration') {
            ...
            const grid = this.quantizeEnabled ? this.quantizeGrid : null;
            const minDur = Math.max(grid || 0.01, 0.01);
            const dx = (e.clientX - this.dragData.startX) / cellW;

            this.dragData.notes.forEach(entry => {
                const note = entry.note;
                let newDur = entry.origDur + dx;
                if (grid) newDur = Math.round(newDur / grid) * grid;
