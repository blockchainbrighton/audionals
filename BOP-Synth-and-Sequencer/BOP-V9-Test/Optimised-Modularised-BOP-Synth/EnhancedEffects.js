/**
 * @file EnhancedEffects.js
 * @description Refactored enhanced effects module as an ES6 class for the BOP Synthesizer component.
 * Manages all audio effects including reverb, delay, filters, modulation, and distortion effects.
 */
...
export class EnhancedEffects {
    ...
    setupEventListeners() {
        // Listen for effect control events from the main component
        this.mainComponent.on('effectToggleChange', (data) => {
            this.toggleEffect(data.effect, data.enabled);
        });

        // Listen for parameter changes
        this.mainComponent.on('filterChange', (data) => {
            if (data.type) {
                this.setFilter({ type: data.type });
            } else {
                const params = {};
                if (data.value !== undefined) {
                    if (data.param === 'frequency') params.frequency = data.value;
                    else if (data.param === 'Q') params.Q = data.value;
                }
                this.setFilter(params);
            }
        });

        this.mainComponent.on('filterLFOChange', (data) => {
            const params = {};
            if (data.param === 'frequency') params.frequency = data.value;
            else if (data.param === 'depth') params.depth = data.value;
            this.setFilterLFO(params);
        });

        this.mainComponent.on('chorusChange', (data) => {
            const params = {};
            if (data.param === 'wet') params.wet = data.value;
            else if (data.param === 'frequency') params.frequency = data.value;
            this.setChorus(params);
        });

        this.mainComponent.on('phaserChange', (data) => {
            const params = {};
            if (data.param === 'wet') params.wet = data.value;
            else if (data.param === 'frequency') params.frequency = data.value;
            this.setPhaser(params);
        });

        this.mainComponent.on('tremoloChange', (data) => {
            const params = {};
            if (data.param === 'wet') params.wet = data.value;
            else if (data.param === 'frequency') params.frequency = data.value;
            this.setTremolo(params);
        });

        this.mainComponent.on('vibratoChange', (data) => {
            const params = {};
            if (data.param === 'wet') params.wet = data.value;
            else if (data.param === 'frequency') params.frequency = data.value;
            this.setVibrato(params);
        });

        this.mainComponent.on('compressorChange', (data) => {
            const params = {};
            if (data.param === 'threshold') params.threshold = data.value;
            else if (data.param === 'ratio') params.ratio = data.value;
            this.setCompressor(params);
        });

        this.mainComponent.on('distortionChange', (data) => {
            const params = {};
            if (data.param === 'wet') params.wet = data.value;
            else if (data.param === 'distortion') params.distortion = data.value;
            this.setDistortion(params);
        });

        this.mainComponent.on('bitCrusherChange', (data) => {
            const params = {};
            if (data.param === 'wet') params.wet = data.value;
            else if (data.param === 'bits') params.bits = data.value;
            this.setBitCrusher(params);
        });

        this.mainComponent.on('delayChange', (data) => {
            const params = {};
            if (data.param === 'wet') params.wet = data.value / 100;
            else if (data.param === 'delayTime') params.delayTime = data.value;
            else if (data.param === 'feedback') params.feedback = data.value;
            this.setDelay(params);
        });

        this.mainComponent.on('reverbChange', (data) => {
            const params = {};
            if (data.param === 'wet') params.wet = data.value / 100;
            else if (data.param === 'roomSize') params.roomSize = data.value;
            else if (data.param === 'decay') params.decay = data.value;
            this.setReverb(params);
        });
    ...
    createEffects() {
        ...
        e.inputGain = new this.Tone.Gain(1);
        e.outputGain = new this.Tone.Gain(0.7);
        e.reverb     = new this.Tone.Reverb(d.reverb);
        e.delay      = new this.Tone.FeedbackDelay(d.delay);
        e.filter     = new this.Tone.Filter(d.filter);
        e.chorus     = new this.Tone.Chorus(d.chorus);
        e.distortion = new this.Tone.Distortion(d.distortion);
        e.phaser     = new this.Tone.Phaser(d.phaser);
        e.tremolo    = new this.Tone.Tremolo({ frequency: d.tremolo.frequency, depth: 0 });
        e.vibrato    = new this.Tone.Vibrato({ frequency: d.vibrato.frequency, depth: 0 });
        e.compressor = new this.Tone.Compressor(d.compressor);
        e.bitCrusher = new this.Tone.BitCrusher(d.bitCrusher);

        // LFOs
        e.filterLFO   = new this.Tone.LFO({ frequency: d.filterLFO.frequency, min: d.filterLFO.min, max: d.filterLFO.max, amplitude: 0 });
        e.tremoloLFO  = new this.Tone.LFO({ frequency: d.tremoloLFO.frequency, min: 0, max: 1, amplitude: 0 });
        e.vibratoLFO  = new this.Tone.LFO({ frequency: d.vibratoLFO.frequency, min: -d.vibratoLFO.depth, max: d.vibratoLFO.depth, amplitude: 0 });
        e.phaserLFO   = new this.Tone.LFO({ frequency: d.phaserLFO.frequency, min: 0.1, max: 10, amplitude: 0 });
    }

    setupAudioChain() {
        const e = this.effects;
        // Distortion -> Compressor
        e.chain1 = new this.Tone.Gain();
        ...
        // BitCrusher
        e.chain3 = new this.Tone.Gain();
        e.inputGain.connect(e.bitCrusher);
        e.bitCrusher.connect(e.chain3);

        // Mixer
        const mixer = new this.Tone.Gain(0.33);
    ...
    /**
     * Clean up and dispose of all effects
     */
    dispose() {
