...
import PresetManager from './synth-preset-manager.js';
...
export class BopSynthUI {
    ...
    init() {
        ...
        this.modules.transport = ensureDestroy(new Transport(
            this.uiElements.transport,
            this.eventBus,
            {
                recordIndicator: this.uiElements.recordIndicator,
                recordStatus: this.uiElements.recordStatus
            }
        ));
        this.modules.presets = ensureDestroy(new PresetManager(
            this.uiElements.presetControls, this.eventBus, this.logic.modules.synthEngine
        ));
        ...
        this.modules.midiControl = ensureDestroy(new MidiControl(this.eventBus, {
            indicator: this.uiElements.midiIndicator,
            label: this.uiElements.midiStatus
        }));
   ...
   // Only capture layout UI state, not parameters!
    ...
    setupTabs() {
        const buttons = Array.isArray(this.uiElements.tabButtons) ? this.uiElements.tabButtons : [];
        const panels = Array.isArray(this.uiElements.tabContents) ? this.uiElements.tabContents : [];
        if (!buttons.length || !panels.length) return;

        this._tabHandlers = buttons.map(btn => {
            const handler = () => this.activateTab(btn.dataset.tab);
            btn.addEventListener('click', handler);
            return { btn, handler };
        });

        const initiallyActive = buttons.find(btn => btn.classList.contains('active'))?.dataset.tab
            || buttons[0]?.dataset.tab;
        if (initiallyActive) {
            this.activateTab(initiallyActive, { suppressEvent: true });
        }
    }

    activateTab(tabId, { suppressEvent = false } = {}) {
        ...
        if (!suppressEvent) {
            this.eventBus.dispatchEvent(new CustomEvent('tab-changed', {
                detail: { tabId }
            }));
            if (tabId === 'midi') {
    ...
    updateStatus(message, type = 'info') {
        const statusElement = this.uiElements.statusMessage;
        if (statusElement) {
            const tone = ['info', 'warn', 'error'].includes(type) ? type : 'info';
