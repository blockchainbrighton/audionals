...
export default EnhancedRecorder;
    ...
    // NEW: Edit note method for PianoRoll integration
    editNote(noteIndex, changes) {
        if (!this.state.seq || noteIndex < 0 || noteIndex >= this.state.seq.length) return;
        
        const note = this.state.seq[noteIndex];
        if (changes.note !== undefined) note.note = changes.note;
        if (changes.start !== undefined) note.start = changes.start;
        if (changes.dur !== undefined) note.dur = changes.dur;
        if (changes.vel !== undefined) note.vel = changes.vel;
        
        this.eventBus.dispatchEvent(new CustomEvent('sequence-changed'));
    }

    // NEW: Delete note method for PianoRoll integration
    deleteNote(noteIndex) {
        if (!this.state.seq || noteIndex < 0 || noteIndex >= this.state.seq.length) return;
        
        this.state.seq.splice(noteIndex, 1);
        this.eventBus.dispatchEvent(new CustomEvent('sequence-changed'));
    }
