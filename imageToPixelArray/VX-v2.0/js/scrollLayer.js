...
// Short selector (replace with import if you have a global $ in ui)
const $ = s => document.querySelector(s);

let scrollCells = [];

// Always call on DOMContentLoaded and any grid size/pixel changes
...
// --- Text color buffer for scrolling text effect ---
export function makeTextColorBuffer(text, scale, hex) {
  ...
  glyphs.forEach(glyph => {
    for (let gr = 0; gr < FONT_H; gr++) for (let gc = 0; gc < FONT_W; gc++) if (glyph[gr][gc] === '1') {
      const px = x + Math.round(gc * scale), py = y + Math.round(gr * scale),
