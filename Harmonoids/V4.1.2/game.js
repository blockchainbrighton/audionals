...
class HarmonoidsGame {
    constructor() {
        ...
        this.gameMode = 'auto'; // 'auto' or 'manual'
        this.manualTrait = {
            waveform: 'sine',
            frequency: 440
        };
    ...
    setupEventListeners() {
        ...
        document.getElementById('manualDropBtn').addEventListener('click', () => {
            this.manualDropHarmonoid();
        });
        // Trait controls
        document.getElementById('waveformSelect').addEventListener('change', (e) => {
            this.manualTrait.waveform = e.target.value;
        });
        document.getElementById('frequencySlider').addEventListener('input', (e) => {
            this.manualTrait.frequency = parseInt(e.target.value);
    ...
    spawnHarmonoid(traits = null) {
        const harmonoid = new Harmonoid(
            this.nextHarmonoidId++,
            this.level.startX,
            this.level.startY,
            this.harmonoidSpeed,
            traits?.frequency || 440 + Math.random() * 220
        );
        this.harmonoids.push(harmonoid);
        this.stats.total++;
        
        // Pass waveform and frequency to audio manager
        this.audioManager.createHarmonoidSound(harmonoid.id, harmonoid.frequency, traits?.waveform || 'sine');
    }
    ...
    update() {
        ...
        if (this.gameMode === 'auto' && this.frameCount % this.harmonoidSpawnRate === 0) {
            this.spawnHarmonoid();
        }
    ...
    setGameMode(mode) {
        this.gameMode = mode;
        document.getElementById('autoModeBtn').disabled = (mode === 'auto');
        document.getElementById('manualModeBtn').disabled = (mode === 'manual');
        document.getElementById('manualDropBtn').style.display = (mode === 'manual') ? 'inline-block' : 'none';
        document.getElementById('traitPanel').style.display = (mode === 'manual') ? 'block' : 'none';
        this.frameCount = 0;
    }

    manualDropHarmonoid() {
        this.spawnHarmonoid(this.manualTrait);
    }
