...
class HarmonoidsGame {
    ...
    update() {
        ...
        if (this.autoSpawn && this.frameCount % this.harmonoidSpawnRate === 0) {
            this.spawnHarmonoid();
        }
...
class Harmonoid {
    ...
    // Toggle between automatic and manual spawn modes
    toggleSpawnMode() {
        this.autoSpawn = !this.autoSpawn;
        const modeBtn = document.getElementById('modeBtn');
        const dropBtn = document.getElementById('dropBtn');
        modeBtn.textContent = `Mode: ${this.autoSpawn ? 'AUTO' : 'MANUAL'}`;
        dropBtn.disabled = this.autoSpawn;
    }
