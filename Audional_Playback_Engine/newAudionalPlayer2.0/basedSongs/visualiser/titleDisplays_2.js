...
(function () {
    ...
    const maxTextElements = 10;
    const textElements = {};
    let clearAnimationsDebounced;

    // Utility function to create and style text elements
    function createAndStyleTextElement(id, text, container, styleProps) {
    ...
    function clearAnimations() {
        if (clearAnimationsDebounced) return;
        clearAnimationsDebounced = true;
        ...
        requestIdleCallback(() => {
            ...
            clearAnimationsDebounced = false;
        });
    }
    ...
    function setupTextDisplays() {
        ...
        // Create and style text elements
        for (let i = 1; i <= maxTextElements; i++) {
            const text = config[`displayText_${i}`] || '';
            textElements[`textDisplay${i}`] = createAndStyleTextElement(
                `text-display-${i}`, text, container, {
