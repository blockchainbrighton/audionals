...
function getColors(o, a, l) {
    ...
    // Event listener for responding to visualizer's request
window.addEventListener('message', (event) => {
    if (event.data.type === 'requestColorLine') {
        let { angle, timestamp, vertices } = event.data.data;
        let colors = getColors(angle, timestamp, vertices);
        let currentTime = Date.now();
        for (let i = 0; i < colors.length; i++) {
            window.postMessage({
                type: 'responseColorLine',
                line: i,
                color: colors[i],
