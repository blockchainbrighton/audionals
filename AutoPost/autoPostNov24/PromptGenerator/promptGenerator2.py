...
HEADER_LINE = "Please generate the following 10 images independently, ensuring each is created without verifying the others. All images should have the same resolution:"
...
def generate_prompt():
    ...
    prompt = f"\"{central_style} 'Letter A' wearing {headphone_type}. Theme: {theme}. Set in {setting}.\""
    return prompt
...
if __name__ == "__main__":
    ...
    try:
        with open("audionalHeadphonePrompts2.txt", "w", encoding="utf-8") as file:
            for group in grouped_prompts:
                ...
                for prompt_number, prompt in group:
                    file.write(f"Image {prompt_number}: {prompt}\n")
                
                # Add a double line space after each group
                file.write("\n\n")
        
        logging.info(f"Successfully saved {len(unique_prompts)} prompts to 'audionalHeadphonePrompts2.txt'.")
