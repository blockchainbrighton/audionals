...
EXECUTION_INTERVAL = 3600  # Interval in seconds (e.g., 3600 seconds = 1 hour)
...
def acquire_lock(lock_file_path):
    """
    Acquires an exclusive lock on the specified file to ensure only one instance runs.
    """
...
def read_first_block(file_path):
    ...
    try:
        ...
        if not blocks:
            ...
            return None, content  # No prompts to send
...
def main():
    ...
    while running:
        ...
        if first_block:
            ...
            logging.info("No prompts to send. Waiting for the next cycle.")
        
        logging.info(f"Automation cycle completed. Waiting for {EXECUTION_INTERVAL} seconds before next execution.")
        
        # Step 8: Wait for the specified interval before next execution
        for _ in range(EXECUTION_INTERVAL):
            if not running:
                break
            time.sleep(1)
