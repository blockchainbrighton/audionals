...
4. Launches Chrome using the specified profile and navigates to a target URL.
...
TARGET_URL = "https://chatgpt.com/"  # URL to navigate to upon launch
...
def launch_chrome_with_profile(profile_directory, url):
    """
    Launch Google Chrome with the specified profile directory and navigate to the given URL.
    """
    logging.debug(f"Launching Chrome with profile directory: {profile_directory} and navigating to URL: {url}")
    if not os.path.exists(CHROME_EXECUTABLE_PATH):
        logging.error("Chrome executable not found.")
        return False
    ...
    try:
        subprocess.Popen([
            ...
            url  # Navigate to the target URL upon launch
        ])
        logging.info(f"Chrome launched with profile '{profile_directory}' and navigating to '{url}'.")
        return True
    except Exception as e:
        logging.error(f"Failed to launch Chrome with profile '{profile_directory}' and URL '{url}': {e}")
        return False
...
def main():
    ...
    success = launch_chrome_with_profile(TARGET_PROFILE_DIRECTORY, TARGET_URL)
    if not success:
        logging.error("Failed to launch Chrome with the specified profile and navigate to the target URL.")
        sys.exit(1)
