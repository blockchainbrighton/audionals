...
export const mapManager = {
    ...
    renderMiniMap: function(ctx, player, camera) {
        if (!ctx || !ctx.canvas) return;
        const { width, height } = ctx.canvas;
        const tileWidth = width / this.game.config.MAP_WIDTH_TILES;
        const tileHeight = height / this.game.config.MAP_HEIGHT_TILES;
        ...
        for (let r = 0; r < this.game.config.MAP_HEIGHT_TILES; r++) {
            for (let c = 0; c < this.game.config.MAP_WIDTH_TILES; c++) {
                ...
                ctx.fillRect(c * tileWidth, r * tileHeight, Math.ceil(tileWidth), Math.ceil(tileHeight));
            }
        }
        ...
        if (camera) {
            ...
            ctx.lineWidth = 1;
            ctx.strokeRect(
                camera.x / this.game.config.TILE_SIZE * tileWidth,
                camera.y / this.game.config.TILE_SIZE * tileHeight,
                camera.width / this.game.config.TILE_SIZE * tileWidth,
                camera.height / this.game.config.TILE_SIZE * tileHeight
            );
        }
        ...
        if (player) {
            ...
            ctx.beginPath();
            ctx.arc(
                (player.x + player.width / 2) / this.game.config.TILE_SIZE * tileWidth,
                (player.y + player.height / 2) / this.game.config.TILE_SIZE * tileHeight,
                Math.max(2, tileWidth * 0.5),
                0,
                Math.PI * 2
            );
            ctx.fill();
