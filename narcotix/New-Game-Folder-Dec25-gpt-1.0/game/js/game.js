...
import { narcotixCollectionManager } from './narcotixCollectionManager.js';
import { stashManager } from './stashManager.js';
...
export const game = {
    ...
    narcotixCollection: narcotixCollectionManager,
    stashManager: stashManager,

    init: async function(canvas, ctx) {
        this.canvas = canvas;
        this.ctx = ctx;
        this.touchControls.init(this); 
        this.hud.init(this);
        this.miniMapCtx = document.getElementById('miniMapCanvas')?.getContext('2d') || null;
        ...
        this.isDayTime = true;
        this.keysPressed = {};
        this.spawnGraceEndTime = Date.now() + (this.config.PLAYER_GRACE_PERIOD_MS || 0);
        ...
        this.narcotixCollection.init(this);
        ...
        this.stashManager.init(this);
        this.hud.init(this);

        await this.narcotixCollection.ensureReady();

        this.mapManager.generateMap();
        this.itemManager.spawnInitialItems();
        this.enemyManager.spawnInitialEnemies();
        const safeRadiusPx = (this.config.PLAYER_SAFE_RADIUS_TILES || 6) * this.config.TILE_SIZE;
        this.enemyManager.enforcePlayerSafeZone(this.player, safeRadiusPx);


        this.hud.update();
        this.utils.addMessage("NarcotiX Systems Online. Welcome, Xperient.");
        if (this.config.PLAYER_GRACE_PERIOD_MS) {
            const seconds = Math.round(this.config.PLAYER_GRACE_PERIOD_MS / 1000);
            this.utils.addMessage(`Spawn shield online (${seconds}s). Establish footing before hostiles engage.`);
        }
    ...
    update: function(deltaTime) {
        ...
        if (this.gameState === 'PLAYING' || this.gameState === 'INVENTORY_OPEN' || this.gameState === 'QUESTLOG_OPEN' || this.gameState === 'CODEX_OPEN' || this.gameState === 'STASH_OPEN') {
            ...
            if (!this.graceNotified && !this.isPlayerInvulnerable()) {
                this.graceNotified = true;
                this.utils.addMessage("Spawn shield disengaged. Hostile damage enabled.");
            }
        }
        this.hud.update();
        this.utils.processTutorialQueue(this);
    },
    isPlayerInvulnerable: function() {
        return Date.now() < this.spawnGraceEndTime;
    },
    ...
    render: function() {
        ...
        if (this.miniMapCtx) {
            this.mapManager.renderMiniMap(this.miniMapCtx, this.player, this.camera);
        }
    ...
    restartGame: async function() {
        ...
        this.gameState = 'PLAYING';
        await this.init(this.canvas, this.ctx);
    ...
    openStash: function() {
        this.stashManager.open();
        const stashPanel = document.getElementById('stashInterface');
    ...
    toggleCodex: function() {
        ...
        if (panel.style.display === 'block') {
            ...
            if (this.gameState === 'CODEX_OPEN') this.gameState = 'PLAYING';
        } else {
            this.narcotixCollection.renderCodexGrid();
            panel.style.display = 'block';
            this.gameState = 'CODEX_OPEN';
            const inventoryDisplayEl = document.getElementById('inventoryDisplay');
            if (inventoryDisplayEl) inventoryDisplayEl.style.display = 'none';
            const questLogEl = document.getElementById('questLog');
            if (questLogEl) questLogEl.style.display = 'none';
                    ...
                    if(tileProps && tileProps.interactive){
                        ...
                        if (this.utils.distance(this.player.x + this.player.width/2, this.player.y + this.player.height/2, tileWorldX, tileWorldY) <= this.config.TILE_SIZE * 1.2) {
                            if(tileData.interactionType && tileData.interactionTargetId){
                                if (tileData.interactionType === 'exchange_node') {
                                    this.openShop(tileData.interactionTargetId); return;
                                } else if (tileData.interactionType === 'xlounge_stash') {
                                    this.openStash(); return;
