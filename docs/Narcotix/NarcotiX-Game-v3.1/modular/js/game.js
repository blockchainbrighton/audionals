...
class NarcotiXGame {
    constructor() {
        ...
        this.hustleStreak = 0;
        this.hustleMultiplier = 1;
        this.lastHustleTime = 0;
        this.supplyDrops = [];
    ...
    interact() {
        ...
        const nearbyDrop = this.supplyDrops.find(drop =>
            Math.hypot(this.player.x - drop.x, this.player.y - drop.y) < GAME_CONFIG.SUPPLY_DROP_RADIUS
        );
        if (nearbyDrop) {
            this.claimSupplyDrop(nearbyDrop);
        }
    }
    ...
    openTradePanel(buyer) {
        ...
        passBtn.onclick = () => this.closeTradePanel(true);
        options.appendChild(passBtn);
        this.pauseGame('trade');
    }

    closeTradePanel(skip = false) {
        const panel = document.getElementById('tradePanel');
        panel.style.display = 'none';
        if (this.activeBuyer) {
            this.activeBuyer.tradeActive = false;
            if (skip) {
                this.activeBuyer.snoozeUntil = Date.now() + 4000;
            }
        }
        this.activeBuyer = null;
    ...
    sellPillToBuyer(buyer, pill) {
        ...
        const basePrice = buyer.offerValue + Math.floor(pill.marketValue * 0.2);
        const salePrice = Math.floor(basePrice * this.hustleMultiplier);
    ...
    consumePill() {
        ...
        const pill = this.player.selectedPill;
        const consumed = this.player.consumePill(pill);
        this.player.selectedPill = null;

        if (consumed) {
            this.createParticle(this.player.x, this.player.y, 'CONSUMED!', pillColor);
            this.logPillConsumption(pill);
            this.updateInventoryUI();
            this.updateUI();
        } else {
    ...
    registerHustle(reason = 'action') {
        const now = Date.now();
        if (now - this.lastHustleTime > GAME_CONFIG.HUSTLE_TIMEOUT) {
            this.hustleStreak = 0;
        }
        this.hustleStreak = Math.min(GAME_CONFIG.HUSTLE_MAX_STREAK, this.hustleStreak + 1);
        this.hustleMultiplier = Number((1 + this.hustleStreak * GAME_CONFIG.HUSTLE_STEP).toFixed(2));
        this.lastHustleTime = now;
        this.createParticle(this.player.x, this.player.y, `${reason.toUpperCase()} x${this.hustleMultiplier}`, '#ff0');
    }

    logPillConsumption(pill) {
        if (!pill) return;
        this.pillLog.unshift({
            name: pill.name,
            effect: pill.effect,
            rarity: pill.rarity,
            sideEffect: pill.sideEffect,
            timestamp: new Date().toLocaleTimeString()
        });
        this.pillLog = this.pillLog.slice(0, 6);
    }

    decayHustle(deltaSeconds) {
        if (this.hustleStreak === 0) return;
        if (Date.now() - this.lastHustleTime > GAME_CONFIG.HUSTLE_TIMEOUT) {
            this.hustleStreak = 0;
            this.hustleMultiplier = 1;
        }
    }
    ...
    applyDamageToEnemy(enemy, damage, color = '#ff0') {
        ...
        if (enemy.stats.hp <= 0) {
            ...
            const bounty = Math.floor(40 * this.hustleMultiplier);
            this.player.cash += bounty;
            this.player.score += bounty;
            this.createParticle(enemy.x, enemy.y, 'ELIMINATED!', '#f00');
            this.registerHustle('takedown');
        }
    }
    ...
    updateUI() {
        ...
        document.getElementById('hustleMultiplier').textContent = `${this.hustleMultiplier.toFixed(2)}x`;
        document.getElementById('hustleStreak').textContent = this.hustleStreak;
        ...
        const pillLogList = document.getElementById('pillLog');
        if (pillLogList) {
            pillLogList.innerHTML = '';
            this.pillLog.forEach(entry => {
                const li = document.createElement('li');
                li.textContent = `[${entry.timestamp}] ${entry.name} (${entry.rarity}) ➜ ${entry.effect}${entry.sideEffect && entry.sideEffect !== 'NONE' ? ` / Side: ${entry.sideEffect}` : ''}`;
                pillLogList.appendChild(li);
            });
        }
    }
    ...
    checkBuyerCollisions() {
        ...
        const nearbyBuyer = this.pedestrians.find(ped =>
            ...
            !ped.fulfilled &&
            !ped.tradeActive &&
            (!ped.snoozeUntil || ped.snoozeUntil < Date.now()) &&
    ...
    spawnSupplyDrop() {
        const spawn = this.getValidSpawnPoint(8, this.player.x, this.player.y);
        const drop = {
            x: spawn.x,
            y: spawn.y,
            expiresAt: Date.now() + GAME_CONFIG.SUPPLY_DROP_LIFETIME,
            claimed: false
        };
        this.supplyDrops.push(drop);
        this.createParticle(drop.x, drop.y, 'SUPPLY DROP!', '#f0f');
    }

    updateSupplyDrops(deltaSeconds) {
        const now = Date.now();
        if (now - this.lastSupplyDrop > GAME_CONFIG.SUPPLY_DROP_INTERVAL) {
            this.spawnSupplyDrop();
            this.lastSupplyDrop = now;
        }
        this.supplyDrops = this.supplyDrops.filter(drop => drop.expiresAt > now && !drop.claimed);
    }

    claimSupplyDrop(drop) {
        if (!drop) return;
        drop.claimed = true;
        const rewardType = Math.random();
        if (rewardType < 0.4) {
            const cash = Math.floor(300 * this.hustleMultiplier);
            this.player.cash += cash;
            this.player.score += cash;
            this.createParticle(drop.x, drop.y, `+$${cash}`, '#0f0');
        } else if (rewardType < 0.7) {
            const weaponTypes = ['pistol', 'shotgun', 'rifle'];
            const type = weaponTypes[Math.floor(Math.random() * weaponTypes.length)];
            this.player.addWeapon(type);
            this.player.addAmmo(type, 15);
            this.createParticle(drop.x, drop.y, `${type.toUpperCase()} SUPPLY`, '#ff0');
        } else {
            const pill = this.nftCollection.getRandomPill();
            this.player.addToInventory(pill);
            this.createParticle(drop.x, drop.y, pill.name.substring(0, 8), '#f0f');
        }
        this.registerHustle('crate');
        for (let i = 0; i < 3; i++) {
            const spawn = this.getValidSpawnPoint(5, drop.x, drop.y);
            this.enemies.push(new Enemy(spawn.x, spawn.y, 'police', this.nftCollection));
        }
        this.supplyDrops = this.supplyDrops.filter(d => !d.claimed);
    }
    ...
    render() {
        ...
        this.supplyDrops.forEach(drop => {
            ...
            ctx.fillRect((drop.x - 0.5) * GAME_CONFIG.TILE_SIZE, (drop.y - 0.5) * GAME_CONFIG.TILE_SIZE, GAME_CONFIG.TILE_SIZE, GAME_CONFIG.TILE_SIZE);
            ctx.strokeStyle = '#fff';
            ctx.strokeRect((drop.x - 0.5) * GAME_CONFIG.TILE_SIZE, (drop.y - 0.5) * GAME_CONFIG.TILE_SIZE, GAME_CONFIG.TILE_SIZE, GAME_CONFIG.TILE_SIZE);
            ctx.fillStyle = '#fff';
            ctx.font = 'bold 12px monospace';
            ctx.fillText('DROP', drop.x * GAME_CONFIG.TILE_SIZE - 18, drop.y * GAME_CONFIG.TILE_SIZE + 4);
        });
    ...
    gameLoop(timestamp = 0) {
        ...
        if (!this.isPaused()) {
            ...
            this.decayHustle(deltaSeconds);
            this.updateSupplyDrops(deltaSeconds);
