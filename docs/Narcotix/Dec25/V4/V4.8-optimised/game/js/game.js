    ...
    update: function(deltaTime) {
        ...
        if (this.gameState === 'PLAYING' || this.gameState === 'INVENTORY_OPEN' || this.gameState === 'QUESTLOG_OPEN') {
            ...
            if (this.gameState === 'PLAYING') {
                ...
                // Update Entities (Enemies, Items, Projectiles)
                if (this.entities) {
                    this.entities.forEach(e => e.update(deltaTime));
                    this.entities = this.entities.filter(e => !e.markedForDeletion);
                }

                this.enemyManager.updateEnemies(deltaTime); // Keep for legacy list mgmt if needed
                ...
                // this.itemManager.updateItemsOnMap(deltaTime); // Redundant
                // this.projectileManager.update(deltaTime); // Redundant
                this.particleManager.update(deltaTime); // Update particles
                this.floatingTextManager.update(deltaTime); // Update floating text
    ...
    renderCasinoUI: function(container, actions) {
        container.innerHTML = `
            <div style="text-align:center;">
                <h3>HIGH / LOW</h3>
                <p>Wager 50 Creds. Roll > 50 to win double.</p>
                <div id="casinoResult" style="font-size:24px; margin:20px; color:#FFD700;">-</div>
                <button class="button" style="width:200px;" onclick="game.playCasinoHighLow(50)">BET 50c</button>
            </div>
        `;
        actions.innerHTML = `<button class="button" onclick="game.exitLocation()">Leave Casino</button>`;
    },

    playCasinoHighLow: function(bet) {
        if(this.player.payMoney(bet)) {
            const roll = Math.floor(Math.random() * 100) + 1;
            const resEl = document.getElementById('casinoResult');
            
            if (roll > 50) {
                const win = bet * 2;
                this.player.earnMoney(win);
                if(resEl) {
                    resEl.textContent = `ROLLED: ${roll} - WIN! (+${win}c)`;
                    resEl.style.color = '#0F0';
                }
                this.soundManager.playPickup(); // Cha-ching
            } else {
                if(resEl) {
                    resEl.textContent = `ROLLED: ${roll} - LOSS`;
                    resEl.style.color = '#F00';
                }
            }
        } else {
            this.utils.addMessage("Not enough Creds to gamble.");
        }
    },
