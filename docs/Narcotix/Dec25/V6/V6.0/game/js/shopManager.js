...
export const shopManager = {
    ...
    init: function(gameInstance) {
        ...
        this.shops = {
            ...
            'armoury_local': {name:"Munitions Depot 01", welcome:"Ordnance Access Granted. Keep it lethal.", inventory:[{itemId:'ammo_light_rounds',stock:500,basePriceModifier:1.1}, {itemId:'pistol',stock:3,basePriceModifier:1.2}], buys:['weapon']},
    ...
    populateShopUI: function(shopId){
        ...
        // Helper to get Image
        const getImg = (itemDef) => {
            let url = itemDef.imageUrl;
            if (!url) {
                if (itemDef.type === 'weapon' && itemDef.weaponId) {
                    url = `artwork/${itemDef.weaponId}.svg`;
                } else {
                    url = `artwork/${itemDef.id}.svg`;
                }
            }
            if (itemDef.nftTraits && itemDef.nftTraits.id) {
                 url = imageLoader.getUrl(itemDef.nftTraits.id) || url;
            }
            return url;
        };
        ...
        shop.inventory.forEach(entry => {
            ...
            if(iDef){
                ...
                div.onclick = () => this.game.showItemDetails(iDef); // Inspect

                const imgUrl = getImg(iDef);
                ...
                div.innerHTML=`
                    <div style="display:flex; align-items:center; justify-content:space-between; width:100%;">
                        ...
                        <button class="button button-small" onclick="event.stopPropagation(); game.shopManager.buyItem('${shopId}','${entry.itemId}')">Acquire</button>
                    </div>
                `; 
        ...
        this.game.player.inventory.items.forEach(pItem => {
            ...
            if(sellPriceToThisVendor > 0 || (pItem.questItem && this.shops[shopId].buys.includes(pItem.id)) ){
                ...
                div.className='shopItem';
                div.style.cursor = 'pointer';
                div.onclick = () => this.game.showItemDetails(pItem); // Inspect

                const imgUrl = getImg(pItem);
                ...
                div.innerHTML=`
                    <div style="display:flex; align-items:center; justify-content:space-between; width:100%;">
                        ...
                        <div>
                            <button class="button button-sell" onclick="event.stopPropagation(); game.shopManager.sellItem('${shopId}','${pItem.id}',1)">Sell 1</button>
                            ${(pItem.quantity||1)>1 ? `<button class="button button-sell" style="margin-left:5px;" onclick="event.stopPropagation(); game.shopManager.sellItem('${shopId}','${pItem.id}',${pItem.quantity||1})">Sell All</button>` : ''}
                        </div>
                    </div>
                `;
