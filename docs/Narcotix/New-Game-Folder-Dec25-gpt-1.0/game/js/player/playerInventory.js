...
export function addItem(item) {
    ...
    const acquiredLabel = item.nftEntry?.name ? `${item.nftEntry.name} Pill` : item.name;
    this.game.utils.addMessage(`Acquired ${acquiredLabel} (x${item.quantity || 1}). Stored.`);
...
export function dropInventoryItem(itemIndex) {
    const item = this.inventory.items[itemIndex];
    if (!item) return;
    const [removed] = this.inventory.items.splice(itemIndex, 1);
    if (!removed) return;
    const dropX = this.x + this.width / 2 - (this.game.config.TILE_SIZE * 0.4);
    const dropY = this.y + this.height / 2 - (this.game.config.TILE_SIZE * 0.4);
    const dropClone = { ...removed };
    if (removed.type === 'weapon' && removed.type === Weapons.weaponTypes.RANGED) {
        dropClone.currentAmmo = removed.currentAmmo;
    }
    this.game.itemManager.dropItem(dropClone, dropX, dropY);
    this.game.utils.addMessage(`${removed.name} dropped.`);
    this.renderInventory();
    this.game.hud.update();
}
...
export function pickupItems() {
    for (let i = this.game.itemManager.onMapItems.length - 1; i >= 0; i--) {
        ...
        if (this.game.utils.AABBCollision(this, itemOnMap)) {
            ...
            if (itemOnMap.id === 'narcotix_pill' && itemOnMap.nftEntry) {
                if (itemToAdd) itemToAdd.nftEntry = itemOnMap.nftEntry;
                this.game.narcotixCollection.recordPickup(itemOnMap.nftEntry);
            }
...
export function renderInventory() {
    ...
    this.inventory.items.forEach((item, index) => {
        ...
        content += `<button class="button button-close" style="float:right;margin-right:5px;" onclick="game.player.dropInventoryItem(${index})">Drop</button>`;
