...
function openContractCallWrapper(options) {
    const fee = options.fee || 500000;
    journeyLog(`[ContractCall] Fee set to: ${fee} uSTX`);
    
    return new Promise((resolve, reject) => {
        Connect.openContractCall({
            ...options,
            fee: String(fee), // Explicitly cast to string
    ...
    ensureAuth(async () => {
        try {
            ...
            const args = [ 
                ...
                bufferCV(Buffer.from(currentRoot))
            ];
            
            const statusEl = document.getElementById('mint-steps');
            statusEl.innerHTML = `
                <div style="background:#eef; padding:15px; border-radius:5px;">
                    <h3 style="margin-top:0">Step 1: Initialize Upload Session</h3>
                    <p>Creating "Upload Session" on-chain...</p>
                </div>
            `;
            ...
            statusEl.innerHTML = `
                <div style="background:#fff3cd; padding:15px; border-radius:5px; color:#856404;">
                    ...
                    <p>Waiting for Session ID assignment...</p>
            ...
            if (inscriptionId === null) {
                throw new Error("Could not parse Session ID from transaction result.");
            }

            journeyLog(`Auto-detected Session ID: ${inscriptionId}`);
            
            statusEl.innerHTML = `
                <div style="background:#d4edda; padding:15px; border-radius:5px; color:#155724;">
                    <h3 style="margin-top:0">Step 2: Upload Data</h3>
                    <p><strong>Session ID: #${inscriptionId}</strong> ready.</p>
...
document.getElementById('btn-resume-mint').addEventListener('click', async () => {
    ...
    journeyLog(`Attempting to resume Upload Session ID: ${id}`);
    ...
    try {
        ...
        if (!meta) throw new Error("Upload Session ID not found on-chain.");
        
        // INTEGRITY CHECK
        const onChainRoot = meta.value['merkle-root'].value.substring(2); // remove 0x
        const localRoot = bufToHex(currentRoot);
        journeyLog("Integrity Check", { onChain: onChainRoot, local: localRoot });
        
        if (onChainRoot !== localRoot) {
            throw new Error("File Mismatch! The file you loaded does not match the one started in this session.");
        }
        ...
        if (meta.value.sealed.value) {
            statusEl.innerHTML = `<span style="color:green">Session #${id} is already SEALED. Inscription Number: ${meta.value.number.value}</span>`;
            return;
        }
...
async function sealInscriptionTransaction(id, root, statusEl) {
    ...
    journeyLog("Sealing inscription...", { uploadId: id });
    const txData = await openContractCallWrapper({
        contractAddress: address,
        contractName: name,
    ...
    statusEl.innerHTML += `
        <div style="background:#fff3cd; padding:10px; border-radius:5px; color:#856404; margin-top: 10px;">
            <p>Sealing TX Sent: ${txData.txId}</p>
            <p>Waiting for Final Inscription Number...</p>
            <div class="spinner">...</div>
        </div>
    `;

    const txDetails = await waitForTransactionSuccess(txData.txId);
    let finalNumber = null;
    if (txDetails.tx_result.repr.includes('(ok u')) {
            const match = txDetails.tx_result.repr.match(/\(ok u(\d+)\)/);
            if (match && match[1]) finalNumber = parseInt(match[1]);
    }

    statusEl.innerHTML += `
        <div style="background:#d4edda; padding:10px; border-radius:5px; color:#155724; margin-top: 10px;">
            <h3 style="margin-top:0">🎉 Inscription #${finalNumber} Complete!</h3>
            <p>Upload Session #${id} is now Inscription #${finalNumber}.</p>
            <button onclick="document.getElementById('manifest-id-input').value = ${finalNumber}; showPage('play');" style="background:#28a745; color:white; border:none; padding:10px; cursor:pointer;">View Inscription #${finalNumber}</button>
        </div>
    `;
}
...
async function startChunkUploads(id, statusEl, specificIndices = null) {
    ...
    for (let i = 0; i < indicesToUpload.length; i += BATCH_SIZE) {
        const batchIndices = indicesToUpload.slice(i, i + BATCH_SIZE);
        journeyLog(`Preparing Batch ${i/BATCH_SIZE + 1} (Chunks ${batchIndices[0]} - ${batchIndices[batchIndices.length-1]})`);
        
        const p = document.createElement('p');
        p.innerText = `Uploading Batch (Chunks ${batchIndices.join(', ')})...`;
        ...
        try {
            ...
            const batchEntries = batchIndices.map(idx => tupleCV({
                id: uintCV(id),
                index: uintCV(idx),
                data: bufferCV(Buffer.from(currentChunks[idx]))
            }));

            // Calculate Dynamic Fee
            const BASE_FEE = 100000; // 0.1 STX base
            const CHUNK_FEE = 100000; // 0.1 STX per chunk (generous safety margin)
            const batchFee = BASE_FEE + (batchIndices.length * CHUNK_FEE);

            journeyLog(`Calculating Batch Fee: ${batchFee/1000000} STX`);
            ...
            await openContractCallWrapper({
                contractAddress: address,
                contractName: name,
                functionName: 'add-batch',
                functionArgs: [ listCV(batchEntries) ],
                network,
                userSession,
                postConditionMode: PostConditionMode.Allow,
                anchorMode: AnchorMode.Any,
            ...
            p.innerText = `Batch (Chunks ${batchIndices.join(', ')}) - Confirmed ✓`;
            p.style.color = '#28a745';
        } catch (err) {
            p.innerText = `Batch Failed ✕`;
...
async function fetchInscriptionData(inscriptionNumber) {
    const { address, name } = getContractDetails();
    journeyLog(`Fetching inscription data for Number: ${inscriptionNumber}`);
    
    try {
        // 1. Resolve Number -> ID
        const idRes = await callReadOnlyFunction({
            contractAddress: address, contractName: name,
            functionName: 'get-id-by-number', 
            functionArgs: [uintCV(inscriptionNumber)],
            senderAddress: address, network
        });
        const idVal = cvToValue(idRes);
        if (idVal === null || idVal === undefined) {
             throw new Error(`Inscription #${inscriptionNumber} not found.`);
        }
        const id = Number(idVal.value);
        journeyLog(`Resolved Inscription #${inscriptionNumber} to Upload ID #${id}`);
