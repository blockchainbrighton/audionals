...
document.getElementById('btn-play-single').addEventListener('click', async () => {
    ...
    journeyLog(`User clicked 'Play/View' for ID: ${id}`);
    try {
        playerLog("Fetching...");
        const { data, mimeType } = await fetchInscriptionData(id);
        
        journeyLog(`Data fetched. MIME: ${mimeType}`);
        ...
        if (mimeType.startsWith('audio/')) {
            ...
            container.appendChild(vid);
             ...
             playerLog(`Detected ${mimeType}. Rendering...`);
             const blob = new Blob([data], { type: mimeType });
             const url = URL.createObjectURL(blob);
            ...
            playerLog(`Detected ${mimeType}. Showing as text...`);
            const text = new TextDecoder().decode(data);
            const pre = document.createElement('pre');
            pre.innerText = text.substring(0, 2000) + (text.length > 2000 ? '...' : '');
