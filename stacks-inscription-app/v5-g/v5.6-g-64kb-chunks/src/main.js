...
function renderMintFileStats(pendingMeta = null) {
    ...
    const avgChunkSize = currentChunks.length > 0 ? Math.round(currentChunks.reduce((a,b)=>a+b.length,0) / currentChunks.length) : 0;
    
    const missingCount = pendingMeta ? null : chunkCount; 
    ...
    statsEl.innerHTML = `
        ...
        <div class="k">Chunks</div><div class="v">${formatInt(chunkCount)} (Avg ~${formatInt(avgChunkSize)} bytes)</div>
...
const getSelectedChunkSize = () => {
    const el = document.getElementById('chunk-size-select');
    if (!el) return 16384; // default safe
    const val = parseInt(el.value, 10);
    return (val && val > 0) ? val : 16384;
};

async function handleSelectedFile(file) {
    if (!file) return;
    lastSelectedFile = file; // Save for re-processing
    ...
    journeyLog(`Using Chunk Size: ${chunkSize} bytes`);
    
    currentChunks = chunkFile(buf, chunkSize);
...
// Re-process when chunk size changes
document.getElementById('chunk-size-select')?.addEventListener('change', () => {
    if (lastSelectedFile) {
        journeyLog("Chunk size changed, re-processing file...");
        handleSelectedFile(lastSelectedFile);
    }
});
...
async function fetchInscriptionData(id) {
    ...
    try {
        ...
        const fetchChunk = async (i) => {
            ...
            // Use retry wrapper to handle 429/CORS intermittent failures
            const res = await callReadOnlyFunctionWithRetry({
            ...
            }, { retries: 4, baseDelayMs: 1000 }); // More aggressive retries for view
            
            const val = cvToValue(res);
            if (!val) throw new Error(`Chunk ${i} missing.`);
            ...
            if (!bytes || !bytes.length) throw new Error(`Chunk ${i} empty.`);
            return bytes;
        };

        // Queue-based execution
        const queue = Array.from({length: count}, (_, i) => i);
        const workers = Array(Math.min(count, CONCURRENCY)).fill(null).map(async () => {
            while (queue.length > 0) {
                const i = queue.shift();
                try {
                    buffers[i] = await fetchChunk(i);
                } catch (e) {
                    journeyLog(`Failed chunk ${i}`, e);
                    throw e; // Fail fast or implement smarter partial recovery
                }
            }
        });
