...
const renderFeeEstimates = (context = { mode: 'mint', missingCount: null }) => {
    ...
    let feeRateLine = 'Fee-rate estimate: not loaded (click “Fetch Fee Rate”)';
    if (Number.isFinite(networkFeeRateMicroPerByte) && networkFeeRateMicroPerByte > 0) {
        ...
        if (bytesTotal !== null) {
            ...
            feeRateLine = `Fee-rate estimate: ${formatMicroStx(micro)} (rate ${networkFeeRateMicroPerByte} microSTX/byte)`;
        } else {
            feeRateLine = `Fee-rate loaded: ${networkFeeRateMicroPerByte} microSTX/byte`;
        }
    }
...
document.getElementById('btn-fetch-fee-rate')?.addEventListener('click', async () => {
    try {
        const res = await fetch(`${network.coreApiUrl}/v2/fees/transfer`, { cache: 'no-store' });
        const data = res.ok ? await res.json() : null;
        const rate = data && (data.fee_rate ?? data.feeRate);
        const parsed = Number(rate);
        if (!Number.isFinite(parsed) || parsed <= 0) throw new Error('Fee rate unavailable');
        networkFeeRateMicroPerByte = parsed;
        journeyLog('Network fee rate loaded', { microSTXPerByte: networkFeeRateMicroPerByte });
        renderFeeEstimates({ mode: 'mint', missingCount: null });
    } catch (e) {
        journeyLog('Fee rate fetch failed', { error: e?.message || String(e) });
        renderMintProgress('<div style="color:#856404">Could not fetch fee rate; using configured fee/tx.</div>');
    }
});
