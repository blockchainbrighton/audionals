...
Instructions for working in this directory tree (`v4-codex/cv4.2`).
...
## Key Files
- `src/main.js`: Thin Vite entrypoint (boots the app).
- `src/app/initApp.js`: App composition/root wiring.
- `src/features/auth/*`: Stacks Connect auth + diagnostics.
- `src/features/mint/*`: File selection, mint/resume pipeline, mint UI rendering.
- `src/features/viewer/*`: Universal viewer + gallery + manifest playback.
- `src/stacks/contractService.js`: Contract IO (deploy, calls, read-only retry, tx polling).
...
- `docs/ARCHITECTURE.md`: cv4.2 structure + conventions.
...
## Editing Guidance
- Preserve existing element IDs in `index.html` (they’re referenced from the feature modules).
