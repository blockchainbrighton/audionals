...
export function initApp() {
  ...
  // Use local proxy in dev to avoid CORS, direct URL in prod
  const networkUrl = import.meta.env.DEV ? '/hiro-api' : 'https://api.testnet.hiro.so';
  ...
  const mintFeature = createMintFeature({
    ...
    feeFeature,
    mintState,
    onInscriptionSealed: async (id) => {
      journeyLog(`MintFeature reported seal for #${id}. Updating viewer...`);
      await viewerFeature.refreshInscription(id);
    },
  });
  ...
  installNavigation({
    ...
    onShow: {
      ...
      play: () => {
        ...
        void viewerFeature.scanForNewInscriptions();
      },
    },
  });
