...
export function createViewerFeature({ journeyLog, network, contractService } = {}) {
  ...
  async function scanForNewInscriptions() {
    ...
    while (gapCount < MAX_GAP) {
      // Fetch in small batches to speed up discovery
      const batchSize = 5;
      const promises = [];
  ...
  async function renderGalleryPage(page) {
    ...
    await Promise.all(
      idsToFetch.map(async (id) => {
        ...
        if (!card) {
            // If we received data for an ID we didn't expect (e.g. discovery happened during render), append it
            // But strict grid management is easier if we just rely on the placeholders.
            return;
        }
