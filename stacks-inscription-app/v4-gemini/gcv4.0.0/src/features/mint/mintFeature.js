...
export function createMintFeature({ journeyLog, userSession, network, ensureAuth, contractService, feeFeature, mintState, onInscriptionSealed } = {}) {
  ...
  async function sealInscriptionTransaction(id, root, statusEl) {
    ...
    // Notify listener (e.g., ViewerFeature) that this ID is now sealed/updated
    if (onInscriptionSealed && typeof onInscriptionSealed === 'function') {
      try {
        await onInscriptionSealed(id);
      } catch (err) {
        console.error('onInscriptionSealed callback failed:', err);
      }
    }
