...
### Viewer & Minting Optimization
*   **Viewer Feature:**
    *   Implemented `maxKnownId` tracking to dynamically discover the number of inscriptions.
    *   Added `scanForNewInscriptions` to automatically find new inscriptions on load and navigation.
    *   Added `refreshInscription(id)` to allow targeted cache invalidation.
    *   Improved gallery rendering to respect the `maxKnownId` and avoid rendering invalid placeholders.
    *   **Rate Limiting**: Implemented batching (concurrency limit of 4) for gallery fetches and scan logic to prevent HTTP 429 (Too Many Requests) errors from the Stacks node.
    *   **Smart Caching**: Added contract-aware caching. Switching contracts now invalidates the metadata cache and resets `maxKnownId`, preventing data leaks between environments.
    *   **Optimization**: Removed aggressive `scanForNewInscriptions` call on navigation to the Viewer tab. Discovery now happens primarily on initialization or manual refresh, reducing redundant API load.
*   **Mint Feature:**
    *   Added `onInscriptionSealed` callback to `createMintFeature`.
    *   Wired up `initApp.js` to notify the viewer when a new inscription is sealed, ensuring instant UI updates.

### Infrastructure
*   **CORS Fix**: Added a Vite proxy configuration (`/hiro-api`) to bypass CORS errors when developing locally against the Hiro Testnet API. Updated `initApp.js` to use this proxy path in development mode.
*   **Network Configuration**: Updated `initApp.js` to use an absolute URL (`window.location.origin + '/hiro-api'`) for the development proxy. This prevents `StacksTestnet` from treating the relative path as invalid or undefined, ensuring transaction polling works correctly via the proxy.

---

# Update Log - Gemini Context Setup

## Date: 2026-01-03

### Documentation
*   **GEMINI.md**: Established `GEMINI.md` as the central context file, replacing `AGENTS.md`.
*   **Version**: Updated architecture documentation to `v4-gemini`.
