...
// Public Stacks node API endpoints (not Hiro-hosted). Using these for read-only calls avoids
// Hiro platform rate limits ("upgrade your plan") for high-volume chunk reads.
const STACKS_NODE_API = {
    testnet: 'https://stacks-node-api.testnet.stacks.co',
    mainnet: 'https://stacks-node-api.mainnet.stacks.co',
};

const PENDING_DEPLOY_KEY = 'pending-deploy:last';
...
const HIRO_TESTNET_BASE_URL = resolveHiroBaseUrl('testnet');
// Use Stacks node API for call-read heavy operations (viewer grid, chunk fetches).
// Hiro remains available for fee-rate lookups and tx status endpoints.
const network = new StacksTestnet({ url: STACKS_NODE_API.testnet });

journeyLog("Network configured", { readOnlyApiUrl: network.coreApiUrl, hiroApiUrl: HIRO_TESTNET_BASE_URL });

const loadPendingDeploy = () => {
    try {
        const raw = localStorage.getItem(PENDING_DEPLOY_KEY);
        if (!raw) return null;
        const parsed = JSON.parse(raw);
        if (!parsed?.txId || !parsed?.contract) return null;
        return parsed;
    } catch {
        return null;
    }
...
const savePendingDeploy = (state) => {
    pendingDeploy = state;
    try {
        if (!state) localStorage.removeItem(PENDING_DEPLOY_KEY);
        else localStorage.setItem(PENDING_DEPLOY_KEY, JSON.stringify(state));
    } catch {
        // ignore
    }
};
...
const renderDeployStatus = () => {
    ...
    if (!pendingDeploy) {
        ...
        el.innerText = 'No pending deploy tracked.';
        return;
    }
    const ageSec = Math.floor((Date.now() - (pendingDeploy.startedAtMs || Date.now())) / 1000);
    el.className = 'status warn';
    el.innerText = `Deploy pending (${ageSec}s): ${pendingDeploy.contract} • TX ${pendingDeploy.txId.slice(0, 12)}…`;
};

// Load deploy state early
pendingDeploy = loadPendingDeploy();
renderDeployStatus();
void checkHiroProxyActive().then((r) => {
    if (!r.active) {
        journeyLog('WARNING: Hiro proxy not active', r);
        setAuthStatus(
            'Dev proxy not active: /hiro-testnet is serving the app HTML. Restart `npm run dev` from v3.0.1 to enable proxy.',
            'warn'
        );
    }
});
...
const maybeFetchFeeRates = async ({ maxAgeMs = 60_000 } = {}) => {
    ...
    try {
        const current = await fetchFeeRateWithFallback('current', [HIRO_TESTNET_BASE_URL]);
...
async function fetchTxStatus(txId) {
    const candidates = [
        HIRO_TESTNET_BASE_URL,
        network.coreApiUrl, // stacks-node-api.* often supports /extended/v1/*
        HIRO_API.direct.testnet,
    ].filter(Boolean);
    ...
    for (const baseUrl of candidates) {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(`TX status fetch failed (${res.status}) via ${baseUrl}: ${text.slice(0, 180)}`);
            }

            if (!isJson) {
                const text = await res.text().catch(() => '');
                // Common case when a proxy isn't active: dev server serves index.html (<!DOCTYPE ...)
                throw new Error(`TX status non-JSON via ${baseUrl}: ${text.slice(0, 80)}`);
            }

            return await res.json();
        } catch (e) {
            lastErr = e;
            journeyLog('TX status fetch attempt failed', { baseUrl, error: getErrorMessage(e) });
        }
    }

    throw lastErr || new Error('TX status fetch failed');
}

async function waitForTxSuccessRobust(txId, { timeoutMs = 12 * 60 * 1000, baseDelayMs = 3000 } = {}) {
    const start = Date.now();
    let delay = baseDelayMs;
    while (Date.now() - start < timeoutMs) {
        try {
            const data = await fetchTxStatus(txId);
            if (data?.tx_status === 'success') return data;
            if (data?.tx_status === 'abort_by_response' || data?.tx_status === 'abort_by_post_condition') {
                const repr = data?.tx_result?.repr ? ` ${data.tx_result.repr}` : '';
                throw new Error(`Transaction failed: ${data.tx_status}${repr}`);
            }
        } catch (e) {
            // If Hiro rate limits tx status too, back off.
            if (isRateLimitError(e)) delay = Math.min(60_000, Math.floor(delay * 1.6));
            else delay = Math.min(30_000, Math.floor(delay * 1.2));
        }
        await sleep(delay);
    }
...
async function checkContractInterfaceAvailable({ address, name }) {
    const baseUrls = [HIRO_TESTNET_BASE_URL, network.coreApiUrl].filter(Boolean);
    let lastErr = null;
    for (const baseUrl of baseUrls) {
        const url = `${baseUrl}/v2/contracts/interface/${address}/${name}`;
        try {
            const res = await fetch(url, { cache: 'no-store' });
            if (!res.ok) return { ok: false, status: res.status, baseUrl };
            const json = await res.json().catch(() => null);
            return { ok: Boolean(json), status: res.status, json, baseUrl };
        } catch (e) {
            lastErr = e;
            journeyLog('Contract interface preflight fetch failed', { baseUrl, error: getErrorMessage(e) });
        }
    }
    return { ok: false, status: 0, baseUrl: baseUrls[0] || null, error: getErrorMessage(lastErr) };
}

async function checkHiroProxyActive() {
    if (!HIRO_API.useProxy) return { active: true, baseUrl: HIRO_TESTNET_BASE_URL };
    const url = `${HIRO_TESTNET_BASE_URL}/v2/info`;
    try {
        const res = await fetch(url, { cache: 'no-store' });
        const ct = res.headers.get('content-type') || '';
        if (!res.ok) return { active: false, baseUrl: HIRO_TESTNET_BASE_URL, status: res.status };
        if (!ct.includes('application/json')) {
            const text = await res.text().catch(() => '');
            return { active: false, baseUrl: HIRO_TESTNET_BASE_URL, status: res.status, error: text.slice(0, 80) };
        }
        await res.json().catch(() => null);
        return { active: true, baseUrl: HIRO_TESTNET_BASE_URL, status: res.status };
    } catch (e) {
        return { active: false, baseUrl: HIRO_TESTNET_BASE_URL, status: 0, error: getErrorMessage(e) };
    }
}
...
(define-map Inscriptions
  uint
  { owner: principal, mime-type: (string-ascii 64), total-size: uint, chunk-count: uint, sealed: bool, merkle-root: (buff 32), created-at: uint, sealed-at: uint }
)
(define-map Chunks { id: uint, index: uint } (buff 8192))
(define-public (begin-inscription (mime (string-ascii 64)) (total-size uint) (chunk-count uint))
  (let ((id (var-get next-id)))
    (map-insert Inscriptions id { owner: tx-sender, mime-type: mime, total-size: total-size, chunk-count: chunk-count, sealed: false, merkle-root: EMPTY-ROOT, created-at: block-height, sealed-at: u0 })
    (var-set next-id (+ id u1))
    (ok id)))
(define-public (add-chunk (id uint) (index uint) (data (buff 8192)))
  (let ((meta (unwrap! (map-get? Inscriptions id) ERR-NOT-FOUND)))
...
(define-read-only (get-chunks-page (id uint) (page uint))
  (match (map-get? Inscriptions id)
    meta
    (let ((base (* page PAGE-SIZE)) (count (get chunk-count meta)))
      (some
        (map (lambda (i) (let ((idx (+ base i))) (if (< idx count) (map-get? Chunks {id: id, index: idx}) none))) PAGE-INDICES)))
    none))
(define-read-only (get-chunk-exists-page (id uint) (page uint))
  (match (map-get? Inscriptions id)
    ...
    (let ((base (* page PAGE-SIZE)) (count (get chunk-count meta)))
      (some
        (map (lambda (i) (let ((idx (+ base i))) (and (< idx count) (is-some (map-get? Chunks {id: id, index: idx}))))) PAGE-INDICES)))
    none))
`;
...
window.showPage = (page) => {
    ...
    // Stop viewer-grid background work when navigating away.
    if (page !== 'play') {
        try {
            viewerGalleryRenderToken++;
        } catch {
            // ignore
        }
        try {
...
document.getElementById('btn-fetch-fee-rate')?.addEventListener('click', async () => {
    ...
    try {
        const results = await Promise.allSettled([
            fetchFeeRateWithFallback('current', [HIRO_TESTNET_BASE_URL]),
...
document.getElementById('btn-deploy-contract').addEventListener('click', async () => {
    ...
    if (lastRateLimitAtMs && Date.now() - lastRateLimitAtMs < 2 * 60_000) {
        setAuthStatus('Recent API rate limit detected (429). Waiting ~2 minutes can help wallet deploy succeed.', 'warn');
        journeyLog('Deploy warning: recent rate limit detected', { lastRateLimitAtMs });
    }
    ...
    Connect.openContractDeploy({
        ...
        onFinish: (data) => {
            ...
            if (data?.txId) {
                savePendingDeploy({
                    txId: data.txId,
                    contract: `${getContractDetails({ silent: true }).address}.${name}`,
                    startedAtMs: Date.now(),
                });
                renderDeployStatus();
                void waitForTxSuccessRobust(data.txId)
                    .then(() => {
                        journeyLog('Deploy confirmed', { txId: data.txId });
                        savePendingDeploy(null);
                        renderDeployStatus();
                        setAuthStatus('Contract deploy confirmed on-chain.', 'ok');
                    })
                    .catch((e) => {
                        journeyLog('Deploy confirmation check failed', { error: getErrorMessage(e), txId: data.txId });
                    });
            }
...
document.getElementById('btn-deploy-contract-v2')?.addEventListener('click', async () => {
    journeyLog("User clicked 'Deploy Contract (v2)'");
    try {
        await ensureAuth({ action: 'deploy the contract (v2)' });
    } catch (e) {
        journeyLog("Deploy blocked (auth not completed)", { error: e?.message || String(e) });
        return;
    }
    ...
    const v2Name = details.name.endsWith('-v2') ? details.name : `${details.name}-v2`;
    const deployParams = {
        contractName: v2Name,
        codeBody: CONTRACT_SOURCE_V2,
        network,
        userSession,
        fee: getFeePerTxMicroStx(),
    };

    journeyLog("Requesting Contract Deploy (v2)...", { contractName: v2Name });
    Connect.openContractDeploy({
        ...deployParams,
        onFinish: (data) => {
            journeyLog("Deploy onFinish (v2)", data);
            if (data?.txId) {
                savePendingDeploy({
                    txId: data.txId,
                    contract: `${details.address}.${v2Name}`,
                    startedAtMs: Date.now(),
                });
                renderDeployStatus();
                void waitForTxSuccessRobust(data.txId)
                    .then(() => {
                        journeyLog('Deploy confirmed (v2)', { txId: data.txId });
                        savePendingDeploy(null);
                        renderDeployStatus();
                        setAuthStatus('Contract deploy (v2) confirmed on-chain.', 'ok');
                    })
                    .catch((e) => {
                        journeyLog('Deploy confirmation check failed (v2)', { error: getErrorMessage(e), txId: data.txId });
                    });
            }
            try {
                const input = document.getElementById('contract-address-input');
                if (input) input.value = `${details.address}.${v2Name}`;
            } catch {
                // ignore
            }
            alert(`Contract Deployed (v2)! TX: ${data.txId}\nContract: ${details.address}.${v2Name}`);
        },
        onCancel: () => journeyLog("Deploy onCancel (v2)"),
    });
});
...
async function waitForTransactionSuccess(txId, { timeoutMs = 10 * 60 * 1000, pollMs = 5000 } = {}) {
    ...
    const apiUrl = HIRO_TESTNET_BASE_URL; // stacks-blockchain-api (Hiro) base
...
document.getElementById('btn-start-mint').addEventListener('click', async () => {
    ...
    try {
        ...
        journeyLog('Mint target contract', { contract: `${address}.${name}`, readOnlyApiUrl: network.coreApiUrl });

        // If we just deployed this contract, it may not be confirmed/indexed yet. Xverse can return a generic
        // "Internal error" in that case. Preflight the contract interface before attempting begin-inscription.
        const iface = await checkContractInterfaceAvailable({ address, name });
        if (!iface.ok) {
            const hint =
                pendingDeploy?.txId
                    ? `Deploy TX pending: ${pendingDeploy.txId.slice(0, 12)}…`
                    : 'If you just deployed, wait for confirmation.';
            renderMintProgress(`
                <div style="background:#fff3cd; padding:10px; border-radius:5px; color:#856404;">
                    <div><strong>Contract not available yet.</strong></div>
                    <div>Status: ${iface.status ? `HTTP ${iface.status}` : 'network error'} (checked via ${escapeHtml(iface.baseUrl || 'unknown')}).</div>
                    <div>${escapeHtml(iface.error || '')}</div>
                    <div style="margin-top:6px;">${hint}</div>
                    <div style="margin-top:6px;">Go to Deploy and click “Check Last Deploy Status”, then retry.</div>
                </div>
            `);
            journeyLog('Mint blocked: contract interface not available', {
                status: iface.status,
                error: iface.error || null,
        ...
        const msg = getErrorMessage(e);
        journeyLog("Minting Process Error", { error: msg });
        ...
        document.getElementById('mint-steps').innerHTML += `
            <div style="background:#f8d7da; padding:15px; border-radius:5px; color:#721c24; margin-top:10px;">
                ...
                <p style="color:#856404;">
                    If you see an Xverse “Internal error”, it often means the contract is not yet confirmed/indexed or the wallet cannot fetch nonce/fees.
                    Try: Deploy page → “Check Last Deploy Status”, wait for success, then retry.
                </p>
...
document.getElementById('btn-check-deploy')?.addEventListener('click', async () => {
    pendingDeploy = loadPendingDeploy();
    renderDeployStatus();
    if (!pendingDeploy?.txId) return;
    try {
        const data = await fetchTxStatus(pendingDeploy.txId);
        journeyLog('Deploy status', { txId: pendingDeploy.txId, tx_status: data?.tx_status });
        if (data?.tx_status === 'abort_by_response' || data?.tx_status === 'abort_by_post_condition') {
            const repr = data?.tx_result?.repr || null;
            const contractId = data?.smart_contract?.contract_id || null;
            journeyLog('Deploy failed on-chain', { txId: pendingDeploy.txId, tx_status: data?.tx_status, tx_result: repr, contract_id: contractId });
            const deployEl = document.getElementById('deploy-status');
            if (deployEl) {
                deployEl.className = 'status error';
                deployEl.innerText = `Deploy failed: ${data?.tx_status}${repr ? ` • ${repr}` : ''}`;
            }
            setAuthStatus('Deploy failed on-chain (abort). Fix deploy before minting.', 'error');
            return;
        }
        if (data?.tx_status === 'success') {
            savePendingDeploy(null);
            renderDeployStatus();
            setAuthStatus('Last deploy confirmed on-chain.', 'ok');
        } else {
            setAuthStatus(`Deploy status: ${data?.tx_status || 'unknown'}`, 'warn');
        }
    } catch (e) {
        journeyLog('Deploy status check failed', { error: getErrorMessage(e) });
        setAuthStatus(`Deploy status check failed: ${getErrorMessage(e)}`, 'warn');
    }
});

document.getElementById('btn-clear-deploy')?.addEventListener('click', () => {
    savePendingDeploy(null);
    renderDeployStatus();
    setAuthStatus('Cleared deploy tracking state.', 'info');
});
