...
import CONTRACT_V3_SOURCE from '../contracts/inscription-core.clar?raw';
import CONTRACT_V2_SOURCE from '../contracts/inscription-core-v2.clar?raw';
...
const renderMintGuidance = () => {
    ...
    if (!el) return;
    el.innerText =
        'Large inscriptions require many sequential wallet signatures. Keep the wallet open, avoid refreshing, and consider smaller files if you see wallet “internal error” or repeated cancels. If interrupted, use Resume with the Draft/Job ID.';
};
...
const persistLastDraftId = (id) => {
    lastDraftId = id;
    try {
        localStorage.setItem('last-draft-id', String(id));
    } catch {
        // ignore
    }
};

const persistLastSealedId = (id) => {
    lastSealedId = id;
    try {
        localStorage.setItem('last-inscription-id', String(id));
        localStorage.removeItem('last-draft-id');
    } catch {
        // ignore
    }
};

const restoreLastDraftOrSealedId = () => {
    try {
        const draftRaw = localStorage.getItem('last-draft-id');
        if (draftRaw) {
            const draftId = Number(draftRaw);
            if (Number.isFinite(draftId)) return { kind: 'draft', id: draftId };
        }

        const raw = localStorage.getItem('last-inscription-id');
        if (!raw) return null;
        const id = Number(raw);
        return Number.isFinite(id) ? { kind: 'sealed', id } : null;
    } catch {
        return null;
    }
};
...
// CONTRACT CONFIG
const DEFAULT_DEPLOY_VERSION = 'v3';
const CONTRACT_SOURCES = {
    v3: CONTRACT_V3_SOURCE,
    v2: CONTRACT_V2_SOURCE,
};
const DEPLOY_DEFAULT_NAMES = {
    v3: 'xStrata-v3',
    v2: 'xStrata-v2',
};

const getActiveStxAddress = () => {
    try {
...
const persistLastStxAddress = (address) => {
    if (!address) return;
    try {
        localStorage.setItem('last-stx-address', String(address));
    } catch {
        // ignore
    }
};

const restoreLastStxAddress = () => {
    try {
        const raw = localStorage.getItem('last-stx-address');
        return raw || null;
    } catch {
        return null;
    }
...
const getContractDetails = ({ silent = false } = {}) => {
    ...
    const activeAddr = getActiveStxAddress();
    const resolvedKey = `${input}|${activeAddr || ''}`;
    if (cachedContractDetails && cachedContractKey === resolvedKey) return cachedContractDetails;
    ...
    if (input.startsWith('.')) {
        name = input.slice(1);
        address = activeAddr || restoreLastStxAddress();
        if (!address) {
            if (!silent) journeyLog('ERROR: No address available for .CONTRACT_NAME shorthand', { input });
            throw new Error('Connect a wallet (or enter ADDRESS.CONTRACT_NAME) to use .CONTRACT_NAME.');
        }
    } else {
        const parts = input.split('.');
        if (parts.length !== 2) {
            if (!silent) journeyLog('ERROR: Invalid contract address format', { input });
            throw new Error('Invalid Address Format. Expected: ADDRESS.CONTRACT_NAME or .CONTRACT_NAME');
        }
        address = parts[0];
        name = parts[1];
    }
    ...
    if (!name) {
        if (!silent) journeyLog('ERROR: Missing contract name', { input });
        throw new Error('Missing contract name.');
    }

    const details = { address, name };
    cachedContractKey = resolvedKey;
...
async function handleSelectedFile(file) {
    ...
    renderFeeEstimates({ mode: 'mint', missingCount: null });
    const restored = restoreLastDraftOrSealedId();
    if (restored !== null) {
        document.getElementById('resume-id-input').value = restored.id;
        renderMintProgress(
            `<div><strong>Last ${restored.kind === 'draft' ? 'Draft/Job' : 'Inscription'} ID:</strong> #${restored.id} (pre-filled for resume)</div>`
        );
    }
}
...
window.showPage = (page) => {
    ...
    if (page === 'play') {
        ...
        void renderGalleryPage(viewerGalleryPage || 0).catch((e) => {
            const status = document.getElementById('gallery-status');
            if (status) {
                status.innerText =
                    'Set Configuration → Contract Address to ADDRESS.CONTRACT_NAME (or connect a wallet) to browse inscriptions.';
            }
...
const updateUI = () => {
    ...
    if (signedIn) {
        ...
        if (addr && addr !== 'Unknown address') persistLastStxAddress(addr);
...
document.getElementById('btn-deploy-contract').addEventListener('click', async () => {
    ...
    const versionEl = document.getElementById('deploy-contract-version');
    const nameEl = document.getElementById('deploy-contract-name');
    const version = (versionEl?.value || DEFAULT_DEPLOY_VERSION).toLowerCase();
    const codeBody = CONTRACT_SOURCES[version] || CONTRACT_SOURCES[DEFAULT_DEPLOY_VERSION];
    const defaultName = DEPLOY_DEFAULT_NAMES[version] || DEPLOY_DEFAULT_NAMES[DEFAULT_DEPLOY_VERSION];
    const rawName = String(nameEl?.value || defaultName).trim();
    ...
    journeyLog("Requesting Contract Deploy...", { contractName: name, version });
    Connect.openContractDeploy({
        ...deployParams,
        onFinish: (data) => {
            journeyLog("Deploy onFinish", data);
            const addr = getActiveStxAddress();
            if (addr) {
                persistLastStxAddress(addr);
                const input = document.getElementById('contract-address-input');
                if (input) input.value = `${addr}.${name}`;
                cachedContractKey = null;
                cachedContractDetails = null;
                journeyLog('Contract address updated', { contract: `${addr}.${name}` });
            }
...
// Deploy UI convenience: swap default contract name when switching versions (without overwriting custom names).
document.getElementById('deploy-contract-version')?.addEventListener('change', (e) => {
    const version = String(e?.target?.value || DEFAULT_DEPLOY_VERSION).toLowerCase();
    const nameEl = document.getElementById('deploy-contract-name');
    if (!nameEl) return;
    const current = String(nameEl.value || '').trim();
    const defaults = Object.values(DEPLOY_DEFAULT_NAMES);
    if (defaults.includes(current) || defaults.includes(current.startsWith('.') ? current.slice(1) : current) || current === '') {
        nameEl.value = DEPLOY_DEFAULT_NAMES[version] || DEPLOY_DEFAULT_NAMES[DEFAULT_DEPLOY_VERSION];
    }
});
...
async function waitForTransactionSuccess(txId, { timeoutMs = 10 * 60 * 1000, pollMs = 5000 } = {}) {
    ...
    const baseUrl = network.coreApiUrl; // in dev: `${origin}/hiro-testnet`
    ...
    const fetchJson = async (url) => {
        ...
        if (contentType.includes('application/json')) {
            try {
                ...
                const preview = text ? text.slice(0, 200) : '';
                throw new Error(`Invalid JSON from ${url} (${contentType || 'unknown content-type'}): ${preview}`);
            }
        } else {
            const preview = text ? text.slice(0, 200) : '';
            throw new Error(`Non-JSON response from ${url} (${contentType || 'unknown content-type'}): ${preview}`);
        }
        return { res, json };
    };

    const fetchTxDetails = async () => {
        // Prefer Hiro "extended" (richer fields), but fall back to stacks-node `/v2/transactions/...` if needed.
        const urls = [
            `${baseUrl}/extended/v1/tx/${txId}`,
            `${baseUrl}/v2/transactions/${txId}`,
        ];
        let lastErr = null;
        ...
        throw lastErr || new Error('Unable to fetch tx status');
    };
    ...
    return new Promise((resolve, reject) => {
        ...
        const interval = setInterval(async () => {
            ...
            try {
                const data = await fetchTxDetails();
                consecutiveErrors = 0;

                const status = data?.tx_status || data?.txStatus || null;
                if (status === 'success') {
                    clearInterval(interval);
                    resolve(data);
                    return;
                ...
                if (status === 'abort_by_response' || status === 'abort_by_post_condition') {
                    clearInterval(interval);
                    reject(new Error(`Transaction failed: ${status}`));
                    return;
                }
                // If pending / not found / unknown, continue polling
            } catch (e) {
                consecutiveErrors++;
                const msg = getErrorMessage(e);
                journeyLog('Polling error', { consecutiveErrors, error: msg, baseUrl });
                // If we keep receiving HTML from the dev server (proxy not applied), fail fast with guidance.
                if (consecutiveErrors >= 3) {
                    clearInterval(interval);
                    reject(
                        new Error(
                            `TX polling failed (non-JSON). If you see HTML here, your Hiro proxy may not be in use. baseUrl=${baseUrl}. Error: ${msg}`
                        )
                    );
                    return;
                }
...
function parseOkUintFromTxResultRepr(repr) {
    ...
    try {
        ...
        statusEl.innerHTML = `
            <div style="background:#fff3cd; padding:15px; border-radius:5px; color:#856404;">
                ...
                <p>Waiting for Draft/Job ID assignment...</p>
        ...
        const inscriptionId = parseOkUintFromTxResultRepr(txDetails?.tx_result?.repr);
        
        if (inscriptionId === null) {
            throw new Error("Could not parse Draft/Job ID from transaction result.");
        }

        journeyLog(`Auto-detected Draft/Job ID: ${inscriptionId}`);
        persistLastDraftId(inscriptionId);
        ...
        renderMintProgress(`
            <div style="background:#d4edda; padding:10px; border-radius:5px; color:#155724;">
                <div><strong>Your Draft/Job ID:</strong> #${inscriptionId}</div>
        ...
        statusEl.innerHTML = `
            <div style="background:#d4edda; padding:15px; border-radius:5px; color:#155724;">
                ...
                <p><strong>Draft/Job ID: #${inscriptionId}</strong> ready.</p>
        ...
        if (lastDraftId !== null) {
            renderMintProgress(`
                <div style="background:#fff3cd; padding:10px; border-radius:5px; color:#856404;">
                    ...
                    <div>Resume with Draft/Job ID: <strong>#${lastDraftId}</strong></div>
...
document.getElementById('btn-resume-mint').addEventListener('click', async () => {
    ...
    if (!idInput.value) return alert("Please enter a Draft/Job ID.");
    ...
    journeyLog(`Attempting to resume Draft/Job ID: ${id}`);
    ...
    try {
        ...
        if (!meta) throw new Error("Draft/Job ID not found on-chain (or already sealed).");
...
async function sealInscriptionTransaction(id, root, statusEl) {
    ...
    const sealTxId = sealTx?.txId || null;
    let finalInscriptionId = id;
    if (sealTxId) {
        // Always confirm seal so we can extract the final public inscription ID (v3 assigns it at seal-time).
        const sealDetails = await waitForTransactionSuccess(sealTxId);
        const parsed = parseOkUintFromTxResultRepr(sealDetails?.tx_result?.repr);
        if (parsed !== null) finalInscriptionId = parsed;
        persistLastSealedId(finalInscriptionId);
    }

    statusEl.innerHTML += `
        <div style="background:#d4edda; padding:10px; border-radius:5px; color:#155724; margin-top: 10px;">
            <h3 style="margin-top:0">Inscription Complete</h3>
            <div><strong>Final Inscription ID:</strong> #${finalInscriptionId}</div>
            ${finalInscriptionId !== id ? `<div style="color:#555; font-size:12px; margin-top:4px;">Draft/Job ID was #${id}</div>` : ''}
            <button onclick="document.getElementById('manifest-id-input').value = ${finalInscriptionId}; showPage('play');" style="background:#28a745; color:white; border:none; padding:10px; cursor:pointer; margin-top:8px;">View Inscription</button>
        </div>
    `;
}
