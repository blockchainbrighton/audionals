...
Instructions for working in this directory tree (`V3-Codex/v3.0.1`), packaged specifically as the **v6.1gc serverless build**.

## Project Summary
This is the same vanilla JS + Vite Stacks “inscription” app, but packaged for static/serverless deployment:
...
## Serverless Notes
- Treat this tree as a static bundle: run `npm run build` and deploy the `dist/` output (along with `manifest.json`) to a CDN, Netlify, Cloudflare Pages, or similar.
- `public/manifest.json` and `/vite.svg` must remain reachable at the host root because Stacks Connect fetches those paths directly.
- `netlify.toml` and `public/_redirects` are included to keep the SPA routes working on platforms that require explicit rewrites for `index.html`.
- The new README highlights the “build + deploy” workflow for testing before uploading the static assets.
