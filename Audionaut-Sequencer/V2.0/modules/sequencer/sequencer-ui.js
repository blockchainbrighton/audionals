import { projectState, runtimeState, getCurrentSequence, createNewChannel, initializeProject, ensureSamplerChannelDefaults, samplerChannelDefaults, createSequenceFromTemplate, ensureChannelInsertSettings, markSampleRecentlyUsed, pruneInactiveSampleCaches, ensureInstrumentChannelDefaults } from './sequencer-state.js';
...
import { updateAllChannelGains, updateChannelGain, forceChannelSilence } from './sequencer-channel-mixer.js';
import { SimpleSampleLoader } from './sequencer-sample-loader.js';
import { playSamplerChannel } from './sequencer-sampler-playback.js';
import { setChannelInsertEnabled, setChannelInsertParameter, updateInsertSuspensionForSequence, updateInsertSuspensionForChannel } from './plugins/channel-insert-manager.js';
import { attachInstrumentEditor, pruneUnusedInstrumentEditors } from './sequencer-instrument-piano-roll.js';
import { getAudioContextCurrentTime } from './utils/audio-context.js';
import { getInsertQualityOptions } from './plugins/insert-quality.js';
import { activateInstrumentChannel, deactivateInstrumentChannel, isInstrumentChannelActive, isInstrumentRecording, toggleInstrumentRecording } from './instrument-live-controller.js';
import { getInstrumentDefinition, listInstrumentOptions } from './instrument-registry.js';
...
let liveStateRenderHandle = null;
...
function scheduleRenderFromLiveState() {
    if (liveStateRenderHandle !== null) return;
    const win = getWindowRef();
    const callback = () => {
        liveStateRenderHandle = null;
        try {
            render();
        } catch (err) {
            console.error('[UI] Failed to update sequencer after instrument live-state change.', err);
        }
    };
    if (win?.requestAnimationFrame) {
        liveStateRenderHandle = win.requestAnimationFrame(callback);
    } else if (typeof setTimeout === 'function') {
        liveStateRenderHandle = setTimeout(callback, 0);
    } else {
        callback();
    }
}
...
const INSERT_QUALITY_OPTIONS = getInsertQualityOptions();
...
const INSERT_PLUGIN_CONFIG = [
    ...
    },
    {
        ...
        params: [
            { key: 'wet', label: 'Mix', min: 0, max: 1, step: 0.01, format: formatPercent },
            { key: 'quality', label: 'Quality', controlType: 'select', options: INSERT_QUALITY_OPTIONS, defaultValue: 'normal' }
        ]
    },
    {
        id: 'reverb',
...
const PROJECT_STORAGE_KEY = 'audionautSequencerProject';
...
function updateInsertPanelVisibility(channelKey, open) {
    const doc = getDocumentRef();
    if (!doc) return;
    const escaped = escapeSelector(channelKey);
    const panelState = runtimeState.insertPanelState.get(channelKey);
    const panel = doc.querySelector(`.channel-inserts[data-panel-key="${escaped}"]`);
    if (panel) {
        panel.hidden = !open;
        panel.classList.toggle('is-open', open);
        if (open) {
            const scrollTop = Number.isFinite(panelState?.scrollTop)
                ? panelState.scrollTop
                : Number(panel.dataset.scrollTop) || 0;
            panel.scrollTop = scrollTop;
            panel.dataset.scrollTop = scrollTop;
        }
    }
    const trigger = doc.querySelector(`.channel-insert-trigger[data-panel-key="${escaped}"]`);
    if (trigger) {
...
const pendingInsertParamUpdates = new Map();
let pendingInsertParamHandle = null;
let pendingInsertParamScheduler = null;
const pendingMixUpdateChannels = new Set();
let pendingMixUpdateHandle = null;
let pendingMixUpdateScheduler = null;
...
function closeAllInsertPanels() {
    let mutated = false;
    runtimeState.insertPanelState.forEach((state, key) => {
        if (state?.open) {
            state.open = false;
            updateInsertPanelVisibility(key, false);
            mutated = true;
        }
    });
    return mutated;
...
function resetSequencerUICaches() {
    channelLayoutState.clear();
    pendingInsertParamUpdates.clear();
    if (pendingInsertParamHandle !== null) {
        if (pendingInsertParamScheduler === 'raf' && typeof cancelAnimationFrame === 'function') {
            cancelAnimationFrame(pendingInsertParamHandle);
        } else if (pendingInsertParamScheduler === 'timeout') {
            clearTimeout(pendingInsertParamHandle);
        }
        pendingInsertParamHandle = null;
        pendingInsertParamScheduler = null;
    }
    pendingMixUpdateChannels.clear();
    if (pendingMixUpdateHandle !== null) {
        if (pendingMixUpdateScheduler === 'raf' && typeof cancelAnimationFrame === 'function') {
            cancelAnimationFrame(pendingMixUpdateHandle);
        } else if (pendingMixUpdateScheduler === 'timeout') {
            clearTimeout(pendingMixUpdateHandle);
        }
        pendingMixUpdateHandle = null;
        pendingMixUpdateScheduler = null;
    }
    stopAllWaveformAnimations();
    channelWaveformAnimations.clear();
    channelWaveformRegistry = new WeakMap();
}

function queueInsertParamUpdate(channel, pluginId, param, value) {
    if (!channel) return;
    let pluginMap = pendingInsertParamUpdates.get(channel);
    if (!pluginMap) {
        pluginMap = new Map();
        pendingInsertParamUpdates.set(channel, pluginMap);
    }
    let paramMap = pluginMap.get(pluginId);
    if (!paramMap) {
        paramMap = new Map();
        pluginMap.set(pluginId, paramMap);
    }
    paramMap.set(param, value);
    if (pendingInsertParamHandle !== null) return;
    const raf = typeof window !== 'undefined' && typeof window.requestAnimationFrame === 'function'
        ...
        : (typeof globalThis.requestAnimationFrame === 'function'
            ...
            : null);
    const scheduleFn = () => {
        flushPendingInsertParamUpdates();
        pendingInsertParamHandle = null;
        pendingInsertParamScheduler = null;
    };
    if (raf) {
        pendingInsertParamScheduler = 'raf';
        pendingInsertParamHandle = raf(scheduleFn);
    } else {
        pendingInsertParamScheduler = 'timeout';
        pendingInsertParamHandle = setTimeout(scheduleFn, 0);
    }
}

function flushPendingInsertParamUpdates() {
    pendingInsertParamHandle = null;
    pendingInsertParamScheduler = null;
    pendingInsertParamUpdates.forEach((pluginMap, channel) => {
        pluginMap.forEach((paramMap, pluginId) => {
            paramMap.forEach((value, param) => {
                setChannelInsertParameter(channel, pluginId, param, value);
            });
            paramMap.clear();
        });
        pluginMap.clear();
...
function scheduleChannelMixUpdate(channel) {
    if (!channel) return;
    pendingMixUpdateChannels.add(channel);
    if (pendingMixUpdateHandle !== null) return;
    const raf = typeof window !== 'undefined' && typeof window.requestAnimationFrame === 'function'
        ? window.requestAnimationFrame.bind(window)
        : (typeof globalThis.requestAnimationFrame === 'function'
            ? globalThis.requestAnimationFrame.bind(globalThis)
            : null);
    const scheduleFn = () => {
        flushPendingChannelMixUpdates();
        pendingMixUpdateHandle = null;
        pendingMixUpdateScheduler = null;
    };
    if (raf) {
        pendingMixUpdateScheduler = 'raf';
        pendingMixUpdateHandle = raf(scheduleFn);
    } else {
        pendingMixUpdateScheduler = 'timeout';
        pendingMixUpdateHandle = setTimeout(scheduleFn, 0);
    }
}

function flushPendingChannelMixUpdates() {
    const sequence = getCurrentSequence();
    pendingMixUpdateChannels.forEach(channel => {
        updateChannelGain(channel, sequence);
        updateInsertSuspensionForChannel(channel);
        if (channel.volume <= 0) {
            forceChannelSilence(channel);
        }
    });
    pendingMixUpdateChannels.clear();
...
const channelLayoutState = new Map();

const getChannelLayoutKey = (sequenceIndex, channelIndex) => `${sequenceIndex}:${channelIndex}`;

function isChannelDetailsVisible(sequenceIndex, channelIndex) {
    const key = getChannelLayoutKey(sequenceIndex, channelIndex);
    if (!channelLayoutState.has(key)) {
        channelLayoutState.set(key, { detailsVisible: true });
    }
    const state = channelLayoutState.get(key);
    return state.detailsVisible !== false;
}

function setChannelDetailsVisible(sequenceIndex, channelIndex, visible) {
    const key = getChannelLayoutKey(sequenceIndex, channelIndex);
    const state = channelLayoutState.get(key) || {};
    state.detailsVisible = !!visible;
    channelLayoutState.set(key, state);
}

const pendingPlayheadTimers = new Set();
const pendingPlayheadFrames = new Set();
let lastHighlightedStepIndex = null;
let loaderOverlayDismissed = false;
let channelWaveformRegistry = new WeakMap();
const channelWaveformAnimations = new Map();
 ...
 */
function resetSamplerChannelAfterSampleChange(channel, nextIndex) {
    if (!channel || channel.type !== 'sampler') return;
    const targetIndex = Number.isInteger(nextIndex) ? nextIndex : 0;
    channel.selectedSampleIndex = targetIndex;
    channel.selectedSampleOrdinal = SimpleSampleLoader.getOrdinalIdByIndex(targetIndex) ?? null;
...
function renderSamplerControls(targetContainer, channelData, chIndex) {
    ...
    select.onchange = async () => {
        ...
        if (!channel) return;
        resetSamplerChannelAfterSampleChange(channel, idx);
        markSampleRecentlyUsed(idx);
...
async function handleOrdinalSampleLoad(chIndex) {
    ...
    try {
        ...
        if (buffer) {
            ...
            markSampleRecentlyUsed(index);
        }

        resetSamplerChannelAfterSampleChange(channel, index);
 ...
 * Appends them to the provided infoContainer.
 */
const INSTRUMENT_OPTIONS = listInstrumentOptions();

function renderInstrumentControls(targetContainer, channelData, chIndex) {
    ensureInstrumentChannelDefaults(channelData);
    const instrumentControls = document.createElement('div');
    instrumentControls.className = 'instrument-controls';

    const sequenceIndex = projectState.currentSequenceIndex ?? 0;
    const instrumentId = channelData.instrumentId;
    const instrument = instrumentId ? runtimeState.instrumentRack[instrumentId] : null;
    const hasInstrument = !!instrument;
    const selectedType = channelData.pendingInstrumentType || channelData.instrumentType;
    const selectedDefinition = getInstrumentDefinition(selectedType);
    const activeType = instrument?.type || channelData.instrumentType || selectedDefinition.id;
    const activeDefinition = getInstrumentDefinition(activeType);

    const selectLabel = document.createElement('label');
    selectLabel.className = 'instrument-select-label';
    selectLabel.textContent = 'Instrument';
    const select = document.createElement('select');
    ...
    INSTRUMENT_OPTIONS.forEach(option => {
        ...
        optEl.selected = option.id === (channelData.pendingInstrumentType || selectedDefinition.id);
        select.appendChild(optEl);
    });
    select.addEventListener('change', event => {
        channelData.pendingInstrumentType = event.target.value;
        render();
    });
    selectLabel.appendChild(select);
    instrumentControls.appendChild(selectLabel);

    const loadBtn = document.createElement('button');
    loadBtn.type = 'button';
    loadBtn.className = 'instrument-button instrument-button--load';
    loadBtn.textContent = hasInstrument
        ? `Replace with ${selectedDefinition.label}`
        : `Load ${selectedDefinition.label}`;
    loadBtn.onclick = () => {
        if (channelData.pendingInstrumentType && instrument?.type !== channelData.pendingInstrumentType) {
            channelData.patch = null;
            channelData.patchInstrumentType = null;
        }
        createInstrumentForChannel(sequenceIndex, chIndex);
        render();
    };
    if (!hasInstrument) {
        ...
        loadBtn.textContent = channelData.pendingInstrumentType === instrument.type
            ? `Reload ${activeDefinition.label}`
            : `Replace with ${selectedDefinition.label}`;
    }

    const configGroup = document.createElement('div');
    configGroup.className = 'instrument-controls-group instrument-controls-group--config';
    configGroup.appendChild(loadBtn);

    const openBtn = document.createElement('button');
    openBtn.type = 'button';
    openBtn.className = 'instrument-button instrument-button--open';
    openBtn.textContent = `View ${activeDefinition.label}`;
    openBtn.onclick = () => openSynthUI(chIndex);
    configGroup.appendChild(openBtn);

    instrumentControls.appendChild(configGroup);

    const liveGroup = document.createElement('div');
    liveGroup.className = 'instrument-controls-group instrument-controls-group--live';

    const isActive = isInstrumentChannelActive(channelData);
    const activateBtn = document.createElement('button');
    activateBtn.type = 'button';
    activateBtn.className = 'instrument-button instrument-button--activate';
    activateBtn.textContent = isActive ? 'Live Active' : 'Play Live';
    activateBtn.title = isActive
        ? 'Click to release this channel from live MIDI control.'
        : 'Send your MIDI controller to this synth without opening the UI.';
    activateBtn.setAttribute('aria-pressed', String(isActive));
    activateBtn.onclick = () => {
        if (isActive) {
            deactivateInstrumentChannel(instrumentId);
        } else {
            activateInstrumentChannel(sequenceIndex, chIndex);
        }
        render();
    };
    liveGroup.appendChild(activateBtn);

    const recordMode = isInstrumentRecording(instrumentId);
    const recordBtn = document.createElement('button');
    recordBtn.type = 'button';
    recordBtn.className = 'instrument-button instrument-button--record';
    recordBtn.textContent = recordMode ? 'Recording' : 'Record';
    recordBtn.title = 'Toggle recording to capture notes into this channel.';
    recordBtn.setAttribute('aria-pressed', String(recordMode));
    recordBtn.classList.toggle('is-active', recordMode);
    recordBtn.classList.toggle('is-recording', recordMode);
    recordBtn.onclick = () => {
        if (!isInstrumentChannelActive(channelData)) {
            activateInstrumentChannel(sequenceIndex, chIndex);
        }
        const nextState = toggleInstrumentRecording(instrumentId);
        recordBtn.textContent = nextState ? 'Recording' : 'Record';
        recordBtn.setAttribute('aria-pressed', String(nextState));
        recordBtn.classList.toggle('is-active', nextState);
        recordBtn.classList.toggle('is-recording', nextState);
    };
    liveGroup.appendChild(recordBtn);

    instrumentControls.appendChild(liveGroup);
    targetContainer.appendChild(instrumentControls);
}

function renderChannelInsertControls(channelData, chIndex) {
    if (!channelData || channelData.type !== 'sampler') {
    ...
    panel.addEventListener('scroll', () => {
        ...
        panel.dataset.scrollTop = panelState.scrollTop;
    });
    ...
    trigger.addEventListener('click', event => {
        ...
        runtimeState.insertPanelState.set(channelKey, panelState);
        updateInsertPanelVisibility(channelKey, panelState.open);
    });
    ...
    closeBtn.addEventListener('click', event => {
        ...
        runtimeState.insertPanelState.set(channelKey, panelState);
        updateInsertPanelVisibility(channelKey, false);
    });

    panelHeader.append(title, headerStatus, closeBtn);
    panel.appendChild(panelHeader);

    const updatePanelActivityStatus = () => {
        const seq = getCurrentSequence();
        const channel = seq?.channels?.[chIndex];
        const count = channel
            ? INSERT_PLUGIN_CONFIG.filter(def => channel.insertSettings[def.id]?.enabled).length
            : activeCount;
        const label = count ? `${count} active` : 'All bypassed';
        triggerStatus.textContent = label;
    ...
    INSERT_PLUGIN_CONFIG.forEach(plugin => {
        ...
        toggle.addEventListener('change', () => {
            ...
            pluginRow.classList.toggle('bypassed', !enabled);
            status.textContent = enabled ? 'Active' : 'Bypassed';
            pluginPanel.hidden = !enabled ? true : !panelState.expandedPlugins[plugin.id];
            nameButton.disabled = !enabled;
            nameButton.classList.toggle('active', enabled && !!panelState.expandedPlugins[plugin.id]);
            pluginPanel.querySelectorAll('input, select').forEach(el => {
                if (el !== toggle) {
                    el.disabled = !enabled;
                }
            });
            updatePanelActivityStatus();
        ...
        plugin.params.forEach(param => {
            ...
            if (param.controlType === 'select') {
                const select = document.createElement('select');
                (param.options || INSERT_QUALITY_OPTIONS).forEach(opt => {
                    const option = document.createElement('option');
                    option.value = opt.value;
                    option.textContent = opt.label;
                    select.appendChild(option);
                });
                const selectValue = typeof pluginState[param.key] === 'string'
                    ? pluginState[param.key]
                    : (param.defaultValue ?? param.options?.[0]?.value ?? '');
                select.value = selectValue;
                select.disabled = !pluginState.enabled;
                select.addEventListener('change', event => {
                    const seq = getCurrentSequence();
                    const channel = seq?.channels?.[chIndex];
                    if (!channel) return;
                    const nextValue = event.target.value;
                    pluginState[param.key] = nextValue;
                    queueInsertParamUpdate(channel, plugin.id, param.key, nextValue);
                });
            ...
            const currentValue = typeof pluginState[param.key] === 'number'
                ...
                slider.addEventListener('input', (event) => {
                    ...
                    pluginState[param.key] = rawValue;
                    queueInsertParamUpdate(channel, plugin.id, param.key, rawValue);
                    valueReadout.textContent = formatter(rawValue);
                });

                control.append(slider, valueReadout);
            }
    ...
    const initialScroll = panelState.scrollTop || 0;
    panel.scrollTop = initialScroll;
    panel.dataset.scrollTop = initialScroll;
...
function renderChannelMixControls(channelData, chIndex) {
    ...
    muteBtn.onclick = () => {
        ...
        updateInsertSuspensionForSequence(seq);
    ...
    volumeSlider.oninput = event => {
        ...
        scheduleChannelMixUpdate(channel);
    };
...
function renderStepGrid(channelEl, channelData, chIndex) {
    ...
    stepsContainer.style.setProperty('--steps-per-row', String(config.TOTAL_STEPS));

    const rowDiv = document.createElement('div');
    rowDiv.className = 'step-row';

    for (let stepIndex = 0; stepIndex < config.TOTAL_STEPS; stepIndex++) {
        rowDiv.appendChild(createStepElement(channelData, chIndex, stepIndex));
    }
...
function getWaveformEntry(channel) {
    return channelWaveformRegistry.get(channel) || null;
}

function hideChannelWaveformPlayhead(channel) {
    const entry = getWaveformEntry(channel);
    if (!entry) return;
    if (entry.playhead) {
        entry.playhead.classList.remove('is-visible');
        entry.playhead.hidden = true;
    }
    if (entry.button) {
...
function cancelChannelWaveformAnimation(channel) {
    const state = channelWaveformAnimations.get(channel);
    if (!state) {
        hideChannelWaveformPlayhead(channel);
        return;
    }
    if (state.frame) cancelAnimationFrame(state.frame);
    if (state.timeout) clearTimeout(state.timeout);
    channelWaveformAnimations.delete(channel);
    hideChannelWaveformPlayhead(channel);
}

function scheduleChannelWaveformPlayhead(channel, detail) {
    if (!channel || !detail) return;
    cancelChannelWaveformAnimation(channel);

    const entry = {
        frame: 0,
        timeout: 0,
        startTime: Number.isFinite(detail.startTime) ? detail.startTime : null,
        duration: Math.max(0.01, Number(detail.duration) || 0.01),
        rangeStart: clampNormalized(detail.range?.start, 0),
        rangeEnd: clampNormalized(detail.range?.end, detail.range?.start ?? 1)
    };
    const span = entry.rangeEnd - entry.rangeStart;
    entry.rangeSpan = Math.abs(span) <= 1e-6 ? 0 : span;

    const updatePosition = () => {
        const now = getAudioContextCurrentTime(runtimeState.Tone);
        ...
        const progress = entry.duration > 0 ? elapsed / entry.duration : 1;
        const clamped = Math.min(Math.max(progress, 0), 1);
        const normalized = entry.rangeStart + clamped * entry.rangeSpan;
        const percent = Math.min(100, Math.max(0, normalized * 100));

        const registry = getWaveformEntry(channel);
        const playheadEl = registry?.playhead;
        const buttonEl = registry?.button;
        if (!playheadEl || !buttonEl || !playheadEl.isConnected || !buttonEl.isConnected) {
            if (progress >= 1) {
                cancelChannelWaveformAnimation(channel);
                return;
            }
            entry.frame = requestAnimationFrame(updatePosition);
            channelWaveformAnimations.set(channel, entry);
            return;
        }
    ...
    const startAnimation = () => {
        const now = getAudioContextCurrentTime(runtimeState.Tone);
        if (typeof now === 'number' && (entry.startTime === null || Number.isNaN(entry.startTime))) {
            entry.startTime = now;
        }
        if (typeof entry.startTime !== 'number') {
            entry.startTime = now ?? 0;
        }
        channelWaveformAnimations.set(channel, entry);
        updatePosition();
    };
    ...
    const now = getAudioContextCurrentTime(runtimeState.Tone);
    if (typeof now === 'number' && typeof entry.startTime === 'number') {
        const delayMs = (entry.startTime - now) * 1000;
        if (delayMs > 1) {
...
function handleSamplerPlaybackEvent(event) {
    const detail = event?.detail;
    if (!detail?.channel) return;
    scheduleChannelWaveformPlayhead(detail.channel, detail);
}

function handleSamplerPlaybackStopEvent(event) {
    const detail = event?.detail;
    if (!detail?.channel) return;
    cancelChannelWaveformAnimation(detail.channel);
}

function stopAllWaveformAnimations() {
    Array.from(channelWaveformAnimations.keys()).forEach(channel => cancelChannelWaveformAnimation(channel));
}


function ensureSampleBuffer(sampleIndex) {
    if (runtimeState.allSampleBuffers[sampleIndex]) {
        markSampleRecentlyUsed(sampleIndex);
...
function createChannelWaveform(channelData) {
    const container = document.createElement('div');
    container.className = 'channel-waveform channel-waveform--inline';
    ...
    playhead.className = 'waveform-preview-playhead';
    playhead.hidden = true;

    previewButton.append(previewCanvas, playhead, overlay);
    ...
    channelWaveformRegistry.set(channelData, {
        button: previewButton,
        playhead,
        canvas: previewCanvas
    });
    hideChannelWaveformPlayhead(channelData);
...
function renderInstrumentStepView(channelEl, channelData, chIndex) {
    const stepsContainer = document.createElement('div');
    stepsContainer.className = 'steps steps--instrument';
    stepsContainer.style.setProperty('--steps-per-row', String(config.TOTAL_STEPS));
    ...
    if (channelData.notes && runtimeState.Tone) {
        const totalDuration = runtimeState.Tone.Time('4m').toSeconds();
        for (const note of channelData.notes) {
            const timeInSeconds = runtimeState.Tone.Time(note.time).toSeconds();
            const stepIndex = Math.floor((timeInSeconds / totalDuration) * config.TOTAL_STEPS);
            
            if (stepIndex >= 0 && stepIndex < config.TOTAL_STEPS) {
...
export function render() {
    ...
    currentSeq.channels.forEach((channelData, chIndex) => {
        ...
        if (channelData.type === 'instrument') channelEl.classList.add('channel--instrument');
        if (channelData.type === 'sampler') channelEl.classList.add('channel--sampler');
        if (channelData.type === 'instrument' && isInstrumentChannelActive(channelData)) {
            channelEl.classList.add('channel--instrument-live');
        }
        channelEl.dataset.channelIndex = chIndex;

        const detailsVisible = isChannelDetailsVisible(sequenceIndex, chIndex);
        channelEl.classList.toggle('channel--details-hidden', !detailsVisible);

        const headerBar = document.createElement('div');
        headerBar.className = 'channel-header';

        const label = document.createElement('div');
        label.className = 'channel-label';

        const headerActions = document.createElement('div');
        headerActions.className = 'channel-header-actions';
        ...
        const insertControls = channelData.type === 'sampler'
            ? renderChannelInsertControls(channelData, chIndex)
            : null;
        ...
        if (channelData.type === 'sampler') {
            ...
            const activeInstrument = channelData.instrumentId
                ? runtimeState.instrumentRack[channelData.instrumentId]
                : null;
            const labelDefinition = activeInstrument
                ? getInstrumentDefinition(activeInstrument.type)
                : getInstrumentDefinition(channelData.pendingInstrumentType || channelData.instrumentType);
            label.textContent = activeInstrument
                ? labelDefinition.label
                : `Load ${labelDefinition.label}`;
        ...
        layoutToggle.className = 'channel-layout-toggle';
        layoutToggle.textContent = detailsVisible ? 'Hide Details' : 'Show Details';
        layoutToggle.onclick = () => {
            setChannelDetailsVisible(sequenceIndex, chIndex, !detailsVisible);
            render();
        };
        headerActions.appendChild(layoutToggle);

        headerBar.append(label, headerActions);
        channelEl.appendChild(headerBar);

        if (channelData.type === 'sampler') {
            if (detailsVisible) {
                const topControlsRow = document.createElement('div');
                topControlsRow.className = 'channel-controls-row channel-controls-row--top';
                topControlsRow.append(mixControls, typeControlsContainer);
                channelEl.appendChild(topControlsRow);

                const bottomControlsRow = document.createElement('div');
                bottomControlsRow.className = 'channel-controls-row channel-controls-row--bottom';
                bottomControlsRow.append(insertControls, createChannelWaveform(channelData));
                channelEl.appendChild(bottomControlsRow);

            } else {
                cancelChannelWaveformAnimation(channelData);
                channelWaveformRegistry.delete(channelData);
            }
            renderStepGrid(channelEl, channelData, chIndex);
        } else if (channelData.type === 'instrument') {
            if (detailsVisible) {
                const detailsContainer = document.createElement('div');
                detailsContainer.className = 'channel-details';

                const controlsRow = document.createElement('div');
                controlsRow.className = 'channel-controls-row';
                controlsRow.append(mixControls, typeControlsContainer);
                detailsContainer.appendChild(controlsRow);

                const editorHost = document.createElement('div');
                editorHost.className = 'instrument-editor';
                const attached = attachInstrumentEditor(channelData, editorHost);
                detailsContainer.appendChild(editorHost);
                channelEl.appendChild(detailsContainer);

                if (attached && channelData.instrumentId) {
                    activeInstrumentIds.add(channelData.instrumentId);
                }
            } else {
                renderInstrumentStepView(channelEl, channelData, chIndex);
            }
        }
    ...
    if (projectState.isPlaying && lastHighlightedStepIndex !== null) {
        applyPlayheadHighlight(lastHighlightedStepIndex);
    }
...
export function setLoaderStatus(text, isError = false, options = {}) {
    ...
    if (els.loadingOverlay) {
        ...
        if (isError || options.forceOverlay) {
            loaderOverlayDismissed = false;
            overlay.classList.remove('is-hidden');
            return;
        }

        const dismissText = /(ready|loaded|success|saved|cleared)/i.test(String(text));
        const shouldDismiss = !options.persistent && (options.dismissOverlay || dismissText || loaderOverlayDismissed);
...
export function bindEventListeners() {
    ...
    if (win?.addEventListener) {
        win.addEventListener('step', evt => {
            ...
            schedulePlayheadHighlight(evt.detail.stepIndex, evt.detail?.scheduledTime, evt.detail?.cycle);
        });

        win.addEventListener('transport-stop', clearTransportHighlight);
        win.addEventListener('sampler-playback', handleSamplerPlaybackEvent);
        win.addEventListener('sampler-playback-stop', handleSamplerPlaybackStopEvent);
        win.addEventListener('sequencer:project-reset', resetSequencerUICaches);
    }
...
function schedulePlayheadHighlight(stepIndex, scheduledTime, _cycle) {
    const scheduleFrame = () => {
        const frameId = requestAnimationFrame(() => {
            pendingPlayheadFrames.delete(frameId);
            applyPlayheadHighlight(stepIndex);
        });
        pendingPlayheadFrames.add(frameId);
    };

    if (typeof scheduledTime === 'number') {
        const currentTime = getAudioContextCurrentTime(runtimeState.Tone);
        if (typeof currentTime === 'number') {
            const delayMs = Math.max(0, (scheduledTime - currentTime) * 1000);
            if (delayMs > 1) {
                const timeoutId = setTimeout(() => {
                    pendingPlayheadTimers.delete(timeoutId);
                    scheduleFrame();
                }, delayMs);
                pendingPlayheadTimers.add(timeoutId);
                return;
            }
        }
    }
...
function applyPlayheadHighlight(stepIndex) {
    ...
    lastHighlightedStepIndex = normalizedIndex;
}

function clearTransportHighlight() {
    pendingPlayheadFrames.forEach(frameId => cancelAnimationFrame(frameId));
    pendingPlayheadFrames.clear();
    pendingPlayheadTimers.forEach(timeoutId => clearTimeout(timeoutId));
    pendingPlayheadTimers.clear();
    stopAllWaveformAnimations();
