/**
 * @file midi.js
 * @description MIDI input controller shared by sequencer instruments.
 */
...
export class MidiControl {
    constructor(eventBus, statusElements = {}) {
        console.log('[MIDI] Initializing MidiControl...');
        this.eventBus = eventBus;
        this.midi = null;
        this.attachedInputs = new Set();

        this.midiInd = statusElements?.indicator || null;
        this.midiStat = statusElements?.label || null;
    ...
    onMIDI(ev) {
        ...
        const normalizedVelocity = Math.max(0, Math.min(velocity / 127, 1));

        if (status === 0x90 && velocity > 0) {
            this.eventBus.dispatchEvent(new CustomEvent('midi-note-on', {
                detail: { note, velocity: normalizedVelocity }
            }));
