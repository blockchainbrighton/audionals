...
export default class LuminaSynth {
    ...
    injectStyles() {
        ...
        .header-bar {
            ...
            background: var(--panel); padding: 5px 15px; border-radius: 8px; border-bottom: 2px solid var(--accent);
        }
        .brand { font-size: 1.2rem; font-weight: bold; color: var(--text); letter-spacing: 4px; }
        .brand span { color: var(--accent); }
        .preset-bar {
            display: grid; grid-template-columns: repeat(auto-fit, minmax(80px, 1fr)); gap: 5px;
            background: var(--panel); padding: 5px; border-radius: 8px;
        }
        .preset-btn {
            background: var(--knob-bg); border: 1px solid #444; color: var(--text-dim);
            padding: 4px; border-radius: 4px; cursor: pointer; font-size: 0.7rem; text-align: center;
        }
        .preset-btn:hover { border-color: var(--accent); color: var(--text); }
        .preset-btn.active { background: var(--accent-dim); border-color: var(--accent); color: #fff; }
        .controls-area { display: grid; grid-template-columns: 1fr 1fr 1fr 180px; gap: 5px; }
        @media(max-width: 800px) { .controls-area { grid-template-columns: 1fr 1fr; } }
        .module {
            background: var(--panel); border-radius: 8px; padding: 5px;
            display: flex; flex-direction: column; border: 1px solid #333;
        }
        .module-title {
            font-size: 0.7rem; text-transform: uppercase; color: var(--text-dim);
            border-bottom: 1px solid #333; padding-bottom: 4px; margin-bottom: 5px; letter-spacing: 1px;
        }
        .knob-group { display: flex; flex-wrap: wrap; gap: 10px; justify-content: center; align-items: center; flex: 1; }
        .knob-container { display: flex; flex-direction: column; align-items: center; width: 50px; }
        .knob-dial {
            width: 40px; height: 40px; border-radius: 50%; background: var(--knob-bg);
        ...
        .knob-label { font-size: 0.6rem; margin-top: 4px; color: var(--text-dim); }
        .knob-val { font-size: 0.6rem; color: var(--accent); margin-top: 2px; opacity: 1; }
        .vis-module {
            position: relative; display: flex; flex-direction: column;
            background: #000; border: 1px solid #333; border-radius: 8px; overflow: hidden; min-height: 100px;
        }
    ...
    initVisuals() {
        ...
        // Reduced geometry detail (detail level 2 -> 1)
        ...
        const loop = (t) => {
            ...
            frame++;
            if(frame % 2 !== 0) return; // 30fps throttle
            ...
            if(this.state.waveform && this.Tone) {
                ...
                let sum = 0;
                // Sample fewer points for scale calc
                for(let i=0; i<buffer.length; i+=20) sum += Math.abs(buffer[i]);
                const avg = sum / (buffer.length/20);
