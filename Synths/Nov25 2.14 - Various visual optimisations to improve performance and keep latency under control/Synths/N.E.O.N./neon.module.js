...
export default class NeonSynth {
    ...
    initVisuals() {
        ...
        if(THREE) {
            ...
            // Reduce geometry from 40x40 to 20x20
            ...
            const loop = () => {
                ...
                if(this.state.fx.analyzer) { 
                    const values = this.state.fx.analyzer.getValue(); 
                    // Analyzer returns array, use lower freq for opacity
                    const bass = values[0]; 
                    material.opacity = 0.2 + ((bass + 100) / 200); 
                }
