...
export default class ThrordSynth {
    ...
    injectStyles() {
        ...
        .thrord-wrapper {
            ...
            --knob-size: 40px; --border: 1px solid #444;
            background-color: #121212; color: var(--text-main);
            font-family: 'Courier New', Courier, monospace;
            display: flex; justify-content: center; align-items: center;
            width: 100%; height: 100%; overflow-y: auto; padding: 5px;
        }
        #app { width: 100%; max-width: 900px; background: var(--bg-dark); border: 2px solid #555; box-shadow: 0 0 20px rgba(0,0,0,0.8); padding: 5px; border-radius: 4px; }
        .header { display: flex; justify-content: space-between; align-items: center; padding: 5px; background: #000; border-bottom: 2px solid var(--accent); margin-bottom: 5px; }
        h1 { margin: 0; font-size: 18px; color: var(--accent); letter-spacing: 2px; }
        .panel-row { display: flex; gap: 5px; margin-bottom: 5px; }
        .module { background: var(--bg-panel); border: var(--border); padding: 5px; flex: 1; display: flex; flex-direction: column; align-items: center; }
        .module-title { font-size: 10px; font-weight: bold; color: #888; margin-bottom: 5px; width: 100%; text-align: center; border-bottom: 1px solid #444; padding-bottom: 2px; }
        .controls-flex { display: flex; gap: 5px; justify-content: center; flex-wrap: wrap; }
        .control-group { display: flex; flex-direction: column; align-items: center; width: 40px; }
        .control-label { font-size: 9px; margin-top: 2px; text-align: center; }
        input[type=range] { -webkit-appearance: none; width: 10px; height: 60px; background: #000; outline: none; border-radius: 5px; writing-mode: bt-lr; -webkit-appearance: slider-vertical; }
        .switch { position: relative; display: inline-block; width: 30px; height: 15px; margin: 5px 0; }
        ...
        .display-screen { background: #220; color: var(--accent); padding: 5px; font-family: 'Courier New', monospace; border: 2px inset #444; width: 120px; text-align: center; margin-bottom: 5px; text-transform: uppercase; font-size: 12px; }
        .btn { background: #444; color: white; border: 1px solid #666; padding: 2px 8px; cursor: pointer; font-size: 10px; }
        .btn:hover { background: #666; }
        .btn.active { background: var(--accent); color: black; }
        .sequencer-container { background: #222; padding: 5px; border: var(--border); overflow-x: auto; }
        .seq-grid { display: grid; grid-template-columns: 60px repeat(16, 1fr); gap: 1px; }
        .seq-row-label { font-size: 9px; display: flex; align-items: center; color: #888; }
        .step-col { display: flex; flex-direction: column; align-items: center; gap: 1px; min-width: 20px; }
        .seq-btn { width: 16px; height: 16px; background: #333; border: 1px solid #555; cursor: pointer; }
        .seq-btn.active { background: var(--accent); box-shadow: 0 0 5px var(--accent); }
        .seq-btn.current { border-color: #fff; }
        .seq-note-select { width: 20px; font-size: 8px; background: #111; color: #fff; border: none; -webkit-appearance: none; text-align: center; }
        #keyboard { margin-top: 5px; }
        canvas { width: 100%; height: 40px; background: #000; border: 1px solid #333; }
        `;
