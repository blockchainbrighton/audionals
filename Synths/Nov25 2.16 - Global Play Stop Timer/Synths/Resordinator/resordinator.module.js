...
export default class ResordinatorSynth {
    ...
    triggerVisualString(midi, velocity) {
        ...
        const idx = Math.max(0, Math.min(71, midi - 24));
        const str = this.state.visuals.strings[idx];
        if(str) {
            str.userData.amp = velocity * 1.5;
            str.userData.active = true;
        }
    }
    ...
    initVisuals() {
        ...
        // Camera positioned closer (Z=40) for a much wider, immersive view
        ...
        const numStrings = 72;
        // Doubled spacing to 2.5 to fill the entire width of the screen
        const spacing = 2.5; 
        const startX = -(numStrings * spacing) / 2;

        for(let i=0; i<numStrings; i++) {
            const points = [];
            // Y from -20 to +20 (40 units tall)
            for(let j=0; j<=16; j++) points.push(new THREE.Vector3(startX + i*spacing, (j*2.5)-20, 0));

            const geo = new THREE.BufferGeometry().setFromPoints(points);
            
            // AdditiveBlending + No Depth Test = Glowing, Super Bright Lines
            const mat = new THREE.LineBasicMaterial({ 
                color: 0xffffff, 
                transparent: true, 
                opacity: 0.6, // High base opacity
        ...
        const loop = () => {
            ...
            for(let i=0; i<strings.length; i++) {
                ...
                if(line.userData.amp > 0.001) {
                    line.userData.amp *= 0.95; 
                    
                    // Ultra bright mapping: Base 0.6 -> Max 1.0
                    const targetOp = 0.6 + Math.min(0.4, line.userData.amp * 0.4);
                    line.material.opacity = targetOp;

                    const pos = line.geometry.attributes.position.array;
                    for(let j=0; j<=16; j++) { 
                        const y = (j*2.5)-20;
                        const vib = Math.sin(y*0.4 + time*2.5) * line.userData.amp * Math.sin((j/16)*Math.PI);
                    ...
                    line.userData.amp = 0;
                    line.material.opacity = 0.6; // Reset to bright idle
                    const pos = line.geometry.attributes.position.array;
                    for(let j=0; j<=16; j++) pos[j*3] = line.userData.baseX; 
                    line.geometry.attributes.position.needsUpdate = true;
