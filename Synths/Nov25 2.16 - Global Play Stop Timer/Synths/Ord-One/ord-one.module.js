...
export default class OrdOneSynth {
    ...
    injectStyles() {
        ...
        #synth-rack {
            display: grid; grid-template-columns: repeat(4, 1fr); grid-template-rows: auto auto auto 100px;
            gap: 5px; background: #000; padding: 8px; border: 4px solid #333; border-radius: 8px;
            box-shadow: 0 0 40px rgba(0,0,0,0.8); max-width: 1000px; width: 100%;
        }
        .module { background: var(--panel-bg); border: var(--border-style); padding: 5px; position: relative; display: flex; flex-direction: column; }
        .module h3 { margin: 0 0 5px 0; font-size: 0.7rem; text-transform: uppercase; border-bottom: 2px solid #555; padding-bottom: 2px; color: #aaa; text-align: center; }
        .controls-row { display: flex; justify-content: space-around; flex-wrap: wrap; gap: 4px; flex: 1; }
        .control-group { display: flex; flex-direction: column; align-items: center; width: 40px; }
        .label { font-size: 0.5rem; margin-top: 2px; text-align: center; color: #888; }
        .knob-container { width: 32px; height: 32px; position: relative; }
        .knob {
        ...
        canvas#oscilloscope { width: 100%; height: 40px; background: #000; border: 1px solid #444; }
        #keyboard { grid-column: span 4; background: #111; border-top: 2px solid #333; padding-top: 2px; }
        select { background: #000; color: #fff; border: 1px solid #555; width: 100%; font-family: inherit; font-size: 0.6rem; }
        `;
