...
(() => {
  ...
  const load = (url,i) => new Promise(res => {
    ...
    img.onload  = () => { ok++;  loaded[i] = img;  console.log(`ImageLoadMgmt: ✔ [${i+1}/${urls.length}] ${url}`); res(img); };
    img.onerror = () => { fail++;loaded[i] = null; console.error(`ImageLoadMgmt: ✖ [${i+1}/${urls.length}] ${url}`); res(null); };
    img.src = url;
  });
