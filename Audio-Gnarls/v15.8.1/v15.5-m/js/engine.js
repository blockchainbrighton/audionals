...
export function Engine(app) {
  ...
  const bufferHumChain = async () => {
    ...
    try {
      const osc = new (T.__Oscillator || T.Oscillator)('A0', 'sine').start(), filter = new T.Filter(80, 'lowpass'); filter.Q.value = .5;
  ...
  const bufferShapeChain = async shape => {
    ...
    try {
      const o1 = new (T.__Oscillator || T.Oscillator)(pr.osc1[1], pr.osc1[0]).start(), o2 = pr.osc2 ? new (T.__Oscillator || T.Oscillator)(pr.osc2[1], pr.osc2[0]).start() : null;
...
class ToneLoader extends HTMLElement {
  ...
  connectedCallback() {
    ...
    const importTone = () => import(url).then(mod => {
      ...
      window.removeEventListener('pointerdown', importToneOnce, true);
      window.removeEventListener('keydown', importToneOnce, true);
    }).catch(err => console.error('Failed to load Tone.js:', err));
    const importToneOnce = () => { importTone(); };
    // Defer Tone import until first user gesture to avoid "AudioContext is suspended" warning
    window.addEventListener('pointerdown', importToneOnce, true);
    window.addEventListener('keydown', importToneOnce, true);
  }
}
