...
# ---- Inputs ----
# Put this script in the same folder as bundle.js
BUNDLE_PATH = Path(__file__).with_name("bundle.js")
OUTPUT_DIR = Path("generated_html_inline")
...
if not BUNDLE_PATH.exists():
    raise FileNotFoundError(f"bundle.js not found next to this script at: {BUNDLE_PATH}")

bundle_js = BUNDLE_PATH.read_text(encoding="utf-8")

# Prevent accidental </script> termination inside inline JS
# (rare, but safe) by escaping the sequence.
bundle_js_safe = bundle_js.replace("</script>", "<\\/script>")
...
<html lang="en" data-seed="{seed}">
  <head>
    <meta charset="utf-8">
    <title>osc-app — {seed}</title>
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <style>
      html,body {{ height: 100%; margin: 0; background: #0b0b0c; color: #e5e7eb; }}
      osc-app {{ display:block; height:100%; }}
      /* Optional: center a loading hint in case the custom element renders async */
      .hint {{ position:fixed; inset:0; display:grid; place-items:center; font:16px system-ui, sans-serif; opacity:.4; }}
    </style>
  </head>
  <body>
    <osc-app></osc-app>
    <div class="hint">Loading <code>&lt;osc-app&gt;</code>…</div>
    <script type="module">
{bundle}
    </script>
  </body>
...
<html lang="en">
  <head>
    <meta charset="utf-8">
    <title>osc-app — seed index</title>
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <style>
      html,body {{ margin:0; font:16px system-ui, sans-serif; background:#0b0b0c; color:#e5e7eb; }}
      main {{ max-width: 900px; margin: 40px auto; padding: 0 16px; }}
      h1 {{ font-size: 22px; margin: 0 0 16px; }}
      ol {{ columns: 3; column-gap: 24px; padding-left: 18px; }}
      li {{ margin: 4px 0; break-inside: avoid; }}
      a {{ color: #93c5fd; text-decoration: none; }}
      a:hover {{ text-decoration: underline; }}
      .note {{ opacity: .65; margin: 12px 0 0; font-size: 14px; }}
    </style>
  </head>
  <body>
    <main>
      <h1>osc-app — 256 seeds</h1>
      <p class="note">Each file is standalone with the full inlined <code>bundle.js</code>.</p>
      <ol>
"""
...
for seed in seeds:
    html_path = OUTPUT_DIR / f"{seed}.html"
    html = HTML_TEMPLATE.format(seed=html_escape(seed),
                                bundle="\n".join("      " + line for line in bundle_js_safe.splitlines()))
    html_path.write_text(html, encoding="utf-8")
    print(f"Created {html_path}")

# Create an index.html to quickly open each seed
index_path = OUTPUT_DIR / "index.html"
with index_path.open("w", encoding="utf-8") as idx:
    idx.write(INDEX_HEADER)
    for seed in seeds:
        idx.write(f'        <li><a href="{seed}.html">{seed}</a></li>\n')
    idx.write(INDEX_FOOTER)

print(f"\nDone. {len(seeds)} HTML files + index generated in: {OUTPUT_DIR.resolve()}")
