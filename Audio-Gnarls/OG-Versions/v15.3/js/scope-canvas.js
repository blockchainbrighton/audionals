  ...
  class ScopeCanvas extends HTMLElement {
    constructor(){
      ...
      this.drawFuncs = {
        ...
        butterfly:(d,t)=>this._withCtx(()=>this._traceParam(d,(i,n,_,c)=>{ const th=i/n*PI*28+t*.00035, a=pow(this._ampAt(d,i),1.25);
          const s=Math.exp(.85*cos(th))-1.6*cos(5*th)+pow(sin(th/10),7), r=s*.22*_.cw*(.5+.5*a);
          return [c+sin(th)*r,c+cos(th)*r]; },{close:true})),
