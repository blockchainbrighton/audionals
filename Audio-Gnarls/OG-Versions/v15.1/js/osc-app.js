...
const addEvents = (el, pairs) => pairs.forEach(([t, h]) => on(el, t, h));
/** Batch removeEventListener from an array of [type, handler] pairs. */
const removeEvents = (el, pairs) => pairs.forEach(([t, h]) => off(el, t, h));

/* ============================================================================
 * <osc-controls> Web Component — with Latch toggle (responsive)
 * ----------------------------------------------------------------------------
 * UI-only component that emits semantic events:
 *  - start-request, mute-toggle, shape-change, toggle-sequencer, audio-signature
...
class OscControls extends HTMLElement {
  constructor() {
    ...
    // dispatch helper bound to this element for composed, bubbling events
    ...
    // expose only the helpers OscApp depends on
  ...
  updateState({
    ...
    if (isBool(isAudioSignaturePlaying)) {
    ...
    if (isBool(isAudioStarted)) {
      ...
      [this._muteBtn, this._audioSigBtn, this._latchBtn, this._loopBtn, this._sigModeBtn, this._vol]
...
class OscApp extends HTMLElement {
  ...
  constructor() {
    ...
    // Engine + Signatures mix into this instance (public API dependency)
  ...
  connectedCallback() {
    ...
    // Prepare the shape data for the dropdown (+ prepend "hum")
  ...
  _updateControls(patch = {}) {
    ...
    if (typeof c.updateState === 'function') c.updateState(enriched);
      ...
      // Fallback data- attributes for unknown controls impls
  ...
  _fitLayout() {
    try {
      ...
      const side = Math.round(Math.max(1, Math.min(vw, availableH))); // avoid half-pixels
  ...
  _onToneReady() {
    ...
    // Pick an initial shape deterministically by seed (Engine supplies _rng)
    ...
    try {
      ...
      const onClick = async (ev) => {
        ...
        try { await this.unlockAudioAndBufferInitial?.(); } catch (e) { console.error('Power-on unlock failed:', e); }
          ...
          // Re-render or remove overlay based on unlock success.
          setTimeout(() => {
  ...
  _onShapeStep({ detail }) {
    ...
    const s = this.state;
    // Only step through actual shapes, not 'hum'
    const shapes = this.shapes;

    // Establish current index; if 'hum' or unknown, create sensible start:
