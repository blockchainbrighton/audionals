...
export function Engine(app) {
  ...
  const _updateKeyboardSynthVoice = (shapeKey) => {
    ...
    const voiceConfig = {
      // Voice 0 (osc1)
      voice0: {
        oscillator: { type: preset.osc1[0] },
        envelope: preset.envelope,
        filter: { Q: preset.filterQ, type: 'lowpass' },
      },
      // Voice 1 (osc2)
      voice1: preset.osc2
        ? {
            oscillator: { type: preset.osc2[0] },
            envelope: preset.envelope,
            filter: { Q: preset.filterQ, type: 'lowpass' },
          }
        : { volume: -Infinity }, // Mute voice1 if not used in preset
      vibratoAmount: preset.lfo[0] > 2 ? 0.1 : 0, // Add some vibrato for fast LFOs
      vibratoRate: preset.lfo[0],
    };
