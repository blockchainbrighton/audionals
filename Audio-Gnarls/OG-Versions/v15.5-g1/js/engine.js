...
// Shared audio helpers: these functions centralise analyser creation,
// linear gain conversion and chain management. See `shared/audio.js`.
import {
  createAnalyser,
  linToDb as _linToDbHelper,
  rampParamLinear,
  fadeOutChain as _fadeOutChain,
  disposeChain as _disposeAudioChain
...
export function Engine(app) {
  ...
  // Bind imported helpers into local names for backwards compatibility.  These
  // wrappers ensure the Engine continues to expose the same private API
  // functions while delegating to the shared audio module.
  const _createAnalyser = T => createAnalyser(T);
  const _linToDb = v => _linToDbHelper(v);

  // Linear ramp helper delegates to the shared helper to avoid code
  // duplication. The wrapper preserves the original signature (param,
  // targetValue, duration, Tone).
  const _rampLinear = (p, t, s, T) => {
    rampParamLinear(p, t, s, T);
  };

  // Silence all chains gracefully. Instead of reimplementing fade logic
  // here, delegate to the shared fade helper for each chain. The helper
  // returns a promise resolving after the fade completes so we can await
  // the slowest fade before proceeding.
  const _silenceAllChains = async (f = 0.012) => {
    const T = app.state?.Tone; if (!T) return;
    const chains = app.state?.chains || {};
    // Collect promises for each chain fade and wait for all to settle.
    const tasks = [];
    for (const key in chains) {
      const ch = chains[key];
      tasks.push(_fadeOutChain(ch, T, f));
    }
    await Promise.all(tasks);
  };

  // Dispose a single chain by first fading it out and then stopping,
  // disposing and disconnecting all nodes. This wrapper preserves
  // backwards compatibility with the previous internal API signature.
  const _disposeChain = async (ch) => {
    const T = app.state?.Tone;
    if (!ch || !T) return;
    await _disposeAudioChain(ch, T, 0.012);
  };
