...
// v20.2 - Use shared clamp01 from utils.js
...
class SeqApp extends HTMLElement {
  ...
  #handleDragVelocity(i,e,slot,bar){
    ...
    const v=clamp01(d.baseVel+((d.startY-e.clientY)/150)*(e.shiftKey?.25:1)); this.#setVel(i,v); this.#setTooltip(slot,v,`Alt-drag${e.shiftKey?" + Shift":""}`); bar.style.height=`${Math.round(v*100)}%`;
  }
