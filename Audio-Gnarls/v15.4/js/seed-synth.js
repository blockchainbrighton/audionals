// seed-synth.js (further condensed, identical public API/behavior)
...
const HUM = 'hum', HUM_L = 'Power Hum', SID = 'seedInput';
...
export class SeedSynthElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.setOptions({ seed: this.getAttribute('seed') ?? DEF.seed, showSequencer: this.hasAttribute('show-sequencer') });
    this._render();
  ...
  attributeChangedCallback(n, o, v) {
    if (!this._initialized || o === v) return;
    n === 'seed' ? (this.seed = v || DEF.seed) : n === 'show-sequencer' && this._updateSequencerVisibility();
  }

  // ---- public API (unchanged surface) ----
  setOptions(opts = {}) { this._options = { ...this._options, ...opts }; this._osc() && this._applyOptionsToOscApp(); }

  get seed() { return this._options.seed; }
  set seed(v) {
    const s = String(v || DEF.seed);
    this._options.seed = s;
    this.setAttribute('seed', s);
    const o = this._osc(); if (!o) return;
    o.resetToSeed?.(s);
    o.shadowRoot?.getElementById(SID)?.value = s;
  }

  get options() {
    const o = this._osc(); if (!o) return [];
    const { shapes = [], humKey = HUM, humLabel = HUM_L } = o;
  ...
  async start() { if (!this._osc()) throw new Error('Component not ready'); this._osc()?._onStartRequest?.(); }

  stop() {
    const s = this._state(); if (!s) return;
    s.isPlaying && this._osc()?._onStartRequest?.();
    s.sequencePlaying && this.stopSequence();
  }

  mute(forceBool) {
    const o = this._osc(); if (!o) return;
    const m = !!this._dest()?.mute;
    typeof forceBool === 'boolean' ? (m !== forceBool && o._onMuteToggle?.()) : o._onMuteToggle?.();
  }
  ...
  setStepTime(ms) { const s = this._state(); s && ((s.stepTime = ms), this._seq()?.setStepTime?.(ms)); }

  getAnalyser() { return this._state()?.chains?.[this.currentKey]?.analyser ?? null; }

  getState() {
    const s = this._state(); if (!s) return null;
    return {
      seed: s.seed,
  ...
  _render() {
    this.shadowRoot.innerHTML = `<style>:host{display:block;width:100%;height:100%}osc-app{width:100%;height:100%}</style><osc-app></osc-app>`;
  ...
  _applyOptionsToOscApp() {
    ...
    this._osc()?.shadowRoot?.getElementById(SID)?.value = this._options.seed;
  }
  ...
  _updateSequencerVisibility() {
    ...
    const s = this._state(); s && (s.isSequencerMode = !!show);
  }
}
