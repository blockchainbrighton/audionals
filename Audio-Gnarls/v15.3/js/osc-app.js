...
const on = (el, t, h, o) => el?.addEventListener?.(t, h, o);
const off = (el, t, h, o) => el?.removeEventListener?.(t, h, o);
const setText = (el, s) => el && (el.textContent = s);
const setPressed = (b, v) => b?.setAttribute?.('aria-pressed', String(!!v));
const toggleClass = (el, c, v) => el?.classList?.toggle?.(c, !!v);
const byId = (root, id) => root.getElementById(id);
const isBool = (v) => typeof v === 'boolean';
const isNum = (v) => typeof v === 'number' && !Number.isNaN(v);
const setDisabledAll = (els, d) => { for (const el of els) el.disabled = !!d; };
const addEvents = (el, pairs) => { for (let i = 0; i < pairs.length; i++) on(el, pairs[i][0], pairs[i][1], pairs[i][2]); };
const removeEvents = (el, pairs) => { for (let i = 0; i < pairs.length; i++) off(el, pairs[i][0], pairs[i][1], pairs[i][2]); };
