...
### 🛡️ Token Spending & Robustness (Safety Checks)
- [x] **Mandatory Confirmation Modals:**
    - [x] **Full Book Generation:** Show total estimated cost, count of missing (uncached) segments, and total characters before starting.
    - [x] **Chapter/Chunk Regeneration:** Warn the user when clicking "Regenerate" on an already completed premium segment.
- [x] **"Dry Run" Analysis:**
    - [x] Add an "Analyze Cache" step that checks the server for existing hashes before showing the final cost, providing a "Real Cost" (missing chunks) vs "Full Cost".
- [x] **Loop & Error Protection:**
    - [x] **Stop/Pause Button:** Implement a global "Halt Queue" button to stop an ongoing mass generation.
    - [x] **Retry Limits:** Prevent the client from indefinitely retrying failed API calls. Max 3 attempts per chunk with exponential backoff.
- [x] **Rate Limiting (UI Level):**
    - [x] Disable generation buttons immediately after click to prevent double-triggering.
- [x] **Usage Thresholds:**
    - [x] Allow users to set a "Session Budget" (e.g., "Warn me if I spend more than $5.00 this session").
