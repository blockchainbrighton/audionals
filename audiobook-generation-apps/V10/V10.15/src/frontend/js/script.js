...
const TextParser = {
    ...
    detectStartingVoice: function(chapterText, chapterTitle, voiceNames = []) {
        const candidates = voiceNames.map((n, i) => ({ name: n.trim(), index: i })).filter(c => c.name);
        ...
        // 2. Check Content (Ignore empty lines, check first 10 meaningful lines)
        // Note: chapterText might have the header prepended, so we skip standard header lines if possible or just rely on regex
