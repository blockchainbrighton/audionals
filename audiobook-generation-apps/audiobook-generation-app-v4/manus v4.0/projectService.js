...
function createProjectService(options = {}) {
  ...
  async function processChapter(project, chapterIndex) {
    ...
    try {
      ...
      if (project.mode === 'single') {
        ...
        if (showcaseActive && Array.isArray(chapter.sequences)) {
          ...
          for (const sequence of chapter.sequences) {
            ...
            for (const step of steps) {
              ...
              for (let i = 0; i < stepLimitedChunks.length; i++) {
                ...
                const chunkFile = path.join(chunksDir, `${chapterId}_${segmentCounter}_${chunkHash}.mp3`);
          ...
          for (let i = 0; i < chunks.length; i++) {
            ...
            const chunkHash = createHash('sha1').update(chunkText).digest('hex').slice(0, 16);
            const chunkFile = path.join(chunksDir, `${chapterId}_${i}_${chunkHash}.mp3`);
        ...
        for (const segment of segments) {
          ...
          for (let i = 0; i < chunks.length; i++) {
            ...
            const chunkText = isDemo ? chunks[i].slice(0, demoCharLimit) : chunks[i];
            const chunkHash = createHash('sha1').update(chunkText).digest('hex').slice(0, 16);
            const chunkFile = path.join(chunksDir, `${chapterId}_${segmentIndex}_${chunkHash}.mp3`);
      ...
      const chapterFile = path.join(chaptersDir, `${chapterId}.mp3`);
      const normalizedFile = path.join(chaptersDir, `${chapterId}_normalized.mp3`);
  ...
  async function compileBook(projectId, options = {}) {
    ...
    const taskPromise = (async () => {
      ...
      const bookFile = path.join(bookDir, `${projectId}_audiobook.mp3`);
