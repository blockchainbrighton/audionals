...
<body>
    ...
    <div class="content">
      ...
      <div id="new-book" class="tab-content">
        ...
        <form id="new-book-form">
          ...
          <div class="form-row">
            <div class="form-group">
              <label for="chapter-start-number">Starting Chapter Number
                <span class="info-icon" tabindex="0" role="img" aria-label="Starting chapter number info" data-tooltip="Use this when you are generating a mid-series batch and want file names to align with the real chapter numbering.">i</span>
              </label>
              <input type="number" id="chapter-start-number" min="1" placeholder="Defaults to 1">
            </div>
            
            <div class="form-group">
              <label class="checkbox-label" style="margin-top: 32px;">
                <input type="checkbox" id="chapter-auto-detect" checked>
                <span>Auto-detect chapter numbers from titles</span>
              </label>
              <span class="help-text">When enabled, numbers found in chapter titles override the starting number.</span>
            </div>
          </div>
  ...
  <script>
    ...
    async function loadProjects() {
      try {
        ...
        container.innerHTML = projects.map(project => {
          ...
          const startNumber = project.chapterStartNumber || 1;
          const numberingLabel = project.autoDetectChapterNumbers === false
            ? \`Manual from \${startNumber}\`
            : (project.chapterStartNumber ? \`Auto (start \${startNumber})\` : 'Auto-detect');
          ...
          return \`
            <div class="project-card" onclick="openProject('\${project.id}')">
              ...
              <p><strong>Chapter Numbering:</strong> \${numberingLabel}</p>
    ...
    document.getElementById('new-book-form').addEventListener('submit', async (e) => {
      ...
      const chapterStartValue = document.getElementById('chapter-start-number').value.trim();
      if (chapterStartValue) {
        const parsedStart = parseInt(chapterStartValue, 10);
        if (Number.isNaN(parsedStart) || parsedStart <= 0) {
          showFlashMessage('Starting chapter number must be a positive integer', 'error', 5000);
          return;
        }
        data.chapterStartNumber = parsedStart;
      }
      data.autoDetectChapterNumbers = document.getElementById('chapter-auto-detect').checked;
      ...
      try {
        ...
        updateDemoOptionsVisibility();
        updatePresetSampleVisibility();
        document.getElementById('chapter-auto-detect').checked = true;
    ...
    async function openProject(projectId) {
      try {
        ...
        const startNumber = project.chapterStartNumber || 1;
        const numberingLabel = project.autoDetectChapterNumbers === false
          ? \`Manual sequence from \${startNumber}\`
          : \`Auto-detect\${project.chapterStartNumber ? ' (start ' + startNumber + ')' : ''}\`;
             ...
             \${project.chapters.map((chapter, index) => \`
               <div class="chapter-item">
                 <div class="chapter-header">
                    <span class="chapter-title">\${'Chapter ' + (chapter.sequenceNumber || (index + 1))}</span>
                    \${chapter.title && chapter.title.trim() && (!/chapter\s*\d+/i.test(chapter.title) || (chapter.sequenceNumber && !chapter.title.includes(String(chapter.sequenceNumber)))) ? '<span class="chapter-subtitle">' + escapeHtml(chapter.title) + '</span>' : ''}
