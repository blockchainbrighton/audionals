    ...
    return new Promise((resolve, reject) => {
      for (const filePath of inputFiles) {
        if (!filePath || !fs.existsSync(filePath)) {
          return reject(new Error(`Audio merge failed: missing input file ${filePath}`));
        }
      }
      ...
      ffmpeg.on('error', (error) => {
        if (fs.existsSync(listFile)) {
          try {
            ...
            console.warn('Unable to remove ffmpeg list file:', cleanupError.message);
          }
        }
        reject(error);
      });
    ...
    return new Promise((resolve, reject) => {
      ...
      ffmpeg.on('close', (code) => {
        if (code === 0) {
          ...
          const message = stderr.trim() || `ffmpeg normalization failed with code ${code}`;
          reject(new Error(message));
        }
      });
    ...
    try {
      ...
      if (project.mode === 'single') {
        ...
        if (showcaseActive && Array.isArray(chapter.sequences)) {
          ...
          const previousChunks = Array.isArray(segment.chunks) ? segment.chunks : [];
          const updatedChunks = [];
        ...
        for (const segment of segments) {
          ...
          for (let i = 0; i < chunks.length; i++) {
            ...
            const previousChunk = previousChunks[i];
            if (previousChunk && previousChunk.hash === chunkHash && previousChunk.file && fs.existsSync(previousChunk.file)) {
              chunkFiles.push(previousChunk.file);
              updatedChunks.push(previousChunk);
              segment.status = 'completed';
              segment.file = previousChunk.file;
              segment.hash = previousChunk.hash;
            } else {
              if (previousChunk && previousChunk.file && fs.existsSync(previousChunk.file)) {
                try {
                  fs.unlinkSync(previousChunk.file);
                } catch (error) {
                  console.warn('Unable to remove stale chunk file:', previousChunk.file, error.message);
                }
              }
              ...
              const chunkMeta = { file: chunkFile, hash: chunkHash };
              updatedChunks.push(chunkMeta);
              segment.status = 'completed';
          ...
          for (let i = updatedChunks.length; i < previousChunks.length; i++) {
            const stale = previousChunks[i];
            if (stale && stale.file && fs.existsSync(stale.file)) {
              try {
                fs.unlinkSync(stale.file);
              } catch (error) {
                console.warn('Unable to remove stale chunk file:', stale.file, error.message);
              }
            }
          }

          segment.chunks = updatedChunks;
          if (updatedChunks.length > 0) {
            const lastChunk = updatedChunks[updatedChunks.length - 1];
            segment.file = lastChunk.file;
            segment.hash = lastChunk.hash;
          } else {
            delete segment.file;
            delete segment.hash;
          }
