...
const STATE = {
    ...
    playback: {
        sessionId: 0,
        mode: 'idle', // 'idle' | 'chunk' | 'chapter' | 'all'
...
const TextParser = {
    ...
    getVoiceCandidates: function(voiceNames = []) {
        ...
        aliasMap.forEach((aliases, index) => {
            aliases.forEach(name => out.push({ name, index }));
        });
        return out;
    },
    ...
    detectStartingVoice: function(chapterText, chapterTitle, voiceNames = []) {
        ...
        const check = (str) => {
            ...
            for (const c of candidates) {
                // Regex: Start of line, Name, Word Boundary (or punctuation)
                // Matches "Cora", "Cora.", "Cora:" but avoids "Coraline" if name is "Cora"
                // Also ensures case-insensitive match
                const esc = this.escapeRegex(c.name);
                const re = new RegExp(`^[\\s"'“”‘’\\(\\[\\{\\-–—]*${esc}(?:[^\\w\\u00C0-\\u024F]|$)`, 'i');
                if (re.test(str)) return c.index;
            }
            return null;
        };

        // 1. Check chapter opening speaker line only (UI character mapping rule).
        const openingLine = this.getChapterOpeningLine(chapterText, chapterTitle);
        let res = check(openingLine);
        if (res !== null) return res;

        // 2. Fallback: some books put speaker in chapter title.
        res = check(chapterTitle);
        if (res !== null) return res;
...
class AudioEngine{
    ...
    playChunk(chunk,overrideVid=null){if(dom.manuscript){dom.manuscript.value=chunk.text;dom.manuscript.scrollTop=0}if(chunk.audioUrl) return this.playAudioBuffer(chunk.audioUrl);const vid=overrideVid||chunk.voiceId||STATE.project.voiceIds[0];const v=STATE.voices.find(vo=>vo.id===vid);if(v&&v.type==='premium')return Promise.reject(new Error('Audio not generated'));if(v&&v.ref){if(!this.synth)return Promise.reject(new Error('No TTS'));return new Promise((res,rej)=>{this.stop(true);const ut=new SpeechSynthesisUtterance(chunk.text);ut.voice=v.ref;
    this.cancelCurrent=()=>{this.cancelCurrent=null;res()};
    ut.onend=()=>{this.cancelCurrent=null;res()};ut.onerror=()=>{this.cancelCurrent=null;rej(new Error('Playback failed'))};this.synth.speak(ut)})}return Promise.reject(new Error('Voice unavailable'))}
    playAudioBuffer(url){return new Promise((res,rej)=>{this.initCtx();this.stop(true);const aud=new Audio(url);aud.crossOrigin="anonymous";this.activeAudio=aud;const src=this.ctx.createMediaElementSource(aud);src.connect(this.analyser);this.analyser.connect(this.ctx.destination);this.draw();
    this.cancelCurrent=()=>{this.cancelCurrent=null;res()};
    aud.onended=()=>{this.cancelCurrent=null;cancelAnimationFrame(this.animId);this.activeAudio=null;res()};aud.onerror=()=>{this.cancelCurrent=null;cancelAnimationFrame(this.animId);this.activeAudio=null;rej(new Error('Play failed'))};if(this.ctx.state==='suspended')this.ctx.resume();aud.play().catch(rej)})}
    ...
    if(!int)this.updateBtn(false);if(this.animId)cancelAnimationFrame(this.animId);if(this.cCtx&&this.canvas)this.cCtx.clearRect(0,0,this.canvas.width,this.canvas.height)}
...
function syncPlaybackFlags(){
    const activeChunkId = STATE.playback.activeChunkId;
    STATE.isPlaying = STATE.playback.mode !== 'idle';
    if(activeChunkId) STATE.activePlaybackId = `chunk_${activeChunkId}`;
    else if(STATE.playback.mode === 'chapter' && STATE.playback.requestedChapterIndex !== null) STATE.activePlaybackId = `chapter_${STATE.playback.requestedChapterIndex}`;
    else STATE.activePlaybackId = null;
    engine.updateBtn(STATE.isPlaying);
}

function applyPlaybackState(patch={},render=true){
    STATE.playback = {...STATE.playback,...patch};
    syncPlaybackFlags();
    if(render) renderTimeline();
}

function isPlaybackSessionCurrent(sessionId,mode=null){
    return STATE.playback.sessionId === sessionId && (mode ? STATE.playback.mode === mode : STATE.playback.mode !== 'idle');
}

function stopPlayback({invalidateSession=true,clearProgress=true}={}){
    engine.stop(true);
    applyPlaybackState({
        sessionId: invalidateSession ? STATE.playback.sessionId + 1 : STATE.playback.sessionId,
        mode: 'idle',
        requestedChapterIndex: null,
        activeChapterIndex: null,
        activeChunkId: null
    });
    if(clearProgress && dom.masterProgress) dom.masterProgress.value = 0;
}

function beginPlaybackSession(mode,{requestedChapterIndex=null,activeChapterIndex=null,activeChunkId=null,resetProgress=true}={}){
    engine.stop(true);
    const sessionId = STATE.playback.sessionId + 1;
    if(resetProgress && dom.masterProgress) dom.masterProgress.value = 0;
    applyPlaybackState({sessionId,mode,requestedChapterIndex,activeChapterIndex,activeChunkId});
    return sessionId;
}

function isChunkControlActive(chunkId){
    return STATE.isPlaying && STATE.playback.activeChunkId === chunkId;
}

function isChapterControlActive(chapterIndex){
    return STATE.isPlaying && STATE.playback.activeChapterIndex === chapterIndex;
}

function findChunkById(id){
    for(let chapterIndex=0;chapterIndex<STATE.chapters.length;chapterIndex++){
        const chunkIndex = STATE.chapters[chapterIndex].chunks.findIndex(chunk=>chunk.id===id);
        if(chunkIndex !== -1){
            return {
                chunk: STATE.chapters[chapterIndex].chunks[chunkIndex],
                chapterIndex,
                chunkIndex
            };
        }
...
function buildChapterPlaybackQueue(chapterIndex){
    const chapter = STATE.chapters[chapterIndex];
    if(!chapter) return [];
    return chapter.chunks.map((chunk,chunkIndex)=>({chunk,chapterIndex,chunkIndex}));
}

function buildFullPlaybackQueue(){
    return STATE.chapters.flatMap((chapter,chapterIndex)=>chapter.chunks.map((chunk,chunkIndex)=>({chunk,chapterIndex,chunkIndex})));
}
...
async function playSequence(items,{mode,requestedChapterIndex=null,trackProgress=false,scrollChunks=false}={}){
    if(!items.length)return;
    const first = items[0];
    const sessionId = beginPlaybackSession(mode,{
        requestedChapterIndex,
        activeChapterIndex: first.chapterIndex,
        activeChunkId: null,
        resetProgress: trackProgress
    });

    try{
        for(let i=0;i<items.length;i++){
            if(!isPlaybackSessionCurrent(sessionId,mode)) return;
            const item = items[i];
            applyPlaybackState({
                sessionId,
                mode,
                requestedChapterIndex,
                activeChapterIndex: item.chapterIndex,
                activeChunkId: item.chunk.id
            });
            if(scrollChunks) scrollChunkIntoView(item.chunk.id);
            if(trackProgress && dom.masterProgress) dom.masterProgress.value = (i/items.length)*100;
            await engine.playChunk(item.chunk);
            if(trackProgress && dom.masterProgress) dom.masterProgress.value = ((i+1)/items.length)*100;
        }
    }catch(e){
        if(isPlaybackSessionCurrent(sessionId,mode)) LOG.add(e.message,'error');
    }finally{
        if(!isPlaybackSessionCurrent(sessionId,mode)) return;
        if(trackProgress && dom.masterProgress) dom.masterProgress.value = 100;
        stopPlayback({invalidateSession:false,clearProgress:!trackProgress});
    }
}
...
async function playSingleChunk(id){
    ...
    if(isChunkControlActive(id)){stopPlayback();return}
    const found = findChunkById(id);
    if(!found)return;
    const {chunk,chapterIndex} = found;
    const sessionId = beginPlaybackSession('chunk',{
    ...
    try{
        ...
        if(isPlaybackSessionCurrent(sessionId,'chunk')) LOG.add(e.message,'error');
    }finally{
        if(isPlaybackSessionCurrent(sessionId,'chunk')) stopPlayback({invalidateSession:false});
    }
}
...
async function playChapter(idx,e){
    ...
    if(isChapterControlActive(idx)){stopPlayback();return}
    await playSequence(buildChapterPlaybackQueue(idx),{
        mode: 'chapter',
...
function resolveChunkVoiceId(chunk){
    if(chunk.voiceId) return chunk.voiceId;
    if(typeof chunk.voiceIndex === 'number' && STATE.project.voiceIds[chunk.voiceIndex]) return STATE.project.voiceIds[chunk.voiceIndex];
    return STATE.project.voiceIds[0] || null;
}

function isServerCacheableVoiceId(voiceId){
    if(!voiceId) return false;
    if(String(voiceId).startsWith('sys_')) return false;
    const voice = STATE.voices.find(v => v.id === voiceId);
    if(voice) return voice.type === 'premium';
    return true;
}

async function checkProjectCache(){
    const mid=dom.elModel.value,all=[],flat=[],cacheable=[];
    STATE.chapters.forEach((ch,i)=>ch.chunks.forEach((ck,j)=>{
        const voiceId = resolveChunkVoiceId(ck);
        const entry = {id:ck.id,text:ck.text,voiceId,chapterIndex:i,chunkIndex:j};
        all.push(entry);
        flat.push(ck);
        if(isServerCacheableVoiceId(voiceId)) cacheable.push(entry);
    }));
    try{
        const res = cacheable.length ? await APIService.checkCache(STATE.project.id,mid,cacheable) : {chunks:[]};
        const resultById = new Map(res.chunks.map(r => [r.id,r]));
        let cached=0,miss=0,pending=0,stale=0;
        const liveSkipped = all.length - cacheable.length;

        all.forEach(c => {
            const chunk = STATE.chapters[c.chapterIndex].chunks[c.chunkIndex];
            if(!isServerCacheableVoiceId(c.voiceId)){
                if(chunk.status !== 'done') pending++;
                return;
            }
            ...
            if(r && r.exists){
                cached++;
                chunk.audioUrl = r.url;
                chunk.filename = r.filename;
                chunk.status = 'done';
                return;
            }
            ...
            pending++;
            miss += c.text.length;
            const hadServerAudio = Boolean(
                chunk.filename ||
                (typeof chunk.audioUrl === 'string' && chunk.audioUrl.includes('/output/chunks/')) ||
                chunk.status === 'done'
            );
            if(hadServerAudio) stale++;
        });

        if(stale){
            LOG.add(`Cache check: ${cached} exact matches, ${pending} segments need generation. ${stale} stale audio reference${stale===1?'':'s'} detected.`,'warning');
        }else{
            LOG.add(`Cache check: ${cached} exact matches, ${pending} segments need generation.`,'info');
        }
        if(liveSkipped){
            LOG.add(`Skipped server cache check for ${liveSkipped} browser/system segment${liveSkipped===1?'':'s'}.`,'info');
        }
...
async function generateSingleChunk(id,e){
    ...
    const voiceId = resolveChunkVoiceId(chunk);
    if(isServerCacheableVoiceId(voiceId)){
        try {
            const res=await APIService.checkCache(STATE.project.id,mid,[{id:chunk.id,text:chunk.text,voiceId,chapterIndex:chIdx,chunkIndex:ckIdx}]);
            isCached=Boolean(res.chunks[0] && res.chunks[0].exists);
        } catch (e) {
            LOG.add(`Cache check error: ${e.message||e}`,'error');
            return;
        }
    }
    const needsGeneration = isServerCacheableVoiceId(voiceId) ? (isCached?0:1) : (chunk.status==='done'?0:1);
    const cost=isCached?0:(isServerCacheableVoiceId(voiceId)?chunk.text.length*TextParser.getModelCreditMultiplier(mid)*0.000165:0);
    ...
    document.getElementById('confirm-new').innerText=needsGeneration;
...
async function generateChapter(idx,e){
    ...
    const mid=dom.elModel.value,checks=ch.chunks.map((k,i)=>({id:k.id,text:k.text,voiceId:resolveChunkVoiceId(k),chapterIndex:idx,chunkIndex:i}));
    const cacheable = checks.filter(c => isServerCacheableVoiceId(c.voiceId));
    let results = [];
    if(cacheable.length){
        try {
            const res=await APIService.checkCache(STATE.project.id,mid,cacheable);
            results=res.chunks;
        } catch (e) {
            LOG.add(`Cache check error: ${e.message||e}`,'error');
            return;
    ...
    let miss=0,total=0,cached=0,pending=0;
    checks.forEach((c,i)=>{
        total+=c.text.length;
        if(!isServerCacheableVoiceId(c.voiceId)){
            if(ch.chunks[i].status !== 'done') pending++;
            return;
        }
        const r = resultById.get(c.id);
        if(r && r.exists) cached++;
        else{
            pending++;
            miss+=c.text.length;
        }
    });
    document.getElementById('safety-message').innerText=`Generate ${ch.title}?`;document.getElementById('confirm-chars').innerText=total;document.getElementById('confirm-new').innerText=pending;document.getElementById('confirm-cached').innerText=cached;document.getElementById('confirm-cost').innerText=`$${(miss*TextParser.getModelCreditMultiplier(mid)*0.000165).toFixed(2)}`;
    openSafetyModal();
...
function renderTimeline() {
    ...
    dom.timeline.innerHTML = STATE.chapters.map((ch, idx) => {
        ...
        const playing = isChapterControlActive(idx);
        const collapsed = ch.collapsed;
        
        const chunksHtml = !collapsed ? ch.chunks.map((ck, cIdx) => {
            const isPlaying = isChunkControlActive(ck.id);
...
dom.btnPlay.addEventListener('click',async()=>{
    ...
    await playSequence(buildFullPlaybackQueue(),{
        mode: 'all',
        trackProgress: true,
        scrollChunks: true
    });
