...
const server = http.createServer(async (req, res) => {
    ...
    if (pathname === '/api/projects' && req.method === 'GET') {
        try {
            ...
            // Filename format: projectId_chX_chkY_hash.mp3
            const chunkFiles = fs.readdirSync(CHUNKS_DIR).filter(f => f.endsWith('.mp3'));
            const ghostIds = new Set();
            
            chunkFiles.forEach(f => {
                // Heuristic: Extract ID (everything before _ch\d)
            ...
            // Filter out IDs that already exist in saved projects
            const existingIds = new Set(projectList.map(p => p.id));
            
            ghostIds.forEach(gid => {
                if (!existingIds.has(gid)) {
                    // Infer title from ID (usually safeTitle_hash)
                    // e.g. "mybook_123abc" -> "mybook"
                    ...
                    projectList.push({
                        ...
                        author: 'Unknown',
                        updatedAt: new Date().toISOString(), // No real date, use now or file time
                        type: 'ghost'
                    });
                }
            });
    ...
    if (pathname.startsWith('/api/projects/') && req.method === 'GET') {
        ...
        // Attempt Reconstruction from Chunks (Ghost Project Recovery)
        try {
