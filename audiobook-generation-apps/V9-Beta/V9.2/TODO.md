...
### 🎨 UI/UX Enhancements
- [ ] **Rich Text Editor:** Replace `<textarea>` with a lightweight rich text editor (e.g., Quill, TipTap) to support:
    - [ ] Bold/Italic for emphasis (mapped to ElevenLabs stability settings?).
    - [ ] Visual delimiters for chapters/segments instead of raw text splitting.
- [ ] **"Karaoke" Playback:** Highlight the active sentence/segment in the editor while the audio plays.
- [ ] **Waveform Visualization:** Replace the simple progress bar with a generated waveform of the audio for better seeking.
- [ ] **Project Management UI:**
    - [ ] A "Load Project" dashboard to switch between books without overwriting `localStorage`.
    - [ ] "Version History" for chunks (Undo/Redo generation).

### 🛠️ Feature Requests
- [ ] **Advanced Audio Controls:**
    - [ ] **Pause Insertion:** Ability to add `[pause: 2s]` tags in the text that the engine respects (either via API or by generating silence chunks).
    - [ ] **Pronunciation Dictionary:** A "Find & Replace" map for phonemes (e.g., "Siobhan" -> "Shiv-awn").
- [ ] **Export & Mastering:**
    ...
    - [ ] **Loudness Normalization:** Use `ffmpeg-normalize` on the backend to ensure consistent volume (-16 or -23 LUFS) across chapters.
- [ ] **Voice "Casting" Board:** A visual drag-and-drop interface to assign voices to characters detected in the text.

### ⚙️ Backend & Architecture
- [ ] **Secure API Proxy:** Move the ElevenLabs API calls entirely to the server-side. The client should never see or store the API Key (store it in a `.env` file or secure server-side storage).
- [ ] **Job Queue System:** If generating a whole book, implement a queue (e.g., `bull` or simple in-memory queue) to handle rate limits gracefully and allow background processing.
- [ ] **Unit Tests:**
    - [ ] Test text splitting logic (edge cases, quotes, dialogue).
...
*   **Phase 1 (Cleanup):** Layout fixes, text splitter improvements, accessibility.
*   **Phase 2 (Core):** Rich text editor, Project Dashboard, Pronunciation Dictionary.
*   **Phase 3 (Pro):** ID3/M4B Export, Loudness Normalization, Server-side Queues.