...
<style>
*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);min-height:100vh;padding:20px}.container{max-width:1400px;margin:0 auto;background:#fff;border-radius:12px;box-shadow:0 20px 60px rgba(0,0,0,.3);overflow:hidden}header{background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:#fff;padding:30px;text-align:center}header h1{font-size:2.5em;margin-bottom:10px}header p{opacity:.9;font-size:1.1em}.content{padding:30px}.tabs{display:flex;gap:10px;border-bottom:2px solid #e0e0e0;margin-bottom:30px}.tab{padding:15px 30px;background:none;border:none;font-size:1.1em;cursor:pointer;color:#666;border-bottom:3px solid transparent;transition:all .3s}.tab:hover{color:#667eea}.tab.active{color:#667eea;border-bottom-color:#667eea}.tab-content{display:none}.tab-content.active{display:block}.form-group{margin-bottom:20px}label{display:block;margin-bottom:8px;font-weight:600;color:#333}.sub-label{margin-top:10px;margin-bottom:6px;font-weight:500;color:#555}.checkbox-label{display:flex;align-items:center;gap:10px;font-weight:500}.checkbox-label input[type="checkbox"]{width:auto;margin:0}input[type="text"],input[type="number"],textarea,select{width:100%;padding:12px;border:2px solid #e0e0e0;border-radius:6px;font-size:1em;font-family:inherit;transition:border-color .3s}input:focus,textarea:focus,select:focus{outline:none;border-color:#667eea}textarea{min-height:200px;resize:vertical}.form-row{display:grid;grid-template-columns:1fr 1fr;gap:20px}button{background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:#fff;border:none;padding:14px 28px;font-size:1em;border-radius:6px;cursor:pointer;transition:transform .2s,box-shadow .2s;font-weight:600}button:hover{transform:translateY(-2px);box-shadow:0 5px 15px rgba(102,126,234,.4)}button:active{transform:translateY(0)}button:disabled{opacity:.5;cursor:not-allowed;transform:none}.btn-secondary{background:#6c757d}.btn-success{background:#28a745}.btn-danger{background:#dc3545}.btn-small{padding:8px 16px;font-size:.9em}.projects-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:20px;margin-top:20px}.btn-delete-project{position:absolute;top:15px;right:15px;background:#dc3545;color:#fff;border:none;border-radius:50%;width:30px;height:30px;font-size:1.2em;line-height:1;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background .3s;z-index:10}.btn-delete-project:hover{background:#c82333}.project-card{background:#f8f9fa;border-radius:8px;padding:20px;cursor:pointer;transition:transform .2s,box-shadow .2s;position:relative}.project-card:hover{transform:translateY(-5px);box-shadow:0 10px 30px rgba(0,0,0,.1)}.project-card h3{margin-bottom:10px;color:#333}.project-card p{color:#666;font-size:.9em}.status-badge{display:inline-block;padding:4px 12px;border-radius:20px;font-size:.8em;font-weight:600;text-transform:uppercase}.status-pending{background:#ffc107;color:#000}.status-processing{background:#17a2b8;color:#fff}.status-completed{background:#28a745;color:#fff}.status-ready_for_merge{background:#6f42c1;color:#fff}.status-paused{background:#fd7e14;color:#fff}.status-error{background:#dc3545;color:#fff}.status-skipped{background:#6c757d;color:#fff}.voice-config{background:#f8f9fa;padding:20px;border-radius:8px;margin-bottom:20px}.voice-config h3{margin-bottom:15px}.hidden{display:none!important}.loading{text-align:center;padding:40px}.spinner{width:40px;height:40px;border:4px solid #e0e0e0;border-top-color:#667eea;border-radius:50%;animation:spin 1s linear infinite;margin:0 auto 15px}@keyframes spin{to{transform:rotate(360deg)}}.chapter-list{margin-top:20px}.chapter-item{background:#f8f9fa;padding:15px;border-radius:8px;margin-bottom:10px;border-left:4px solid #667eea}.chapter-title{font-weight:600;margin-bottom:5px}.chapter-meta{font-size:.85em;color:#666}.controls{display:flex;gap:10px;margin-top:15px;flex-wrap:wrap;align-items:center}.progress-bar{width:100%;height:8px;background:#e0e0e0;border-radius:4px;overflow:hidden;margin-top:10px}.progress-fill{height:100%;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);transition:width .3s}.segment-table{width:100%;border-collapse:collapse;margin-top:15px}.segment-table th,.segment-table td{padding:10px;text-align:left;border-bottom:1px solid #e0e0e0}.segment-table th{background:#f8f9fa;font-weight:600}.activity-log{background:#f8f9fa;border-radius:8px;padding:15px;margin-top:20px;max-height:300px;overflow-y:auto}.log-entry{padding:8px 0;border-bottom:1px solid #e0e0e0;font-size:.9em}.log-time{color:#666;font-size:.8em}.help-text{font-size:.85em;color:#666;margin-top:5px}.section-heading{font-size:1.1em;font-weight:600;margin-top:25px;margin-bottom:10px;color:#333}.section-subtext{font-size:.9em;color:#666;margin-bottom:15px}.voice-name-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:15px}.title-intro-preview{margin-top:15px}.title-intro-preview audio{width:100%;margin-top:10px}.slider-group{margin-bottom:15px}.slider-group input[type="range"]{width:100%;margin-top:5px}.info-icon{display:inline-flex;align-items:center;justify-content:center;width:16px;height:16px;border-radius:50%;background:#667eea;color:#fff;font-size:10px;font-weight:700;cursor:help;margin-left:5px;position:relative}.info-icon::after{content:attr(data-tooltip);position:absolute;bottom:calc(100% + 8px);left:50%;transform:translateX(-50%) translateY(4px);background:rgba(33,37,41,.95);color:#fff;padding:8px 12px;border-radius:6px;font-size:12px;font-weight:400;min-width:200px;max-width:280px;text-align:left;opacity:0;pointer-events:none;transition:opacity .15s,transform .15s;z-index:10;white-space:normal}.info-icon::before{content:'';position:absolute;bottom:110%;left:50%;transform:translateX(-50%);border-width:6px;border-style:solid;border-color:rgba(33,37,41,.95) transparent transparent transparent;opacity:0;transition:opacity .15s;pointer-events:none}.info-icon:hover::after,.info-icon:focus::after{opacity:1;transform:translate(-50%,-4px)}.info-icon:hover::before,.info-icon:focus::before{opacity:1}.slider-group label.slider-label{display:flex;align-items:center;gap:10px;font-weight:600;color:#333}.slider-group label .slider-label-text{flex:1}.slider-group label .slider-value{font-weight:600;color:#667eea}
body.dark-mode{background:linear-gradient(135deg,#1a1a2e 0%,#16213e 100%);color:#e0e0e0}body.dark-mode .container,body.dark-mode .project-card,body.dark-mode #project-modal>div,body.dark-mode #delete-modal>div,body.dark-mode .voice-config,body.dark-mode .activity-log{background:#1f2937;color:#e0e0e0;border-color:#374151}body.dark-mode h1,body.dark-mode h2,body.dark-mode h3,body.dark-mode .section-heading,body.dark-mode label,body.dark-mode .chapter-title,body.dark-mode .project-card h3,body.dark-mode .slider-group label.slider-label{color:#f3f4f6}body.dark-mode .sub-label,body.dark-mode .section-subtext,body.dark-mode .chapter-meta,body.dark-mode .help-text,body.dark-mode .project-card p,body.dark-mode .log-time{color:#9ca3af}body.dark-mode input,body.dark-mode textarea,body.dark-mode select{background:#374151;border-color:#4b5563;color:#fff}body.dark-mode .tab{color:#9ca3af}body.dark-mode .tab.active{color:#818cf8;border-bottom-color:#818cf8}body.dark-mode .segment-table th{background:#374151;color:#e0e0e0}body.dark-mode .segment-table td,body.dark-mode .segment-table th{border-bottom-color:#4b5563}body.dark-mode .chapter-item{border-color:#4b5563}body.dark-mode .log-entry{border-bottom-color:#4b5563}body.dark-mode #chapter-settings-modal>div{background:#1f2937;color:#e0e0e0}
</style>
</head>
<body>
<div class="container">
...
modal.classList.remove('hidden');setupEventSource(pid)}catch(e){alert('Error loading project: '+e.message)}}
...
async function processChapter(pid,idx){try{const r=await fetch('/api/projects/'+pid+'/process-chapter',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({index:idx})});if(!r.ok){const e=await r.json();throw new Error(e.error)}}catch(e){alert('Error: '+e.message)}}
async function startProject(pid,limit=null){try{const r=await fetch('/api/projects/'+pid+'/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({limit})});if(!r.ok){const e=await r.json();throw new Error(e.error)}alert(limit?'Started '+limit+' chapter(s).':'Started all pending chapters.');loadProjects();openProject(pid)}catch(e){alert('Error: '+e.message)}}
async function pauseProject(pid){try{await fetch('/api/projects/'+pid+'/pause',{method:'POST'});alert('Paused');loadProjects();openProject(pid)}catch(e){alert('Error: '+e.message)}}
async function resumeProject(pid,limit=null){try{const r=await fetch('/api/projects/'+pid+'/resume',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({limit})});if(!r.ok){const e=await r.json();throw new Error(e.error)}alert(limit?'Resumed with batch '+limit+'.':'Resumed all.');loadProjects();openProject(pid)}catch(e){alert('Error: '+e.message)}}
async function finalizeProject(pid){try{const r=await fetch('/api/projects/'+pid+'/finalize',{method:'POST'});const result=await r.json();if(!r.ok)throw new Error(result.error);alert('Audiobook merged!');loadProjects();openProject(pid)}catch(e){alert('Error: '+e.message)}}
async function appendManuscript(pid){const ms=document.getElementById('additional-manuscript'),tk=document.getElementById('additional-voice-token');if(!ms||!ms.value.trim()){alert('Please paste manuscript text.');return}try{const r=await fetch('/api/projects/'+pid+'/chapters',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({manuscript:ms.value,voiceSwitchToken:tk&&tk.value?tk.value:undefined})});const result=await r.json();if(!r.ok)throw new Error(result.error);alert('Added '+result.added+' chapter(s).');ms.value='';loadProjects();openProject(pid)}catch(e){alert('Error: '+e.message)}}
async function saveVoiceNames(pid){const inputs=Array.from(document.querySelectorAll('input[data-voice-name-index]'));if(!inputs.length){alert('No voice fields.');return}const voiceNames=[];inputs.forEach(inp=>{const idx=parseInt(inp.dataset.voiceNameIndex,10);if(Number.isFinite(idx))voiceNames[idx]=inp.value.trim()});try{const r=await fetch('/api/projects/'+pid+'/voices',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({voiceNames})});const result=await r.json();if(!r.ok)throw new Error(result.error);alert('Voice names updated.');loadProjects();openProject(pid)}catch(e){alert('Error: '+e.message)}}
async function saveFinalizeSettings(pid){const inp=document.getElementById('pause-between-chapters-setting');if(!inp){alert('Setting not found.');return}const v=parseFloat(inp.value);if(Number.isNaN(v)||v<0){alert('Must be zero or positive.');return}try{const r=await fetch('/api/projects/'+pid+'/merge-settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pauseBetweenChaptersSeconds:v})});const result=await r.json();if(!r.ok)throw new Error(result.error);alert('Settings saved.');loadProjects();openProject(pid)}catch(e){alert('Error: '+e.message)}}
async function generateTitleIntro(pid){const ta=document.getElementById('project-title-intro-text');if(!ta||!ta.value.trim()){alert('Enter title intro script.');return}const vs=document.getElementById('project-title-intro-voice');let voiceIdx=0;if(vs){const p=parseInt(vs.value,10);if(Number.isFinite(p)&&p>=0)voiceIdx=p}try{const r=await fetch('/api/projects/'+pid+'/title-intro',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:ta.value,voiceIndex:voiceIdx})});const result=await r.json();if(!r.ok)throw new Error(result.error);alert('Title intro updated.');loadProjects();openProject(pid)}catch(e){alert('Error: '+e.message)}}
async function saveProjectSettings(pid){const data={settings:{stability:parseFloat(document.getElementById('project-stability').value),similarity_boost:parseFloat(document.getElementById('project-similarity').value),style:parseFloat(document.getElementById('project-style').value),speed:parseFloat(document.getElementById('project-speed').value)},modelVersion:document.getElementById('project-model').value,demoMode:document.getElementById('project-demo-mode').checked};try{const r=await fetch('/api/projects/'+pid+'/settings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});const result=await r.json();if(!r.ok)throw new Error(result.error);alert('Voice settings saved. Changes apply to future processing.');loadProjects();openProject(pid)}catch(e){alert('Error: '+e.message)}}
async function resetChapter(pid,idx){if(!confirm('Reset this chapter? This will delete the generated audio and allow re-processing with current settings.'))return;try{const r=await fetch('/api/projects/'+pid+'/chapters/'+idx+'/reset',{method:'POST'});const result=await r.json();if(!r.ok)throw new Error(result.error);alert('Chapter reset. You can now process it with new settings.');loadProjects();openProject(pid)}catch(e){alert('Error: '+e.message)}}
