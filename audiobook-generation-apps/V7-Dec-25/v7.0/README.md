...
*   **Web Interface:** Intuitive dashboard to manage projects, edit manuscripts, and monitor progress.
*   **ElevenLabs Integration:** Full support for ElevenLabs' top-tier AI models (Multilingual v2, Turbo v2.5, Flash v2, etc.) and voice cloning.
*   **Smart Audio Processing:** Automatically splits text, generates audio chunks, merges them into chapters, and applies professional loudness normalization using `ffmpeg`.
*   **Dual Voice Mode:** innovative support for dialogue-heavy books, allowing you to switch between narrators or characters automatically using delimiters (e.g., `***`).
...
*   **Manuscript:** Paste your book's text. The app automatically detects chapters using Markdown headings (e.g., `# Chapter 1`, `## Chapter Two`) or standard "Chapter" keywords.
*   **Voice Selection:**
...
*   The app will break down the text into optimized chunks and send them to ElevenLabs.
*   You can watch the progress bar as chunks are created and merged into chapters.

### 5. Finalize & Download
*   Once all chapters are generated, click **"Finalize Book"**.
