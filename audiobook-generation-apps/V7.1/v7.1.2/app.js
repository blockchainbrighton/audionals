...
function getChapterHeadingRegex() {
  const keywords = [
    'Chapter', 'Kapitel',             // English, German
    'Capítulo', 'Capitulo',           // Spanish, Portuguese
    'Chapitre',                       // French
    'Capitolo',                       // Italian
    'Hoofdstuk',                      // Dutch
    'Rozdział',                       // Polish
    'Глава',                          // Russian
    'Bölüm',                          // Turkish
    'Part', 'Parte', 'Partie', 'Teil', // Generic 'Part'
    'Livre', 'Libro', 'Buch',         // Generic 'Book'
    'Luku',                           // Finnish
    'Fejezet',                        // Hungarian
    'Kapitola',                       // Czech
    'Κεφάλαιο',                       // Greek
...
${MODEL_SELECT_OPTIONS_HTML}
        ...
        <div class="form-group">
          ...
          <div style="margin-top: 10px; display: flex; align-items: center; gap: 15px;">
            <button type="button" class="btn-secondary btn-small" onclick="estimateCost()">Calculate Usage Estimate</button>
            <span id="cost-estimate-display" style="font-weight: 600; color: #667eea;"></span>
          </div>
        </div>
  ...
  <script>
    ...
    async function estimateCost() {
      ...
      if (!text) {
        alert('Please enter manuscript text first.');
        return;
      }
      ...
      try {
        const response = await fetch('/api/estimate', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ text, modelVersion })
        });
        ...
        const formattedCost = result.estimatedCost.toLocaleString('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 4 });
        
        display.innerHTML = \`
          <span title="\${result.charCount.toLocaleString()} chars">Char: \${result.charCount.toLocaleString()}</span> &nbsp;|&nbsp; 
          <span title="\${result.multiplier}x multiplier">Credits: \${result.creditCount.toLocaleString()}</span> &nbsp;|&nbsp; 
          <span title="Based on Scale plan ($330/2M credits)">Est. Cost: \${formattedCost}</span>
        \`;
      } catch (error) {
    ...
    document.getElementById('new-book-form').addEventListener('submit', async (e) => {
      ...
      const data = {
        ...
        // titleIntroText extracted automatically from manuscript preamble
      ...
      const pauseValue = pauseInput && pauseInput.value !== ''
        ? parseFloat(pauseInput.value)
        : 2;
      if (!Number.isNaN(pauseValue)) {
        data.pauseBetweenChaptersSeconds = pauseValue;
      }
      ...
      try {
        ...
        // Save preferences
        localStorage.setItem('preferredModel', data.modelVersion);
        localStorage.setItem('preferredSpeed', data.settings.speed);
