// scriptLoader_NB_coreScripts.js

window.coreScriptsToLoad = [
...
window.loadNextScripts = function(scriptList, callback) {
    if (scriptList.length === 0) {
        callback();
        return;
    }
    const scriptSrc = scriptList.shift();
    const script = document.createElement('script');
    script.src = scriptSrc;
    script.onload = () => loadNextScripts(scriptList, callback);
    document.head.appendChild(script);
};

// Load the core scripts and then load title and color palette scripts
loadNextScripts(window.coreScriptsToLoad, () => {
    const titleColorPaletteLoader = document.createElement('script');
    titleColorPaletteLoader.src = 'scriptLoader_NB_titleColorPaletteScripts.js';
    document.head.appendChild(titleColorPaletteLoader);
});
