    ...
    async handleMessage(event) {
        if (event.data.type === 'INIT_WASM') {
            ...
            try {
                ...
                // IMPORTANT: All AudioWorkletProcessor instances share the same module scope.
                // wasm-bindgen loaders keep module-level state (`let wasm;`), so re-initializing would
                // invalidate previously-created BvstSynth instances. Initialize once globally.
                ...
                const bytes = event.data.wasmBytes;
                const sig = (() => {
                    try {
                ...
                if (!state.initPromise) {
                    state.signature = sig;
                    state.initPromise = Promise.resolve(wasmInit(bytes)).then(() => {
                        state.ready = true;
                    });
                }
