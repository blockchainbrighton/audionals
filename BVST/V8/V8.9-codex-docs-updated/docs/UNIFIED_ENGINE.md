...
1. `System/host.html` loads the selected plugin UI inside an iframe (`Plugins/.../gui.html`).
...
4. `System/host.html` receives those and forwards them to the AudioWorklet via `workletNode.port.postMessage(...)`.
5. `System/shared/processor_unified.js` runs the audio thread, holds the `BvstSynth` instance, and calls:
   - `set_param(id, value)`
