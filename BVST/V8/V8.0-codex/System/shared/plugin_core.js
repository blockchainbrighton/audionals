...
export const BVST = {
        ...
        this.instanceId = instanceId;
	        ...
	        const debugMidiParam = (getQueryParam('debugMidi') || '').toString().trim().toLowerCase();
	        const debugMidi = debugMidiParam === '1' || debugMidiParam === 'true' || debugMidiParam === 'yes';
	        const hostMidi =
	            (config && (config.hostMidi === true || config.hostMidi === 1)) ||
	            hostMidiParam === '1' ||
	            hostMidiParam === 'true' ||
	            hostMidiParam === 'yes';
	        this.hostMidi = hostMidi;
	        this._hostMidiStatus = { connected: false, name: '', hasDevices: false };
        ...
        if (config.keyboard !== false) {
            ...
            if (config.keyboard && config.keyboard.onNoteOff) {
	            ...
	            if (!hostMidi) {
	                ...
	                const applyHostMidiStatus = (st) => {
	                    if (!this.hostMidi) return;
	                    const connected = !!(st && st.connected);
	                    const name = st && typeof st.name === 'string' ? st.name : '';
	                    const hasDevices = !!(st && st.hasDevices);
	                    this._hostMidiStatus = { connected, name, hasDevices };
	                    ...
	                    if (midiSel) {
	                        ...
	                        o.innerText = connected
	                            ? `Host: ${name || 'device'}`
	                            : (hasDevices ? 'Host: select MIDI In (rack)' : 'Host: no device');
	                        midiSel.appendChild(o);
	                        midiSel.disabled = true;
	                    }
	                    if (midiLed) {
	                        midiLed.style.display = 'inline-block';
	                        midiLed.style.background = connected ? '#0f0' : '#333';
	                        midiLed.style.boxShadow = 'none';
	                    }
	                };
	                if (midiSel) {
	                    // Start pessimistic; host will send BVST_HOST_MIDI_STATUS after load.
	                    applyHostMidiStatus({ connected: false, name: '', hasDevices: false });
	                }
	            }
	        } else {
        ...
        if (!this._listenerAdded) {
	            window.addEventListener('message', (event) => {
	                if (event.data && event.data.type === 'BVST_HOST_MIDI_STATUS') {
	                    // Host-level MIDI status broadcast (runtime only).
	                    if (this.hostMidi) {
	                        ...
	                        const manufacturer = typeof data.manufacturer === 'string' ? data.manufacturer : '';
	                        const connection = typeof data.connection === 'string' ? data.connection : '';
	                        const hasDevices = !!data.hasDevices;
	                        if (debugMidi) {
	                            console.log('BVST_MIDI_PLUGIN', { selected, connected, verified, active, manufacturer, name, connection, hasDevices, instanceId: this.instanceId });
	                        }
	                        if (midiSel) {
	                            midiSel.replaceChildren();
	                            const o = document.createElement('option');
	                            o.value = '';
	                            const label = `${manufacturer ? `${manufacturer} ` : ''}${name || 'device'}`.trim();
	                            o.innerText = connected
	                                ? (active ? `Host: ${label} (active)` : (verified ? `Host: ${label} (ready)` : `Host: ${label} (waiting)`))
	                                : (hasDevices ? (selected ? (connection === 'open' ? 'Host: selected device disconnected' : 'Host: selected device not open') : 'Host: select MIDI In (rack)') : 'Host: no devices');
	                            midiSel.appendChild(o);
	                            midiSel.disabled = true;
	                        }
	                        if (midiLed) {
	                            midiLed.style.display = 'inline-block';
	                            midiLed.style.background = active ? '#00f0ff' : (verified ? '#0f0' : '#333');
	                            midiLed.style.boxShadow = 'none';
	                        }
	                    }
	                }
