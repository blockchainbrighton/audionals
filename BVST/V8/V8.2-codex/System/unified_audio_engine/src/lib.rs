...
use bvst_lib::{Param};
use bvst_lib::dsp::{
    Oscillator, WaveType, Svf, Adsr, Delay, Drive, NoiseGen, mtof, clamp,
};
...
impl Engine {
    ...
    fn note_on(&mut self, note: f32, velocity: f32) {
        if let Engine::UniversalSynth(e) = self {
...
impl BvstSynth {
    ...
    pub fn new(sample_rate: f32, plugin_id: String) -> Self {
        ...
        let engine = match plugin_norm.as_str() {
            "universalsynth" | "mother" | "mothersynth" | "synth" => Engine::UniversalSynth(UniversalSynth::new(sample_rate)),
            "universalfx" | "fx" | "effect" => Engine::UniversalFX(UniversalFX::new(sample_rate)),
            "universalmod" | "mod" | "modulation" => Engine::UniversalMod(UniversalMod::new(sample_rate)),
            "universaldynamics" | "dynamics" => Engine::UniversalDynamics(UniversalDynamics::new(sample_rate)),
            "primarysourceenhancer" | "pse" => Engine::PrimarySourceEnhancer(PrimarySourceEnhancer::new(sample_rate)),
            "universalutility" | "utility" => Engine::UniversalUtility(UniversalUtility::new(sample_rate)),
    ...
    pub fn load_sample(&mut self, _data: &[f32]) {}
