...
export class Controls {
    ...
    attachListeners() {
        ...
        document.querySelectorAll(this.knobSelector).forEach(el => {
            if (el.dataset.bvstBound === '1') return;
            el.dataset.bvstBound = '1';
            const handler = (e) => this.handleDown(e, el, 'knob');
            el.addEventListener('mousedown', handler);
            el.addEventListener('touchstart', handler, { passive: false });
            el.addEventListener('dblclick', (e) => this.handleDblClick(e, el));
            el.addEventListener('wheel', (e) => this.handleWheel(e, el), { passive: false });
        });
    ...
    handleMove(e) {
        ...
        if (type === 'knob') {
            ...
            if (isLog) {
                ...
                let norm = normStart + ((deltaY / 200) * speedFactor);
                norm = Math.max(0, Math.min(1, norm));
                newVal = this.normToVal(norm, min, max);
            } else {
                const change = ((deltaY / 200) * range) * speedFactor;
            ...
            if (el.classList.contains('horizontal')) {
                ...
                const pixelRange = rect.width || 150; // Fallback
                change = ((deltaX / pixelRange) * range) * speedFactor;
            } else {
                const deltaY = this.startPos.y - clientY; // Up is positive
                const pixelRange = rect.height || 150;
                change = ((deltaY / pixelRange) * range) * speedFactor;
            }
            newVal = this.startVal + change;
            newVal = Math.max(min, Math.min(max, newVal));
        }
