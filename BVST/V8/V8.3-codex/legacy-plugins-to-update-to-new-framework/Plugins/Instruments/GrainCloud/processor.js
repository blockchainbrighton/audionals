import init, { BvstSynth } from "../../../System/shared/wasm_loader_sampler.js";

// --- BVST AudioWorkletProcessor ---
// Optimized to use shared WASM loader for Samplers
...
class BvstProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        console.log("BVST: Processor constructed (Shared Sampler Loader).");
    }

    async handleMessage(event) {
        if (event.data.type === 'INIT_WASM') {
            console.log("BVST: Initializing WASM via Shared Sampler Loader...");
            try {
    ...
    process(inputs, outputs, parameters) {
        ...
        if (this.synth) {
            try {
                // Samplers use 4-arg process in the shared loader
