...
const BVST_BUILD = 'presets-autoload-v5';
...
export const BVST = {
    init: function(config) {
        ...
        console.log(`[BVST] init '${config.name || 'Unnamed'}' (${BVST_BUILD})`, { instanceId });
        ...
        const controls = new Controls({
            onChange: (id, val) => {
                ...
                if (id === 'btn-power' && val === 1) {
                    window.parent.postMessage({ type: 'BVST_POWER', instanceId: this.instanceId }, '*');
        ...
        if (config.presets && uiRefs.presetSelector) {
            ...
            const applyPreset = (presetName) => {
                console.log('[BVST] applyPreset', { plugin: config.name, presetName });
                const p = config.presets[presetName];
                if (!p) {
                    console.warn('[BVST] preset not found', { plugin: config.name, presetName, available: presetNames });
                    return;
                }
            ...
            if (presetNames.length > 0) {
                ...
                console.log('[BVST] presets detected', { plugin: config.name, desired, presetNames });
                ...
                setTimeout(() => {
                    ...
                    console.log('[BVST] preset selector state', {
                        plugin: config.name,
                        value: selector.value,
                        selectedIndex: selector.selectedIndex,
                        selectedText: selector.options[selector.selectedIndex] ? selector.options[selector.selectedIndex].text : null
                    });
                }, 0);
            } else {
                const o = document.createElement('option');
        ...
        if (config.keyboard !== false) {
            ...
            if (!hostMidi) {
                ...
                console.log('[BVST] hostMidi enabled; skipping WebMIDI in plugin UI', { plugin: config.name, instanceId: this.instanceId });
            }
        } else {
    ...
    _initSampler: function(samplerConfig) {
        this.sampler = new SamplerUI({
            ...
            onSampleLoad: (data) => {
                window.parent.postMessage({ type: 'BVST_LOAD_SAMPLE_FROM_GUI', instanceId: this.instanceId, samples: data }, '*');
