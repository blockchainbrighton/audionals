...
5.  If you need new DSP/models, extend `System/unified_audio_engine/src/lib.rs` (single shared WASM).

### 3. Building
Run the build script with your plugin name. It bumps the manifest version and creates a distribution folder (audio engine + worklet are shared).

```bash
python3 System/scripts/build.py MyNewSynth
# If you changed the unified Rust engine and/or shared worklet:
python3 System/scripts/build.py MyNewSynth --rebuild-wasm --rebuild-processor
```

**Output:** `Plugins/.../MyNewSynth/dist/vX.Y.Z/` contains the deployable files (`gui.html`, optional `patch.json`).
...
3.  **Unified Worklet:** All plugins share `System/shared/processor_unified.js` and `System/shared/bvst_unified_bg.wasm` (single copy, inscribed once).
