...
// 1: Loop Start (0-1)
// 2: Loop End (0-1)
// 3: Loop Enable (Switch)
// 10: Filter Cutoff
// 11: Filter Res
// 12: Filter Mode (LP/HP/BP)
// 20: Amp Attack
// 21: Amp Release
// 22: Drive
// 23: Delay Send
...
struct Voice {
    ...
    env: Adsr,
    sample_rate: f32,
    seek_pos: f32, // Normalized 0.0 - 1.0. -1.0 means inactive.
}
...
impl Voice {
    ...
    fn trigger(&mut self, note: u32, vel: f32) {
        ...
        self.note = note;
        self.velocity = vel;
        self.sampler.trigger(); // Reset phase to 0
        
        // Apply Seek Position if set
        if self.seek_pos >= 0.0 {
            self.sampler.seek(self.seek_pos);
            self.seek_pos = -1.0; // Reset
        }
    ...
    fn process_block(&mut self, params: &EngineParams, output: &mut [f32]) {
        ...
        let env_val = self.env.next(); // Control Rate Envelope (simplified)
        ...
        // MIDI Note 60 (C4) = 1.0 speed.
        let note_ratio = 2.0_f32.powf((self.note as f32 - 60.0) / 12.0);
        let total_speed = params.speed * note_ratio;
        let freq = 261.63 * total_speed; // Sampler uses base_freq logic

        // Filter Coeffs (Control Rate)
        ...
        if total_len > 0 {
            ...
            if end <= self.sampler.loop_start { end = self.sampler.loop_start + 1; } // Safety
        ...
        for out_sample in output.iter_mut() {
            let raw = self.sampler.process(freq);
            ...
            let mut processed = self.filter.process_with_coeffs(raw, &coeffs, mode);
...
pub struct BvstSynth {
    params: EngineParams,
    voice: Voice, // Monophonic sampler for now to keep it simple/safe with large buffers
    delay: Delay,
    ...
    pub fn set_param(&mut self, id: u32, value: f32) {
        match id {
            ...
            30 => {
                ...
                // If voice is already active (race condition where Trigger arrived before Seek),
    ...
    pub fn process(&mut self, _in_l: &[f32], _in_r: &[f32], output_l: &mut [f32], output_r: &mut [f32]) {
        ...
        for chunk_start in (0..len).step_by(BLOCK_SIZE) {
            ...
            for i in 0..chunk_len {
                let dry = sub_out[i];
                let wet = self.delay.process(dry, 0.4, 0.5, self.params.delay_send); // Fixed delay time for now
                let final_val = dry + wet; // wet is actually mix in Delay struct impl? 
                // wait, Delay.process returns mixed signal? 
                // Checking lib.rs: lerp(input, delayed, wet). Yes.
                // But here I want Send behavior.
                // If delay_send is 0, wet returns dry.
                // If delay_send is 1, wet returns delayed.
