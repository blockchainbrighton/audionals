...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def end_headers(self):
        # Development UX: prevent aggressive caching of ES modules/patches during iteration.
    ...
    def _infer_subgroup(self, manifest, plugin_dir, top_category):
        ...
        if plugin_type == "effect":
            lname = name.lower()
            if "reverb" in lname or "verb" in lname:
                return "Reverb"
            if "delay" in lname:
                return "Delay"
            if "chorus" in lname:
                return "Chorus"
            if "flange" in lname or "flanger" in lname:
                return "Flanger"
            if "eq" in lname:
                return "EQ"
            if lname in ("hpf", "lpf") or "filter" in lname:
                return "Filters"
            if "gate" in lname or "expander" in lname:
                return "Gate"
