...
enum Engine {
    ...
    UniversalReverb(UniversalReverb),
...
impl Engine {
    fn set_param(&mut self, id: u32, value: f32) {
        match self {
            ...
            Engine::UniversalReverb(e) => e.set_param(id, value),
            Engine::UniversalFilter(e) => e.set_param(id, value),
            Engine::ParametricEQ(e) => e.set_param(id, value),
            Engine::GraphicEQ(e) => e.set_param(id, value),
    ...
    fn process(&mut self, in_l: &[f32], in_r: &[f32], out_l: &mut [f32], out_r: &mut [f32]) {
        match self {
            ...
            Engine::UniversalReverb(e) => e.process(in_l, in_r, out_l, out_r),
            Engine::UniversalFilter(e) => e.process(in_l, in_r, out_l, out_r),
            Engine::ParametricEQ(e) => e.process(in_l, in_r, out_l, out_r),
            Engine::GraphicEQ(e) => e.process(in_l, in_r, out_l, out_r),
    ...
    fn get_descriptor(&self) -> &'static str {
        match self {
            ...
            Engine::UniversalReverb(_) => UNIVERSAL_REVERB_DESCRIPTOR,
            Engine::UniversalFilter(_) => UNIVERSAL_FILTER_DESCRIPTOR,
            Engine::ParametricEQ(_) => PARAMETRIC_EQ_DESCRIPTOR,
            Engine::GraphicEQ(_) => GRAPHIC_EQ_DESCRIPTOR,
...
impl BvstSynth {
    ...
    pub fn new(sample_rate: f32, plugin_id: String) -> Self {
        ...
        let engine = match plugin_norm.as_str() {
            ...
            "universalfx" | "fx" | "effect" | "auroradelay" | "tapedelay" | "slapbackdelay" | "tapdelay" | "digitaldelay" => Engine::UniversalFX(UniversalFX::new(sample_rate)),
            "universalmod" | "mod" | "modulation" | "orbitalchorus" | "jetflange" | "widechorus" => Engine::UniversalMod(UniversalMod::new(sample_rate)),
            "universaldynamics" | "dynamics" | "gluebuscomp" => Engine::UniversalDynamics(UniversalDynamics::new(sample_rate)),
            "primarysourceenhancer" | "pse" | "cleangate" => Engine::PrimarySourceEnhancer(PrimarySourceEnhancer::new(sample_rate)),
            "universalutility" | "utility" | "gainpanscope" => Engine::UniversalUtility(UniversalUtility::new(sample_rate)),
            "universalreverb" | "reverb" | "roomreverb" | "platereverb" | "hallreverb" => Engine::UniversalReverb(UniversalReverb::new(sample_rate)),
            "universalfilter" | "filter" | "hpf" | "lpf" | "filterhpf" | "filterlpf" => Engine::UniversalFilter(UniversalFilter::new(sample_rate)),
            "parametriceq" | "parametric_eq" | "peq" => Engine::ParametricEQ(ParametricEQ::new(sample_rate)),
            "graphiceq" | "graphiceq5" | "graphic_eq" | "geq" => Engine::GraphicEQ(GraphicEQ::new(sample_rate)),
...
impl DelayLine {
    fn new(sample_rate: f32, max_time_sec: f32) -> Self {
        let size = ((sample_rate * max_time_sec) as usize).max(2);
        Self { buffer: vec![0.0; size], pos: 0, sample_rate }
    }
    ...
    fn process(&mut self, input: f32, time_sec: f32) -> f32 {
        let delay_samples = (time_sec * self.sample_rate)
            .max(1.0)
            .min(self.buffer.len() as f32 - 1.0);
        ...
        while read_idx_i < 0 {
            read_idx_i += self.buffer.len() as i32;
        }
...
impl UniversalReverb {
    fn new(sr: f32) -> Self {
        Self {
            pre_l: DelayLine::new(sr, 0.25),
            pre_r: DelayLine::new(sr, 0.25),
            verb_l: Reverb::new(sr),
            verb_r: Reverb::new(sr),
            damp_l: Svf::new(sr),
            damp_r: Svf::new(sr),

            p_mix: Param::new(0.0, 1.0, 0.25, sr),
            p_decay: Param::new(0.0, 1.0, 0.5, sr),
            p_predelay: Param::new(0.0, 0.25, 0.0, sr),
            p_tone: Param::new(200.0, 20000.0, 12000.0, sr),
            p_width: Param::new(0.0, 2.0, 1.0, sr),
        }
    }
    ...
    fn set_param(&mut self, id: u32, value: f32) {
        match id {
            0 => self.p_mix.set(value),
            1 => self.p_decay.set(value),
            2 => self.p_predelay.set(value),
            3 => self.p_tone.set(value),
            4 => self.p_width.set(value),
            _ => {}
        }
    }
    ...
    fn process(&mut self, in_l: &[f32], in_r: &[f32], out_l: &mut [f32], out_r: &mut [f32]) {
        ...
        let predelay = self.p_predelay.process();
        ...
        for i in 0..len {
            ...
            let pre_l = self.pre_l.process(dry_l, predelay);
            let pre_r = self.pre_r.process(dry_r, predelay);

            // Wet-only from the reverb (mix=1.0), then apply a damping lowpass.
            let wet_l = self.damp_l.process(self.verb_l.process(pre_l, 1.0, decay), tone, 0.7);
            let wet_r = self.damp_r.process(self.verb_r.process(pre_r, 1.0, decay), tone, 0.7);
            ...
            out_l[i] = dry_l * (1.0 - mix) + wl * mix;
            out_r[i] = dry_r * (1.0 - mix) + wr * mix;
        }
    }
}

// ------------------------
// Engine: UniversalFilter
// ------------------------

struct UniversalFilter {
    drive: Drive,
    filter_l: Svf,
    filter_r: Svf,
...
impl UniversalFilter {
    fn new(sr: f32) -> Self {
        Self {
            drive: Drive::new(),
            filter_l: Svf::new(sr),
            filter_r: Svf::new(sr),
            p_mode: Param::new(0.0, 3.0, 0.0, sr),
            p_cutoff: Param::new(20.0, 20000.0, 8000.0, sr),
    ...
    fn set_param(&mut self, id: u32, value: f32) {
        match id {
            0 => self.p_mode.set(value),
            1 => self.p_cutoff.set(value),
            2 => self.p_res.set(value),
            3 => self.p_drive.set(value),
            4 => self.p_mix.set(value),
            _ => {}
        }
    }

    fn process(&mut self, in_l: &[f32], in_r: &[f32], out_l: &mut [f32], out_r: &mut [f32]) {
        let mode = self.p_mode.process().round().clamp(0.0, 3.0) as u32;
        ...
        for i in 0..len {
            ...
            let mut r = dry_r;
            if drive_amt > 0.001 {
                l = self.drive.process(l, drive_amt);
                r = self.drive.process(r, drive_amt);
            }

            let (lp_l, hp_l, bp_l, nt_l) = self.filter_l.process_multimode(l, cutoff, res);
            let (lp_r, hp_r, bp_r, nt_r) = self.filter_r.process_multimode(r, cutoff, res);
            let wet_l = match mode {
                0 => lp_l,
                1 => hp_l,
                2 => bp_l,
                _ => nt_l,
            };
            let wet_r = match mode {
                0 => lp_r,
                1 => hp_r,
                2 => bp_r,
                _ => nt_r,
...
impl Biquad {
    ...
    fn set_peaking(&mut self, sample_rate: f32, freq_hz: f32, q: f32, gain_db: f32) {
        let freq = clamp(freq_hz, 20.0, (sample_rate * 0.45).max(20.0));
        let q = clamp(q, 0.05, 50.0);
...
impl ParametricEQ {
    fn new(sr: f32) -> Self {
        let mut eq_l = Biquad::new();
        let mut eq_r = Biquad::new();
        eq_l.reset();
        eq_r.reset();
        Self {
            eq_l,
            eq_r,
            sample_rate: sr,
            p_freq: Param::new(20.0, 20000.0, 1000.0, sr),
            p_q: Param::new(0.1, 10.0, 0.7, sr),
            p_gain_db: Param::new(-24.0, 24.0, 0.0, sr),
            p_mix: Param::new(0.0, 1.0, 1.0, sr),
            p_out_db: Param::new(-12.0, 12.0, 0.0, sr),
            update_ctr: 0,
            last_freq: 0.0,
            last_q: 0.0,
            last_gain: 0.0,
    ...
    fn set_param(&mut self, id: u32, value: f32) {
        match id {
            ...
            1 => self.p_q.set(value),
            2 => self.p_gain_db.set(value),
            3 => self.p_mix.set(value),
            4 => self.p_out_db.set(value),
            _ => {}
        }
    }
    ...
    fn process(&mut self, in_l: &[f32], in_r: &[f32], out_l: &mut [f32], out_r: &mut [f32]) {
        ...
        for i in 0..len {
            ...
            let q = self.p_q.process();
            let gain_db = self.p_gain_db.process();
            let mix = self.p_mix.process();
            let out_db = self.p_out_db.process();

            if self.update_ctr == 0 {
                if (freq - self.last_freq).abs() > 0.5 || (q - self.last_q).abs() > 0.01 || (gain_db - self.last_gain).abs() > 0.05 {
                    self.eq_l.set_peaking(self.sample_rate, freq, q, gain_db);
                    self.eq_r.set_peaking(self.sample_rate, freq, q, gain_db);
                    self.last_freq = freq;
                    self.last_q = q;
            ...
            let wet_l = self.eq_l.process(dry_l);
            let wet_r = self.eq_r.process(dry_r);

            let out_gain = 10.0_f32.powf(out_db / 20.0);
            out_l[i] = (dry_l * (1.0 - mix) + wet_l * mix) * out_gain;
            out_r[i] = (dry_r * (1.0 - mix) + wet_r * mix) * out_gain;
        }
    }
}

const GRAPHIC_EQ_FREQS: [f32; 5] = [100.0, 300.0, 1000.0, 3000.0, 10000.0];
...
impl GraphicEQ {
    fn new(sr: f32) -> Self {
        Self {
            ...
            p_g1: Param::new(-24.0, 24.0, 0.0, sr),
            p_g2: Param::new(-24.0, 24.0, 0.0, sr),
            p_g3: Param::new(-24.0, 24.0, 0.0, sr),
            p_g4: Param::new(-24.0, 24.0, 0.0, sr),
            p_g5: Param::new(-24.0, 24.0, 0.0, sr),
            p_out_db: Param::new(-12.0, 12.0, 0.0, sr),
    ...
    fn set_param(&mut self, id: u32, value: f32) {
        match id {
            0 => self.p_g1.set(value),
            1 => self.p_g2.set(value),
            2 => self.p_g3.set(value),
            3 => self.p_g4.set(value),
            4 => self.p_g5.set(value),
            5 => self.p_out_db.set(value),
            _ => {}
        }
    }
    ...
    fn process(&mut self, in_l: &[f32], in_r: &[f32], out_l: &mut [f32], out_r: &mut [f32]) {
        ...
        for i in 0..len {
            ...
            if self.update_ctr == 0 {
                for bi in 0..5 {
                    if (g[bi] - self.last_g[bi]).abs() > 0.05 {
                        let f = GRAPHIC_EQ_FREQS[bi];
                        let q = 0.9;
                        self.eq_l[bi].set_peaking(self.sample_rate, f, q, g[bi]);
                        self.eq_r[bi].set_peaking(self.sample_rate, f, q, g[bi]);
                        self.last_g[bi] = g[bi];
                    }
                }
                self.update_ctr = 64;
            ...
            let mut l = in_l[i];
            let mut r = if i < in_r.len() { in_r[i] } else { l };

            for bi in 0..5 {
                l = self.eq_l[bi].process(l);
                r = self.eq_r[bi].process(r);
            }
...
const UNIVERSAL_REVERB_DESCRIPTOR: &str = r#"{"name":"UniversalReverb","type":"Effect","params":5}"#;
const UNIVERSAL_FILTER_DESCRIPTOR: &str = r#"{"name":"UniversalFilter","type":"Effect","params":5}"#;
const PARAMETRIC_EQ_DESCRIPTOR: &str = r#"{"name":"ParametricEQ","type":"Effect","params":5}"#;
const GRAPHIC_EQ_DESCRIPTOR: &str = r#"{"name":"GraphicEQ","type":"Effect","params":6}"#;
