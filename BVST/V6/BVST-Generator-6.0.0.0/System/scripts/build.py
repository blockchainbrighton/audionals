...
def build():
    ...
    skip_processor_generation = False
    
    if os.path.exists(processor_dest):
        with open(processor_dest, 'r') as f:
            existing_content = f.read()
            if "Shared FX Loader" in existing_content or "Shared Sampler Loader" in existing_content or "Shared Loader" in existing_content:
                print("Detected Shared Loader optimization. Skipping processor generation to preserve optimization.")
                skip_processor_generation = True

    if not skip_processor_generation:
        ...
        print(f"Preserving existing {processor_dest}")
