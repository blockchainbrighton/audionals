...
export class BvstSynth {
    ...
    process(input, output) {
        ...
        var ptr1 = passArrayF32ToWasm0(output, wasm.__wbindgen_export);
        var len1 = WASM_VECTOR_LEN;
        wasm.bvstsynth_process(this.__wbg_ptr, ptr0, len0, ptr1, len1, addHeapObject(output));
    }
    /**
     * @param {number} id
     * @param {number} value
    ...
    async handleMessage(event) {
        if (event.data.type === 'INIT_WASM') {
            ...
            try {
                ...
                if (!wasmInit) { throw new Error("WASM init function not found (expected 'init' or '__wbg_init')."); }
