...
use bvst_lib::{Param, Curve, dsp::{self, Oscillator, Svf, Adsr, Reverb, WaveType}};
...
impl BvstSynth {
    pub fn new(sample_rate: f32) -> BvstSynth {
        BvstSynth {
            ...
            p_verb: Param::new(Curve::Linear { min: 0.0, max: 0.6 }, 0.1),
    ...
    pub fn process(&mut self, output: &mut [f32]) {
        for sample in output.iter_mut() {
            ...
            let wet = self.reverb.process(dry, v_verb, 0.8); // 0.8 size
