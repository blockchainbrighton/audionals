    ...
    (
        ...
        impl $name {
            ...
            pub fn get_descriptor() -> String {
                ...
                $(
                    s.push_str("{\"id\":");
                    s.push_str(stringify!($id));
                    s.push_str(",\"name\":\"");
                    s.push_str(stringify!($p_name));
                    s.push_str("\",\"min\":");
                    s.push_str(stringify!($min));
                    s.push_str(",\"max\":");
                    s.push_str(stringify!($max));
                    s.push_str(",\"def\":");
                    s.push_str(stringify!($def));
                    s.push_str(",\"curve\":\"");
                    s.push_str(stringify!($curve));
                    s.push_str("\"},");
                )*
            ...
            pub fn set_state(&mut self, state: &[f32]) {
                ...
                $(
                    if i < state.len() {
                        self.$p_name.val_norm = state[i];
                        self.$p_name.val_smoothed = state[i];
                        i += 1;
                    }
                )*
            }
