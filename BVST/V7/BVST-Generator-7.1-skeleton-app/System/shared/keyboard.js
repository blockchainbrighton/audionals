        ...
        this.container.ontouchmove = (e) => {
            ...
            if (el && el.classList.contains('key') && el.dataset.midi) {
                ...
                if (!this.activeKeys.includes(midi)) {
                    // Find keys that are active but NOT the current one (imperfect, but works for monophonic slide)
                    // Better: track the *last touched* key.
                    // For polyphonic glissando, we might just add notes.
                    // Let's implement simple "slide triggers new note".
                    // We don't necessarily turn off the old one immediately unless we track "finger 1 is on key X".
                    // Simple approach: Trigger ON for new key.
                    // Users rely on 'touchend' (or leaving the element) to trigger OFF.
                    // BUT: 'touchend' fires on the *target* element (where it started), not where it ends.
                    // So we must manually handle OFF for previous keys if we want true monophonic-style glissando?
                    // Let's stick to: Trigger ON.
                    // Actually, for a keyboard, glissando usually implies dragging triggers notes.
                    // We should trigger ON.
                    // To avoid stuck notes, we need a robust 'All Notes Off' or track which key corresponds to this touch.
                    // Complexity: high.
                    // Simplified Glissando:
                    this._handleNoteOn(midi);
                    
                    // Optional: Auto-off previous key if dragging?
                    // Let's leave it as "add note" for now, user lifts finger to stop.
                    // But if I slide C -> D, C stays on until I lift?
                    // If I lift on D, D fires touchend? No, the *original* element fires touchend.
                    // So C fires touchend.
                    // D never got touchstart, so it has no touchend listener active?
                    // Wait, touchend bubbles?
                    // Use a tracked 'lastMidi' for the touch.
                }
            }
        };
        // We need to handle 'touchend' on the container to clear the note if we slid off?
        // Actually, the original key (where touchstart happened) receives touchend even if we moved off it.
        // So the original note turns off.
        // The NEW note (triggered by touchmove) needs to be turned off when touch ends.
        // Since we called _handleNoteOn(midi) manually, we must ensure _handleNoteOff(midi) is called.
        // We can hook container.ontouchend to turn off *all* active keys? Or just the ones we added via drag?
        this.container.ontouchend = (e) => {
            e.preventDefault();
            // Release all for safety on lift? Or just the active ones?
            // "Release all" is safest for a web toy.
