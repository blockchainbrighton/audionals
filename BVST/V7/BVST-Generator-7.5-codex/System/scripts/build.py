...
def build():
    ...
    # 2. Combine Unified Loader + Glue -> processor.js
    processor_dest = os.path.join(synth_dir, "processor.js")
    
    print("Generating processor.js using Unified Loader...")
    
    # Use the new Unified Loader instead of generated bindings
    unified_loader_path = os.path.join(project_root, "System", "shared", "wasm_loader_unified.js")
    
    if not os.path.exists(unified_loader_path):
        print(f"Error: Unified loader not found at {unified_loader_path}")
        return
    ...
    full_processor_code = loader_code + "\n\n" + glue_code
    ...
    if os.path.exists(gui_src):
        ...
        # Adjust shared-module import paths for dist/ nesting.
    ...
    # Copy patch.json if present (GUI can be patch-driven in newer templates)
    patch_src = os.path.join(synth_dir, "patch.json")
    if os.path.exists(patch_src):
        shutil.copy(patch_src, os.path.join(dist_dir, "patch.json"))
            
    # Cleanup
    if os.path.exists(pkg_dir):
        print(f"Cleaning up {pkg_dir}...")
