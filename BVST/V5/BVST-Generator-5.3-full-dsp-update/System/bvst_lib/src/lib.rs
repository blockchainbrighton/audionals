...
pub mod dsp {
    ...
    impl Oscillator {
        ...
        pub fn next(&mut self, freq: f32, wave: WaveType, pwm: f32) -> f32 {
            ...
            match wave {
                ...
                WaveType::Noise => {
                     // Handled by NoiseGen usually, but if selected here return 0.
                     0.0 
                }
            }
        }
    ...
    impl Svf {
        ...
        // Add Highpass/Bandpass helpers later if needed
    }
    ...
    impl Delay {
        ...
        pub fn process(&mut self, input: f32, time_s: f32, feedback: f32, wet: f32) -> f32 {
            ...
            // time_s clamped to max buffer
            let delay_samps = (time_s * 44100.0).max(1.0) as usize; // Approximation or use sample_rate from struct?
            // We lost sample_rate in struct. Assume standard or store it?
            // Let's just use samples if we want precision, but time_s is friendlier.
            // Let's assume 44100 for calc or store it.
            // RE-DESIGN: Store sample_rate.
    ...
    // --- REVERB (Improved) ---
    // Uses multiple comb/allpass or just a simple Schroeder
    pub struct Reverb { 
        // Simple implementation using a few allpasses + delays would be better but big code.
        // Sticking to a simple feedback delay network for now.
    ...
    impl Reverb {
        ...
        pub fn process(&mut self, input: f32, wet: f32, size: f32) -> f32 {
            // Simple stereo-ish decorrelation
            let s = size * 0.05;
    ...
    impl Compressor {
        ...
        pub fn process(&mut self, input: f32, thresh_db: f32, ratio: f32, attack: f32, release: f32) -> f32 {
            ...
            // Smooth gain? Usually env follower handles smoothing.
