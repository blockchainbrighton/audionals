...
class OscApp2 extends HTMLElement {
  ...
  connectedCallback() {
    ...
    // Compose DOM - fixed the order to avoid the parent-child append error
  ...
  disconnectedCallback() {
    ...
    // Remove sequencer event listeners
  ...
  _onToggleSequencer() {
    ...
    // Layout: when sequencer is visible, make room and allow scroll
    const mainEl = this.shadowRoot.getElementById('main');
    ...
    if (state.isSequencerMode) {
      ...
      // Reset sequencer state when closing
  ...
  _onSeqStepAdvance(e) {
    const { stepIndex, value, isLastStep } = e.detail;
