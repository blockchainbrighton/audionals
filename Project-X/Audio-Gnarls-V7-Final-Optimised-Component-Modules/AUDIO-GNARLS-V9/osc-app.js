...
class OscApp2 extends HTMLElement {
  ...
  defaultState(seed = 'default') {
    return {
      ...
      sequence: Array(8).fill(null), sequencePlaying: false, sequenceIntervalId: null, sequenceStepIndex: 0,
  ...
  connectedCallback() {
    // Structure: [aside/instructions | main(canvas/controls/sequencer)]
    ...
    this._sequencerDiv = $('div', { id: 'sequencer', style: 'display:none;' });
    this._stepSlotsDiv = $('div', { id: 'stepSlots' });

    // Sequence controls
    const seqControlsDiv = $('div', { id: 'sequenceControls' });
    this._playBtn = $('button', { id: 'playBtn', textContent: 'Play Sequence', style: 'display:block;' });
    const stepTimeLabel = $('label', { htmlFor: 'stepTimeInput', style: 'margin-left:1.2em;', textContent: 'Step Time (ms):' });
    this._stepTimeInput = $('input', { type: 'number', id: 'stepTimeInput', min: '50', max: '2000', value: '400', style: 'width:60px;margin-left:0.7em;' });
    seqControlsDiv.append(this._playBtn, stepTimeLabel, this._stepTimeInput);
    this._sequencerDiv.append(this._stepSlotsDiv, seqControlsDiv);
  ...
  _style() { return `
    ...
    #sequencer { text-align:center;width:95%;margin:.8em auto 0 auto;}
  ...
  // ——— Audio: Chain Creation/Buffering ———
  ...
  _onToggleSequencer() {
    ...
    state.isSequencerMode ? this.createSequenceUI() : (state.isRecording = false, state.currentRecordSlot = -1, state.sequencePlaying && this.stopSequence(), this.updateSequenceUI());
  }
  ...
  // ——— Keyboard: shape switching and sequence recording ———
  _handleKeyDown(e) {
  ...
  createSequenceUI() {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      slot.textContent = val === 0 ? '0' : (val != null && val >= 1 && val <= this.shapes.length ? val : '');
      slot.onclick = () => { this.state.isRecording = true; this.state.currentRecordSlot = i; this.updateSequenceUI(); };
      slot.oncontextmenu = ev => { ev.preventDefault(); this.state.sequence[i]=null; if(this.state.isRecording && this.state.currentRecordSlot===i){ this.state.currentRecordSlot=(i+1)%8; if(!this.state.currentRecordSlot) this.state.isRecording=false; } this.updateSequenceUI(); };
      this._stepSlotsDiv.append(slot);
    }
    this._playBtn.onclick = ()=>this.state.sequencePlaying ? this.stopSequence() : this.playSequence();
    this._stepTimeInput.onchange = () => {
      const val = parseInt(this._stepTimeInput.value,10);
      if(val>=50 && val<=2000) { this.state.stepTime = val; if(this.state.sequencePlaying) this.stopSequence(), this.playSequence(); }
      else this._stepTimeInput.value = this.state.stepTime;
    };
    this.updateSequenceUI();
  }
  ...
  updateSequenceUI() {
    this._stepSlotsDiv.querySelectorAll('.step-slot').forEach((slot) => {
      const idx = +slot.dataset.index, val = this.state.sequence[idx];
      slot.textContent = val === 0 ? '0' : (val != null && val >= 1 && val <= this.shapes.length ? val : '');
  ...
  recordStep(number) {
    const s = this.state, idx = s.currentRecordSlot;
    if (!s.isRecording || idx < 0 || idx >= s.sequence.length) return;
    s.sequence[idx] = number;
    s.currentRecordSlot = (idx + 1) % s.sequence.length;
    if (!s.currentRecordSlot) s.isRecording = false;
    this.updateSequenceUI();
  }

  playSequence() {
    const s = this.state;
    if (s.sequencePlaying) return;
    s.sequencePlaying = true; s.sequenceStepIndex = 0; this.updateSequenceUI();
    const stepFn = () => {
      const idx = s.sequenceStepIndex, val = s.sequence[idx];
      let shapeKey;
      if (val === 0) shapeKey = this.humKey;
      else if (val >= 1 && val <= this.shapes.length) shapeKey = this.shapes[val - 1];
      else { s.sequenceStepIndex = (idx + 1) % s.sequence.length; this.updateSequenceUI(); return; }
      this._controls.updateState({ isAudioStarted: s.contextUnlocked, isPlaying: s.isPlaying, isMuted: s.Tone?.Destination?.mute, shapeKey, sequencerVisible: s.isSequencerMode });
      this._onShapeChange({ detail: { shapeKey } });
      s.sequenceStepIndex = (idx + 1) % s.sequence.length;
      this.updateSequenceUI();
    };
    stepFn();
    s.sequenceIntervalId = setInterval(stepFn, s.stepTime);
  }

  stopSequence() {
    const s = this.state;
    if (!s.sequencePlaying) return;
    clearInterval(s.sequenceIntervalId);
    s.sequenceIntervalId = null;
