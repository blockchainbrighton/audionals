 ...
 * ----------------------------
 * UI-only control panel for the Oscilloscope/Synth engine.
 * Encapsulated in Shadow DOM; dispatches events upward for the host app.
 *
 * Controls:
 *  - POWER ON/OFF      → 'start-request'
 *  - Mute/Unmute       → 'mute-toggle'
 *  - Shape select      → 'shape-change' { shapeKey }
 *  - Show Sequencer    → 'toggle-sequencer'
 *  - Audio Signature   → 'audio-signature'
 *  - Loop toggle       → 'loop-toggle'
 *  - Signature Mode    → 'signature-mode-toggle'
 *
 * Methods:
 *  - setShapes(list)   → Populate waveform dropdown.
 *  - disableAll(bool)  → Enable/disable all controls.
 *  - updateState(obj)  → Sync button states with app (see full README).
 *
 * Notes:
 *  - UI only; no audio logic here.
 *  - Toggle states must reflect `aria-pressed` + text.
 *  - When adding new controls: constructor → container → disableAll → event → updateState.
 */
