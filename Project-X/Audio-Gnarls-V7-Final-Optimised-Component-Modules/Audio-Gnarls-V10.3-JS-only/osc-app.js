class OscApp2 extends HTMLElement {
  ...
  defaultState(seed = 'default') {
    return {
      ...
      audioSignatureOnComplete: null, // callback for when a signature finishes (used by signature-mode sequencer)
  ...
  connectedCallback() {
        ...
        <ol>
          ...
          <li><b>Step Sequencer:</b>
            <ul style="margin:0 0 0 1em; padding:0; font-size:.98em;">
              ...
              <li>Press <b>Play Sequence</b> to play ${/* hint about loop */''}(<i>L toggles Loop</i>).</li>
    ...
    // New (optional) controls events for toggles if your UI emits them
  ...
  _updateControls({
    ...
    // Pass extra state to controls if it supports it; safe if ignored.
  ...
  // --- NEW: Global Loop toggle --------------------------------------------
  _onLoopToggle() {
    this.state.isLoopEnabled = !this.state.isLoopEnabled;
    this._updateControls();

    if (this.state.audioSignaturePlaying && !this.state.isSequenceSignatureMode) {
      // If currently playing a standalone signature and loop turned off, let it finish once.
      // If loop turned on, the runner will auto-repeat at completion.
      this._loader.textContent = this.state.isLoopEnabled ? 'Loop enabled.' : 'Loop disabled.';
    }
  }
  ...
  _onSignatureModeToggle() {
    ...
    // If switching modes while seq is running, re-orchestrate
    if (s.sequencePlaying) {
      // Stop both the child sequencer and any running signature
      this.stopSequence();
      this.stopAudioSignature();
      // If toggled on, auto-restart in correct mode (optional UX); keep it simple: don't auto-restart
  ...
  _onAudioSignature() {
    ...
    // Start playback (loop according to global toggle)
    this.playAudioSignature(audioSignatureSequence, algorithm, { loop: s.isLoopEnabled });

    this._loader.textContent = s.isLoopEnabled ? 'Playing Audio Signature (Loop).' : 'Playing Audio Signature...';
  }
  ...
  // --- Updated to support loop + onComplete callback ----------------------
  playAudioSignature(sequence, algorithm = 1, { loop = false, onComplete = null } = {}) {
    const s = this.state;
    // If another signature is playing, stop it first
    if (s.audioSignaturePlaying) this.stopAudioSignature();
    ...
    const playStep = () => {
      ...
      if (s.audioSignatureStepIndex >= sequence.length) {
        ...
        const finishOnce = () => {
          ...
          else this._loader.textContent = 'Audio Signature complete.';
        };
  ...
  _handleKeyDown(e) {
    // Global shortcuts first (not when typing)
  ...
  _onSeqPlayStarted(e) {
    ...
    this.state._seqFirstCycleStarted = false;
    if (typeof stepTime === 'number') this.state.stepTime = stepTime;
    this._updateControls();

    // If we're in Sequencer Signature Mode, we orchestrate playback ourselves
    if (this.state.isSequenceSignatureMode) {
      // Stop child timing so we can drive steps by signature completion
  ...
  _onSeqPlayStopped() {
    ...
    // If our signature-driven sequencer was running, stop it cleanly
    if (s.signatureSequencerRunning) this._stopSignatureSequencer();
  ...
  _onSeqStepAdvance(e) {
    // In normal (non-signature) mode, the child drives timing and we just apply values.
    if (this.state.isSequenceSignatureMode) return; // ignore child advances when we are driving
    ...
    // Play-once logic: detect wrap to index 0 (end of cycle)
    if (this.state.sequencePlaying) {
      if (stepIndex === 0) {
        if (this.state._seqFirstCycleStarted) {
          ...
          this.state._seqFirstCycleStarted = true;
        }
      }
    }
    ...
    else return; // ignore unknown values / nulls
  ...
  // --- NEW: Signature-mode sequencer runner --------------------------------
  async _startSignatureSequencer() {
    const s = this.state;
    if (s.signatureSequencerRunning) this._stopSignatureSequencer();
    s.signatureSequencerRunning = true;

    // Ensure no lingering signature playback
    this.stopAudioSignature();
    ...
    const runOnePass = async () => {
      ...
      for (let i = 0; i < s.sequence.length; i++) {
        ...
        // Handle null/empty step as rest: wait for stepTime then continue
        if (value === null || typeof value !== 'number' || value < 0) {
          await this._sleep(Math.max(50, s.stepTime));
          continue;
        }

        // Map value to shapeKey (0=hum, 1..N=shapes)
        let shapeKey = null;
        if (value === 0) shapeKey = this.humKey;
        else if (value >= 1 && value <= this.shapes.length) shapeKey = this.shapes[value - 1];
        ...
        // Generate the signature for this shape and play fully (no per-signature looping here)
        ...
        await new Promise(resolve => {
          ...
          this.playAudioSignature(seq, algo, {
            loop: false,
            onComplete: () => resolve()
          });
        });

        // Small inter-step gap for clarity (use stepTime as gap)
        if (!s.signatureSequencerRunning) return;
        await this._sleep(Math.max(30, s.stepTime));
      }
    };

    // Run once, then loop if enabled
    await runOnePass();
    if (!s.signatureSequencerRunning) return;
    ...
    // Finished (either completed once or stopped)
    this._stopSignatureSequencer();
    this._sequencerComponent?.stopSequence?.(); // ensure UI reflects stopped
  }
  ...
  updateSequencerState() {
    ...
    this._sequencerComponent.updateState?.({
      ...
      // Optionally expose toggles if the child wants to render them
      isLoopEnabled: this.state.isLoopEnabled,
      isSequenceSignatureMode: this.state.isSequenceSignatureMode
    });
  }
  ...
  stopSequence() {
    // Stop child sequencer if it is running
    this._sequencerComponent?.stopSequence();
    // Also stop our signature-driven runner if active
    if (this.state.signatureSequencerRunning) this._stopSignatureSequencer();
    // Stop any signature playing
    if (this.state.audioSignaturePlaying) this.stopAudioSignature();
    // Update flags/UI
