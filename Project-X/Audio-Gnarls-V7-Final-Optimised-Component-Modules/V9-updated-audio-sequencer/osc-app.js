...
class OscApp2 extends HTMLElement {
  ...
  startToneSequencer(stepTimeMs) {
    ...
    const getSampleTriggers = () =>
      this._sequencerComponent.state.sampleChannels.map(ch => !!ch.steps[this.state.sequenceStepIndex]);

    this._toneStepId = Tone.Transport.scheduleRepeat((time) => {
      // Get current step index and value
      const idx = this.state.sequenceStepIndex;
      const value = this.state.sequence[idx];
      const sampleTriggers = getSampleTriggers();
      ...
      // SAMPLES (precise)
      if (Array.isArray(sampleTriggers) && this.state.contextUnlocked && this.state.initialShapeBuffered) {
        sampleTriggers.forEach((shouldTrigger, idx) => {
          if (shouldTrigger && this.samplePlayers[idx]) {
            try {
              this.samplePlayers[idx].start(time);
            } catch (err) {
              console.warn('Sample trigger error on channel', idx, err);
            }
          }
        });
      }
