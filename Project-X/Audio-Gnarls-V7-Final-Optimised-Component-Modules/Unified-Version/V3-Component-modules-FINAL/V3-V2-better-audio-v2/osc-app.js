...
class OscApp extends HTMLElement {
  ...
  async unlockAudioAndBufferInitial() {
    ...
    try {
      ...
      // Buffer the chain for the initially selected shape.
      ...
      // Switch visualiser to live mode for the initial shape.
      {
        const chain = this.state.chains[shape];
        this.scopeCanvas.setParams({
          shape,
      ...
      // Pre-buffer all remaining chains immediately to ensure zero latency
      // when switching shapes via number keys.  Perform this concurrently
      // to avoid audio glitches when the user changes shapes.
      this.controls.setLoader('Buffering synthesiser chains...');
      const others = this.state.shapes
        .filter((s) => s !== shape)
        .map((s) => this.bufferShapeChain(s));
