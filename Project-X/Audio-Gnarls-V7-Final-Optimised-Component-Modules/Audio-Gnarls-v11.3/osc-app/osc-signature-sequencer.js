...
export function SignatureSequencer(app) {
  return {
    ...
    generateAudioSignature(seed, algorithm = 1, steps = null) {
      // Use provided steps or fall back to current app steps
      ...
      switch (algorithm) {
        ...
        case 2:
          return app._generateSignatureWithConstraints(seed, {
            steps: sequenceSteps, paletteSize: 6, pRepeat: 0.35, pHum: 0.15, pSilence: 0.2, avoidBackAndForth: true
          });
        case 3: {
          const L = 8; const pattern = Array.from({ length: L }, () => Math.floor(rng() * 10));
          return Array.from({ length: sequenceSteps }, (_, i) => pattern[i % L]);
        }
        case 4: {
        ...
        case 5: {
          ...
          for (let i = 0; i < sequenceSteps;) { const len = Math.min((Math.floor(rng() * 6) + 2), sequenceSteps - i);
            for (let j = 0; j < len; j++, i++) seq.push(cluster); cluster = Math.floor(rng() * 10);
          } return seq;
        }
        case 6: {
          const seq = []; for (let i = 0; i < sequenceSteps; i++) seq.push(rng() > 0.7 ? (Math.floor(rng() * 9) + 1) : 0); return seq;
        }
        case 7: {
          const seq = new Array(sequenceSteps).fill(0); let pos = 0; let a = 1, b = 1;
          while (pos < sequenceSteps) { seq[pos] = Math.floor(rng() * 9) + 1; const next = a + b; a = b; b = next; pos += next; }
          return seq;
        }
        case 8: {
          const a = Math.floor(rng() * 10); const b = Math.floor(rng() * 10);
          return Array.from({ length: sequenceSteps }, (_, i) => (i % 2 === 0 ? a : b));
        }
        case 9: {
          const seq = []; let v = Math.floor(rng() * 9) + 1;
          for (let i = 0; i < sequenceSteps; i++) { if (rng() < 0.2 || v === 0) v = Math.floor(rng() * 10);
            seq.push(v); if (rng() > 0.7) v = Math.max(0, v - 1);
          } return seq;
        }
        case 10: {
          const seq = []; let c = Math.floor(rng() * 10);
          for (let i = 0; i < sequenceSteps; i++) { if (i % 8 === 0 || rng() > 0.6) c = Math.floor(rng() * 10); seq.push(c); }
          return seq;
        }
        default:
          return app._generateSignatureWithConstraints(seed, { steps: sequenceSteps });
      }
    },
    ...
    // Step count management for signature sequencer
    setSteps(newSteps) {
