...
class SeqApp extends HTMLElement {
  ...
  // Allow external updates like: <seq-app steps="16">
  ...
  // React to <seq-app steps="..."> changes from HTML/JS
  ...
  updateState(newState) {
    // If steps changed, use the robust setSteps method
    if ('steps' in newState) {
      ...
      if (newSteps !== this.steps) {
        ...
        // Remove steps from newState to avoid double processing
  ...
  // Robust setSteps method that preserves data when expanding and safely truncates when shrinking
  setSteps(newSteps) {
