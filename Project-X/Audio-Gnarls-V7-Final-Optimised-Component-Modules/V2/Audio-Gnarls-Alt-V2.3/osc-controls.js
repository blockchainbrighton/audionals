...
class OscControls extends HTMLElement {
  ...
  _generateSoundPresets(seed) {
    ...
    modes.forEach((mode, index) => {
      ...
      // Determine the number of voices in the bank (8–10).  Each voice
      // represents an independent sound that can be triggered on
      // demand.  Voices are harmonically related by drawing pitches
      // exclusively from the chosen scale.  Variety is achieved by
      // varying the instrument type, envelope and oscillator for each
      // voice.
      ...
      for (let i = 0; i < numVoices; i++) {
        // Always include the root note in the first voice to anchor the
        // harmony.  Subsequent voices choose randomly from the scale.
        const pitch = i === 0 ? scale[0] : choose(scale);
        // Choose an instrument class.  Some instruments (e.g.
        // MembraneSynth, MetalSynth) produce percussive or metallic
        // timbres, while others (Synth, MonoSynth) are more tonal.
        const instType = choose(voiceTypes);
        // Envelope: early voices favour short attack/decay for hard
        // hitting percussive sounds; later voices use slower attack and
        // longer release for evolving pads.
        const isPercussive = i < Math.floor(numVoices / 3);
        const env = isPercussive ? {
          attack: randf(0.005, 0.05),
          decay: randf(0.05, 0.4),
          sustain: randf(0.2, 0.6),
          release: randf(0.1, 1.5)
        } : {
          attack: randf(0.1, 1.0),
          decay: randf(0.2, 1.2),
          sustain: randf(0.5, 1.0),
          release: randf(1.0, 4.0)
        };
        // Oscillator type for Synth/MonoSynth voices; ignored for other
        // instruments.  We still include it for completeness.
        const oscType = choose(['sine','triangle','square','sawtooth']);
        // Volume: stronger voices have louder volume; softer voices are
        // pushed back in the mix.  Percussive voices are typically
        // louder.
        const volume = isPercussive ? -8 + -randf(0, 4) : -12 - randf(0, 6);
        bank.push({
          note: pitch,
          inst: instType,
          envelope: env,
      ...
      // Decide whether to include a base drone oscillator.  When
      // enabled the drone plays the root note at a low volume,
      // underpinning the harmony without cluttering the mix.  The
      // oscillator type and volume are derived deterministically.
      ...
      const preset = {
        ...
        bank,
        drone,
        // Global filter affecting all voices
        filter: {
        ...
        // Global LFO modulating the filter for evolving textures
        lfo: {
        ...
        reverb: rand() > 0.4,
        // Detune can still be applied to voices globally, though voices
        // themselves define their own volumes and envelopes
        detune: randf(-50, 50)
      };
