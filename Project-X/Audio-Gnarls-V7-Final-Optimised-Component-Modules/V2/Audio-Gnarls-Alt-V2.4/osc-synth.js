...
export class OscSynth {
  /**
   * Create a new OscSynth instance. All sounds are sustained or looping. Transients auto-loop at 120 BPM.
  ...
  /**
   * Clamp/sanitize all params, ensuring LFOs are within evolving/“siren” ranges, envelopes not fully transient.
   */
  _sanitizeParams(params) {
    ...
    // LFO
    if (p.lfo) {
      // Allow deep, slow LFOs for evolving or “siren” effects (up to 8 bars at 120BPM)
      // 120 BPM: 1 bar = 2 seconds, so 0.125 Hz is 1 cycle every 8s = 4 bars
      p.lfo.frequency = this._clamp(p.lfo.frequency ?? 0.25, 0.0125, 2.0); // 0.0125 = 1 cycle/80s = 40 bars
    ...
    p.tempo = 120; // Enforce fixed tempo (loop rate is always 120 BPM for transients)

    return p;
  }

  /**
   * Build audio graph. Any voice with short envelope will be auto-looped.
   */
  _buildChain() {
    const T = this.Tone;
    const p = this.params;
    ...
    // === Set up auto-loop for any transient voices ===
    this._setupAutoLoopers();
  }

  /**
   * For voices with short envelope (decay+release < 1.2s), trigger a looping pattern at 120 BPM (1 bar = 2s).
   * For all other voices, just trigger a long note on connect.
   */
  _setupAutoLoopers() {
    ...
    this.voices.forEach((voice, idx) => {
      ...
      // Consider "transient" if total decay+release < 1.2s
      const isTransient = (env.decay ?? 0.2) + (env.release ?? 2.5) < 1.2;
      if (isTransient) {
        // Loop at every bar (2.0s) -- use setInterval, deterministic by voice index
        ...
        // For sustained sounds, trigger a single long note (8 bars = 16s)
        try { voice.triggerAttackRelease(note, `${barLen * 8}s`, undefined, 0.8); } catch {}
      }
    });
  }

  /**
   * Connect the synth to a destination, start everything.
   */
  connect(destination) {
    const T = this.Tone;
    ...
    // Set global tempo (120 BPM)
    T.Transport.bpm.value = 120;

    // Ensure audible (existing logic is sufficient)
    this._ensureAudible();
  ...
  /**
   * Ensure output is audible: if not, boost master gain and reset filter.
   */
  _ensureAudible() {
