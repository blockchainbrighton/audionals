  ...
  _generateSoundPresets(seed) {
    ...
    modes.forEach((mode, index) => {
      ...
      for (let i = 0; i < numVoices; i++) {
        ...
        // Envelope tweaks: ensure every voice has an almost
        // instantaneous attack (<0.02s) so that it is audible immediately.
        // Decay/sustain/release are still varied but clamped to musically
        // sensible ranges. Sustained voices have longer releases.
        const env = isPercussive ? {
          attack: this._clamp(randf(0.005, 0.015), 0.001, 0.02),
          decay: this._clamp(randf(0.05, 0.4), 0.02, 0.6),
          sustain: this._clamp(randf(0.3, 0.8), 0.1, 0.9),
          release: this._clamp(randf(0.2, 2.0), 0.1, 3.0)
        } : {
          attack: this._clamp(randf(0.005, 0.02), 0.001, 0.03),
          decay: this._clamp(randf(0.3, 1.2), 0.1, 2.0),
          sustain: this._clamp(randf(0.6, 1.0), 0.3, 1.0),
          release: this._clamp(randf(2.0, 5.0), 0.5, 6.0)
        };
  
        const oscType = choose(['sine','triangle','square','sawtooth']);
  
        // Ensure volumes remain in an audible range. Percussive voices
        // should cut through the mix while pads sit slightly quieter.
        const volume = this._clamp(
          isPercussive
            ? -8 - randf(0, 4)    // -8 to -12 dB
            : -10 - randf(0, 4),  // -10 to -14 dB
          -18, -3
        );
      ...
      const preset = {
        ...
        tempo, // ✅ Now included
        filter: {
          // Use a slightly higher minimum cutoff to ensure all voices
          // remain audible even if the LFO or resonance pushes the
          // frequency lower. Values between 600 and 6000 Hz are
          // musically useful for pads and leads.
          frequency: this._clamp(randf(600, 6000), 400, 8000),
        ...
        lfo: {
          ...
          // Encourage more complex modulation by selecting a wider
          // frequency range and deeper modulation depths.
          frequency: this._clamp(randf(0.05, 0.5), 0.01, 2),
          depth: this._clamp(randf(0.2, 0.8), 0.05, 1)
        },
        phaser: rand() > 0.5 ? {
