...
class LookaheadScheduler {
  ...
  reset(startTime = this.ctx.currentTime) {
...
class Sequencer {
  ...
  start() {
    ...
    this.position = 0;
    this.scheduler.reset(this.ctx.currentTime);
    this.scheduler.start((time) => {
      this.tick(time);
    });
    this.isPlaying = true;
    if (this.synth.state.sequencer) {
      this.synth.state.sequencer.playing = true;
    }
  }
  stop({ resetPosition = true, updateState = true } = {}) {
    this.scheduler.stop();
    if (resetPosition) {
      this.scheduler.reset(this.ctx.currentTime);
      this.position = 0;
    ...
    this.isPlaying = false;
    if (updateState && this.synth.state.sequencer) {
      this.synth.state.sequencer.playing = false;
    }
  }
  restart() {
    const wasPlaying = this.isPlaying;
    this.stop({ resetPosition: true, updateState: false });
    if (wasPlaying || this.synth.state.sequencer?.playing) {
      this.start();
    }
  }
  tick(time) {
...
class PresetManager {
  ...
  loadFactory() {
    this.presets.set('Deep Liquid Bass', {
      ...
      global: { bpm: 174, swing: 8, key: 'C', masterVolume: 0.82, limiterCeiling: -0.8, quality: 1 },
      oscillators: {
    ...
    this.presets.set('Dark Tech Roller', {
      ...
      global: { bpm: 174, swing: 12, key: 'C', masterVolume: 0.78, limiterCeiling: -1.2, quality: 1 },
      oscillators: {
    ...
    this.presets.set('Rolling Pluck', {
      ...
      global: { bpm: 174, swing: 16, key: 'C', masterVolume: 0.76, limiterCeiling: -1, quality: 1 },
      oscillators: {
    ...
    this.presets.set('Air Pad', {
      ...
      global: { bpm: 174, swing: 10, key: 'C', masterVolume: 0.8, limiterCeiling: -1.4, quality: 2 },
      oscillators: {
...
class MothyGSynth {
  ...
  stopAll() {
    if (!this.initialised) return;
    this.sequencer?.stop({ resetPosition: true, updateState: true });
  ...
  triggerLaneNote(laneId, note, velocity, time, step) {
    ...
    voice.updateFromState(this.state);
    voice.trigger(playNote, velocity, time);
  ...
  setupKeyControls() {
    ...
    KEY_ROOTS.forEach((keyName) => {
      ...
      button.addEventListener('click', () => {
        ...
        this.ensureRhythmState();
        const wasPlaying = this.synth.initialised ? !!this.synth.sequencer?.isPlaying : false;
