...
const TOOLTIP_COPY = {
  ...
  'master.lfo.enabled': 'Enable or disable the dedicated master LFO gate on the main bus.',
  'master.lfo.tempo': 'Sets the base tempo for the master LFO in BPM. Defaults to the patch tempo.',
  'master.lfo.depth': 'Controls how deep the master LFO dips the output. Higher values create sharper gating.',
  'master.lfo.multipliers': 'Instantly jump the master LFO to key musical divisions and multiples.',
  'master.filters.hp': 'High-pass filter cutoff for the master bus. Use to trim rumble before the limiter.',
  'master.filters.lp': 'Low-pass filter cutoff for the master bus. Great for taming harsh highs quickly.',
  'master.delayMix': 'Quick access to delay mix for fast tweaks without opening the FX panel.',
...
class MasterBus {
  constructor(ctx) {
    ...
    this.preSaturation.connect(this.hpFilter);
    this.hpFilter.connect(this.lpFilter);

    this.lfoVca = ctx.createGain();
    this.lfoVca.gain.value = 0;
    this.lpFilter.connect(this.lfoVca);

    this.masterGain = ctx.createGain();
    this.lfoVca.connect(this.masterGain);
    ...
    this.lfoBase = ctx.createConstantSource();
    this.lfoBase.offset.value = 1;

    this.lfoDepthGain.connect(this.lfoVca.gain);
    this.lfoBase.connect(this.lfoVca.gain);
  ...
  setFilters(filters = {}) {
    const now = this.ctx.currentTime;
    const hp = clamp(filters.hp ?? 20, 20, 20000);
    const lpCandidate = clamp(filters.lp ?? 20000, 200, this.ctx.sampleRate / 2);
    const lp = clamp(Math.max(lpCandidate, hp + 10), 200, this.ctx.sampleRate / 2);
    this.hpFilter.frequency.setTargetAtTime(hp, now, 0.05);
    this.lpFilter.frequency.setTargetAtTime(lp, now, 0.05);
  }
  setLFO(settings = {}) {
    const {
      enabled = false,
    ...
    const now = this.ctx.currentTime;
    const safeDepth = clamp(depth ?? 0, 0, 0.5);
    const base = enabled ? Math.max(1 - safeDepth, 0) : 1;
    const oscDepth = enabled ? safeDepth : 0;
    let frequency = (tempo || 1) / 60;
    const safeMultiplier = Number.isFinite(multiplier) ? multiplier : 1;
    frequency *= safeMultiplier || 1;
    frequency = clamp(frequency, 0.01, 40);
    const validShapes = ['sine', 'triangle', 'sawtooth', 'square'];
    const targetShape = validShapes.includes(shape) ? shape : 'sine';
    if (this.lfoOsc.type !== targetShape) {
      this.lfoOsc.type = targetShape;
    }
    this.lfoOsc.frequency.setTargetAtTime(frequency, now, 0.05);
    this.lfoDepthGain.gain.setTargetAtTime(oscDepth, now, 0.05);
    this.lfoBase.offset.setTargetAtTime(base, now, 0.05);
  }
}
...
class PresetManager {
  ...
  loadFactory() {
    ...
    this.generateAlgorithmicPresets(88);
  }
  generateAlgorithmicPresets(targetCount) {
    const desired = Math.max(targetCount, this.presets.size);
    if (this.presets.size >= desired) return;

    const rng = new PRNG(0x5eedc0de);
    const adjectives = [
      'Velvet', 'Azure', 'Crimson', 'Frosted', 'Phantom', 'Solar', 'Noir', 'Luminous', 'Verdant', 'Nebula',
      'Golden', 'Lunar', 'Ghost', 'Prismatic', 'Serene', 'Quantum', 'Radiant', 'Obsidian', 'Amber', 'Cobalt'
    ];
    const nouns = [
      'Pulse', 'Mirage', 'Horizon', 'Drift', 'Chorus', 'Cascade', 'Signal', 'Bloom', 'Current', 'Flux',
      'Eclipse', 'Voyage', 'Beacon', 'Temple', 'Tide', 'Circuit', 'Rift', 'Ember', 'Cloud', 'Halo', 'Orbit', 'Veil'
    ];
    const modSources = ['LFO1', 'LFO2', 'LFO3', 'MODENV', 'Velocity', 'Aftertouch', 'Random', 'EnvelopeFollower', 'Keytrack'];
    const modDestinations = ['oscA.morph', 'oscB.morph', 'filter1.cutoff', 'filter2.cutoff', 'sub.level', 'drive', 'delay.mix', 'reverb.mix', 'chorus.depth'];
    const oscATables = ['mellowTriangle', 'smoothSaw', 'reeseBlend', 'grittySaw'];
    const oscBTables = ['smoothSaw', 'airyBlend', 'hollowPulse', 'metallicFm'];

    const fillModMatrix = (state, minSlots, maxSlots) => {
      const upper = Math.max(maxSlots, minSlots);
      const span = upper - minSlots + 1;
      const slotCount = minSlots + Math.floor(rng.nextRange(0, span > 0 ? span : 1));
      const matrix = [];
      for (let i = 0; i < slotCount; i += 1) {
        let amount = rng.nextRange(-0.6, 0.6);
        if (Math.abs(amount) < 0.12) {
          amount = amount < 0 ? -0.12 : 0.12;
        }
        matrix.push({
          source: rng.pick(modSources),
          destination: rng.pick(modDestinations),
          amount
        });
      }
      state.modMatrix = matrix;
    };
    ...
    const archetypes = [
      {
        label: 'Bass',
        configure: (state) => {
          state.global.bpm = Math.round(rng.nextRange(130, 174));
          state.global.swing = Math.round(rng.nextRange(6, 22));
          state.global.masterVolume = rng.nextRange(0.74, 0.89);
          state.global.limiterCeiling = rng.nextRange(-1.8, -0.6);
          state.global.key = rng.pick(KEY_ROOTS);
          state.global.quality = rng.next() > 0.65 ? 2 : 1;

          state.oscillators.oscA.table = rng.pick(['reeseBlend', 'grittySaw', 'smoothSaw']);
          state.oscillators.oscB.table = rng.pick(['smoothSaw', 'airyBlend', 'metallicFm']);
          state.oscillators.oscA.morph = rng.nextRange(0.2, 0.75);
          state.oscillators.oscB.morph = rng.nextRange(0.15, 0.65);
          state.oscillators.oscA.tune = Math.round(rng.nextRange(-16, -7));
          state.oscillators.oscB.tune = Math.round(rng.nextRange(-12, 5));
          state.oscillators.oscA.fine = rng.nextRange(-0.12, 0.12);
          state.oscillators.oscB.fine = rng.nextRange(-0.15, 0.15);
          state.oscillators.oscA.level = rng.nextRange(0.62, 0.86);
          state.oscillators.oscB.level = rng.nextRange(0.38, 0.65);
          state.oscillators.oscA.pan = rng.nextRange(-0.18, 0.18);
          state.oscillators.oscB.pan = rng.nextRange(-0.2, 0.2);

          state.sub.level = rng.nextRange(0.68, 0.96);
          state.sub.squareBlend = rng.nextRange(0.05, 0.22);
          state.sub.saturation = rng.nextRange(0.32, 0.58);
          state.sub.legato = true;

          state.fm.ratio = rng.pick([1.5, 2, 2.5]);
          state.fm.index = rng.nextRange(0.1, 0.32);
          state.fm.attack = rng.nextRange(0.004, 0.02);
          state.fm.decay = rng.nextRange(0.12, 0.28);
          state.fm.velocityToIndex = rng.nextRange(0.06, 0.2);

          state.filters.filter1.type = 'lowpass';
          state.filters.filter1.cutoff = rng.nextRange(80, 240);
          state.filters.filter1.resonance = rng.nextRange(0.45, 0.95);
          state.filters.filter1.envAmount = rng.nextRange(140, 320);
          state.filters.filter1.drive = rng.nextRange(1.08, 1.35);
          state.filters.filter1.keytrack = rng.nextRange(0.35, 0.65);

          state.filters.filter2.type = rng.pick(['bandpass', 'lowpass']);
          state.filters.filter2.cutoff = rng.nextRange(260, 720);
          state.filters.filter2.resonance = rng.nextRange(0.7, 1.25);
          state.filters.filter2.envAmount = rng.nextRange(40, 120);
          state.filters.filter2.drive = rng.nextRange(1.02, 1.2);
          state.filters.filter2.keytrack = rng.nextRange(0.18, 0.45);
          state.filters.routing.serial = true;
          state.filters.routing.mix = rng.nextRange(0.6, 0.95);

          state.envelopes.amp.attack = rng.nextRange(0.003, 0.012);
          state.envelopes.amp.decay = rng.nextRange(0.12, 0.32);
          state.envelopes.amp.sustain = rng.nextRange(0.55, 0.82);
          state.envelopes.amp.release = rng.nextRange(0.22, 0.4);
          state.envelopes.amp.curve = rng.nextRange(1, 1.4);

          state.envelopes.filter.attack = rng.nextRange(0.004, 0.018);
          state.envelopes.filter.decay = rng.nextRange(0.16, 0.35);
          state.envelopes.filter.sustain = rng.nextRange(0.25, 0.55);
          state.envelopes.filter.release = rng.nextRange(0.18, 0.4);
          state.envelopes.filter.curve = rng.nextRange(1.1, 1.6);

          state.envelopes.mod.attack = rng.nextRange(0.004, 0.012);
          state.envelopes.mod.decay = rng.nextRange(0.14, 0.32);
          state.envelopes.mod.sustain = rng.nextRange(0, 0.3);
          state.envelopes.mod.release = rng.nextRange(0.12, 0.28);
          state.envelopes.mod.curve = rng.nextRange(0.9, 1.3);

          state.lfo = [
            { shape: 'triangle', rate: rng.nextRange(0.16, 0.4), sync: true, amount: rng.nextRange(0.2, 0.48), destination: 'filter1.cutoff', phase: rng.nextRange(0, 1) },
            { shape: 'sine', rate: rng.nextRange(0.22, 0.55), sync: true, amount: rng.nextRange(0.15, 0.32), destination: 'oscB.morph', phase: rng.nextRange(0, 1) },
            { shape: 'sample-hold', rate: rng.nextRange(2.8, 6.5), sync: false, amount: rng.nextRange(0.06, 0.16), destination: 'drive', phase: 0 }
          ];

          state.fx.chorus.depth = rng.nextRange(0.12, 0.32);
          state.fx.chorus.rate = rng.nextRange(0.45, 1.05);
          state.fx.delay.time = rng.nextRange(0.18, 0.36);
          state.fx.delay.feedback = rng.nextRange(0.18, 0.34);
          state.fx.delay.mix = rng.nextRange(0.12, 0.28);
          state.fx.delay.pingPong = rng.next() > 0.55;
          state.fx.reverb.mix = rng.nextRange(0.08, 0.2);

          state.sidechain.amount = rng.nextRange(0.2, 0.42);
          state.sidechain.attack = rng.nextRange(0.028, 0.06);
          state.sidechain.release = rng.nextRange(0.18, 0.36);
          state.sidechain.source = 'internal';

          state.transient.attack = rng.nextRange(1.05, 1.28);
          state.transient.sustain = rng.nextRange(0.78, 0.94);

          fillModMatrix(state, 2, 4);
        }
      },
      {
        label: 'Pad',
        configure: (state) => {
          state.global.bpm = Math.round(rng.nextRange(70, 110));
          state.global.swing = Math.round(rng.nextRange(0, 18));
          state.global.masterVolume = rng.nextRange(0.58, 0.76);
          state.global.limiterCeiling = rng.nextRange(-2.8, -1);
          state.global.key = rng.pick(KEY_ROOTS);
          state.global.quality = 2;

          state.oscillators.oscA.table = rng.pick(['mellowTriangle', 'smoothSaw']);
          state.oscillators.oscB.table = rng.pick(['airyBlend', 'hollowPulse', 'metallicFm']);
          state.oscillators.oscA.morph = rng.nextRange(0.1, 0.55);
          state.oscillators.oscB.morph = rng.nextRange(0.3, 0.85);
          state.oscillators.oscA.tune = Math.round(rng.nextRange(-7, 7));
          state.oscillators.oscB.tune = Math.round(rng.nextRange(0, 12));
          state.oscillators.oscA.fine = rng.nextRange(-0.08, 0.08);
          state.oscillators.oscB.fine = rng.nextRange(-0.08, 0.08);
          state.oscillators.oscA.level = rng.nextRange(0.55, 0.78);
          state.oscillators.oscB.level = rng.nextRange(0.4, 0.62);
          state.oscillators.oscA.pan = rng.nextRange(-0.2, 0.2);
          state.oscillators.oscB.pan = rng.nextRange(-0.3, 0.3);

          state.sub.level = rng.nextRange(0.25, 0.5);
          state.sub.squareBlend = rng.nextRange(0.1, 0.45);
          state.sub.saturation = rng.nextRange(0.15, 0.35);
          state.sub.legato = false;

          state.fm.ratio = rng.pick([0.5, 1, 2]);
          state.fm.index = rng.nextRange(0.05, 0.2);
          state.fm.attack = rng.nextRange(0.05, 0.2);
          state.fm.decay = rng.nextRange(0.4, 0.8);
          state.fm.velocityToIndex = rng.nextRange(0.02, 0.12);

          state.filters.filter1.type = 'lowpass';
          state.filters.filter1.cutoff = rng.nextRange(1200, 4200);
          state.filters.filter1.resonance = rng.nextRange(0.3, 0.65);
          state.filters.filter1.envAmount = rng.nextRange(90, 220);
          state.filters.filter1.drive = rng.nextRange(0.95, 1.15);
          state.filters.filter1.keytrack = rng.nextRange(0.2, 0.45);

          state.filters.filter2.type = rng.pick(['bandpass', 'highpass']);
          state.filters.filter2.cutoff = rng.nextRange(1600, 5200);
          state.filters.filter2.resonance = rng.nextRange(0.4, 0.9);
          state.filters.filter2.envAmount = rng.nextRange(60, 200);
          state.filters.filter2.drive = rng.nextRange(0.9, 1.1);
          state.filters.filter2.keytrack = rng.nextRange(0.15, 0.35);
          state.filters.routing.serial = rng.next() > 0.5;
          state.filters.routing.mix = rng.nextRange(0.45, 0.85);

          state.envelopes.amp.attack = rng.nextRange(0.12, 0.5);
          state.envelopes.amp.decay = rng.nextRange(0.6, 1.4);
          state.envelopes.amp.sustain = rng.nextRange(0.7, 0.95);
          state.envelopes.amp.release = rng.nextRange(0.8, 1.6);
          state.envelopes.amp.curve = rng.nextRange(1.1, 1.6);

          state.envelopes.filter.attack = rng.nextRange(0.08, 0.3);
          state.envelopes.filter.decay = rng.nextRange(0.4, 1.2);
          state.envelopes.filter.sustain = rng.nextRange(0.6, 0.9);
          state.envelopes.filter.release = rng.nextRange(0.7, 1.4);
          state.envelopes.filter.curve = rng.nextRange(1, 1.5);

          state.envelopes.mod.attack = rng.nextRange(0.05, 0.24);
          state.envelopes.mod.decay = rng.nextRange(0.6, 1.4);
          state.envelopes.mod.sustain = rng.nextRange(0.4, 0.7);
          state.envelopes.mod.release = rng.nextRange(0.6, 1.2);
          state.envelopes.mod.curve = rng.nextRange(0.9, 1.4);

          state.lfo = [
            { shape: 'sine', rate: rng.nextRange(0.05, 0.18), sync: true, amount: rng.nextRange(0.2, 0.45), destination: 'oscA.morph', phase: rng.nextRange(0, 1) },
            { shape: 'triangle', rate: rng.nextRange(0.08, 0.25), sync: true, amount: rng.nextRange(0.18, 0.5), destination: 'filter1.cutoff', phase: rng.nextRange(0, 1) },
            { shape: 'sine', rate: rng.nextRange(0.2, 0.5), sync: false, amount: rng.nextRange(0.1, 0.35), destination: 'reverb.mix', phase: rng.nextRange(0, 1) }
          ];

          state.fx.chorus.depth = rng.nextRange(0.25, 0.45);
          state.fx.chorus.rate = rng.nextRange(0.18, 0.5);
          state.fx.delay.time = rng.nextRange(0.35, 0.75);
          state.fx.delay.feedback = rng.nextRange(0.25, 0.55);
          state.fx.delay.mix = rng.nextRange(0.25, 0.45);
          state.fx.delay.pingPong = true;
          state.fx.reverb.mix = rng.nextRange(0.35, 0.65);

          state.sidechain.amount = rng.nextRange(0.05, 0.18);
          state.sidechain.attack = rng.nextRange(0.05, 0.12);
          state.sidechain.release = rng.nextRange(0.6, 1.1);
          state.sidechain.source = 'external';

          state.transient.attack = rng.nextRange(0.85, 1.05);
          state.transient.sustain = rng.nextRange(0.9, 1.05);

          fillModMatrix(state, 3, 5);
        }
      },
      {
        label: 'Pluck',
        configure: (state) => {
          state.global.bpm = Math.round(rng.nextRange(118, 175));
          state.global.swing = Math.round(rng.nextRange(10, 26));
          state.global.masterVolume = rng.nextRange(0.7, 0.86);
          state.global.limiterCeiling = rng.nextRange(-1.4, -0.5);
          state.global.key = rng.pick(KEY_ROOTS);
          state.global.quality = rng.next() > 0.5 ? 2 : 1;

          state.oscillators.oscA.table = rng.pick(['smoothSaw', 'reeseBlend', 'grittySaw']);
          state.oscillators.oscB.table = rng.pick(['airyBlend', 'metallicFm']);
          state.oscillators.oscA.morph = rng.nextRange(0.18, 0.68);
          state.oscillators.oscB.morph = rng.nextRange(0.12, 0.52);
          state.oscillators.oscA.tune = Math.round(rng.nextRange(-5, 5));
          state.oscillators.oscB.tune = rng.pick([7, 12, -12]);
          state.oscillators.oscA.fine = rng.nextRange(-0.08, 0.08);
          state.oscillators.oscB.fine = rng.nextRange(-0.1, 0.1);
          state.oscillators.oscA.level = rng.nextRange(0.6, 0.82);
          state.oscillators.oscB.level = rng.nextRange(0.34, 0.58);
          state.oscillators.oscA.pan = rng.nextRange(-0.12, 0.12);
          state.oscillators.oscB.pan = rng.nextRange(-0.12, 0.12);

          state.sub.level = rng.nextRange(0.2, 0.42);
          state.sub.squareBlend = rng.nextRange(0.05, 0.3);
          state.sub.saturation = rng.nextRange(0.18, 0.4);
          state.sub.legato = false;

          state.fm.ratio = rng.pick([2, 3, 4]);
          state.fm.index = rng.nextRange(0.28, 0.55);
          state.fm.attack = rng.nextRange(0.002, 0.01);
          state.fm.decay = rng.nextRange(0.08, 0.22);
          state.fm.velocityToIndex = rng.nextRange(0.08, 0.2);

          state.filters.filter1.type = 'lowpass';
          state.filters.filter1.cutoff = rng.nextRange(540, 1800);
          state.filters.filter1.resonance = rng.nextRange(0.5, 0.9);
          state.filters.filter1.envAmount = rng.nextRange(260, 520);
          state.filters.filter1.drive = rng.nextRange(1.02, 1.22);
          state.filters.filter1.keytrack = rng.nextRange(0.32, 0.6);

          state.filters.filter2.type = 'bandpass';
          state.filters.filter2.cutoff = rng.nextRange(1200, 2600);
          state.filters.filter2.resonance = rng.nextRange(0.7, 1.2);
          state.filters.filter2.envAmount = rng.nextRange(140, 320);
          state.filters.filter2.drive = rng.nextRange(1, 1.18);
          state.filters.filter2.keytrack = rng.nextRange(0.25, 0.5);
          state.filters.routing.serial = true;
          state.filters.routing.mix = rng.nextRange(0.55, 0.85);

          state.envelopes.amp.attack = rng.nextRange(0.002, 0.01);
          state.envelopes.amp.decay = rng.nextRange(0.18, 0.34);
          state.envelopes.amp.sustain = rng.nextRange(0.2, 0.45);
          state.envelopes.amp.release = rng.nextRange(0.18, 0.32);
          state.envelopes.amp.curve = rng.nextRange(1, 1.4);

          state.envelopes.filter.attack = rng.nextRange(0.003, 0.015);
          state.envelopes.filter.decay = rng.nextRange(0.2, 0.38);
          state.envelopes.filter.sustain = rng.nextRange(0.15, 0.4);
          state.envelopes.filter.release = rng.nextRange(0.18, 0.32);
          state.envelopes.filter.curve = rng.nextRange(1.1, 1.6);

          state.envelopes.mod.attack = rng.nextRange(0.002, 0.01);
          state.envelopes.mod.decay = rng.nextRange(0.22, 0.4);
          state.envelopes.mod.sustain = rng.nextRange(0, 0.25);
          state.envelopes.mod.release = rng.nextRange(0.14, 0.3);
          state.envelopes.mod.curve = rng.nextRange(1, 1.5);

          state.lfo = [
            { shape: 'triangle', rate: rng.nextRange(0.22, 0.5), sync: true, amount: rng.nextRange(0.18, 0.4), destination: 'filter2.cutoff', phase: rng.nextRange(0, 1) },
            { shape: 'sine', rate: rng.nextRange(0.35, 0.8), sync: true, amount: rng.nextRange(0.12, 0.35), destination: 'oscB.morph', phase: rng.nextRange(0, 1) },
            { shape: 'sawtooth', rate: rng.nextRange(0.6, 1.2), sync: true, amount: rng.nextRange(0.1, 0.28), destination: 'delay.mix', phase: rng.nextRange(0, 1) }
          ];

          state.fx.chorus.depth = rng.nextRange(0.16, 0.32);
          state.fx.chorus.rate = rng.nextRange(0.4, 0.9);
          state.fx.delay.time = rng.nextRange(0.2, 0.45);
          state.fx.delay.feedback = rng.nextRange(0.2, 0.42);
          state.fx.delay.mix = rng.nextRange(0.35, 0.55);
          state.fx.delay.pingPong = rng.next() > 0.5;
          state.fx.reverb.mix = rng.nextRange(0.18, 0.35);

          state.sidechain.amount = rng.nextRange(0.1, 0.28);
          state.sidechain.attack = rng.nextRange(0.025, 0.06);
          state.sidechain.release = rng.nextRange(0.22, 0.4);
          state.sidechain.source = 'internal';

          state.transient.attack = rng.nextRange(1.02, 1.22);
          state.transient.sustain = rng.nextRange(0.7, 0.85);
      ...
      },
      {
        label: 'Lead',
        configure: (state) => {
          state.global.bpm = Math.round(rng.nextRange(96, 148));
          state.global.swing = Math.round(rng.nextRange(0, 24));
          state.global.masterVolume = rng.nextRange(0.68, 0.84);
          state.global.limiterCeiling = rng.nextRange(-1.6, -0.6);
          state.global.key = rng.pick(KEY_ROOTS);
          state.global.quality = rng.next() > 0.4 ? 2 : 1;

          state.oscillators.oscA.table = rng.pick(oscATables);
          state.oscillators.oscB.table = rng.pick(oscBTables);
          state.oscillators.oscA.morph = rng.nextRange(0.15, 0.7);
          state.oscillators.oscB.morph = rng.nextRange(0.25, 0.75);
          state.oscillators.oscA.tune = Math.round(rng.nextRange(-7, 7));
          state.oscillators.oscB.tune = rng.pick([0, 7, 12]);
          state.oscillators.oscA.fine = rng.nextRange(-0.1, 0.1);
          state.oscillators.oscB.fine = rng.nextRange(-0.12, 0.12);
          state.oscillators.oscA.level = rng.nextRange(0.58, 0.82);
          state.oscillators.oscB.level = rng.nextRange(0.42, 0.66);
          state.oscillators.oscA.pan = rng.nextRange(-0.22, 0.22);
          state.oscillators.oscB.pan = rng.nextRange(-0.22, 0.22);

          state.sub.level = rng.nextRange(0.28, 0.52);
          state.sub.squareBlend = rng.nextRange(0.08, 0.32);
          state.sub.saturation = rng.nextRange(0.2, 0.42);
          state.sub.legato = rng.next() > 0.4;

          state.fm.ratio = rng.pick([1, 1.5, 2, 3]);
          state.fm.index = rng.nextRange(0.18, 0.42);
          state.fm.attack = rng.nextRange(0.01, 0.04);
          state.fm.decay = rng.nextRange(0.2, 0.5);
          state.fm.velocityToIndex = rng.nextRange(0.06, 0.18);

          state.filters.filter1.type = rng.pick(['lowpass', 'bandpass']);
          state.filters.filter1.cutoff = rng.nextRange(480, 2600);
          state.filters.filter1.resonance = rng.nextRange(0.4, 0.9);
          state.filters.filter1.envAmount = rng.nextRange(180, 420);
          state.filters.filter1.drive = rng.nextRange(1, 1.28);
          state.filters.filter1.keytrack = rng.nextRange(0.25, 0.55);

          state.filters.filter2.type = rng.pick(['bandpass', 'highpass']);
          state.filters.filter2.cutoff = rng.nextRange(1200, 3800);
          state.filters.filter2.resonance = rng.nextRange(0.45, 1.05);
          state.filters.filter2.envAmount = rng.nextRange(120, 320);
          state.filters.filter2.drive = rng.nextRange(0.95, 1.18);
          state.filters.filter2.keytrack = rng.nextRange(0.2, 0.5);
          state.filters.routing.serial = rng.next() > 0.5;
          state.filters.routing.mix = rng.nextRange(0.5, 0.85);

          state.envelopes.amp.attack = rng.nextRange(0.01, 0.08);
          state.envelopes.amp.decay = rng.nextRange(0.18, 0.4);
          state.envelopes.amp.sustain = rng.nextRange(0.45, 0.75);
          state.envelopes.amp.release = rng.nextRange(0.3, 0.55);
          state.envelopes.amp.curve = rng.nextRange(1, 1.4);

          state.envelopes.filter.attack = rng.nextRange(0.01, 0.06);
          state.envelopes.filter.decay = rng.nextRange(0.24, 0.5);
          state.envelopes.filter.sustain = rng.nextRange(0.3, 0.6);
          state.envelopes.filter.release = rng.nextRange(0.3, 0.6);
          state.envelopes.filter.curve = rng.nextRange(1, 1.5);

          state.envelopes.mod.attack = rng.nextRange(0.008, 0.05);
          state.envelopes.mod.decay = rng.nextRange(0.2, 0.5);
          state.envelopes.mod.sustain = rng.nextRange(0.1, 0.45);
          state.envelopes.mod.release = rng.nextRange(0.2, 0.45);
          state.envelopes.mod.curve = rng.nextRange(0.9, 1.4);

          state.lfo = [
            { shape: 'sine', rate: rng.nextRange(4, 8), sync: false, amount: rng.nextRange(0.03, 0.1), destination: 'oscA.morph', phase: rng.nextRange(0, 1) },
            { shape: 'triangle', rate: rng.nextRange(0.2, 0.45), sync: true, amount: rng.nextRange(0.2, 0.4), destination: 'filter2.cutoff', phase: rng.nextRange(0, 1) },
            { shape: 'square', rate: rng.nextRange(0.3, 0.6), sync: true, amount: rng.nextRange(0.1, 0.28), destination: 'delay.mix', phase: rng.nextRange(0, 1) }
          ];

          state.fx.chorus.depth = rng.nextRange(0.18, 0.32);
          state.fx.chorus.rate = rng.nextRange(0.35, 0.9);
          state.fx.delay.time = rng.nextRange(0.22, 0.48);
          state.fx.delay.feedback = rng.nextRange(0.22, 0.42);
          state.fx.delay.mix = rng.nextRange(0.22, 0.4);
          state.fx.delay.pingPong = rng.next() > 0.6;
          state.fx.reverb.mix = rng.nextRange(0.2, 0.38);

          state.sidechain.amount = rng.nextRange(0.12, 0.26);
          state.sidechain.attack = rng.nextRange(0.03, 0.08);
          state.sidechain.release = rng.nextRange(0.24, 0.42);
          state.sidechain.source = rng.next() > 0.5 ? 'internal' : 'external';

          state.transient.attack = rng.nextRange(1.02, 1.18);
          state.transient.sustain = rng.nextRange(0.82, 0.96);
      ...
      },
      {
        label: 'Texture',
        configure: (state) => {
          state.global.bpm = Math.round(rng.nextRange(60, 128));
          state.global.swing = Math.round(rng.nextRange(0, 30));
          state.global.masterVolume = rng.nextRange(0.6, 0.78);
          state.global.limiterCeiling = rng.nextRange(-3, -1);
          state.global.key = rng.pick(KEY_ROOTS);
          state.global.quality = 2;

          state.oscillators.oscA.table = rng.pick(oscATables);
          state.oscillators.oscB.table = rng.pick(oscBTables);
          state.oscillators.oscA.morph = rng.nextRange(0.05, 0.95);
          state.oscillators.oscB.morph = rng.nextRange(0.1, 0.95);
          state.oscillators.oscA.tune = Math.round(rng.nextRange(-12, 12));
          state.oscillators.oscB.tune = Math.round(rng.nextRange(-7, 19));
          state.oscillators.oscA.fine = rng.nextRange(-0.2, 0.2);
          state.oscillators.oscB.fine = rng.nextRange(-0.2, 0.2);
          state.oscillators.oscA.level = rng.nextRange(0.4, 0.7);
          state.oscillators.oscB.level = rng.nextRange(0.4, 0.7);
          state.oscillators.oscA.pan = rng.nextRange(-0.4, 0.4);
          state.oscillators.oscB.pan = rng.nextRange(-0.4, 0.4);

          state.sub.level = rng.nextRange(0.2, 0.5);
          state.sub.squareBlend = rng.nextRange(0.2, 0.6);
          state.sub.saturation = rng.nextRange(0.1, 0.45);
          state.sub.legato = rng.next() > 0.5;

          state.fm.ratio = rng.pick([0.75, 1.25, 2.5, 3.5]);
          state.fm.index = rng.nextRange(0.12, 0.5);
          state.fm.attack = rng.nextRange(0.01, 0.12);
          state.fm.decay = rng.nextRange(0.3, 0.8);
          state.fm.velocityToIndex = rng.nextRange(0.04, 0.18);

          state.filters.filter1.type = rng.pick(['lowpass', 'bandpass', 'highpass']);
          state.filters.filter1.cutoff = rng.nextRange(120, 4200);
          state.filters.filter1.resonance = rng.nextRange(0.2, 1.3);
          state.filters.filter1.envAmount = rng.nextRange(60, 380);
          state.filters.filter1.drive = rng.nextRange(0.9, 1.35);
          state.filters.filter1.keytrack = rng.nextRange(0.1, 0.6);

          state.filters.filter2.type = rng.pick(['bandpass', 'highpass']);
          state.filters.filter2.cutoff = rng.nextRange(300, 6200);
          state.filters.filter2.resonance = rng.nextRange(0.3, 1.4);
          state.filters.filter2.envAmount = rng.nextRange(40, 320);
          state.filters.filter2.drive = rng.nextRange(0.9, 1.25);
          state.filters.filter2.keytrack = rng.nextRange(0.1, 0.6);
          state.filters.routing.serial = rng.next() > 0.5;
          state.filters.routing.mix = rng.nextRange(0.4, 0.9);

          state.envelopes.amp.attack = rng.nextRange(0.03, 0.4);
          state.envelopes.amp.decay = rng.nextRange(0.25, 0.9);
          state.envelopes.amp.sustain = rng.nextRange(0.4, 0.9);
          state.envelopes.amp.release = rng.nextRange(0.6, 1.4);
          state.envelopes.amp.curve = rng.nextRange(0.9, 1.5);

          state.envelopes.filter.attack = rng.nextRange(0.02, 0.2);
          state.envelopes.filter.decay = rng.nextRange(0.25, 0.9);
          state.envelopes.filter.sustain = rng.nextRange(0.3, 0.7);
          state.envelopes.filter.release = rng.nextRange(0.5, 1.2);
          state.envelopes.filter.curve = rng.nextRange(0.9, 1.6);

          state.envelopes.mod.attack = rng.nextRange(0.02, 0.22);
          state.envelopes.mod.decay = rng.nextRange(0.3, 1.1);
          state.envelopes.mod.sustain = rng.nextRange(0.2, 0.65);
          state.envelopes.mod.release = rng.nextRange(0.4, 1);
          state.envelopes.mod.curve = rng.nextRange(0.85, 1.5);

          state.lfo = [
            { shape: 'sample-hold', rate: rng.nextRange(0.18, 1.5), sync: false, amount: rng.nextRange(0.15, 0.4), destination: 'filter1.cutoff', phase: 0 },
            { shape: 'sine', rate: rng.nextRange(0.05, 0.18), sync: true, amount: rng.nextRange(0.22, 0.48), destination: 'oscA.morph', phase: rng.nextRange(0, 1) },
            { shape: 'triangle', rate: rng.nextRange(0.08, 0.26), sync: true, amount: rng.nextRange(0.18, 0.4), destination: 'reverb.mix', phase: rng.nextRange(0, 1) }
          ];

          state.fx.chorus.depth = rng.nextRange(0.2, 0.48);
          state.fx.chorus.rate = rng.nextRange(0.12, 0.6);
          state.fx.delay.time = rng.nextRange(0.4, 0.95);
          state.fx.delay.feedback = rng.nextRange(0.3, 0.6);
          state.fx.delay.mix = rng.nextRange(0.28, 0.5);
          state.fx.delay.pingPong = rng.next() > 0.5;
          state.fx.reverb.mix = rng.nextRange(0.38, 0.7);

          state.sidechain.amount = rng.nextRange(0.05, 0.22);
          state.sidechain.attack = rng.nextRange(0.05, 0.14);
          state.sidechain.release = rng.nextRange(0.4, 0.9);
          state.sidechain.source = rng.next() > 0.6 ? 'external' : 'internal';

          state.transient.attack = rng.nextRange(0.9, 1.2);
          state.transient.sustain = rng.nextRange(0.85, 1.1);

          fillModMatrix(state, 3, 6);
        }
      }
    ];

    let iterations = 0;
    while (this.presets.size < desired && iterations < 600) {
      const archetype = archetypes[iterations % archetypes.length];
      const presetState = deepClone(createDefaultState());
      const startSeed = rng.seed;
      archetype.configure(presetState);
      const adjective = adjectives[iterations % adjectives.length];
      const noun = nouns[Math.floor(iterations / adjectives.length) % nouns.length];
      const baseName = `${adjective} ${noun}`;
      let name = `${baseName} (${archetype.label})`;
      let suffix = 2;
      while (this.presets.has(name)) {
        name = `${baseName} (${archetype.label} ${suffix})`;
        suffix += 1;
      }
      presetState.meta = {
        ...(presetState.meta ?? {}),
      ...
      this.presets.set(name, presetState);
      iterations += 1;
    }
  }
  get(name) {
...
function createDefaultState() {
  return {
    ...
    master: {
      lfo: { enabled: false, tempo: 140, depth: 0.3, multiplier: 1, shape: 'sine' },
      filters: { hp: 20, lp: 18000 }
    },
    oscillators: {
...
class MothyGSynth {
  ...
  ensureMasterState() {
    if (!this.state.master || typeof this.state.master !== 'object') {
      this.state.master = { lfo: {}, filters: {} };
    }
    const master = this.state.master;
    if (!master.lfo || typeof master.lfo !== 'object') {
      master.lfo = {};
    }
    if (!master.filters || typeof master.filters !== 'object') {
      master.filters = {};
    }
    const defaultTempo = Number.isFinite(this.state.global?.bpm) ? this.state.global.bpm : 140;
    const lfo = master.lfo;
    if (typeof lfo.enabled !== 'boolean') lfo.enabled = false;
    if (!Number.isFinite(lfo.tempo)) lfo.tempo = defaultTempo;
    if (!Number.isFinite(lfo.depth)) lfo.depth = 0.3;
    if (!Number.isFinite(lfo.multiplier)) lfo.multiplier = 1;
    if (typeof lfo.shape !== 'string') lfo.shape = 'sine';
    lfo.depth = clamp(lfo.depth, 0, 0.5);
    lfo.multiplier = clamp(lfo.multiplier, 0.0625, 16);
    if (!Number.isFinite(lfo.tempo) || lfo.tempo <= 0) lfo.tempo = defaultTempo;

    const filters = master.filters;
    if (!Number.isFinite(filters.hp)) filters.hp = 20;
    if (!Number.isFinite(filters.lp)) filters.lp = 18000;
    filters.hp = clamp(filters.hp, 20, 20000);
    const lpMax = this.ctx?.sampleRate ? this.ctx.sampleRate / 2 : 20000;
    filters.lp = clamp(filters.lp, 200, lpMax);
    if (filters.lp < filters.hp + 10) {
      filters.lp = clamp(filters.hp + 10, 200, lpMax);
    }
  }
  ensureKey() {
  ...
  applyState() {
    ...
    if (this.masterBus) {
      ...
      this.masterBus.setFilters(this.state.master.filters);
      const lfoSettings = this.state.master.lfo ?? {};
      const tempo = Number.isFinite(lfoSettings.tempo) ? lfoSettings.tempo : this.state.global.bpm;
      this.masterBus.setLFO({
        enabled: lfoSettings.enabled,
        tempo,
        depth: lfoSettings.depth,
        multiplier: lfoSettings.multiplier,
        shape: lfoSettings.shape
      });
    }
...
class UIController {
  constructor() {
    ...
    this.presetNames = [];
    this.masterLfoButtons = Array.from(document.querySelectorAll('[data-lfo-multiplier]'));
    this.masterLfoReadout = document.getElementById('master-lfo-readout');
  ...
  ensureMasterState() {
    ...
    const bpm = Number.isFinite(this.state.global?.bpm) ? this.state.global.bpm : 140;
    const lfo = master.lfo;
    if (typeof lfo.enabled !== 'boolean') lfo.enabled = false;
    if (!Number.isFinite(lfo.tempo)) lfo.tempo = bpm;
    if (!Number.isFinite(lfo.depth)) lfo.depth = 0.3;
    if (!Number.isFinite(lfo.multiplier)) lfo.multiplier = 1;
    if (typeof lfo.shape !== 'string') lfo.shape = 'sine';
    lfo.depth = clamp(lfo.depth, 0, 0.5);
    lfo.multiplier = clamp(lfo.multiplier, 0.0625, 16);
    if (!Number.isFinite(lfo.tempo) || lfo.tempo <= 0) lfo.tempo = bpm;

    const filters = master.filters;
    if (!Number.isFinite(filters.hp)) filters.hp = 20;
    if (!Number.isFinite(filters.lp)) filters.lp = 18000;
    filters.hp = clamp(filters.hp, 20, 20000);
    filters.lp = clamp(filters.lp, 200, 20000);
    if (filters.lp < filters.hp + 10) {
      filters.lp = clamp(filters.hp + 10, 200, 20000);
    }
  }
  ensureUIState() {
    if (!this.state.ui) {
  ...
  updateMasterLfoMultiplierUI() {
    if (!Array.isArray(this.masterLfoButtons)) return;
    const lfo = this.state.master?.lfo ?? {};
    const multiplier = Number.isFinite(lfo.multiplier) ? lfo.multiplier : 1;
    this.masterLfoButtons.forEach((button) => {
      const value = Number(button.dataset.lfoMultiplier);
      const isActive = Math.abs((value ?? 1) - multiplier) < 1e-3;
      button.classList.toggle('is-active', isActive);
      button.setAttribute('aria-pressed', String(isActive));
    });
  }
  updateMasterLfoReadout() {
    this.ensureMasterState();
    const lfo = this.state.master.lfo;
    const baseTempo = Number.isFinite(lfo.tempo) ? lfo.tempo : (Number(this.state.global?.bpm) || 140);
    const multiplier = Number.isFinite(lfo.multiplier) ? lfo.multiplier : 1;
    const effective = clamp(baseTempo * multiplier, 0.01, 960);
    const format = (value) => {
      if (!Number.isFinite(value)) return '--';
      return Math.abs(Math.round(value) - value) < 0.05 ? String(Math.round(value)) : value.toFixed(1);
    };
    if (this.masterLfoReadout) {
      const multLabel = Math.abs(Math.round(multiplier) - multiplier) < 0.001 ? Math.round(multiplier) : multiplier;
      this.masterLfoReadout.textContent = `${format(effective)} BPM (base ${format(baseTempo)} × ${multLabel})`;
    }
    const tempoInput = document.getElementById('master-lfo-tempo');
    const depthInput = document.getElementById('master-lfo-depth');
    const enabledInput = document.getElementById('master-lfo-enabled');
    if (tempoInput && Number(tempoInput.value) !== baseTempo) tempoInput.value = baseTempo;
    if (depthInput && Number(depthInput.value) !== lfo.depth) depthInput.value = lfo.depth;
    if (enabledInput) enabledInput.checked = !!lfo.enabled;
    this.updateMasterLfoMultiplierUI();
  }
  setupKeyControls() {
  ...
  bindMasterControls() {
    ...
    const lfoEnabled = document.getElementById('master-lfo-enabled');
    const lfoTempo = document.getElementById('master-lfo-tempo');
    const lfoDepth = document.getElementById('master-lfo-depth');
    const lfoButtonWrap = document.querySelector('.master-lfo-multipliers');
    const masterHpf = document.getElementById('master-hpf');
    const masterLpf = document.getElementById('master-lpf');
    const masterDelayMix = document.getElementById('master-delay-mix');

    if (lfoEnabled) {
      this.applyTooltip(lfoEnabled.closest('label') ?? lfoEnabled, this.tip('master.lfo.enabled'));
      lfoEnabled.addEventListener('change', () => {
        this.ensureMasterState();
        this.state.master.lfo.enabled = !!lfoEnabled.checked;
        if (this.synth.initialised) {
          const lfo = this.state.master.lfo;
          this.synth.masterBus.setLFO({
            enabled: lfo.enabled,
            tempo: lfo.tempo,
            depth: lfo.depth,
            multiplier: lfo.multiplier,
            shape: lfo.shape
          });
        }
        this.updateMasterLfoReadout();
        this.persistState();
      });
    }
    if (lfoTempo) {
      this.applyTooltip(lfoTempo.closest('label') ?? lfoTempo, this.tip('master.lfo.tempo'));
      lfoTempo.addEventListener('input', () => {
        this.ensureMasterState();
        const tempo = Number(lfoTempo.value);
        this.state.master.lfo.tempo = tempo;
        if (this.synth.initialised) {
          const lfo = this.state.master.lfo;
          this.synth.masterBus.setLFO({
            enabled: lfo.enabled,
    ...
    if (lfoDepth) {
      this.applyTooltip(lfoDepth.closest('label') ?? lfoDepth, this.tip('master.lfo.depth'));
      lfoDepth.addEventListener('input', () => {
        this.ensureMasterState();
        const depth = Number(lfoDepth.value);
        this.state.master.lfo.depth = depth;
        if (this.synth.initialised) {
          const lfo = this.state.master.lfo;
          this.synth.masterBus.setLFO({
            enabled: lfo.enabled,
    ...
    if (lfoButtonWrap) {
      this.applyTooltip(lfoButtonWrap, this.tip('master.lfo.multipliers'));
    }
    if (Array.isArray(this.masterLfoButtons)) {
      this.masterLfoButtons.forEach((button) => {
        button.addEventListener('click', () => {
          const value = Number(button.dataset.lfoMultiplier);
          if (!Number.isFinite(value)) return;
          this.ensureMasterState();
          this.state.master.lfo.multiplier = value;
          if (this.synth.initialised) {
            const lfo = this.state.master.lfo;
            this.synth.masterBus.setLFO({
              enabled: lfo.enabled,
    ...
    if (masterHpf) {
      this.applyTooltip(masterHpf.closest('label') ?? masterHpf, this.tip('master.filters.hp'));
      masterHpf.addEventListener('input', () => {
        this.ensureMasterState();
        this.state.master.filters.hp = Number(masterHpf.value);
        if (this.synth.initialised) {
          this.synth.masterBus.setFilters(this.state.master.filters);
        }
        this.persistState();
      });
    }
    if (masterLpf) {
      this.applyTooltip(masterLpf.closest('label') ?? masterLpf, this.tip('master.filters.lp'));
      masterLpf.addEventListener('input', () => {
        this.ensureMasterState();
        this.state.master.filters.lp = Number(masterLpf.value);
        if (this.synth.initialised) {
          this.synth.masterBus.setFilters(this.state.master.filters);
        }
        this.persistState();
    ...
    if (masterDelayMix) {
      this.applyTooltip(masterDelayMix.closest('label') ?? masterDelayMix, this.tip('master.delayMix'));
      masterDelayMix.addEventListener('input', () => {
        if (!this.state.fx || !this.state.fx.delay) {
          this.state.fx = this.state.fx ?? {};
          this.state.fx.delay = this.state.fx.delay ?? {};
        }
        this.state.fx.delay.mix = Number(masterDelayMix.value);
        if (this.synth.initialised) {
          this.synth.fx.setDelay(this.state.fx.delay);
        }
        this.persistState();
      });
    }
  ...
  renderPresetToolbar(presetNames) {
    const container = document.getElementById('preset-selector-bar');
    if (!container) return;
    container.innerHTML = '';
    const title = document.createElement('span');
    title.className = 'preset-toolbar-title';
    ...
    selectEl.id = 'preset-select';
    selectEl.setAttribute('aria-label', 'Factory Preset');
    ...
    this.presetNames = Array.isArray(presetNames) ? [...presetNames] : [];
    ...
    if (currentPresetName && this.presetNames.includes(currentPresetName)) {
      selectEl.value = currentPresetName;
    } else if (this.presetNames.length) {
      selectEl.value = this.presetNames[0];
    }

    const disableNav = this.presetNames.length <= 1;
    selectEl.disabled = this.presetNames.length === 0;
    prevBtn.disabled = disableNav;
    nextBtn.disabled = disableNav;

    const stepPreset = (direction) => {
      if (!this.presetNames.length) return;
      const current = selectEl.value;
      let index = this.presetNames.indexOf(current);
      if (index === -1) {
        index = direction > 0 ? -1 : 0;
      }
      const nextIndex = (index + direction + this.presetNames.length) % this.presetNames.length;
      const target = this.presetNames[nextIndex];
      if (!target) return;
      selectEl.value = target;
      this.loadPresetByName(target);
    };
  ...
  applyStateToUI() {
    ...
    const filters = this.state.master?.filters ?? {};
    const masterHpf = document.getElementById('master-hpf');
    const masterLpf = document.getElementById('master-lpf');
    if (masterHpf && Number(masterHpf.value) !== filters.hp) masterHpf.value = filters.hp;
    if (masterLpf && Number(masterLpf.value) !== filters.lp) masterLpf.value = filters.lp;
    const delayMix = this.state.fx?.delay?.mix ?? 0;
    const masterDelayMix = document.getElementById('master-delay-mix');
    if (masterDelayMix && Number(masterDelayMix.value) !== delayMix) masterDelayMix.value = delayMix;
    const delayMixPanel = document.getElementById('delay-mix');
    if (delayMixPanel && Number(delayMixPanel.value) !== delayMix) delayMixPanel.value = delayMix;
    if (this.tooltipsToggleInput) {
