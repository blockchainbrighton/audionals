...
class UIController {
  ...
  buildOscPanel() {
    ...
    panel.innerHTML = '';
    const buildOsc = (key) => {
      const oscState = this.state.oscillators[key];
      const catalog = OSCILLATOR_CATALOG[key];
      const tableControl = this.controlElement({
        label: `${catalog.name} Table`,
        type: 'select',
        options: catalog.tables,
        value: oscState.table,
        id: `${key}-table`
      });
      tableControl.input.addEventListener('change', () => {
        oscState.table = tableControl.input.value;
        this.updateEngine();
      });
      panel.appendChild(tableControl.wrap);

      const morphControl = this.controlElement({
        label: 'Morph',
        type: 'range',
        min: 0,
        max: 1,
        step: 0.01,
        value: oscState.morph,
        id: `${key}-morph`
      });
      morphControl.input.addEventListener('input', () => {
        oscState.morph = Number(morphControl.input.value);
        this.updateEngine();
      });
      panel.appendChild(morphControl.wrap);

      const tuneControl = this.controlElement({
        label: 'Coarse Tune',
        type: 'number',
        min: -24,
        max: 24,
      ...
      tuneControl.input.addEventListener('input', () => {
        oscState.tune = Number(tuneControl.input.value);
        this.updateEngine();
      });
      panel.appendChild(tuneControl.wrap);

      const fineControl = this.controlElement({
        label: 'Fine',
        type: 'range',
        min: -1,
        max: 1,
      ...
      fineControl.input.addEventListener('input', () => {
        oscState.fine = Number(fineControl.input.value);
        this.updateEngine();
      });
      panel.appendChild(fineControl.wrap);

      const levelControl = this.controlElement({
        label: 'Level',
        type: 'range',
        min: 0,
        max: 1,
        step: 0.01,
        value: oscState.level,
        id: `${key}-level`
      });
      levelControl.input.addEventListener('input', () => {
        oscState.level = Number(levelControl.input.value);
        this.updateEngine();
      });
      panel.appendChild(levelControl.wrap);

      const panControl = this.controlElement({
        label: 'Pan',
        type: 'range',
        min: -1,
        max: 1,
      ...
      panControl.input.addEventListener('input', () => {
        oscState.pan = Number(panControl.input.value);
        this.updateEngine();
      });
      panel.appendChild(panControl.wrap);
    };
    buildOsc('oscA');
    buildOsc('oscB');
  }
  buildSubPanel() {
    ...
    panel.innerHTML = '';
    const subState = this.state.sub;
    const level = this.controlElement({ label: 'Sub Level', type: 'range', min: 0, max: 1, step: 0.01, value: subState.level, id: 'sub-level' });
    level.input.addEventListener('input', () => {
      subState.level = Number(level.input.value);
      this.updateEngine();
    });
    panel.appendChild(level.wrap);

    const square = this.controlElement({ label: 'Square Blend %', type: 'range', min: 0, max: 1, step: 0.01, value: subState.squareBlend, id: 'sub-square' });
    square.input.addEventListener('input', () => {
      subState.squareBlend = Number(square.input.value);
      this.updateEngine();
    });
    panel.appendChild(square.wrap);

    const saturation = this.controlElement({ label: 'Saturation', type: 'range', min: 0, max: 1, step: 0.01, value: subState.saturation, id: 'sub-sat' });
    saturation.input.addEventListener('input', () => {
      subState.saturation = Number(saturation.input.value);
      this.updateEngine();
    });
    panel.appendChild(saturation.wrap);

    const legato = this.controlElement({ label: 'Mono/Legato', type: 'checkbox', value: subState.legato, id: 'sub-legato' });
    legato.input.addEventListener('change', () => {
      subState.legato = legato.input.checked;
      this.updateEngine();
    });
    panel.appendChild(legato.wrap);
  }
  buildFMPanel() {
    const panel = document.getElementById('osc-panel');
    const fmState = this.state.fm;
    const ratio = this.controlElement({ label: 'FM Ratio', type: 'range', min: 0.25, max: 8, step: 0.01, value: fmState.ratio, id: 'fm-ratio' });
    ratio.input.addEventListener('input', () => {
      fmState.ratio = Number(ratio.input.value);
      this.updateEngine();
    });
    panel.appendChild(ratio.wrap);

    const index = this.controlElement({ label: 'FM Index', type: 'range', min: 0, max: 2, step: 0.01, value: fmState.index, id: 'fm-index' });
    index.input.addEventListener('input', () => {
      fmState.index = Number(index.input.value);
      this.updateEngine();
    });
    panel.appendChild(index.wrap);

    const attack = this.controlElement({ label: 'Index Attack', type: 'range', min: 0, max: 1, step: 0.001, value: fmState.attack, id: 'fm-attack' });
    attack.input.addEventListener('input', () => {
      fmState.attack = Number(attack.input.value);
      this.updateEngine();
    });
    panel.appendChild(attack.wrap);

    const decay = this.controlElement({ label: 'Index Decay', type: 'range', min: 0, max: 1.5, step: 0.001, value: fmState.decay, id: 'fm-decay' });
    decay.input.addEventListener('input', () => {
      fmState.decay = Number(decay.input.value);
      this.updateEngine();
    });
    panel.appendChild(decay.wrap);

    const velIndex = this.controlElement({ label: 'Vel→Index', type: 'range', min: 0, max: 1, step: 0.01, value: fmState.velocityToIndex, id: 'fm-vel' });
    velIndex.input.addEventListener('input', () => {
      fmState.velocityToIndex = Number(velIndex.input.value);
      this.updateEngine();
    });
    panel.appendChild(velIndex.wrap);
  }
  buildFilterPanel() {
    const panel = document.getElementById('filter-panel');
    panel.innerHTML = '';
    const filter1 = this.state.filters.filter1;
    const filter2 = this.state.filters.filter2;
    const routing = this.state.filters.routing;

    const addFilterControls = (filter, idPrefix, options) => {
      const typeCtrl = this.controlElement({ label: `${options.label} Type`, type: 'select', options: [
        { value: 'lowpass', label: 'Low-pass' },
        { value: 'bandpass', label: 'Band-pass' },
        { value: 'highpass', label: 'High-pass' }
      ], value: filter.type, id: `${idPrefix}-type` });
      typeCtrl.input.addEventListener('change', () => {
        filter.type = typeCtrl.input.value;
        this.updateEngine();
      });
      panel.appendChild(typeCtrl.wrap);

      const cutoff = this.controlElement({ label: 'Cutoff (Hz)', type: 'number', min: 20, max: 20000, step: 1, value: filter.cutoff, id: `${idPrefix}-cutoff` });
      cutoff.input.addEventListener('input', () => {
        filter.cutoff = Number(cutoff.input.value);
        this.updateEngine();
      });
      panel.appendChild(cutoff.wrap);

      const res = this.controlElement({ label: 'Resonance', type: 'range', min: 0.1, max: 2.5, step: 0.01, value: filter.resonance, id: `${idPrefix}-res` });
      res.input.addEventListener('input', () => {
        filter.resonance = Number(res.input.value);
        this.updateEngine();
      });
      panel.appendChild(res.wrap);

      const drive = this.controlElement({ label: 'Drive', type: 'range', min: 1, max: 2.5, step: 0.01, value: filter.drive, id: `${idPrefix}-drive` });
      drive.input.addEventListener('input', () => {
        filter.drive = Number(drive.input.value);
        this.updateEngine();
      });
      panel.appendChild(drive.wrap);

      const envAmount = this.controlElement({ label: 'Env Amount', type: 'number', min: 0, max: 2000, step: 1, value: filter.envAmount, id: `${idPrefix}-env` });
      envAmount.input.addEventListener('input', () => {
        filter.envAmount = Number(envAmount.input.value);
        this.updateEngine();
      });
      panel.appendChild(envAmount.wrap);

      const keytrack = this.controlElement({ label: 'Keytrack %', type: 'range', min: 0, max: 1, step: 0.01, value: filter.keytrack, id: `${idPrefix}-keytrack` });
      keytrack.input.addEventListener('input', () => {
        filter.keytrack = Number(keytrack.input.value);
        this.updateEngine();
      });
      panel.appendChild(keytrack.wrap);
    };

    addFilterControls(filter1, 'filter1', { label: 'Filter 1' });
    addFilterControls(filter2, 'filter2', { label: 'Filter 2' });

    const serial = this.controlElement({ label: 'Serial Routing', type: 'checkbox', value: routing.serial, id: 'filters-serial' });
    serial.input.addEventListener('change', () => {
      routing.serial = serial.input.checked;
      this.updateEngine();
    });
    panel.appendChild(serial.wrap);

    const mix = this.controlElement({ label: 'Mix', type: 'range', min: 0, max: 1, step: 0.01, value: routing.mix, id: 'filters-mix' });
    mix.input.addEventListener('input', () => {
      routing.mix = Number(mix.input.value);
      this.updateEngine();
    });
    panel.appendChild(mix.wrap);
  }
  buildEnvelopePanel() {
    ...
    envelopeKeys.forEach((key) => {
      const env = this.state.envelopes[key];
      ['attack', 'decay', 'sustain', 'release', 'curve'].forEach((stage) => {
        const range = stage === 'curve' ? { min: 0.5, max: 3, step: 0.01 } : { min: 0, max: 2, step: 0.001 };
        if (stage === 'sustain') range.max = 1;
        const ctrl = this.controlElement({
        ...
        ctrl.input.addEventListener('input', () => {
          env[stage] = Number(ctrl.input.value);
          this.updateEngine();
        });
        panel.appendChild(ctrl.wrap);
      });
    });
  }
  buildFXPanel() {
    ...
    panel.innerHTML = '';
    const { chorus, delay, reverb } = this.state.fx;
    if (!this.state.sidechain) {
      this.state.sidechain = { source: 'internal', amount: 0.2, attack: 0.04, release: 0.24 };
    }

    const chorusDepth = this.controlElement({ label: 'Chorus Depth', type: 'range', min: 0, max: 1, step: 0.01, value: chorus.depth, id: 'chorus-depth' });
    chorusDepth.input.addEventListener('input', () => {
      chorus.depth = Number(chorusDepth.input.value);
      this.updateEngine();
    });
    panel.appendChild(chorusDepth.wrap);

    const chorusRate = this.controlElement({ label: 'Chorus Rate', type: 'range', min: 0.05, max: 5, step: 0.01, value: chorus.rate, id: 'chorus-rate' });
    chorusRate.input.addEventListener('input', () => {
      chorus.rate = Number(chorusRate.input.value);
      this.updateEngine();
    });
    panel.appendChild(chorusRate.wrap);

    const delayTime = this.controlElement({ label: 'Delay Time', type: 'range', min: 0.05, max: 1.5, step: 0.01, value: delay.time, id: 'delay-time' });
    delayTime.input.addEventListener('input', () => {
      delay.time = Number(delayTime.input.value);
      this.updateEngine();
    });
    panel.appendChild(delayTime.wrap);

    const delayFeedback = this.controlElement({ label: 'Delay Feedback', type: 'range', min: 0, max: 0.95, step: 0.01, value: delay.feedback, id: 'delay-feedback' });
    delayFeedback.input.addEventListener('input', () => {
      delay.feedback = Number(delayFeedback.input.value);
      this.updateEngine();
    });
    panel.appendChild(delayFeedback.wrap);

    const delayMix = this.controlElement({ label: 'Delay Mix', type: 'range', min: 0, max: 1, step: 0.01, value: delay.mix, id: 'delay-mix' });
    delayMix.input.addEventListener('input', () => {
      delay.mix = Number(delayMix.input.value);
      this.updateEngine();
    });
    panel.appendChild(delayMix.wrap);

    const reverbMix = this.controlElement({ label: 'Reverb Mix', type: 'range', min: 0, max: 1, step: 0.01, value: reverb.mix, id: 'reverb-mix' });
    reverbMix.input.addEventListener('input', () => {
      reverb.mix = Number(reverbMix.input.value);
      this.updateEngine();
    });
    panel.appendChild(reverbMix.wrap);

    const sidechain = this.state.sidechain;
    const scAmount = this.controlElement({ label: 'Sidechain Amt', type: 'range', min: 0, max: 1, step: 0.01, value: sidechain.amount, id: 'sidechain-amount' });
    scAmount.input.addEventListener('input', () => {
      sidechain.amount = Number(scAmount.input.value);
      this.updateEngine();
    });
    panel.appendChild(scAmount.wrap);

    const scAttack = this.controlElement({ label: 'SC Attack', type: 'range', min: 0.005, max: 0.2, step: 0.001, value: sidechain.attack, id: 'sidechain-attack' });
    scAttack.input.addEventListener('input', () => {
      sidechain.attack = Number(scAttack.input.value);
      this.updateEngine();
    });
    panel.appendChild(scAttack.wrap);

    const scRelease = this.controlElement({ label: 'SC Release', type: 'range', min: 0.05, max: 1.2, step: 0.01, value: sidechain.release, id: 'sidechain-release' });
    scRelease.input.addEventListener('input', () => {
      sidechain.release = Number(scRelease.input.value);
      this.updateEngine();
    });
    panel.appendChild(scRelease.wrap);

    const scSource = this.controlElement({ label: 'SC Source', type: 'select', options: [
      { value: 'internal', label: 'Internal Drums' },
      { value: 'external', label: 'Master Mix' }
    ], value: sidechain.source, id: 'sidechain-source' });
    scSource.input.addEventListener('change', () => {
      sidechain.source = scSource.input.value;
      this.updateEngine();
    });
    panel.appendChild(scSource.wrap);
  }
  buildModMatrixPanel() {
    const panel = document.getElementById('mod-panel');
    panel.innerHTML = '';
    for (let i = 0; i < 8; i += 1) {
      const row = this.state.modMatrix[i] ?? { source: 'LFO1', destination: 'oscA.morph', amount: 0 };
      this.state.modMatrix[i] = row;
      const source = this.controlElement({ label: `Source ${i + 1}`, type: 'select', options: [
        'LFO1', 'LFO2', 'LFO3', 'MODENV', 'Keytrack', 'Velocity', 'Aftertouch', 'EnvelopeFollower', 'Random'
      ], value: row.source, id: `mod-${i}-source` });
      source.input.addEventListener('change', () => {
        row.source = source.input.value;
        this.updateEngine();
      });
      panel.appendChild(source.wrap);

      const destination = this.controlElement({ label: `Destination ${i + 1}`, type: 'text', value: row.destination, id: `mod-${i}-dest` });
      destination.input.addEventListener('change', () => {
        row.destination = destination.input.value;
        this.updateEngine();
      });
      panel.appendChild(destination.wrap);

      const amount = this.controlElement({ label: `Amount ${i + 1}`, type: 'range', min: -1, max: 1, step: 0.01, value: row.amount, id: `mod-${i}-amount` });
      amount.input.addEventListener('input', () => {
        row.amount = Number(amount.input.value);
        this.updateEngine();
      });
      panel.appendChild(amount.wrap);
    }
  }
  buildPresetPanel() {
