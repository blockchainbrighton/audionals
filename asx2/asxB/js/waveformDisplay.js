...
const getStyle = (k, d) => (getComputedStyle(document.documentElement).getPropertyValue(k).trim() || d),
  waveformColor = () => getStyle('--step-play', '#4caf50'),
...
// Shared: maps [0,1] buffer ratio to canvas X in (possibly zoomed) view
...
// Shared: min/max of audio data over [from,to)
...
export function generateWaveformPathImage(
  ...
  ctx.scale(dpr, dpr); ctx.strokeStyle = color; ctx.lineWidth = 1; ctx.beginPath();
  const samplesPerPixel = n / W, amp = H / 2;
  for (let x = 0; x < W; ++x) {
    if (x && x < W - 1 && x % 1.5 !== 0) continue;
    const a = Math.floor(x * samplesPerPixel), b = Math.floor((x + 1) * samplesPerPixel),
...
export function renderWaveformToCanvas(
  ...
  const {
    ...
    W = canvas.clientWidth, H = canvas.clientHeight || 100;
  if (!W || !H) return;
  ...
  if (bufferToDisplay) {
    ...
    if (seg > 1e-5 && drawW > 0) {
      if (fadeInTime > 0) {
        ...
        ctx.fillStyle = grad; ctx.fillRect(drawStartX, 0, fadeW, H);
      }
      if (fadeOutTime > 0) {
        ...
        ctx.fillStyle = grad; ctx.fillRect(gradStartX, 0, fadeW, H);
      }
    }
  }
