...
const ogSampleUrls = [
   ...
   * @param {string} type - The element type (e.g. 'div', 'button').
   * @param {string|string[]} [className] - Class name or an array of classes.
  ...
  /**
   * Creates a modal overlay and appends the given content.
   * Closes the modal if clicking outside the content.
   * @param {HTMLElement} content - The modal inner content.
  ...
  function createModal(content) {
    ...
    overlay.addEventListener('click', (e) => {
      if (!content.contains(e.target)) closeModal(overlay);
    });
    return overlay;
  }
  ...
  /**
   * Displays a temporary visual message.
   * @param {string} message
   */
  function showVisualMessage(message) {
    const msgDiv = createElement('div', 'visual-message', { textContent: message });
  ...
  /**
   * Retrieves the last part of a URL.
   * @param {string} url
   * @returns {string}
   */
  ...
  /**
   * Sets the channel URL using the given ordinalId.
   * @param {number} index
   * @param {string} ordinalId
   */
  function setOrdinalId(index, ordinalId) {
    window.unifiedSequencerSettings.settings.masterSettings.channelURLs[index] = `https://ordinals.com/content/${ordinalId}`;
  }
  
  /**
   * Sets the text for a load sample button based on the channel name.
   * Uses a default if no name is provided.
   * @param {number} index
   * @param {HTMLElement} button
   */
  function setButtonText(index, button) {
  ...
  /**
   * Creates a container with labeled input fields.
   * @param {Array} fields - Each field has label, placeholder, className.
  ...
  function createInputFields(fields, width) {
    ...
    fields.forEach(({ label, placeholder, className }) => {
      const fieldContainer = createElement('div', 'input-container', { style: 'margin-bottom: 10px;' });
      const labelEl = createElement('label', null, { textContent: label, style: 'margin-bottom: 5px;' });
   ...
   * @param {string} width
   * @param {number} index - For unique id.
   * @returns {HTMLElement}
   */
  function createDropdown(labelText, options, width, index) {
    ...
    const select = createElement('select', null, { 
      id: `audional-dropdown-${index}`, 
      style: `width: ${width};` 
    });
    options.forEach(opt => {
      select.appendChild(createElement('option', null, { textContent: opt.text, value: opt.value }));
    });
  ...
  /**
   * Creates a container with action buttons.
   * @param {Array} actions - Each has text, action, tooltip, className.
   * @returns {HTMLElement}
   */
  ...
  /**
   * Opens the load sample modal for the given channel.
   * @param {number} index
   * @param {HTMLElement} loadSampleButton
   * @returns {HTMLElement} The modal overlay.
   */
  function openLoadSampleModal(index, loadSampleButton) {
    ...
    const inputs = createInputFields([
    ...
    modalContent.append(inputs, dropdown, actions);
  ...
  /**
   * Handles the load action from modal or dropdown.
   * @param {number} index
  ...
  function handleLoad(index, event, loadSampleButton) {
    try {
      let url, sampleName;
      // Look for an existing modal with inputs
      const modal = AppState.openModals.find(m => m.querySelector('.modal-content'));
      const audInput = modal?.querySelector('.audional-input')?.value.trim();
      const ipfsInput = modal?.querySelector('.ipfs-input')?.value.trim();
      const sOrdInput = modal?.querySelector('.sOrdinal-input')?.value.trim();
      const dropdown = modal?.querySelector(`#audional-dropdown-${index}`);
      const dropdownVal = dropdown?.value;
      const dropdownText = dropdown?.selectedOptions[0]?.text;
  
      if (event?.target && dropdownVal) { // Dropdown change event
        url = dropdownVal;
        sampleName = dropdownText;
      } else {
        if (audInput) {
          url = `https://ordinals.com/content/${audInput}`;
          sampleName = audInput.split('/').pop();
        } else if (ipfsInput) {
          url = `https://ipfs.io/ipfs/${ipfsInput}`;
          sampleName = ipfsInput.split('/').pop();
        } else if (sOrdInput) {
          url = `https://content.sordinals.io/inscription-data/${sOrdInput}`;
          sampleName = sOrdInput.split('/').pop();
        } else if (dropdownVal) {
          url = dropdownVal;
          sampleName = dropdownText;
        }
  ...
  function determineErrorMessage(error) {
    ...
    if (msg.includes('404')) return "Audio not found (404). Please check the URL or input.";
    if (msg.includes('network')) return "Network error occurred while loading audio. Please check your connection.";
  ...
  /**
   * Closes any open custom context menu.
   */
  ...
  /**
   * Shows a modal to rename the channel.
   * @param {number} channelIndex
   * @param {HTMLElement} loadSampleButton
   */
  function showChannelNamingModal(channelIndex, loadSampleButton) {
    ...
    const buttons = createActionButtons([
      { text: 'Submit', action: submitAction, className: 'green-button' },
      { text: 'Cancel', action: closeAllModals, className: 'red-button' }
    ]);
  
    modalContent.append(input, buttons);
    const modalOverlay = createModal(modalContent);
    AppState.openModals.push(modalOverlay);
    document.body.appendChild(modalOverlay);
    input.focus();
  ...
  /**
   * Updates the UI and settings for a channel name change.
   * @param {number} channelIndex
   * @param {string} name
   */
  ...
  /**
   * Copies channel configuration data to clipboard.
   * @param {number} channelIndex
   */
    ...
    try {
      ...
      if (navigator.clipboard?.writeText) {
        navigator.clipboard.writeText(serializedData)
          ...
          .catch(err => handleError('copyChannel', err, 'Failed to copy channel to clipboard.'));
      } else {
  ...
  /**
   * Pastes channel configuration data from the clipboard.
   * @param {number} channelIndex
   * @param {HTMLElement} loadSampleButton
   */
    ...
    try {
      if (navigator.clipboard?.readText) {
        navigator.clipboard.readText().then(text => {
          ...
          const isValid = requiredFields.every(field => Object.prototype.hasOwnProperty.call(copiedData, field));
          if (!isValid) throw new Error('Clipboard data does not contain all required channel fields.');
          ...
          setChannelVolume(channelIndex, copiedData.volume); // Assumes setChannelVolume is defined elsewhere
          ...
          Object.entries(copiedData.steps).forEach(([seqKey, steps]) => {
            if (window.unifiedSequencerSettings.settings.masterSettings.projectSequences[seqKey]) {
              window.unifiedSequencerSettings.settings.masterSettings.projectSequences[seqKey][`ch${channelIndex}`] = {
        ...
        }).catch(err => handleError('pasteChannel', err, 'Failed to paste channel from clipboard.'));
      } else {
  ...
  function setupLoadSampleButton(channel, index) {
    ...
    loadSampleButton.textContent = window.unifiedSequencerSettings.settings.masterSettings.projectChannelNames[index] || 'Load New Audio into Channel';
    ...
    // Right-click for custom context menu
    loadSampleButton.addEventListener('contextmenu', (e) => handleContextMenu(e, index, loadSampleButton));
  }
