...
export function renderInventory() {
    ...
    this.inventory.items.forEach((item, index) => {
        ...
        if (item.type === 'weapon') {
            ...
            // Check if it's an NFT pill (has nftTraits) to add Inspect button
            if (item.nftTraits) {
                // Pass the item object via a temporary global lookup or find it by ID?
                // Easiest is to look it up by index again in the handler, but game.showItemDetails expects an item object.
                // We can just pass the index to a helper or lookup logic.
                // Let's make showItemDetails handle a lookup if passed an ID or index, OR simply construct the call.
                // Since we can't easily serialize the object in HTML, let's use a helper on game.player
                content += `<button class="button" style="float:right; margin-left:5px; font-size:10px; padding:4px 8px;" onclick="game.player.inspectItem(${index})">Inspect</button>`;
            }
