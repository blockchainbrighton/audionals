...
export function renderInventory() {
    ...
    this.inventory.items.forEach((item, index) => {
        ...
        // Resolve Image
        let imgUrl = item.imageUrl || `artwork/${item.id}.svg`;
        ...
        let mainActionBtn = '';
        if (item.type === 'weapon') {
            mainActionBtn = `<button class="button button-small" onclick="game.player.useItem(${index})">Equip</button>`;
        } else if (item.type === 'consumable' || (item.effect && typeof item.effect === 'function')) {
            mainActionBtn = `<button class="button button-small" onclick="game.player.useItem(${index})">Use</button>`;
        }
        ...
        const content = `
            ...
            <div class="item-actions">
                ${mainActionBtn}
                <button class="button button-small" onclick="game.player.inspectItem(${index})">Info</button>
                <button class="button button-small button-danger" onclick="game.player.dropItem(${index})">Drop</button>
            </div>
        `;
