        ...
        for (const enemy of sortedEnemies) {
            ...
            if (distToEnemy < this.equippedWeapon.effectiveRange) {
                ...
                if (angleDiff < (this.equippedWeapon.accuracyCone || 0.26)) { 
                    ...
                    if (this.game.particleManager) {
                        this.game.particleManager.createEffect(enemy.x + enemy.width/2, enemy.y + enemy.height/2, 'blood', 8);
                    }
...
export function reloadEquippedWeapon() {
    ...
    // Check if we have ammo in inventory to reload
    // (Simplified: infinite ammo for now? Or check inventory for 'ammo' item)
    // The original code in playerWeapons.js 'reloadWeapon' checked inventory.
    // We'll trust the process will succeed if we start it, actual deduction happens at end.
...
export function attemptActiveReload() {
    ...
    if (progress >= 0.5 && progress <= 0.7) {
        ...
        this.completeReload();
        // Play success sound
        if(this.game.soundManager) this.game.soundManager.playPickup(); // distinct sound
    } else {
        this.game.utils.addMessage("Jam! Reload delayed.");
        this.reloadDuration += 1000; // Penalty
        this.activeReloadTriggered = true; // Prevent spamming
        // Play fail sound
    }
}
