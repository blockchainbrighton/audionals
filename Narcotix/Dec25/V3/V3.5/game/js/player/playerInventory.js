...
export function addItem(item) {
    ...
    // Show NFT popup if applicable (Pauses game via NFT_VIEW)
    if (item.nftTraits && this.game.hud.showNftPickup) {
        this.game.hud.showNftPickup(item);
    } else if (item.type === 'weapon' || item.type === 'gadget' || item.type === 'consumable' || item.type === 'quest_item') {
...
export function renderInventory() {
    ...
    this.inventory.items.forEach((item, index) => {
        ...
        if (item.nftTraits && item.nftTraits.id) {
             url = imageLoader.getUrl(item.nftTraits.id);
        }

        if (url) {
            imageHtml = `<img src="${url}" style="width:32px; height:32px; vertical-align:middle; margin-right:8px; border-radius:4px; border:1px solid #444;" loading="lazy" referrerpolicy="no-referrer">`;
        }
        ...
        // Inspect button for all items
        content += `<button class="button" style="float:right; margin-left:5px; margin-right:5px; font-size:10px; padding:4px 8px;" onclick="game.player.inspectItem(${index})">Inspect</button>`;
