...
export function renderCharacterDetails(ctx) {
    ...
    if (this.faceExpression) {
        // Adjust for drawExpression which expects a bounding box (x, y, width, height)
        // The head is a circle with center (headX, headY) and radius this.headRadius
        // Bounding box would be:
        const faceSize = this.headRadius * 2;
        const faceX = headX - this.headRadius;
        const faceY = headY - this.headRadius;
        
        // Pass the expression to be drawn. 
        // Note: drawExpression rotates 90deg by default for pills. 
        // For the player face, we might want it upright relative to the screen, 
        // but the function assumes a pill laying horizontally. 
        // However, the function rotates +90deg inside. 
        // If we pass a square box, it will rotate 90deg. 
        // We might need to counteract this or update drawExpression to take rotation as arg.
        // Or simpler: Save context, rotate -90, call function, restore?
        // Let's modify drawExpression's rotation logic slightly or wrap it here.
        
        // Actually, drawExpression rotates 90deg because pills are wide.
        // If we want it upright, we should pre-rotate -90deg so the +90deg makes it 0.
        
        ctx.save();
        ctx.translate(headX, headY);
        // Removed ctx.rotate(-Math.PI / 2); as drawExpression already handles rotation for upright display.
        ctx.translate(-headX, -headY);
        
        drawExpression(ctx, faceX, faceY, faceSize, faceSize, this.faceExpression);
        ctx.restore();
    }
