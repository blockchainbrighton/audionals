    ...
    renderItemDetails: function(item) {
        ...
        if (inInventory) {
            ...
            if (item.type === 'weapon' || item.type === 'gadget') {
                 actionButtons += `<button class="button" onclick="game.player.useItem(${index}); game.hideItemDetails();">Equip</button>`;
            } 
            // Use/Consume Action
            else if (item.type === 'consumable' || item.effect) {
                 actionButtons += `<button class="button" onclick="game.player.useItem(${index}); game.hideItemDetails();">Use</button>`;
            }
            
            // Drop Action (Available for all inventory items)
            actionButtons += `<button class="button" onclick="game.player.dropItem(${index});">Drop</button>`;
        }

        // Close Action (Always available)
        actionButtons += `<button class="button" onclick="game.hideItemDetails()">Close</button>`;
