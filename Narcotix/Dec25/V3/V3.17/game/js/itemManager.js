            ...
            'walkman': { 
                ...
                effect: (player) => {
                    ...
                    if (player.hasWalkmanActive) {
                        this.game.hud.showItemDetails(this.itemDefinitions['walkman']);
                        ...
                        this.itemDefinitions[itemId] = {
                            ...
                            effect: (player) => {
                                ...
                                if (!player.gang && row.expression) {
                                    player.joinExpressionGang(row.expression);
                                }
        ...
        this.onMapItems.forEach(item => {
            if (item.x + this.game.config.TILE_SIZE > this.game.camera.x &&
                ...
                if (item.id && item.id.includes('pill')) {
                    ...
                    // Use the custom renderPill function
                    // Adjust height slightly for the pill shape (e.g., half height) to look nice
                    const pillHeight = item.height * 0.6; 
                    const pillY = item.y + (item.height - pillHeight) / 2; // Center vertically
                    
                    this.game.utils.renderPill(this.game.ctx, item.x, pillY, item.width, pillHeight, pillData);
                    return; // Skip standard character rendering
                }

                // Render Walkman SVG
                if (item.id === 'walkman') {
                    const imgPath = 'artwork/walkman.svg';
                    if (!this.itemImages[imgPath]) {
                        this.itemImages[imgPath] = new Image();
                        this.itemImages[imgPath].src = imgPath;
                    }
                    const img = this.itemImages[imgPath];
                    if (img.complete && img.naturalHeight !== 0) {
                        this.game.ctx.drawImage(img, item.x, item.y, item.width, item.height);
                        return;
                    }
                }
