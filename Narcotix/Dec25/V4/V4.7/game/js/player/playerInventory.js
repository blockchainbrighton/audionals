...
export function renderInventory() {
    ...
    if (this.equippedWeapon) {
        ...
        equippedDiv.className = 'inventory-equipped-card';
        
        let weaponImg = `artwork/${this.equippedWeapon.id}.svg`;
        // Check if equipped weapon has specific image from inventory item (if we tracked it)
        // For now, use default artwork based on ID
        ...
        let equippedContent = `
            ...
            <div class="item-details">
                ...
                <div class="item-meta">
                    ${this.equippedWeapon.type === Weapons.weaponTypes.RANGED ? `Ammo: ${this.equippedWeapon.currentAmmo}/${this.equippedWeapon.ammoCapacity}` : 'Melee'}
                </div>
            </div>
            <div class="item-actions">
                ${this.equippedWeapon.id !== Weapons.UNARMED_STATS.id ? `<button class="button button-small" onclick="game.player.unequipWeapon()">Unequip</button>` : ''}
            </div>
        `;
    ...
    this.inventory.items.forEach((item, index) => {
        ...
        itemDiv.className = 'inventory-item-card';
        // Click card to inspect
        itemDiv.onclick = () => game.player.inspectItem(index);
        itemDiv.style.cursor = 'pointer'; // Visual cue
        
        // Resolve Image
        let imgUrl = item.imageUrl;
        if (!imgUrl) {
            if (item.type === 'weapon' && item.weaponId) {
                imgUrl = `artwork/${item.weaponId}.svg`;
            } else {
                imgUrl = `artwork/${item.id}.svg`;
            }
        }
        
        if (item.nftTraits && item.nftTraits.id) {
             // Try to get cached URL, fallback to default logic
             imgUrl = imageLoader.getUrl(item.nftTraits.id) || imgUrl;
        }

        // Action Buttons
        let mainActionBtn = '';
        if (item.type === 'weapon') {
            mainActionBtn = `<button class="button button-small" onclick="event.stopPropagation(); game.player.useItem(${index})">Equip</button>`;
        } else if (item.type === 'consumable' || (item.effect && typeof item.effect === 'function')) {
            mainActionBtn = `<button class="button button-small" onclick="event.stopPropagation(); game.player.useItem(${index})">Use</button>`;
        }
        ...
        const content = `
            ...
            <div class="item-actions">
                ${mainActionBtn}
                <button class="button button-small" onclick="event.stopPropagation(); game.player.inspectItem(${index})">Info</button>
                <button class="button button-small button-danger" onclick="event.stopPropagation(); game.player.dropItem(${index})">Drop</button>
            </div>
        `;
