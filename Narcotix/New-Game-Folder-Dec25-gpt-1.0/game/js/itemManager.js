...
export const itemManager = {
        ...
        for (let i = 0; i < totalItemsToSpawn; i++) {
            ...
            if (attempts < 20) {
                ...
                if (spawnTypeRoll < 0.6 && spawnableGenericItems.length > 0) { // 60% chance for generic/ammo
                    ...
                    if (iDef) {
                        const nftEntry = (rId === 'narcotix_pill') ? this.game.narcotixCollection.getRandomEntry() : null;
                        mapItemEntry = {
                            type: iDef.type, // 'consumable' or 'ammo'
                            id: rId, // Item definition ID
                            x: itemSpawnX, y: itemSpawnY, width: itemWidth, height: itemHeight,
                            name: nftEntry?.name || iDef.name,
                            quantity: rId === 'narcotix_pill'
                                ? 1
                                : (iDef.stackable ? (Math.floor(Math.random() * (iDef.type === 'ammo' ? 20 : 3)) + 1) : 1),
    ...
    renderItemsOnMap: function() {
        this.onMapItems.forEach(item => {
            if (item.x + this.game.config.TILE_SIZE > this.game.camera.x &&
                ...
                if (item.id === 'narcotix_pill' && item.nftEntry) {
                    this.renderCollectionPill(item);
                    return;
                }
    ...
    renderCollectionPill: function(item) {
        const ctx = this.game.ctx;
        ctx.save();

        const paddingX = item.width * 0.05;
        const paddingY = item.height * 0.2;
        const pillWidth = item.width - paddingX * 2;
        const pillHeight = item.height - paddingY * 2;
        const radius = pillHeight / 2;

        const startX = item.x + paddingX;
        const startY = item.y + paddingY;

        const gradient = ctx.createLinearGradient(startX, startY, startX + pillWidth, startY + pillHeight);
        gradient.addColorStop(0, item.nftEntry.colors.primary);
        gradient.addColorStop(1, item.nftEntry.colors.secondary);

        ctx.fillStyle = gradient;
        ctx.beginPath();
        ctx.moveTo(startX + radius, startY);
        ctx.lineTo(startX + pillWidth - radius, startY);
        ctx.quadraticCurveTo(startX + pillWidth, startY, startX + pillWidth, startY + radius);
        ctx.lineTo(startX + pillWidth, startY + pillHeight - radius);
        ctx.quadraticCurveTo(startX + pillWidth, startY + pillHeight, startX + pillWidth - radius, startY + pillHeight);
        ctx.lineTo(startX + radius, startY + pillHeight);
        ctx.quadraticCurveTo(startX, startY + pillHeight, startX, startY + pillHeight - radius);
