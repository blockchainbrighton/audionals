...
import { SAFEHOUSE_TILE, TILE_SIZE } from '../config.js';
...
export function initCore(gameInstance) {
    ...
    const spawnTile = SAFEHOUSE_TILE || { x: Math.floor(this.game.config.MAP_WIDTH_TILES / 2), y: Math.floor(this.game.config.MAP_HEIGHT_TILES / 2) };
    this.x = spawnTile.x * TILE_SIZE + TILE_SIZE / 2 - this.width / 2;
    this.y = spawnTile.y * TILE_SIZE + TILE_SIZE / 2 - this.height / 2;
