...
export const enemyManager = {
    ...
    spawnEnemy: function(typeId, tileX, tileY, patrolPath=[]) {
        const spawnX = tileX*this.game.config.TILE_SIZE;
        const spawnY = tileY*this.game.config.TILE_SIZE;
        const centerX = spawnX + this.game.config.TILE_SIZE/2;
        const centerY = spawnY + this.game.config.TILE_SIZE/2;
        if (this.shouldBlockSpawn(centerX, centerY)) {
            return;
        }
        const newE=this.createEnemy(typeId, spawnX, spawnY, patrolPath.map(p=>({x:p.x*this.game.config.TILE_SIZE,y:p.y*this.game.config.TILE_SIZE})));
    ...
    shouldBlockSpawn: function(worldX, worldY) {
        if (!this.game || !this.game.player) return false;
        const graceActive = typeof this.game.isPlayerInvulnerable === 'function' ? this.game.isPlayerInvulnerable() : false;
        if (!graceActive) return false;
        const safeRadiusTiles = this.game.config.PLAYER_SAFE_RADIUS_TILES || 6;
        const safeRadiusPx = safeRadiusTiles * this.game.config.TILE_SIZE;
        const playerCenterX = this.game.player.x + this.game.player.width/2;
        const playerCenterY = this.game.player.y + this.game.player.height/2;
        return this.game.utils.distance(playerCenterX, playerCenterY, worldX, worldY) < safeRadiusPx;
    },

    enforcePlayerSafeZone: function(player, radiusPx) {
        if (!player || !radiusPx) return;
        const playerCenterX = player.x + player.width/2;
        const playerCenterY = player.y + player.height/2;
        this.list = this.list.filter(enemy => {
            const enemyCenterX = enemy.x + enemy.width/2;
            const enemyCenterY = enemy.y + enemy.height/2;
            return this.game.utils.distance(playerCenterX, playerCenterY, enemyCenterX, enemyCenterY) >= radiusPx;
        });
    },
