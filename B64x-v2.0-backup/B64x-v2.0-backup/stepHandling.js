...
(function () {
    ...
    const EMPTY_TOGGLE_STEPS = Object.freeze([]);
    let cachedChannels = null;

    const getChannelElements = () => {
      if (window.channelsStore && window.channelsStore.length) {
        cachedChannels = window.channelsStore;
      } else if (!cachedChannels || !cachedChannels.length) {
        cachedChannels = Array.from(document.querySelectorAll('.channel[id^="channel-"]'));
      }
    ...
    function handleStep(channel, channelData, totalStepCount) {
      ...
      const toggleSteps = channelData?.toggleMuteSteps || EMPTY_TOGGLE_STEPS;
      const isToggleMuteStep = toggleSteps.includes(totalStepCount);
    ...
    function playStep() {
      ...
      const channels = getChannelElements();
      ...
      for (let channelIndex = 0; channelIndex < numChannels; channelIndex++) {
        ...
        const channelData = presetData?.channels?.[channelIndex];
