...
(function (window) {
    ...
    let iframeChannelMappings = null;
    ...
    window.addEventListener("storage", (event) => {
      if (event.key === "channelIframeMappings") {
        refreshIframeChannelMappings();
      }
    });
     ...
     */
    function refreshIframeChannelMappings() {
      try {
        iframeChannelMappings = JSON.parse(localStorage.getItem("channelIframeMappings")) || {};
      } catch (error) {
        console.error("[getIframeIdByChannelIndex] Failed to parse channelIframeMappings from localStorage", error);
        iframeChannelMappings = {};
      }
    }

    function getIframeIdByChannelIndex(channelIndex) {
      if (!iframeChannelMappings) {
        refreshIframeChannelMappings();
      }
      return iframeChannelMappings?.[channelIndex];
    }
