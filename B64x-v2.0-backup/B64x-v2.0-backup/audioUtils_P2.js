...
(function (window) {
  ...
  function playSound(currentSequence, channel, currentStep) {
    ...
    const broadcastMessage = isActive ? "startArpeggiator" : "stopArpeggiator";
    bc.postMessage({ type: broadcastMessage, channelIndex });
    if (iframeBC) {
      iframeBC.postMessage({ type: broadcastMessage, channelIndex });
    }
    bc.close();
    if (iframeBC) {
      iframeBC.close();
